...
import w from"./vendor-swal-BSk0fVSb.js";import{a as Ae}from"./vendor-DHtNC8Ux.js";const Ne="true".toLowerCase()==="true";function Ee(){return Ne}function X(){const e=window.location.pathname,o="/dole-system/",s=e.toLowerCase().indexOf(o.toLowerCase());if(s!==-1)return e.substring(0,s+o.length);const a=e.indexOf("/frontend/");if(a!==-1)return e.substring(0,a+1);const r=e.indexOf("/backend/");return r!==-1?e.substring(0,r+1):"/"}function we(e="Incorrect Username or Password"){w.fire({html:`
    ...
    `,document.body.appendChild(t);const l=e?0:1e3,p=e?600:1500;setTimeout(()=>{o&&o.classList.add("animate-slide-left"),s&&s.classList.add("animate-slide-right"),setTimeout(()=>{window.location.href=`${X()}frontend/dashboard/`},p)},l)}function et(){const e=document.getElementById("logoutBtn");e&&e.addEventListener("click",()=>{localStorage.removeItem("isLoggedIn"),window.location.href=`${X()}`})}function tt(){const e=document.getElementById("mobile-splash"),o=document.getElementById("show-login-btn"),s=document.getElementById("back-to-splash"),a=document.getElementById("mobile-bg-content"),r=document.getElementById("mobile-welcome-text"),t=document.getElementById("reopen-login-drawer"),u=document.getElementById("request-notifications-btn"),n=async()=>{if("Notification"in window){const f=await Notification.requestPermission();console.log("Notification permission:",f),f==="granted"&&u&&u.classList.add("hidden")}};Notification.permission==="default"&&u&&(u.classList.remove("hidden"),u.addEventListener("click",n));const l=()=>{e&&(e.style.transform="translateY(-100%)",setTimeout(()=>{e.style.visibility="hidden",e.style.pointerEvents="none",e.style.zIndex="-1",Notification.permission==="default"&&n();const f=document.getElementById("drawer-login");f&&f.classList.remove("translate-y-full")},800))},p=()=>{e&&(e.style.zIndex="9999",e.style.pointerEvents="auto",e.style.visibility="visible",e.style.transform="translateY(0)")};o&&o.addEventListener("click",l),document.querySelectorAll(".forgot-password-link").forEach(f=>{f.addEventListener("click",y=>{y.target.closest("#mobile-splash")&&l()})}),s&&s.addEventListener("click",()=>{const f=document.getElementById("drawer-login");if(f){f.classList.add("translate-y-full");const y=f.querySelector("[data-drawer-hide]");y&&y.click()}p()});const c=document.getElementById("drawer-login"),g=document.getElementById("curved-welcome"),v=document.getElementById("peoples-bg");c&&new MutationObserver(y=>{y.forEach($=>{$.attributeName==="class"&&(c.classList.contains("translate-y-full")?(a&&(a.style.transform="translateY(0)"),r&&(r.style.opacity="1",r.style.transform="translateY(0) scale(1)"),g&&(g.style.opacity="0",g.style.transform="scale(0.5)"),t&&e&&e.style.visibility==="hidden"&&(t.style.opacity="1",t.style.transform="scale(1)"),v&&(v.classList.add("opacity-0","scale-0"),v.classList.remove("opacity-40","scale-[1.6]"))):(a&&(a.style.transform="translateY(-35%)"),r&&(r.style.opacity="0",r.style.transform="translateY(20px) scale(0.9)"),g&&(g.style.opacity="1",g.style.transform="scale(1)"),t&&(t.style.opacity="0",t.style.transform="scale(0)"),v&&(v.classList.remove("opacity-0","scale-0"),v.classList.add("opacity-40","scale-[1.6]"))))})}).observe(c,{attributes:!0})}const le=()=>"false".toLowerCase()==="true";function _e(e){try{return JSON.stringify(e)}catch{return"[unserializable]"}}const ae={debug(...e){le()&&console.debug(...e)},info(...e){le()&&console.info(...e)},warn(...e){le()&&console.warn(...e)},error(...e){console.error(...e)},table(e){le()&&console.table(e)},json(e,o){le()&&console.debug(e,_e(o))}},ce=new Map;async function ne(e,o={}){const a=`${X()}${e}`;let r=null;try{const i=JSON.parse(localStorage.getItem("user"));i&&(r=i.user_id||i.id||null)}catch{}const t={headers:{"Content-Type":"application/json",...r?{"X-User-Id":r}:{},...o.headers},...o},n=(t.method||"GET").toUpperCase()==="GET"?2:1;let l=null;for(let i=1;i<=n;i++)try{if(ae.debug("[API] Request",{url:a,method:t.method||"GET",hasUserId:!!r}),t.body)try{ae.json("[API] Payload",JSON.parse(t.body))}catch{ae.debug("[API] Payload (raw)",t.body)}const c=await fetch(a,t);if(!c.ok)throw new Error(`HTTP ${c.status}: ${c.statusText}`);const g=await c.json();return ce.has(a)&&(ce.delete(a),ae.info?.("[API] Recovered",{url:a})),ae.debug("[API] Response",{url:a,ok:!0}),{success:!0,data:g}}catch(c){if(l=c,c instanceof TypeError&&/fetch/i.test(c.message||"")&&i<n){await new Promise(f=>setTimeout(f,1200));continue}}return l instanceof TypeError&&/fetch/i.test(l.message||"")?ce.get(a)||(ce.set(a,!0),ae.error("API Request Network Error (suppressed for repeats):",{url:a,message:l.message})):ae.error("API Request Error:",l),{success:!1,error:l?.message||"Unknown request error"}}async function Y(e){return ne(e,{method:"GET"})}async function xe(e,o){return ne(e,{method:"POST",body:JSON.stringify(o)})}async function Pe(e,o){return ne(e,{method:"PUT",body:JSON.stringify(o)})}async function rt(e,o){const s=new URLSearchParams(o).toString();return ne(`${e}?${s}`,{method:"PATCH"})}class Oe{constructor(){this.intervals=new Map,this.isPageVisible=!0,document.addEventListener("visibilitychange",()=>{this.isPageVisible=!document.hidden,this.isPageVisible?console.log("[Polling] Page visible - resuming normal activity"):console.log("[Polling] Page hidden - reducing activity")})}start(o,s,a=1e4){this.stop(o),s();const r=setInterval(async()=>{this.isPageVisible&&await s()},a);this.intervals.set(o,r),console.log(`[Polling] Started: ${o} (every ${a}ms)`)}stop(o){this.intervals.has(o)&&(clearInterval(this.intervals.get(o)),this.intervals.delete(o),console.log(`[Polling] Stopped: ${o}`))}stopAll(){this.intervals.forEach((o,s)=>this.stop(s)),console.log("[Polling] All intervals stopped")}getActivePolls(){return Array.from(this.intervals.keys())}}const Te=new Oe;function ot(){typeof window.initFlowbite=="function"?(window.initFlowbite(),console.log("[Flowbite] Components re-initialized")):console.warn("[Flowbite] initFlowbite() not available globally")}function at(e,o="",s="info"){w.fire({toast:!0,position:"top-end",icon:s,title:e,text:o,showConfirmButton:!1,timer:3e3,timerProgressBar:!0})}function st(e){return JSON.stringify(e)}window.addEventListener("beforeunload",()=>{Te.stopAll()});const He="modulepreload",qe=function(e,o){return new URL(e,o).href},ke={},De=function(o,s,a){let r=Promise.resolve();if(s&&s.length>0){let p=function(i){return Promise.all(i.map(c=>Promise.resolve(c).then(g=>({status:"fulfilled",value:g}),g=>({status:"rejected",reason:g}))))};const u=document.getElementsByTagName("link"),n=document.querySelector("meta[property=csp-nonce]"),l=n?.nonce||n?.getAttribute("nonce");r=p(s.map(i=>{if(i=qe(i,a),i in ke)return;ke[i]=!0;const c=i.endsWith(".css"),g=c?'[rel="stylesheet"]':"";if(a)for(let f=u.length-1;f>=0;f--){const y=u[f];if(y.href===i&&(!c||y.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${i}"]${g}`))return;const v=document.createElement("link");if(v.rel=c?"stylesheet":He,c||(v.as="script"),v.crossOrigin="",v.href=i,l&&v.setAttribute("nonce",l),document.head.appendChild(v),c)return new Promise((f,y)=>{v.addEventListener("load",f),v.addEventListener("error",()=>y(new Error(`Unable to preload CSS for ${i}`)))})}))}function t(u){const n=new Event("vite:preloadError",{cancelable:!0});if(n.payload=u,window.dispatchEvent(n),!n.defaultPrevented)throw u}return r.then(u=>{for(const n of u||[])n.status==="rejected"&&t(n.reason);return o().catch(t)})};let me=null;if(Ee()){const e="https://llnddycvbcetztzwbdpx.supabase.co",o="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxsbmRkeWN2YmNldHp0endiZHB4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ1MDMwMTEsImV4cCI6MjA5MDA3OTAxMX0.apIpDlZYja7SNuA-joXgFSvu_wtbBcVVP3OQSPrh7fA";try{me=Ae(e,o),console.log("[SUPABASE SDK] Client Initialized Successfully")}catch(s){console.error("[SUPABASE SDK] Failed to initialize client:",s)}}else console.log("[SUPABASE SDK] Supabase mode is disabled (Localhost PHP mode active).");const ie="color-theme",Ve=3600*24*365;function Fe(e,o,s){document.cookie=`${e}=${o}; max-age=${s}; path=/; SameSite=Lax`}function Ce(e){const o=document.cookie.match(new RegExp("(?:^|; )"+e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+"=([^;]*)"));return o?decodeURIComponent(o[1]):null}function Le(){const e=localStorage.getItem(ie)||Ce(ie);return e==="dark"||e==="light"?e:"light"}function ue(e){const o=document.documentElement;e==="dark"?o.classList.add("dark"):o.classList.remove("dark"),localStorage.setItem(ie,e),Fe(ie,e,Ve),Ge(e),document.dispatchEvent(new CustomEvent("themeChanged",{detail:{theme:e}}))}function ve(){const e=Le();ue(e==="dark"?"light":"dark")}function Ge(e){const o=e==="dark",s=document.getElementById("pref-dark-mode");s&&(s.checked=o);const a=document.getElementById("theme-toggle-dark-icon"),r=document.getElementById("theme-toggle-light-icon");a&&r&&(a.classList.toggle("hidden",o),r.classList.toggle("hidden",!o));const t=document.getElementById("sidebar-theme-label");t&&(t.textContent=o?"Light Mode":"Dark Mode")}function nt(){const e=Le();ue(e);const o=document.getElementById("pref-dark-mode");o&&o.addEventListener("change",()=>{ue(o.checked?"dark":"light")});const s=document.getElementById("theme-toggle-btn");s&&s.addEventListener("click",ve),document.querySelectorAll("[data-theme-toggle]").forEach(a=>{a.addEventListener("click",ve)}),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",a=>{localStorage.getItem(ie)||Ce(ie)||ue(a.matches?"dark":"light")})}function te(){return document.documentElement.classList.contains("dark")}window.toggleTheme=ve;window.isDarkMode=te;const ge={queue:[],currentIndex:0,isActive:!1,isAutoSave:!1,lastInteractionTime:0,init(){this.showUploadModal()},showUploadModal(){const e=te(),o={bgCard:e?"bg-slate-900/40":"bg-gray-50/40",borderCard:e?"border-slate-800":"border-gray-100",textHeading:e?"text-green-500":"text-[#2e7d32]",textSubtitle:e?"text-slate-500":"text-gray-400 dark:text-gray-300",bgUpload:e?"bg-slate-800":"bg-white",borderUpload:e?"border-slate-700":"border-gray-200",textUpload:e?"text-slate-400":"text-gray-500",hoverUpload:e?"hover:bg-slate-800/80 hover:border-blue-500":"hover:bg-blue-50/50 hover:border-royal-blue",iconText:e?"text-green-400":"text-[#2e7d32]",iconBg:e?"bg-green-900/20":"bg-[#e8f5e9]",iconBorder:e?"border-green-800/30":"border-[#c8e6c9]"},s=`
        ...
        `;w.fire({html:s,width:"600px",showConfirmButton:!1,showCloseButton:!0,customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:a=>{const r=a.querySelector("#csv-upload"),t=a.querySelector('label[for="csv-upload"]');["dragenter","dragover","dragleave","drop"].forEach(n=>{t.addEventListener(n,u,!1)});function u(n){n.preventDefault(),n.stopPropagation()}["dragenter","dragover"].forEach(n=>{t.addEventListener(n,()=>{t.classList.add("border-blue-500","bg-blue-50/50"),e&&t.classList.add("dark:bg-slate-800/80")},!1)}),["dragleave","drop"].forEach(n=>{t.addEventListener(n,()=>{t.classList.remove("border-blue-500","bg-blue-50/50"),e&&t.classList.remove("dark:bg-slate-800/80")},!1)}),r.addEventListener("change",n=>{const l=n.target.files[0];if(l){const p=a.querySelector("#auto-save-toggle");this.isAutoSave=p?p.checked:!1,this.handleFile(l)}}),t.addEventListener("drop",n=>{const p=n.dataTransfer.files[0];if(p){const i=a.querySelector("#auto-save-toggle");this.isAutoSave=i?i.checked:!1,this.handleFile(p)}},!1)}})},handleFile(e){if(!e.name.toLowerCase().endsWith(".csv")&&!e.name.toLowerCase().endsWith(".txt")){w.fire("Invalid File","Please upload a valid .csv or .txt file.","error");return}const o=new FileReader;o.onload=s=>{const a=s.target.result;this.parseCSV(a)},o.readAsText(e)},async parseCSV(e){let o=[],s="",a=!1;for(let r=0;r<e.length;r++){let t=e[r];t==='"'&&(a=!a),!a&&(t===`
`||t==="\r")?(t==="\r"&&e[r+1]===`
`&&r++,s.trim()!==""&&o.push(s),s=""):s+=t}s.trim()!==""&&o.push(s),this.queue=[];for(let r=0;r<o.length;r++){let t=o[r].trim();if(!t)continue;let u=[],n="",l=!1;for(let p=0;p<t.length;p++){let i=t[p];i==='"'?l=!l:i===","&&!l?(u.push(n.replace(/(^"|"$)/g,"").trim()),n=""):n+=i}if(u.push(n.replace(/(^"|"$)/g,"").trim()),u.length>=2){const p=u[3];if(!p||isNaN(parseInt(p)))continue;const i=u[1];if(!i||i.toLowerCase()==="name"||i.toLowerCase()==="full name")continue;const c=u[2];let g=u[4]?u[4].toUpperCase().trim():"",v="";(g==="F"||g.includes("FEMALE"))&&(v="Female"),(g==="M"||g.includes("MALE"))&&(v="Male");const f=u[5],y=u[6],$=u[7],T=this.formatDate(u[8]),R=this.formatDate(u[9]);this.queue.push({name:i,address:c,age:p,gender:v,education:f,startDate:T,endDate:R,office:y,designation:$})}}if(this.queue.length>0){try{w.fire({title:"Checking duplicates...",html:'<p class="text-sm">Please wait while we cross-reference your data.</p>',allowOutsideClick:!1,didOpen:()=>{w.showLoading()}});const r=this.queue.map(n=>n.name),u=await(await fetch(`${X()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({names:r})})).json();if(u.success&&u.duplicates&&u.duplicates.length>0){const n=new Set(u.duplicates.map(p=>p.toLowerCase().trim())),l=this.queue.length;this.queue=this.queue.filter(p=>{const i=n.has(p.name.toLowerCase().trim());return i&&console.warn(`%c[Bulk Add] SKIPPED: ${p.name} already exists in database.`,"color: #ff9800; font-weight: bold;"),!i}),console.log(`[Bulk Add] Removed ${l-this.queue.length} duplicates ahead of time.`)}}catch(r){console.error("Bulk duplicate check failed:",r)}if(this.queue.length===0){w.fire("All Skipped","All beneficiaries in the CSV already exist in the database.","info");return}this.isActive=!0,this.currentIndex=0,w.close(),this.processNext()}else w.fire("Error","No valid beneficiary data found. Please ensure your CSV formatting matches the requirements.","error")},showProgressModal(){te();const e=Math.round(this.currentIndex/this.queue.length*100),o=`
...
<div class="mt-8 pt-2 border-t border-default relative">
    ...
    <div id="drawer-page-0" class="flex-1 flex flex-col gap-4">
        ...
        <div class="bg-[#e8f5e9]/50 dark:bg-green-900/10 p-4 rounded-xl border border-[#c8e6c9] dark:border-green-900/30 mt-2">
            ...
            <div class="flex flex-col gap-2">
                <div class="flex justify-between items-start group">
                    ...
                    <span class="font-black text-[#1b5e20] dark:text-green-400 text-xs text-right whitespace-nowrap">${e.absorbDate?new Date(e.absorbDate).toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric"}).toUpperCase():"N/A"}</span>
                </div>
                <div class="flex justify-between items-start group">
        ...
        <div class="bg-[#ffebee]/50 dark:bg-red-900/10 p-4 rounded-xl border border-[#ffcdd2] dark:border-red-900/30 mt-2">
            ...
            <div class="flex flex-col gap-2">
                <div class="flex justify-between items-start group">
                    <span class="text-gray-500 font-medium text-[10px] uppercase font-bold tracking-widest mr-4">Date</span>
                    <span class="font-black text-[#b71c1c] dark:text-red-400 text-xs text-right whitespace-nowrap">${e.resignedDate?new Date(e.resignedDate).toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric"}).toUpperCase():"N/A"}</span>
                </div>
                <div class="flex justify-between items-start group">
    ...
    <div id="drawer-page-1" class="hidden flex-1 flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-3">
            ${l.map(c=>{const g=c.status.toUpperCase(),f={VERIFIED:"COMPLETED",DECLINED:"REJECTED",PENDING:"PENDING"}[g]||g,y=f==="COMPLETED",$=f==="REJECTED",T=y?"text-green-500":$?"text-red-500":"text-gray-400 dark:text-gray-500",R=y?"bg-green-50/50 dark:bg-green-900/10":$?"bg-red-50/50 dark:bg-red-900/10":"bg-gray-50/50 dark:bg-slate-800/50",J=y?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800 hover:bg-green-200 cursor-pointer":$?"bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800 hover:bg-red-200 cursor-pointer":"bg-white text-gray-500 border-gray-200 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer";let H='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';return y?H='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>':$&&(H='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>'),`
                <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-0.5 ${R}">
                    ...
                    <button type="button" class="ml-3 ${J} text-[10px] font-black px-3 py-1.5 rounded-full border uppercase tracking-widest transition-colors flex-shrink-0 drawer-doc-btn" data-id="${c.id}" data-name="${c.name}" data-status="${f} cursosr-pointer">
                        ${f}
                    </button>
                </div>
                `}).join("")}
    ...
    <div id="drawer-page-2" class="hidden flex-1 flex flex-col gap-6">
         ...
         <div class="flex flex-col gap-5">
            <div>
                ...
                <div class="space-y-2">
                    ${t.length?t.map(c=>{const g=c.status||"PENDING";let v=g==="VERIFIED"||g==="COMPLETED"?"text-green-500":g==="REJECTED"||g==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500",f=c.date||c.createdAt,y=f;if(f){const $=/^\d{4}-\d{2}-\d{2}$/.test(f)?new Date(f+"T00:00:00Z"):new Date(f);isNaN($)||(y=$.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric",timeZone:"Asia/Manila"}).toUpperCase())}return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-blue-100 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors edit-log-btn" data-type="dtr" data-id="${c.id}" data-val="${f}" data-status="${g}">
                            <span class="text-xs font-black text-royal-blue dark:text-blue-400 capitalize whitespace-nowrap pointer-events-none">${y}</span>
            ...
            <div>
                ...
                <div class="space-y-2">
                    ${r.length?r.map(c=>{const g=c.status||"PENDING";let v=g==="VERIFIED"||g==="COMPLETED"?"text-green-500":g==="REJECTED"||g==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500",f=c.period||c.createdAt,y=f;if(f){const $=/^\d{4}-\d{2}-\d{2}$/.test(f)?new Date(f+"T00:00:00Z"):new Date(f);isNaN($)||(y=$.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric",timeZone:"Asia/Manila"}).toUpperCase())}return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-orange-100 dark:border-orange-900/50 bg-orange-50/50 dark:bg-orange-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-orange-100 dark:hover:bg-orange-900/40 transition-colors edit-log-btn" data-type="ar" data-id="${c.id}" data-val="${f}" data-status="${g}">
                            <span class="text-xs font-black text-orange-600 dark:text-orange-400 capitalize whitespace-nowrap pointer-events-none">${y}</span>
                            <span class="text-[11px] font-bold ${v} uppercase tracking-widest truncate max-w-[50%] text-right pr-6 group-hover:pr-12 pointer-events-none transition-all">${g}</span>
...
<style>
    ...
    `;let i=document.getElementById("beneficiary-drawer-container");i&&i.remove(),i=document.createElement("div"),i.id="beneficiary-drawer-container",i.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-neutral-primary-soft dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl",i.setAttribute("tabindex","-1"),i.setAttribute("data-drawer-backdrop","true"),i.innerHTML=p,document.body.appendChild(i),De(async()=>{const{Drawer:c}=await import("./vendor-flowbite-B7rSfpuT.js").then(g=>g.b);return{Drawer:c}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:c})=>{const g={placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{i&&i.parentNode&&i.remove()},300)}},v=new c(i,g);v.show(),i.querySelector("#close-drawer-btn").addEventListener("click",()=>v.hide());const y=i.querySelector("#drawer-prev-btn"),$=i.querySelector("#drawer-next-btn"),T=2,R=()=>{i.querySelectorAll("[id^=drawer-page-]").forEach((C,j)=>{C.classList.toggle("hidden",j!==a)}),y.disabled=a===0,$.disabled=a===T,y.classList.toggle("opacity-50",a===0),$.classList.toggle("opacity-50",a===T)};y.addEventListener("click",()=>{a>0&&a--,R()}),$.addEventListener("click",()=>{a<T&&a++,R()}),R(),i.querySelectorAll(".drawer-doc-btn").forEach(C=>{C.addEventListener("click",async()=>{const j=C.dataset.name,M=C.dataset.status,B="flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[9px] gap-2 transition-all duration-300 ",I=await w.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Update Document</span>',html:`
                        <div class="font-montserrat text-left">
                            ...
                            <div class="grid grid-cols-3 gap-3">
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-doc-status" value="PENDING" class="peer sr-only" ${M==="PENDING"?"checked":""}>
                                ...
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-doc-status" value="COMPLETED" class="peer sr-only" ${M==="COMPLETED"?"checked":""}>
                                ...
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-doc-status" value="REJECTED" class="peer sr-only" ${M==="REJECTED"?"checked":""}>
                    ...
                    `,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Update Status</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Cancel</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2"},buttonsStyling:!1,preConfirm:()=>{const L=document.querySelector('input[name="swal-doc-status"]:checked');return L?L.value:null}});if(I.isConfirmed){const L=I.value;if(L===M)return;try{const Q={COMPLETED:"VERIFIED",REJECTED:"DECLINED",PENDING:"PENDING"}[L]||L,k=await xe("api/logs.php?type=docs",{gip_id:e.id,doc_name:j,status:Q}),b=k.success?k.data:{success:!1,error:k.error};b.success?(w.fire({toast:!0,position:s,icon:"success",title:"Status updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):w.fire("Error",b.error||"Failed to update","error")}catch(G){w.fire("Error",G.message,"error")}}})});const J=()=>{if(!t.length)return new Date().toISOString().split("T")[0];const C=new Date(t[0].date);return C.setDate(C.getDate()+1),C.toISOString().split("T")[0]},H=()=>{if(!r.length){const B=new Date,I=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"][B.getMonth()];return B.getDate()<=15?`${I} 1-15`:`${I} 16-${new Date(B.getFullYear(),B.getMonth()+1,0).getDate()}`}const C=r[0].period||"",j=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"],M=C.toUpperCase().match(/([A-Z]{3})\s+(\d+)-(\d+)/);if(M){let B=M[1],I=parseInt(M[2]),L=j.indexOf(B);if(L!==-1)return I===1?`${B} 16-${new Date(new Date().getFullYear(),L+1,0).getDate()}`:`${j[(L+1)%12]} 1-15`}return"NEW PERIOD"},re=async(C,j)=>{w.fire({title:"Adding...",allowOutsideClick:!1,showConfirmButton:!1}),w.showLoading();try{const M={gip_id:e.id};C==="dtr"&&(M.record_date=j),C==="ar"&&(M.period=j);const B=await xe(`api/logs.php?type=${C}`,M);(B.success?B.data:{success:!1,error:B.error}).success?(w.fire({toast:!0,position:s,icon:"success",title:"Auto-Added!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):w.fire("Error","Failed to add log.","error")}catch(M){w.fire("Error",M.message,"error")}},U=async(C,j,M,B,I)=>{const L=j==="dtr"?"Record Date":"Period";te();const G="flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[10px] sm:text-xs gap-2 ",Q='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>',k='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>',{value:b}=await w.fire({title:`<span class="text-xl font-black text-heading uppercase tracking-tight">Edit ${C} Log</span>`,html:`
                    <div class="flex flex-col gap-5 text-left font-montserrat mt-2">
                        <div>
                            <label class="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-1">${L}</label>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-log-status" value="VERIFIED" class="peer sr-only" ${I==="VERIFIED"?"checked":""}>
                                ...
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-log-status" value="REJECTED" class="peer sr-only" ${I==="REJECTED"||I==="DECLINED"?"checked":""}>
                ...
                `,focusConfirm:!1,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">SAVE REVISIONS</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">CANCEL</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2"},buttonsStyling:!1,preConfirm:()=>{const m=document.querySelector('input[name="swal-log-status"]:checked');return{val:document.getElementById("swal-log-val").value.trim().toUpperCase(),status:m?m.value:"PENDING"}}});if(b&&(b.val!==B||b.status!==I))try{const m={type:j,id:M,status:b.status};j==="dtr"&&(m.record_date=b.val),j==="ar"&&(m.period=b.val);const D=await Pe("api/logs.php",m),A=D.success?D.data:{success:!1,error:D.error};A.success?(w.fire({toast:!0,position:s,icon:"success",title:"Log Updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):w.fire("Error",A.error||"Failed to update log.","error")}catch(m){w.fire("Error",m.message,"error")}},Z=i.querySelector("#add-dtr-log-btn");Z&&Z.addEventListener("click",()=>re("dtr",J()));const q=i.querySelector("#add-ar-log-btn");q&&q.addEventListener("click",()=>re("ar",H())),i.querySelectorAll(".edit-log-btn").forEach(C=>{C.addEventListener("click",j=>{if(j.target.closest(".delete-log-btn"))return;const M=C.dataset.type,B=C.dataset.id,I=C.dataset.val,L=C.dataset.status;U(M.toUpperCase(),M,B,I,L)})}),i.querySelectorAll(".delete-log-btn").forEach(C=>{C.addEventListener("click",async()=>{const j=C.dataset.id,M=C.dataset.type;if((await w.fire({title:'<span class="text-xl font-black text-philippine-red uppercase tracking-tight">Delete item?</span>',text:"This action cannot be undone.",icon:"warning",showCancelButton:!0,confirmButtonText:'<span class="font-black tracking-widest uppercase">Delete</span>',cancelButtonText:'<span class="font-black tracking-widest uppercase">Wait</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100",confirmButton:"bg-philippine-red text-white hover:bg-red-700 text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs px-6 py-2.5 rounded-xl border border-gray-200 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1})).isConfirmed)try{const I=await xe(`api/logs.php?type=${M}`,{log_id:j,action:"delete"});(I.success?I.data:{success:!1,error:I.error}).success?(w.fire({toast:!0,position:s,icon:"success",title:"Deleted",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):w.fire("Error","Failed to delete data.","error")}catch(I){w.fire("Error",I.message,"error")}})})}).catch(console.error)}function ze(e){const o=te(),s=`w-full bg-transparent border-b-2 ${o?"border-slate-700 text-white focus:border-brand placeholder-slate-600":"border-gray-200 text-gray-900 focus:border-brand placeholder-gray-300"} px-1 py-1 text-sm font-black outline-none transition-all focus:ring-0`,a=`w-full bg-transparent border-none ${o?"text-white":"text-royal-blue"} px-0 py-0 text-xl sm:text-2xl font-black leading-tight tracking-tight focus:ring-0 outline-none placeholder-gray-300 resize-none overflow-hidden`,r=["DOLE Field Office","LGU","DEPED","DICT","PCA"];function t(p){if(!p)return"";const i=new Date(p),c=new Date;let g=c.getFullYear()-i.getFullYear();const v=c.getMonth()-i.getMonth();return(v<0||v===0&&c.getDate()<i.getDate())&&g--,g>=0?g:0}function u(p){if(!p)return"bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-white border-gray-200 dark:border-slate-700";const i=String(p).toUpperCase();return i==="ONGOING"?"bg-green-100 text-green-700 border-green-200":i==="EXPIRED"?"bg-red-400 text-white border-red-400":i==="RESIGNED"?"bg-[#ce1126] text-white border-[#ce1126]":i==="ABSORBED"?"bg-[#2e7d32] text-white border-[#2e7d32]":"bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-white border-gray-200 dark:border-slate-700"}const n=`
    ...
    <div class="flex flex-col gap-4 text-sm mt-3 px-1 pb-24">
        ...
        <div class="flex flex-col gap-2 pt-3 mt-1 border-t border-gray-50 dark:border-slate-800/60">
            ...
            <div id="edit-education-suggestions-box" class="hidden mt-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                ${Be.map(p=>`<button type="button" class="edit-education-option w-full text-left px-3 py-2 text-[11px] font-bold text-slate-700 dark:text-slate-200 hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer"><span class="option-text">${p.name}</span></button>`).join("")}
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col gap-4 text-sm mt-4 px-1">
        ...
        <div class="flex flex-col gap-2">
            ...
            <div id="edit-designation-suggestions-box" class="hidden mt-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                ${Ie.map(p=>`<button type="button" class="edit-designation-option w-full text-left px-3 py-2 text-[11px] font-bold text-slate-700 dark:text-slate-200 hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer"><span class="option-text">${p}</span></button>`).join("")}
            </div>
        </div>
...
.pulse-highlight {
    ...
    `;let l=document.getElementById("edit-drawer-container");l&&l.remove(),l=document.createElement("div"),l.id="edit-drawer-container",l.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-white dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",l.setAttribute("tabindex","-1"),l.innerHTML=n,document.body.appendChild(l),setTimeout(()=>{const p=l.querySelector('textarea[name="name"]');p&&(p.style.height="auto",p.style.height=p.scrollHeight+"px")},10),(async()=>{try{const p=await ne("api/beneficiaries.php?get_offices=1");if(p.success&&p.data.offices){const i=l.querySelector("#edit-office-suggestions-box");if(i){const c=["DOLE Field Office","LGU","DEPED","DICT","PCA"],g=[...new Set([...c,...p.data.offices])];i.innerHTML=g.map(v=>`
                    ...
                    `).join("")}}}catch(p){console.error("Error fetching office suggestions:",p)}})(),De(async()=>{const{Drawer:p}=await import("./vendor-flowbite-B7rSfpuT.js").then(i=>i.b);return{Drawer:p}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:p})=>{const i=new p(l,{placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{l&&l.parentNode&&l.remove()},400)}});i.show(),window.initFlowbite&&window.initFlowbite();const c=()=>{document.activeElement&&typeof document.activeElement.blur=="function"&&document.activeElement.blur(),i.hide()};l.querySelector("#close-edit-drawer-btn").addEventListener("click",c),l.querySelector("#edit-drawer-cancel-btn").addEventListener("click",c);const g=l.querySelector("#edit-beneficiary-drawer-form"),v=l.querySelector("#edit-bday-input"),f=l.querySelector("#edit-age-display"),y=l.querySelector("#edit-startDate-input"),$=l.querySelector("#edit-endDate-input"),T=l.querySelector('input[name="seriesNo"]'),R=l.querySelector('input[name="gip_id"]'),J=l.querySelector("#edit-drawer-remarks"),H=l.querySelector("#edit-extension-log-container"),re=()=>{if(!H)return;const k=J.value,b=te();if(k==="ABSORBED"){const D=e.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});H.innerHTML=`
                    <div class="mt-4 pt-4 border-t ${b?"border-slate-800":"border-gray-100"}">
                        <p class="text-[9px] uppercase font-black ${b?"text-green-500":"text-[#2e7d32]"} border-b ${b?"border-slate-800":"border-green-100"} pb-1 flex items-center gap-2 mb-3"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg> Absorption Details</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                            <div class="group">
                                <label class="text-[9px] ${b?"text-green-500":"text-[#2e7d32]"} font-black uppercase block mb-1">Absorption Date</label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                        <svg class="w-3.5 h-3.5 ${b?"text-green-400":"text-green-600"}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                                    </div>
                                    <input type="text" name="absorbDate" value="${D}" class="absorb-date-picker w-full ${b?"bg-slate-800 text-white border-slate-700":"bg-green-50 text-slate-900 border-green-200"} rounded-lg pl-9 pr-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm font-mono" placeholder="MM/DD/YYYY HH:MM AM/PM">
                                </div>
                            </div>
                            <div class="group">
                                <label class="text-[9px] ${b?"text-slate-500":"text-gray-400"} font-black uppercase block mb-1">Where?</label>
                                <input type="text" name="absorb_where" value="${e.absorb_where||""}" class="w-full ${b?"bg-slate-800 text-white border-slate-700":"bg-gray-50 text-slate-900 border-gray-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm" placeholder="Where to absorb?">
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                            <div class="group">
                                <label class="text-[9px] ${b?"text-slate-500":"text-gray-400"} font-black uppercase block mb-1">Position</label>
                            ...
                            <div class="group">
                                <label class="text-[9px] ${b?"text-slate-500":"text-gray-400"} font-black uppercase block mb-1">Agency</label>
                                <input type="text" name="absorb_agency" value="${e.absorb_agency||""}" class="w-full ${b?"bg-slate-800 text-white border-slate-700":"bg-gray-50 text-slate-900 border-gray-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm" placeholder="On what agency?">
                            </div>
                        </div>
                    </div>
                `}else if(k==="RESIGNED"){const D=e.resignedDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});H.innerHTML=`
                    <div class="mt-4 pt-4 border-t ${b?"border-slate-800":"border-gray-100"}">
                        <p class="text-[9px] uppercase font-black ${b?"text-red-500":"text-[#ce1126]"} border-b ${b?"border-slate-800":"border-red-100"} pb-1 flex items-center gap-2 mb-3"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg> Resignation Details</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                            <div class="group">
                                <label class="text-[9px] ${b?"text-red-500":"text-[#ce1126]"} font-black uppercase block mb-1">Resignation Date</label>
                                <div class="relative">
                                    <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
                                        <svg class="w-3.5 h-3.5 ${b?"text-red-400":"text-red-600"}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                    </div>
                                    <input type="text" name="resignedDate" value="${D}" class="resign-date-picker w-full ${b?"bg-slate-800 text-white border-slate-700":"bg-red-50 text-slate-900 border-red-200"} rounded-lg pl-9 pr-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm font-mono" placeholder="MM/DD/YYYY HH:MM AM/PM">
                                </div>
                            </div>
                            <div class="group">
                                <label class="text-[9px] ${b?"text-slate-500":"text-gray-400"} font-black uppercase block mb-1">Reason (Optional)</label>
                                <input type="text" name="resigned_reason" value="${e.resigned_reason||""}" class="w-full ${b?"bg-slate-800 text-white border-slate-700":"bg-gray-50 text-slate-900 border-gray-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm" placeholder="Why resigned?">
                            </div>
                        </div>
                    </div>
                `}else H.innerHTML="";const m=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);m&&H.querySelectorAll(".absorb-date-picker, .resign-date-picker").forEach(A=>{new m(A,{format:"M dd, yyyy",autohide:!0,orientation:"bottom"})})};J&&J.addEventListener("change",k=>{const b="text-[10px] sm:text-[11px] font-black px-2.5 py-2.5 rounded-lg border uppercase tracking-widest shadow-sm outline-none focus:ring-2 focus:ring-brand w-full cursor-pointer transition-colors duration-300 h-[42px]";J.className=`${u(k.target.value)} ${b} editable-indicator`,re(),(J.value==="ABSORBED"||J.value==="RESIGNED")&&setTimeout(()=>{H.scrollIntoView({behavior:"smooth",block:"center"}),H.classList.add("pulse-highlight"),setTimeout(()=>H.classList.remove("pulse-highlight"),1500)},50)}),re();let U=!1,Z=!1;const q=(k,b)=>{k.addEventListener("input",m=>{const D=m.target.value,A=window.__maskDate(D);if(D!==A&&(m.target.value=A),A.length===10){const _=window.__parseFormattedDate(A);_&&b&&(Z||b(_),document.activeElement===k&&k.blur())}}),k.addEventListener("changeDate",m=>{m.detail&&m.detail.date&&b&&(Z||b(m.detail.date),k._datepicker&&k._datepicker.hide())})};v&&q(v,k=>{f&&(!U||!f.value)&&(f.value=window.calculateAge(k))}),y&&q(y,k=>{if($){const m=new Date(k);m.setMonth(m.getMonth()+3),m.setDate(m.getDate()+1);const D=String(m.getMonth()+1).padStart(2,"0"),A=String(m.getDate()).padStart(2,"0"),_=m.getFullYear();$.value=`${D}/${A}/${_}`}const b=k.getFullYear();b>1900&&R&&T&&Promise.all([Y(`api/beneficiaries.php?next_id&year=${encodeURIComponent(b)}`),Y(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(b)}`)]).then(([m,D])=>{const A=m.success&&m.data?.success?m.data.nextId:null,_=D.success&&D.data?.success?D.data.nextSeries:null,oe=String(R.value||"").match(/^ROX-RD-ESIG-(\d{4})-\d{4}$/),d=String(T.value||"").match(/^(\d{4})-\d{2}-\d{3}$/),h=oe?Number(oe[1]):null,x=d?Number(d[1]):null;A&&(h===null||h!==b)&&(R.value=A),_&&(x===null||x!==b)&&(T.value=_)}).catch(m=>{console.error("Edit drawer identifier sync error:",m)})}),$&&q($);const C=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null),j=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null);C&&v&&(v._datepicker=new C(v,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"}));const M=l.querySelector("#edit-date-range-picker");if(j&&M){const k=new j(M,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0,orientation:"auto"});y&&(y._datepicker=k.datepickers[0]),$&&($._datepicker=k.datepickers[1])}e.id&&(Z=!0,Y(`api/beneficiaries.php?id=${encodeURIComponent(e.id)}`).then(k=>{if(k.success&&k.data&&k.data.beneficiary){const b=k.data.beneficiary;if(y&&b.startDate){const m=new Date(b.startDate);isNaN(m)||(y.value=b.startDateFormatted||"",y._datepicker&&y._datepicker.setDate(m))}if($&&b.endDate){const m=new Date(b.endDate);isNaN(m)||($.value=b.endDateFormatted||"",$._datepicker&&$._datepicker.setDate(m))}}setTimeout(()=>{Z=!1},100)}).catch(k=>{console.error("Error fetching accurate beneficiary dates:",k),Z=!1})),f&&f.addEventListener("input",()=>U=!0);const B=(k,b,m)=>{const D=l.querySelector(k),A=l.querySelector(b);if(!D||!A)return;const _=()=>A.classList.add("hidden"),oe=()=>A.classList.remove("hidden");D.addEventListener("focus",oe),D.addEventListener("input",()=>{const d=D.value.toLowerCase().trim();let h=0;A.querySelectorAll(m).forEach(x=>{const E=(x.querySelector(".option-text")?.textContent||x.textContent||"").toLowerCase().includes(d);x.style.display=E?"block":"none",E&&h++}),h>0?oe():_()}),A.addEventListener("click",d=>{const h=d.target.closest(m);h&&(D.value=(h.querySelector(".option-text")?.textContent||h.textContent||"").trim(),_(),D.dispatchEvent(new Event("change")))}),document.addEventListener("click",d=>{!D.contains(d.target)&&!A.contains(d.target)&&_()})};B("#edit-education-input","#edit-education-suggestions-box",".edit-education-option"),B("#edit-designation-input","#edit-designation-suggestions-box",".edit-designation-option"),B("#edit-office-input","#edit-office-suggestions-box",".edit-office-option");const I=l.querySelector("#edit-replacement-input"),L=l.querySelector("#edit-replacement-suggestions-box"),G=l.querySelector("#edit-replacement-loading");let Q=null;I&&L&&(I.addEventListener("input",k=>{const b=k.target.value.trim();clearTimeout(Q),L.classList.add("hidden"),!(b.length<2)&&(G&&G.classList.remove("hidden"),Q=setTimeout(async()=>{try{const m=await ne(`api/beneficiaries.php?replacement_candidates=1&q=${encodeURIComponent(b)}&limit=10`);m.success&&m.data&&m.data.candidates&&m.data.candidates.length>0?(L.innerHTML=m.data.candidates.map(D=>`
                            ...
                            `).join(""),L.classList.remove("hidden")):(L.innerHTML='<div class="px-4 py-3 text-xs text-gray-500 italic text-center font-medium">No active beneficiaries found.</div>',L.classList.remove("hidden"))}catch(m){console.error("Replacement fetch error:",m)}finally{G&&G.classList.add("hidden")}},400))}),L.addEventListener("click",k=>{const b=k.target.closest("button");b&&(I.value=b.dataset.name,L.classList.add("hidden"))}),document.addEventListener("click",k=>{!I.contains(k.target)&&!L.contains(k.target)&&L.classList.add("hidden")})),g.querySelectorAll("input, select, textarea").forEach(k=>{const b=(k.getAttribute("type")||"").toLowerCase(),m=!k.disabled&&!k.readOnly&&b!=="hidden";k.classList.remove("editable-indicator"),m&&k.classList.add("editable-indicator")}),g.addEventListener("submit",k=>{k.preventDefault();const b=new FormData(g),m={};b.forEach((D,A)=>m[A]=D),m.id=e.id,m.gip_id=e.id,window.addBeneficiaryData&&(async()=>await window.addBeneficiaryData(m,!0,!1)&&(c(),window.viewBeneficiary&&setTimeout(()=>window.viewBeneficiary({id:e.id},0),100),w.fire({toast:!0,position:"top-end",icon:"success",title:"Record Updated",showConfirmButton:!1,timer:3e3})))()})})}function it(){window.__maskDate=function(e){let o=e.replace(/\D/g,"").slice(0,8);return o.length>2&&o.length<=4?o=o.slice(0,2)+"/"+o.slice(2):o.length>4&&(o=o.slice(0,2)+"/"+o.slice(2,4)+"/"+o.slice(4)),o},window.__parseFormattedDate=function(e){if(!e)return null;const o=e.split("/");if(o.length===3){const s=parseInt(o[0])-1,a=parseInt(o[1]),r=parseInt(o[2]);if(r>1e3&&s>=0&&s<12&&a>0&&a<=31)return new Date(r,s,a)}return null},window.calculateAge=function(e){if(!e)return"";const o=e instanceof Date?e:new Date(e),s=new Date;let a=s.getFullYear()-o.getFullYear();const r=s.getMonth()-o.getMonth();return(r<0||r===0&&s.getDate()<o.getDate())&&a--,a>=0?a:0},window.viewBeneficiary=async function(e,o=0){const s=e?.id||e?.gip_id||null;if(!s)return;const a=!!(e?.name&&e?.office&&e?.remarks);let r={...e,id:s};if(!a){const n=await Y(`api/beneficiaries.php?id=${encodeURIComponent(s)}`);n.success&&n.data?.success&&n.data?.beneficiary&&(r={...n.data.beneficiary,...r,id:s})}const t=`logs_cache_${s}`;let u=!1;try{if(window.__doleDB&&window.__doleDB.getSecureCache){const $=await window.__doleDB.getSecureCache(t);$&&(r.arLogs=$.arLogs||[],r.dtrLogs=$.dtrLogs||[],r.docs=$.docs||[],pe(r,o),u=!0)}let n=[],l=[],p=[];const[i,c,g,v]=await Promise.all([Y(`api/logs.php?type=ar&gip_id=${encodeURIComponent(s)}`),Y(`api/logs.php?type=dtr&gip_id=${encodeURIComponent(s)}`),Y(`api/logs.php?type=docs&gip_id=${encodeURIComponent(s)}`),Y(`api/logs.php?type=absorption&gip_id=${encodeURIComponent(s)}`)]);n=i.success&&i.data?.success?i.data.logs:[],l=c.success&&c.data?.success?c.data.logs:[],p=g.success&&g.data?.success?g.data.logs:[];const f=v.success&&v.data?.success?v.data.logs:[];if(f.length>0){const $=f[0];r.absorbDate=$.absorption_datetime,r.absorb_where=$.where||$.absorb_where,r.absorb_position=$.position||$.absorb_position,r.absorb_agency=$.agency||$.absorb_agency}const y={arLogs:n,dtrLogs:l,docs:p};window.__doleDB&&window.__doleDB.setSecureCache&&await window.__doleDB.setSecureCache(t,y),r.arLogs=n,r.dtrLogs=l,r.docs=p,u?document.getElementById("beneficiary-drawer-container")&&pe(r,o):pe(r,o)}catch(n){console.error("Error fetching logs/docs:",n),u||(r.arLogs=[],r.dtrLogs=[],r.docs=[],pe(r,o))}},window.showAddDataModal=function(e){be(e)},window.editBeneficiary=function(e){ze(e)},window.showExportConfigModal=function(e){Je(e)},window.showProfileModal=function(){Ye()}}async function Ye(){try{if(Ee()&&me){let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=a.id)}catch{}if(!e)throw new Error("User not logged in");const{data:o,error:s}=await me.from("users").select("*").eq("id",e).single();if(s)throw s;Se(o)}else{let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=`?user_id=${a.id}`)}catch{}const s=await(await fetch(`${X()}api/profile.php${e}`)).json();if(s.success){const a=s.profile;Se(a)}else w.fire({icon:"error",title:"Error",text:s.error||"Failed to load profile"})}}catch(e){console.error("Error fetching profile:",e)}}function Se(e){const o=e.profile_picture_path?`${X()}${e.profile_picture_path}`:null,s=e.full_name?e.full_name.split(" ").map(r=>r[0]).join("").substring(0,2).toUpperCase():"US",a=`
    ...
    `;w.fire({html:u,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:n=>{window.initFlowbite&&window.initFlowbite();const l=n.querySelector("#cancel-modal-btn");l&&l.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),w.close(),e?._isBulk&&ge.onCancel()});const p=n.querySelector("#bulk-add-btn");p&&p.addEventListener("click",()=>{w.close(),ge.init()});const i=(d,h)=>{d.addEventListener("input",x=>{const S=window.__maskDate(x.target.value);if(x.target.value!==S&&(x.target.value=S),S.length===10){const E=window.__parseFormattedDate(S);E&&h&&(h(E),document.activeElement===d&&d.blur())}}),d.addEventListener("changeDate",x=>{if(x.detail&&x.detail.date&&h){h(x.detail.date);const S=d._datepicker||d.parentNode&&d.parentNode._datepicker;S&&typeof S.hide=="function"&&S.hide()}})},c=n.querySelector("#birthday-input"),g=n.querySelector("#age-display");if(c){i(c,h=>{g&&(g.value=window.calculateAge(h),g.classList.add("animate-pulse"),setTimeout(()=>g.classList.remove("animate-pulse"),400))});const d=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);d&&(c._datepicker=new d(c,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"}))}const v=n.querySelector("#name-input-field"),f=n.querySelector("#duplicate-warning");if(v&&f){let d;v.addEventListener("input",h=>{const x=h.target.value.trim();if(clearTimeout(d),x.length<3){f.classList.add("hidden");return}d=setTimeout(async()=>{try{const E=await(await fetch(`${X()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:x})})).json();E.success&&E.exists?(f.classList.remove("hidden"),te()?f.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":f.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):f.classList.add("hidden")}catch(S){console.error("Duplicate check error:",S)}},500)}),e?.name&&(f.classList.add("hidden"),(async()=>{const x=await(await fetch(`${X()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();x.success&&x.exists&&(f.classList.remove("hidden"),te()?f.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":f.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const y=n.querySelector("#full-id-input"),$=n.querySelector("#series-no-input"),T=n.querySelector('input[name="startDate"]'),R=n.querySelector('input[name="endDate"]'),J=n.querySelectorAll('input[name="remarks"]'),H=n.querySelector("#extension-log-container"),re=async d=>{if(!d)return;const h=`cache_next_id_${d}`,x=`cache_next_series_${d}`,S=`cache_expiry_${d}`,E=Date.now(),V=localStorage.getItem(h),P=localStorage.getItem(x),z=localStorage.getItem(S);if(V&&P&&z&&E<parseInt(z,10)){y&&(y.value=V,y.classList.remove("placeholder:text-gray-300")),$&&($.value=P);return}const F=[y,$].filter(Boolean);F.forEach(N=>{N.classList.add("animate-pulse"),N.placeholder="Syncing..."});try{const[N,O]=await Promise.all([Y(`api/beneficiaries.php?next_id&year=${encodeURIComponent(d)}`),Y(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(d)}`)]),K=N.success&&N.data?.success?N.data.nextId:null,W=O.success&&O.data?.success?O.data.nextSeries:null;K&&y&&(y.value=K,localStorage.setItem(h,K)),W&&$&&($.value=W,localStorage.setItem(x,W)),localStorage.setItem(S,(E+3e5).toString())}catch(N){console.error("ID Sync error:",N)}finally{F.forEach(N=>N.classList.remove("animate-pulse"))}},U=n.querySelector("#replacement-search-input"),Z=n.querySelector("#replacement-hidden"),q=n.querySelector("#replacement-suggestions"),C=d=>{const h=(d.name||"").toUpperCase().trim(),x=d.startDateFormatted||d.startDate||"N/A",S=d.endDateFormatted||d.endDate||"N/A";return`${h} (${x.toUpperCase()} - ${S.toUpperCase()})`},j=d=>{if(q){if(!d.length){q.innerHTML=`<div class="px-3 py-2 text-[10px] font-bold ${t.textCourseOpt}">No matching beneficiary found.</div>`,q.classList.remove("hidden");return}q.innerHTML=d.map(h=>{const x=C(h);return`
                    ...
                    `}).join(""),q.classList.remove("hidden"),q.querySelectorAll(".replacement-option").forEach(h=>{h.addEventListener("click",()=>{const x=h.getAttribute("data-replacement")||"";U&&(U.value=x),Z&&(Z.value=x),q.classList.add("hidden")})})}};let M=null;const B=async(d="")=>{const h=(d||"").trim(),x=`api/beneficiaries.php?replacement_candidates=1&limit=20${h?`&q=${encodeURIComponent(h)}`:""}`,S=await Y(x);S.success&&S.data?.success&&Array.isArray(S.data.candidates)&&j(S.data.candidates)};U&&Z&&q&&(U.addEventListener("focus",()=>{B(U.value||"")}),U.addEventListener("input",()=>{Z.value=U.value.trim(),clearTimeout(M),M=setTimeout(()=>{B(U.value||"")},250)}),document.addEventListener("click",d=>{!U.contains(d.target)&&!q.contains(d.target)&&q.classList.add("hidden")}));const I=()=>{const d=n.querySelector('input[name="remarks"]:checked');return d?d.value:"ONGOING"},L=d=>{const h=n.querySelector(`input[name="remarks"][value="${d}"]`);h&&(h.checked=!0,Q())},G=()=>{if(R&&R.value){const d=window.__parseFormattedDate(R.value);if(!d)return;const h=new Date;h.setHours(0,0,0,0);let x="ONGOING";d<h&&(x="EXPIRED"),L(x)}},Q=()=>{if(!H)return;const d=I();if(d==="ABSORBED"){const h=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});H.innerHTML=`
                    ...
                    `}else if(d==="RESIGNED"){const h=e?.resignedDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});H.innerHTML=`
                        <div class="mt-4 pt-4 border-t ${r?"border-slate-800":"border-gray-100"}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                                <div class="${r?"bg-red-900/10 border-red-900/30":"bg-[#ffebee]/50 border-[#ffcdd2]"} rounded-lg p-2.5 flex items-center justify-between border">
                                    <div>
                                        <label class="text-[8px] ${r?"text-red-500":"text-[#ce1126]"} font-bold uppercase block mb-0.5">Resignation Date</label>
                                        <p class="text-[10px] font-black ${r?"text-red-400":"text-[#b71c1c]"} uppercase">${h}</p>
                                        <input type="hidden" name="resignedDate" value="${h}">
                                    </div>
                                    <svg class="w-4 h-4 ${r?"text-red-400":"text-[#ce1126]"}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                </div>
                                <div class="group">
                    ...
                    `}else H.innerHTML=""};if(T){let d=null;i(T,E=>{const V=E.getFullYear();if(R){const P=new Date(E);P.setMonth(P.getMonth()+3),P.setDate(P.getDate()+1);const z=String(P.getMonth()+1).padStart(2,"0"),F=String(P.getDate()).padStart(2,"0"),N=P.getFullYear();R.value=`${z}/${F}/${N}`}G(),V>1900&&V!==d&&(d=V,re(V))}),R&&i(R,()=>G());const h=n.querySelector("#date-range-picker"),x=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null),S=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);if(x&&h&&T&&R){const E=new x(h,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0,orientation:"auto"});T._datepicker=E.datepickers?.[0]||null,R._datepicker=E.datepickers?.[1]||null}else S&&(T&&(T._datepicker=new S(T,{format:"mm/dd/yyyy",autohide:!0,orientation:"auto"})),R&&(R._datepicker=new S(R,{format:"mm/dd/yyyy",autohide:!0,orientation:"auto"})));if(!o){const E=new Date().getFullYear();re(E)}}J.forEach(d=>d.addEventListener("change",Q));const k=n.querySelector("#resign-btn"),b=n.querySelector("#absorb-btn");k&&k.addEventListener("click",()=>L("RESIGNED")),b&&b.addEventListener("click",()=>L("ABSORBED")),n.querySelectorAll('input[type="text"], textarea').forEach(d=>{["id-number-input","full-id-input"].includes(d.id)||d.addEventListener("input",()=>{const h=d.selectionStart,x=d.selectionEnd;d.value=d.value.toUpperCase(),d.setSelectionRange(h,x)})}),G(),Q(),A("education-input","course-suggestions","course-option"),A("designation-input","work-suggestions","work-option"),A("office-input","office-suggestions","office-option");const m={publicSector:["DOLE","LGU","DEPED"],privateSector:["DICT","PCA"]},D=(d=[])=>{const h=n.querySelector("#office-suggestions");if(!h)return;const x=N=>String(N||"").trim().toUpperCase(),S=new Set(m.publicSector.map(x)),E=new Set(m.privateSector.map(x)),V=new Set([...S,...E]),z=[...new Set((d||[]).map(x).filter(Boolean))].filter(N=>!V.has(N)),F=(N,O)=>O.length?`
                        <div class="px-2.5 py-1 text-[8px] font-black uppercase tracking-widest ${t.textLabel} opacity-70">${N}</div>
                        ${O.map(K=>`
                            <div class="office-option px-3 py-2 text-[10px] font-bold ${t.textCourseOpt} ${t.courseHover} rounded-md cursor-pointer transition-colors flex items-center gap-2.5 active:scale-[0.98]">
                                <span class="option-text">${K}</span>
                            </div>
                        `).join("")}
                    `:"";h.innerHTML=[F("Public Sector",[...S]),F("Private Sector",[...E]),F("Other Offices",z)].join("")};D(),(async()=>{try{const d=await Y("api/beneficiaries.php?get_offices=1");d.success&&d.data?.success&&Array.isArray(d.data.offices)&&D(d.data.offices)}catch(d){console.error("Office suggestion sync failed:",d)}})();function A(d,h,x){const S=n.querySelector(`#${d}`),E=n.querySelector(`#${h}`);if(!S||!E)return;let V=!1;S.addEventListener("focus",()=>E.classList.remove("hidden")),document.addEventListener("click",P=>{!S.contains(P.target)&&!E.contains(P.target)&&E.classList.add("hidden")}),S.addEventListener("input",()=>{if(V){V=!1;return}const P=S.value.toLowerCase(),z=E.querySelectorAll(`.${x}`);let F=!1;z.forEach(N=>{const O=N.querySelector(".option-text");(O?O.innerText:N.innerText).toLowerCase().includes(P)?(N.style.display="block",F=!0):N.style.display="none"}),F?E.classList.remove("hidden"):E.classList.add("hidden")}),E.addEventListener("click",P=>{const z=P.target.closest(`.${x}`);if(!z)return;const F=z.querySelector(".option-text");S.value=F?F.innerText.trim():z.innerText.trim(),V=!0,E.classList.add("hidden"),S.dispatchEvent(new Event("change"))})}const _=n.querySelector("#add-beneficiary-form"),oe="add_beneficiary_draft";if(!o){const d=localStorage.getItem(oe);if(d)try{const h=JSON.parse(d);Object.entries(h).forEach(([x,S])=>{const E=_.elements[x];E&&E.type!=="file"&&E.type!=="hidden"&&(E.value=S)})}catch(h){console.error("Error loading draft",h)}}_.addEventListener("input",d=>{if(!o){const h=new FormData(_),x={};h.forEach((S,E)=>x[E]=S),localStorage.setItem(oe,JSON.stringify(x))}}),_&&_.addEventListener("submit",d=>{d.preventDefault(),_.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(W=>{W.classList.remove("ring-2","ring-red-500","!border-red-500")});const x=new FormData(_);let S=!1;const E=W=>{const ee=_.querySelector(`[name="${W}"]`);ee&&ee.classList.add("ring-2","ring-red-500","!border-red-500"),S=!0},V=x.get("name"),P=x.get("contact"),z=x.get("startDate"),F=x.get("endDate"),N=(x.get("designation")||"").trim();if((!V||V.trim()===""||/[0-9]/.test(V))&&E("name"),P&&P.trim()!==""&&/[^0-9]/.test(P.replace(/[\s\-\+\(\)]/g,""))&&E("contact"),z||E("startDate"),F||E("endDate"),S)return;const O={};x.forEach((W,ee)=>{if(["birthday","startDate","endDate"].includes(ee)){const se=window.__parseFormattedDate(W);if(se){const fe=se.getFullYear(),he=String(se.getMonth()+1).padStart(2,"0"),Me=String(se.getDate()).padStart(2,"0");O[ee]=`${fe}-${he}-${Me}`}else O[ee]=W}else O[ee]=W}),N||(O.designation="N/A"),O.replacement||(O.replacement="");const K=n.querySelector("#full-id-input")?.value;o?(O.id=e?.id,K&&(O.gip_id=K)):(O.id=null,K&&(O.gip_id=K)),window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(O)){if(!o){const ee="add_beneficiary_draft",se=_.querySelector('[name="office"]')?.value||"",fe=_.querySelector('[name="designation"]')?.value||"",he=_.querySelector('[name="education"]')?.value||"";localStorage.setItem(ee,JSON.stringify({office:se,designation:fe,education:he}))}w.close(),setTimeout(()=>{w.fire({toast:!0,position:"top-end",icon:"success",title:`Record ${o?"Updated":"Added"} Successfully`,showConfirmButton:!1,timer:3e3,timerProgressBar:!0}),!o&&e?._isBulk&&ge.onSaveSuccess()},100)}else w.fire({icon:"error",title:"Save Failed",text:"There was an error saving the record to the database."})})()})}})}window.handleContactSubmit=async function(e){e.preventDefault();const o=e.target,s=o.querySelector('button[type="submit"]'),a=s.innerHTML;s.disabled=!0,s.innerHTML=`
        <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        Sending...
    `;try{const r=new FormData(o);if((await fetch(o.action,{method:"POST",body:r,headers:{Accept:"application/json"}})).ok)w.fire({toast:!0,position:"top-end",icon:"success",title:"Message Sent Successfully!",text:"Thank you for reaching out. I will get back to you soon!",showConfirmButton:!1,timer:4e3,timerProgressBar:!0,didOpen:u=>{u.addEventListener("mouseenter",w.stopTimer),u.addEventListener("mouseleave",w.resumeTimer)},customClass:{popup:"rounded-2xl border-l-4 border-teal-500 shadow-2xl"}}),o.reset();else throw new Error("Failed to send")}catch{w.fire({toast:!0,position:"top-end",icon:"error",title:"Oops! Something went wrong.",text:"Could not send your message. Please try again later.",showConfirmButton:!1,timer:4e3,customClass:{popup:"rounded-2xl border-l-4 border-red-500 shadow-2xl"}})}finally{s.disabled=!1,s.innerHTML=a}return!1};export{De as _,Y as a,ne as b,rt as c,st as d,at as e,Ke as f,X as g,nt as h,Ee as i,Qe as j,et as k,ae as l,tt as m,it as n,Te as p,ot as r,me as s,Ze as u};
