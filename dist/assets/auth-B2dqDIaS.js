        ...
        <div class="flex flex-col gap-3">
            ${n.map(i=>{const u=i.status.toUpperCase(),x={VERIFIED:"COMPLETED",DECLINED:"REJECTED",PENDING:"PENDING"}[u]||u,b=x==="COMPLETED",m=x==="REJECTED",L=b?"text-green-500":m?"text-red-500":"text-gray-400 dark:text-gray-500",S=b?"bg-green-50/50 dark:bg-green-900/10":m?"bg-red-50/50 dark:bg-red-900/10":"bg-gray-50/50 dark:bg-slate-800/50",I=b?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800 hover:bg-green-200 cursor-pointer":m?"bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800 hover:bg-red-200 cursor-pointer":"bg-white text-gray-500 border-gray-200 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer";let A='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';return b?A='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>':m&&(A='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>'),`
                <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-0.5 ${S}">
                    ...
                    <button type="button" class="ml-3 ${I} text-[10px] font-black px-3 py-1.5 rounded-full border uppercase tracking-widest transition-colors flex-shrink-0 drawer-doc-btn" data-id="${i.id}" data-name="${i.name}" data-status="${x} cursosr-pointer">
                        ${x}
                    </button>
                </div>
                `}).join("")}
...
.editable-indicator {
    ...
    `;let n=document.getElementById("edit-drawer-container");n&&n.remove(),n=document.createElement("div"),n.id="edit-drawer-container",n.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-white dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",n.setAttribute("tabindex","-1"),n.innerHTML=d,document.body.appendChild(n),setTimeout(()=>{const c=n.querySelector('textarea[name="name"]');c&&(c.style.height="auto",c.style.height=c.scrollHeight+"px")},10),(async()=>{try{const c=await re("api/beneficiaries.php?get_offices=1");if(c.success&&c.data.offices){const l=n.querySelector("#edit-office-suggestions-box");if(l){const i=["DOLE Field Office","LGU","DEPED","DICT","PCA"],u=[...new Set([...i,...c.data.offices])];l.innerHTML=u.map(g=>`
                    ...
                    `).join("")}}}catch(c){console.error("Error fetching office suggestions:",c)}})(),ue(async()=>{const{Drawer:c}=await import("./vendor-flowbite-B2TFTxm5.js");return{Drawer:c}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:c})=>{const l=new c(n,{placement:"right",backdrop:!0,bodyScrolling:!1,backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{n.parentNode&&n.remove()},300)}});l.show(),n.querySelector("#close-edit-drawer-btn").addEventListener("click",()=>l.hide()),n.querySelector("#edit-drawer-cancel-btn").addEventListener("click",()=>l.hide());const i=n.querySelector("#edit-beneficiary-drawer-form"),u=n.querySelector("#edit-bday-input"),g=n.querySelector("#edit-age-display");let x=!1;u&&g&&(g.addEventListener("input",()=>{x=!0}),u.addEventListener("change",m=>{m.target.value&&(!x||!g.value)&&(g.value=t(m.target.value))}));const b=(m,L,S)=>{const I=n.querySelector(m),A=n.querySelector(L);if(!I||!A)return;const P=()=>A.classList.add("hidden"),F=()=>A.classList.remove("hidden");I.addEventListener("focus",F),I.addEventListener("input",()=>{const N=I.value.toLowerCase().trim();let O=0;A.querySelectorAll(S).forEach(w=>{const E=(w.querySelector(".option-text")?.textContent||w.textContent||"").toLowerCase().includes(N);w.style.display=E?"block":"none",E&&O++}),O>0?F():P()}),A.addEventListener("click",N=>{const O=N.target.closest(S);O&&(I.value=(O.querySelector(".option-text")?.textContent||O.textContent||"").trim(),P(),I.dispatchEvent(new Event("change")))}),document.addEventListener("click",N=>{!I.contains(N.target)&&!A.contains(N.target)&&P()})};b("#edit-education-input","#edit-education-suggestions-box",".edit-education-option"),b("#edit-designation-input","#edit-designation-suggestions-box",".edit-designation-option"),b("#edit-office-input","#edit-office-suggestions-box",".edit-office-option"),i.querySelectorAll("input, select, textarea").forEach(m=>{const L=(m.getAttribute("type")||"").toLowerCase(),S=!m.disabled&&!m.readOnly&&L!=="hidden";m.classList.remove("editable-indicator"),S&&m.classList.add("editable-indicator")}),i.addEventListener("submit",m=>{m.preventDefault();const L=new FormData(i),S={};L.forEach((I,A)=>S[A]=I),S.id=e.id,S.gip_id=e.id,window.addBeneficiaryData&&(async()=>await window.addBeneficiaryData(S,!0,!1)&&(l.hide(),window.viewBeneficiary&&setTimeout(()=>window.viewBeneficiary({id:e.id},0),100),p.fire({toast:!0,position:"top-end",icon:"success",title:"Record Updated",showConfirmButton:!1,timer:3e3})))()})})}function ie(e="Incorrect Username or Password"){p.fire({html:`
            <div class="p-6">
    ...
    `;p.fire({html:d,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:n=>{const c=n.querySelector("#cancel-modal-btn");c&&c.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),p.close(),e?._isBulk&&Z.onCancel()});const l=n.querySelector("#bulk-add-btn");l&&l.addEventListener("click",()=>{p.close(),Z.init()});const i=n.querySelector("#birthday-input"),u=n.querySelector("#age-display");i&&u&&(i.addEventListener("change",f=>{f.target.value&&(u.value=le(f.target.value))}),i.value&&!e?.age&&(u.value=le(i.value)));const g=n.querySelector("#name-input-field"),x=n.querySelector("#duplicate-warning");if(g&&x){let f;g.addEventListener("input",h=>{const $=h.target.value.trim();if(clearTimeout(f),$.length<3){x.classList.add("hidden");return}f=setTimeout(async()=>{try{const v=await(await fetch(`${D()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:$})})).json();v.success&&v.exists?(x.classList.remove("hidden"),_()?x.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":x.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):x.classList.add("hidden")}catch(k){console.error("Duplicate check error:",k)}},500)}),e?.name&&(x.classList.add("hidden"),(async()=>{const $=await(await fetch(`${D()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();$.success&&$.exists&&(x.classList.remove("hidden"),_()?x.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":x.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const b=n.querySelector("#full-id-input"),m=n.querySelector("#series-no-input"),L=n.querySelector('input[name="startDate"]'),S=n.querySelector('input[name="endDate"]'),I=n.querySelectorAll('input[name="remarks"]'),A=n.querySelector("#extension-log-container"),P=f=>{o||(b&&(b.classList.add("animate-pulse"),b.placeholder="Loading...",fetch(`${D()}api/beneficiaries.php?next_id&year=${f}`).then(h=>h.json()).then(h=>{b.classList.remove("animate-pulse"),h.success&&h.nextId&&(b.value=h.nextId)})),m&&(m.classList.add("animate-pulse"),m.placeholder="Loading...",fetch(`${D()}api/beneficiaries.php?next_series_no&year=${f}`).then(h=>h.json()).then(h=>{m.classList.remove("animate-pulse"),h.success&&h.nextSeries&&(m.value=h.nextSeries)})))},F=()=>{const f=n.querySelector('input[name="remarks"]:checked');return f?f.value:"ONGOING"},N=f=>{const h=n.querySelector(`input[name="remarks"][value="${f}"]`);h&&(h.checked=!0,w())},O=()=>{if(S&&S.value){const f=S.value.split("-"),h=new Date(f[0],f[1]-1,f[2]),$=new Date;$.setHours(0,0,0,0);let k="ONGOING";h<$&&(k="EXPIRED"),N(k)}},w=()=>{if(!A)return;if(F()==="ABSORBED"){const h=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});A.innerHTML=`
