            ...
            ${d.map(l=>{const u=l.status.toUpperCase(),m={VERIFIED:"COMPLETED",DECLINED:"REJECTED",PENDING:"PENDING"}[u]||u,x=m==="COMPLETED",v=m==="REJECTED",E=x?"text-green-500":v?"text-red-500":"text-gray-400 dark:text-gray-500",I=x?"bg-green-50/50 dark:bg-green-900/10":v?"bg-red-50/50 dark:bg-red-900/10":"bg-gray-50/50 dark:bg-slate-800/50",M=x?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800 hover:bg-green-200 cursor-pointer":v?"bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800 hover:bg-red-200 cursor-pointer":"bg-white text-gray-500 border-gray-200 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer";let P='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';return x?P='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>':v&&(P='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>'),`
                <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-0.5 ${I}">
                    <div class="flex items-center gap-3 w-full">
                        <div class="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center bg-white dark:bg-slate-800 shadow-sm border border-gray-100 dark:border-slate-700 ${E}">
                            ${P}
                        </div>
                        <span class="text-xs sm:text-sm font-black ${x?"text-heading":"text-gray-500 dark:text-gray-400"} uppercase tracking-tight flex-1">${l.name}</span>
                    </div>
    ...
    <div id="drawer-page-2" class="hidden flex-1 flex flex-col gap-6">
         ...
         <div class="flex flex-col gap-5">
            <div>
                ...
                <div class="space-y-2">
                    ${t.length?t.map(l=>{const u=l.status||"PENDING";let b=u==="VERIFIED"||u==="COMPLETED"?"text-green-500":u==="REJECTED"||u==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500";return`
            ...
            <div>
                ...
                <div class="space-y-2">
                    ${r.length?r.map(l=>{const u=l.status||"PENDING";let b=u==="VERIFIED"||u==="COMPLETED"?"text-green-500":u==="REJECTED"||u==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500";return`
...
<style>
    ...
    `;let i=document.getElementById("beneficiary-drawer-container");i&&i.remove(),i=document.createElement("div"),i.id="beneficiary-drawer-container",i.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-neutral-primary-soft dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl",i.setAttribute("tabindex","-1"),i.setAttribute("data-drawer-backdrop","true"),i.innerHTML=p,document.body.appendChild(i),ye(async()=>{const{Drawer:l}=await import("./vendor-flowbite-bBKPL--D.js");return{Drawer:l}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:l})=>{const u={placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{i&&i.parentNode&&i.remove()},300)}},b=new l(i,u);b.show(),i.querySelector("#close-drawer-btn").addEventListener("click",()=>b.hide());const x=i.querySelector("#drawer-prev-btn"),v=i.querySelector("#drawer-next-btn"),E=2,I=()=>{i.querySelectorAll("[id^=drawer-page-]").forEach((k,S)=>{k.classList.toggle("hidden",S!==a)}),x.disabled=a===0,v.disabled=a===E,x.classList.toggle("opacity-50",a===0),v.classList.toggle("opacity-50",a===E)};x.addEventListener("click",()=>{a>0&&a--,I()}),v.addEventListener("click",()=>{a<E&&a++,I()}),I(),i.querySelectorAll(".drawer-doc-btn").forEach(k=>{k.addEventListener("click",async()=>{const S=k.dataset.name,C=k.dataset.status,L=await f.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Update Document</span>',html:`
...
.editable-indicator {
    ...
    `;f.fire({html:c,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:n=>{const d=n.querySelector("#cancel-modal-btn");d&&d.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),f.close(),e?._isBulk&&de.onCancel()});const p=n.querySelector("#bulk-add-btn");p&&p.addEventListener("click",()=>{f.close(),de.init()});const i=n.querySelector("#birthday-input"),l=n.querySelector("#age-display");i&&l&&(i.addEventListener("change",g=>{g.target.value&&(l.value=ve(g.target.value))}),i.value&&!e?.age&&(l.value=ve(i.value)));const u=n.querySelector("#name-input-field"),b=n.querySelector("#duplicate-warning");if(u&&b){let g;u.addEventListener("input",h=>{const w=h.target.value.trim();if(clearTimeout(g),w.length<3){b.classList.add("hidden");return}g=setTimeout(async()=>{try{const B=await(await fetch(`${_()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:w})})).json();B.success&&B.exists?(b.classList.remove("hidden"),z()?b.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":b.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):b.classList.add("hidden")}catch(y){console.error("Duplicate check error:",y)}},500)}),e?.name&&(b.classList.add("hidden"),(async()=>{const w=await(await fetch(`${_()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();w.success&&w.exists&&(b.classList.remove("hidden"),z()?b.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":b.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const m=n.querySelector("#full-id-input"),x=n.querySelector("#series-no-input"),v=n.querySelector('input[name="startDate"]'),E=n.querySelector('input[name="endDate"]'),I=n.querySelectorAll('input[name="remarks"]'),M=n.querySelector("#extension-log-container"),P=async g=>{if(!o){if(m){m.classList.add("animate-pulse"),m.placeholder="Loading...";try{const h=await F(`api/beneficiaries.php?next_id&year=${encodeURIComponent(g)}`);h.success&&h.data?.success&&h.data?.nextId&&(m.value=h.data.nextId)}finally{m.classList.remove("animate-pulse")}}if(x){x.classList.add("animate-pulse"),x.placeholder="Loading...";try{const h=await F(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(g)}`);h.success&&h.data?.success&&h.data?.nextSeries&&(x.value=h.data.nextSeries)}finally{x.classList.remove("animate-pulse")}}}},N=n.querySelector("#replacement-search-input"),O=n.querySelector("#replacement-hidden"),D=n.querySelector("#replacement-suggestions"),U=g=>{const h=(g.name||"").toUpperCase().trim(),w=g.startDateFormatted||g.startDate||"N/A",y=g.endDateFormatted||g.endDate||"N/A";return`${h} (${w.toUpperCase()} - ${y.toUpperCase()})`},k=g=>{if(D){if(!g.length){D.innerHTML=`<div class="px-3 py-2 text-[10px] font-bold ${t.textCourseOpt}">No matching beneficiary found.</div>`,D.classList.remove("hidden");return}D.innerHTML=g.map(h=>{const w=U(h);return`
                    ...
                    `}).join(""),D.classList.remove("hidden"),D.querySelectorAll(".replacement-option").forEach(h=>{h.addEventListener("click",()=>{const w=h.getAttribute("data-replacement")||"";N&&(N.value=w),O&&(O.value=w),D.classList.add("hidden")})})}};let S=null;const C=async(g="")=>{const h=(g||"").trim(),w=`api/beneficiaries.php?replacement_candidates=1&limit=20${h?`&q=${encodeURIComponent(h)}`:""}`,y=await F(w);y.success&&y.data?.success&&Array.isArray(y.data.candidates)&&k(y.data.candidates)};N&&O&&D&&(N.addEventListener("focus",()=>{C(N.value||"")}),N.addEventListener("input",()=>{O.value=N.value.trim(),clearTimeout(S),S=setTimeout(()=>{C(N.value||"")},250)}),document.addEventListener("click",g=>{!N.contains(g.target)&&!D.contains(g.target)&&D.classList.add("hidden")}));const L=()=>{const g=n.querySelector('input[name="remarks"]:checked');return g?g.value:"ONGOING"},$=g=>{const h=n.querySelector(`input[name="remarks"][value="${g}"]`);h&&(h.checked=!0,G())},j=()=>{if(E&&E.value){const g=E.value.split("-"),h=new Date(g[0],g[1]-1,g[2]),w=new Date;w.setHours(0,0,0,0);let y="ONGOING";h<w&&(y="EXPIRED"),$(y)}},G=()=>{if(!M)return;if(L()==="ABSORBED"){const h=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});M.innerHTML=`
