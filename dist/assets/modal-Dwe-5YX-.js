...
import m from"./vendor-swal-BSk0fVSb.js";import{a as Ae}from"./vendor-DHtNC8Ux.js";const Ne="true".toLowerCase()==="true";function Ee(){return Ne}function W(){const e=window.location.pathname,o="/dole-system/",s=e.toLowerCase().indexOf(o.toLowerCase());if(s!==-1)return e.substring(0,s+o.length);const a=e.indexOf("/frontend/");if(a!==-1)return e.substring(0,a+1);const r=e.indexOf("/backend/");return r!==-1?e.substring(0,r+1):"/"}function we(e="Incorrect Username or Password"){m.fire({html:`
            <div class="p-6">
...
<div class="mt-8 pt-2 border-t border-default relative">
    ...
    <div id="drawer-page-1" class="hidden flex-1 flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-3">
            ${l.map(c=>{const g=c.status.toUpperCase(),f={VERIFIED:"COMPLETED",DECLINED:"REJECTED",PENDING:"PENDING"}[g]||g,k=f==="COMPLETED",y=f==="REJECTED",T=k?"text-green-500":y?"text-red-500":"text-gray-400 dark:text-gray-500",j=k?"bg-green-50/50 dark:bg-green-900/10":y?"bg-red-50/50 dark:bg-red-900/10":"bg-gray-50/50 dark:bg-slate-800/50",te=k?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800 hover:bg-green-200 cursor-pointer":y?"bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800 hover:bg-red-200 cursor-pointer":"bg-white text-gray-500 border-gray-200 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer";let Y='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';return k?Y='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>':y&&(Y='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>'),`
                <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-0.5 ${j}">
                    <div class="flex items-center gap-3 w-full">
                        ...
                        <span class="text-xs sm:text-sm font-black ${k?"text-heading":"text-gray-500 dark:text-gray-400"} uppercase tracking-tight flex-1">${c.name}</span>
                    </div>
                    <button type="button" class="ml-3 ${te} text-[10px] font-black px-3 py-1.5 rounded-full border uppercase tracking-widest transition-colors flex-shrink-0 drawer-doc-btn" data-id="${c.id}" data-name="${c.name}" data-status="${f} cursosr-pointer">
                        ${f}
                    </button>
                </div>
                `}).join("")}
    ...
    <div id="drawer-page-2" class="hidden flex-1 flex flex-col gap-6">
         ...
         <div class="flex flex-col gap-5">
            <div>
                ...
                <div class="space-y-2">
                    ${t.length?t.map(c=>{const g=c.status||"PENDING";let h=g==="VERIFIED"||g==="COMPLETED"?"text-green-500":g==="REJECTED"||g==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500",f=c.date||c.createdAt,k=f;if(f){const y=/^\d{4}-\d{2}-\d{2}$/.test(f)?new Date(f+"T00:00:00Z"):new Date(f);isNaN(y)||(k=y.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric",timeZone:"Asia/Manila"}).toUpperCase())}return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-blue-100 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors edit-log-btn" data-type="dtr" data-id="${c.id}" data-val="${f}" data-status="${g}">
                            <span class="text-xs font-black text-royal-blue dark:text-blue-400 capitalize whitespace-nowrap pointer-events-none">${k}</span>
                            <span class="text-[11px] font-bold ${h} uppercase tracking-widest truncate max-w-[50%] text-right pr-6 group-hover:pr-12 pointer-events-none transition-all">${g}</span>
            ...
            <div>
                ...
                <div class="space-y-2">
                    ${r.length?r.map(c=>{const g=c.status||"PENDING";let h=g==="VERIFIED"||g==="COMPLETED"?"text-green-500":g==="REJECTED"||g==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500",f=c.period||c.createdAt,k=f;if(f){const y=/^\d{4}-\d{2}-\d{2}$/.test(f)?new Date(f+"T00:00:00Z"):new Date(f);isNaN(y)||(k=y.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric",timeZone:"Asia/Manila"}).toUpperCase())}return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-orange-100 dark:border-orange-900/50 bg-orange-50/50 dark:bg-orange-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-orange-100 dark:hover:bg-orange-900/40 transition-colors edit-log-btn" data-type="ar" data-id="${c.id}" data-val="${f}" data-status="${g}">
                            <span class="text-xs font-black text-orange-600 dark:text-orange-400 capitalize whitespace-nowrap pointer-events-none">${k}</span>
                            <span class="text-[11px] font-bold ${h} uppercase tracking-widest truncate max-w-[50%] text-right pr-6 group-hover:pr-12 pointer-events-none transition-all">${g}</span>
...
<style>
    ...
    `;let i=document.getElementById("beneficiary-drawer-container");i&&i.remove(),i=document.createElement("div"),i.id="beneficiary-drawer-container",i.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-neutral-primary-soft dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl",i.setAttribute("tabindex","-1"),i.setAttribute("data-drawer-backdrop","true"),i.innerHTML=p,document.body.appendChild(i),Ce(async()=>{const{Drawer:c}=await import("./vendor-flowbite-B7rSfpuT.js").then(g=>g.b);return{Drawer:c}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:c})=>{const g={placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{i&&i.parentNode&&i.remove()},300)}},h=new c(i,g);h.show(),i.querySelector("#close-drawer-btn").addEventListener("click",()=>h.hide());const k=i.querySelector("#drawer-prev-btn"),y=i.querySelector("#drawer-next-btn"),T=2,j=()=>{i.querySelectorAll("[id^=drawer-page-]").forEach((L,A)=>{L.classList.toggle("hidden",A!==a)}),k.disabled=a===0,y.disabled=a===T,k.classList.toggle("opacity-50",a===0),y.classList.toggle("opacity-50",a===T)};k.addEventListener("click",()=>{a>0&&a--,j()}),y.addEventListener("click",()=>{a<T&&a++,j()}),j(),i.querySelectorAll(".drawer-doc-btn").forEach(L=>{L.addEventListener("click",async()=>{const A=L.dataset.name,B=L.dataset.status,D="flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[9px] gap-2 transition-all duration-300 ",M=await m.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Update Document</span>',html:`
                        <div class="font-montserrat text-left">
                            <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-4 ps-1">Set status for <span class="text-brand font-black">${A}</span></label>
                            
                            <div class="grid grid-cols-3 gap-3">
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-doc-status" value="PENDING" class="peer sr-only" ${B==="PENDING"?"checked":""}>
                                ...
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-doc-status" value="COMPLETED" class="peer sr-only" ${B==="COMPLETED"?"checked":""}>
                                ...
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-doc-status" value="REJECTED" class="peer sr-only" ${B==="REJECTED"?"checked":""}>
                    ...
                    `,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Update Status</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Cancel</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2"},buttonsStyling:!1,preConfirm:()=>{const R=document.querySelector('input[name="swal-doc-status"]:checked');return R?R.value:null}});if(M.isConfirmed){const R=M.value;if(R===B)return;try{const E={COMPLETED:"VERIFIED",REJECTED:"DECLINED",PENDING:"PENDING"}[R]||R,x=await xe("api/logs.php?type=docs",{gip_id:e.id,doc_name:A,status:E}),C=x.success?x.data:{success:!1,error:x.error};C.success?(m.fire({toast:!0,position:s,icon:"success",title:"Status updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):m.fire("Error",C.error||"Failed to update","error")}catch(w){m.fire("Error",w.message,"error")}}})});const te=()=>{if(!t.length)return new Date().toISOString().split("T")[0];const L=new Date(t[0].date);return L.setDate(L.getDate()+1),L.toISOString().split("T")[0]},Y=()=>{if(!r.length){const D=new Date,M=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"][D.getMonth()];return D.getDate()<=15?`${M} 1-15`:`${M} 16-${new Date(D.getFullYear(),D.getMonth()+1,0).getDate()}`}const L=r[0].period||"",A=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"],B=L.toUpperCase().match(/([A-Z]{3})\s+(\d+)-(\d+)/);if(B){let D=B[1],M=parseInt(B[2]),R=A.indexOf(D);if(R!==-1)return M===1?`${D} 16-${new Date(new Date().getFullYear(),R+1,0).getDate()}`:`${A[(R+1)%12]} 1-15`}return"NEW PERIOD"},K=async(L,A)=>{m.fire({title:"Adding...",allowOutsideClick:!1,showConfirmButton:!1}),m.showLoading();try{const B={gip_id:e.id};L==="dtr"&&(B.record_date=A),L==="ar"&&(B.period=A);const D=await xe(`api/logs.php?type=${L}`,B);(D.success?D.data:{success:!1,error:D.error}).success?(m.fire({toast:!0,position:s,icon:"success",title:"Auto-Added!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):m.fire("Error","Failed to add log.","error")}catch(B){m.fire("Error",B.message,"error")}},G=async(L,A,B,D,M)=>{const R=A==="dtr"?"Record Date":"Period";oe();const w="flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[10px] sm:text-xs gap-2 ",E='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>',x='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>',{value:C}=await m.fire({title:`<span class="text-xl font-black text-heading uppercase tracking-tight">Edit ${L} Log</span>`,html:`
                    <div class="flex flex-col gap-5 text-left font-montserrat mt-2">
                        <div>
                            <label class="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-1">${R}</label>
                ...
                `,focusConfirm:!1,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">SAVE REVISIONS</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">CANCEL</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2"},buttonsStyling:!1,preConfirm:()=>{const I=document.querySelector('input[name="swal-log-status"]:checked');return{val:document.getElementById("swal-log-val").value.trim().toUpperCase(),status:I?I.value:"PENDING"}}});if(C&&(C.val!==D||C.status!==M))try{const I={type:A,id:B,status:C.status};A==="dtr"&&(I.record_date=C.val),A==="ar"&&(I.period=C.val);const q=await Oe("api/logs.php",I),Z=q.success?q.data:{success:!1,error:q.error};Z.success?(m.fire({toast:!0,position:s,icon:"success",title:"Log Updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):m.fire("Error",Z.error||"Failed to update log.","error")}catch(I){m.fire("Error",I.message,"error")}},ee=i.querySelector("#add-dtr-log-btn");ee&&ee.addEventListener("click",()=>K("dtr",te()));const F=i.querySelector("#add-ar-log-btn");F&&F.addEventListener("click",()=>K("ar",Y())),i.querySelectorAll(".edit-log-btn").forEach(L=>{L.addEventListener("click",A=>{if(A.target.closest(".delete-log-btn"))return;const B=L.dataset.type,D=L.dataset.id,M=L.dataset.val,R=L.dataset.status;G(B.toUpperCase(),B,D,M,R)})}),i.querySelectorAll(".delete-log-btn").forEach(L=>{L.addEventListener("click",async()=>{const A=L.dataset.id,B=L.dataset.type;if((await m.fire({title:'<span class="text-xl font-black text-philippine-red uppercase tracking-tight">Delete item?</span>',text:"This action cannot be undone.",icon:"warning",showCancelButton:!0,confirmButtonText:'<span class="font-black tracking-widest uppercase">Delete</span>',cancelButtonText:'<span class="font-black tracking-widest uppercase">Wait</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100",confirmButton:"bg-philippine-red text-white hover:bg-red-700 text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs px-6 py-2.5 rounded-xl border border-gray-200 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1})).isConfirmed)try{const M=await xe(`api/logs.php?type=${B}`,{log_id:A,action:"delete"});(M.success?M.data:{success:!1,error:M.error}).success?(m.fire({toast:!0,position:s,icon:"success",title:"Deleted",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):m.fire("Error","Failed to delete data.","error")}catch(M){m.fire("Error",M.message,"error")}})})}).catch(console.error)}function ze(e){const o=oe(),s=`w-full bg-transparent border-b-2 ${o?"border-slate-700 text-white focus:border-brand placeholder-slate-600":"border-gray-200 text-gray-900 focus:border-brand placeholder-gray-300"} px-1 py-1 text-sm font-black outline-none transition-all focus:ring-0`,a=`w-full bg-transparent border-none ${o?"text-white":"text-royal-blue"} px-0 py-0 text-xl sm:text-2xl font-black leading-tight tracking-tight focus:ring-0 outline-none placeholder-gray-300 resize-none overflow-hidden`,r=["DOLE Field Office","LGU","DEPED","DICT","PCA"];function t(p){if(!p)return"";const i=new Date(p),c=new Date;let g=c.getFullYear()-i.getFullYear();const h=c.getMonth()-i.getMonth();return(h<0||h===0&&c.getDate()<i.getDate())&&g--,g>=0?g:0}function u(p){if(!p)return"bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-white border-gray-200 dark:border-slate-700";const i=String(p).toUpperCase();return i==="ONGOING"?"bg-green-100 text-green-700 border-green-200":i==="EXPIRED"?"bg-red-400 text-white border-red-400":i==="RESIGNED"?"bg-[#ce1126] text-white border-[#ce1126]":i==="ABSORBED"?"bg-[#2e7d32] text-white border-[#2e7d32]":"bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-white border-gray-200 dark:border-slate-700"}const n=`
...
.editable-indicator {
    ...
    `;let l=document.getElementById("edit-drawer-container");l&&l.remove(),l=document.createElement("div"),l.id="edit-drawer-container",l.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-white dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",l.setAttribute("tabindex","-1"),l.innerHTML=n,document.body.appendChild(l),setTimeout(()=>{const p=l.querySelector('textarea[name="name"]');p&&(p.style.height="auto",p.style.height=p.scrollHeight+"px")},10),(async()=>{try{const p=await ne("api/beneficiaries.php?get_offices=1");if(p.success&&p.data.offices){const i=l.querySelector("#edit-office-suggestions-box");if(i){const c=["DOLE Field Office","LGU","DEPED","DICT","PCA"],g=[...new Set([...c,...p.data.offices])];i.innerHTML=g.map(h=>`
                    ...
                    `).join("")}}}catch(p){console.error("Error fetching office suggestions:",p)}})(),Ce(async()=>{const{Drawer:p}=await import("./vendor-flowbite-B7rSfpuT.js").then(i=>i.b);return{Drawer:p}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:p})=>{const i=new p(l,{placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{l&&l.parentNode&&l.remove()},400)}});i.show(),window.initFlowbite&&window.initFlowbite();const c=()=>{document.activeElement&&typeof document.activeElement.blur=="function"&&document.activeElement.blur(),i.hide()};l.querySelector("#close-edit-drawer-btn").addEventListener("click",c),l.querySelector("#edit-drawer-cancel-btn").addEventListener("click",c);const g=l.querySelector("#edit-beneficiary-drawer-form"),h=l.querySelector("#edit-bday-input"),f=l.querySelector("#edit-age-display"),k=l.querySelector("#edit-startDate-input"),y=l.querySelector("#edit-endDate-input"),T=l.querySelector('input[name="seriesNo"]'),j=l.querySelector('input[name="gip_id"]'),te=l.querySelector("#edit-drawer-remarks");te&&te.addEventListener("change",w=>{const E="text-[10px] sm:text-[11px] font-black px-2.5 py-2.5 rounded-lg border uppercase tracking-widest shadow-sm outline-none focus:ring-2 focus:ring-brand w-full cursor-pointer transition-colors duration-300";te.className=`${u(w.target.value)} ${E} editable-indicator`});let Y=!1,K=!1;const G=(w,E)=>{w.addEventListener("input",x=>{const C=x.target.value,I=window.__maskDate(C);if(C!==I&&(x.target.value=I),I.length===10){const q=window.__parseFormattedDate(I);q&&E&&(K||E(q),document.activeElement===w&&w.blur())}}),w.addEventListener("changeDate",x=>{x.detail&&x.detail.date&&E&&(K||E(x.detail.date),w._datepicker&&w._datepicker.hide())})};h&&G(h,w=>{f&&(!Y||!f.value)&&(f.value=window.calculateAge(w))}),k&&G(k,w=>{if(y){const x=new Date(w);x.setMonth(x.getMonth()+3),x.setDate(x.getDate()+1);const C=String(x.getMonth()+1).padStart(2,"0"),I=String(x.getDate()).padStart(2,"0"),q=x.getFullYear();y.value=`${C}/${I}/${q}`}const E=w.getFullYear();E>1900&&j&&T&&Promise.all([z(`api/beneficiaries.php?next_id&year=${encodeURIComponent(E)}`),z(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(E)}`)]).then(([x,C])=>{const I=x.success&&x.data?.success?x.data.nextId:null,q=C.success&&C.data?.success?C.data.nextSeries:null,Z=String(j.value||"").match(/^ROX-RD-ESIG-(\d{4})-\d{4}$/),O=String(T.value||"").match(/^(\d{4})-\d{2}-\d{3}$/),J=Z?Number(Z[1]):null,d=O?Number(O[1]):null;I&&(J===null||J!==E)&&(j.value=I),q&&(d===null||d!==E)&&(T.value=q)}).catch(x=>{console.error("Edit drawer identifier sync error:",x)})}),y&&G(y);const ee=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null),F=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null);ee&&h&&(h._datepicker=new ee(h,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"}));const L=l.querySelector("#edit-date-range-picker");if(F&&L){const w=new F(L,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0,orientation:"auto"});k&&(k._datepicker=w.datepickers[0]),y&&(y._datepicker=w.datepickers[1])}e.id&&(K=!0,z(`api/beneficiaries.php?id=${encodeURIComponent(e.id)}`).then(w=>{if(w.success&&w.data&&w.data.beneficiary){const E=w.data.beneficiary;if(k&&E.startDate){const x=new Date(E.startDate);isNaN(x)||(k.value=E.startDateFormatted||"",k._datepicker&&k._datepicker.setDate(x))}if(y&&E.endDate){const x=new Date(E.endDate);isNaN(x)||(y.value=E.endDateFormatted||"",y._datepicker&&y._datepicker.setDate(x))}}setTimeout(()=>{K=!1},100)}).catch(w=>{console.error("Error fetching accurate beneficiary dates:",w),K=!1})),f&&f.addEventListener("input",()=>Y=!0);const A=(w,E,x)=>{const C=l.querySelector(w),I=l.querySelector(E);if(!C||!I)return;const q=()=>I.classList.add("hidden"),Z=()=>I.classList.remove("hidden");C.addEventListener("focus",Z),C.addEventListener("input",()=>{const O=C.value.toLowerCase().trim();let J=0;I.querySelectorAll(x).forEach(d=>{const b=(d.querySelector(".option-text")?.textContent||d.textContent||"").toLowerCase().includes(O);d.style.display=b?"block":"none",b&&J++}),J>0?Z():q()}),I.addEventListener("click",O=>{const J=O.target.closest(x);J&&(C.value=(J.querySelector(".option-text")?.textContent||J.textContent||"").trim(),q(),C.dispatchEvent(new Event("change")))}),document.addEventListener("click",O=>{!C.contains(O.target)&&!I.contains(O.target)&&q()})};A("#edit-education-input","#edit-education-suggestions-box",".edit-education-option"),A("#edit-designation-input","#edit-designation-suggestions-box",".edit-designation-option"),A("#edit-office-input","#edit-office-suggestions-box",".edit-office-option");const B=l.querySelector("#edit-replacement-input"),D=l.querySelector("#edit-replacement-suggestions-box"),M=l.querySelector("#edit-replacement-loading");let R=null;B&&D&&(B.addEventListener("input",w=>{const E=w.target.value.trim();clearTimeout(R),D.classList.add("hidden"),!(E.length<2)&&(M&&M.classList.remove("hidden"),R=setTimeout(async()=>{try{const x=await ne(`api/beneficiaries.php?replacement_candidates=1&q=${encodeURIComponent(E)}&limit=10`);x.success&&x.data&&x.data.candidates&&x.data.candidates.length>0?(D.innerHTML=x.data.candidates.map(C=>`
                            ...
                            `).join(""),D.classList.remove("hidden")):(D.innerHTML='<div class="px-4 py-3 text-xs text-gray-500 italic text-center font-medium">No active beneficiaries found.</div>',D.classList.remove("hidden"))}catch(x){console.error("Replacement fetch error:",x)}finally{M&&M.classList.add("hidden")}},400))}),D.addEventListener("click",w=>{const E=w.target.closest("button");E&&(B.value=E.dataset.name,D.classList.add("hidden"))}),document.addEventListener("click",w=>{!B.contains(w.target)&&!D.contains(w.target)&&D.classList.add("hidden")})),g.querySelectorAll("input, select, textarea").forEach(w=>{const E=(w.getAttribute("type")||"").toLowerCase(),x=!w.disabled&&!w.readOnly&&E!=="hidden";w.classList.remove("editable-indicator"),x&&w.classList.add("editable-indicator")}),g.addEventListener("submit",w=>{w.preventDefault();const E=new FormData(g),x={};E.forEach((C,I)=>x[I]=C),x.id=e.id,x.gip_id=e.id,window.addBeneficiaryData&&(async()=>await window.addBeneficiaryData(x,!0,!1)&&(c(),window.viewBeneficiary&&setTimeout(()=>window.viewBeneficiary({id:e.id},0),100),m.fire({toast:!0,position:"top-end",icon:"success",title:"Record Updated",showConfirmButton:!1,timer:3e3})))()})})}function it(){window.__maskDate=function(e){let o=e.replace(/\D/g,"").slice(0,8);return o.length>2&&o.length<=4?o=o.slice(0,2)+"/"+o.slice(2):o.length>4&&(o=o.slice(0,2)+"/"+o.slice(2,4)+"/"+o.slice(4)),o},window.__parseFormattedDate=function(e){if(!e)return null;const o=e.split("/");if(o.length===3){const s=parseInt(o[0])-1,a=parseInt(o[1]),r=parseInt(o[2]);if(r>1e3&&s>=0&&s<12&&a>0&&a<=31)return new Date(r,s,a)}return null},window.calculateAge=function(e){if(!e)return"";const o=e instanceof Date?e:new Date(e),s=new Date;let a=s.getFullYear()-o.getFullYear();const r=s.getMonth()-o.getMonth();return(r<0||r===0&&s.getDate()<o.getDate())&&a--,a>=0?a:0},window.viewBeneficiary=async function(e,o=0){const s=e?.id||e?.gip_id||null;if(!s)return;const a=!!(e?.name&&e?.office&&e?.remarks);let r={...e,id:s};if(!a){const n=await z(`api/beneficiaries.php?id=${encodeURIComponent(s)}`);n.success&&n.data?.success&&n.data?.beneficiary&&(r={...n.data.beneficiary,...r,id:s})}const t=`logs_cache_${s}`;let u=!1;try{if(window.__doleDB&&window.__doleDB.getSecureCache){const y=await window.__doleDB.getSecureCache(t);y&&(r.arLogs=y.arLogs||[],r.dtrLogs=y.dtrLogs||[],r.docs=y.docs||[],pe(r,o),u=!0)}let n=[],l=[],p=[];const[i,c,g,h]=await Promise.all([z(`api/logs.php?type=ar&gip_id=${encodeURIComponent(s)}`),z(`api/logs.php?type=dtr&gip_id=${encodeURIComponent(s)}`),z(`api/logs.php?type=docs&gip_id=${encodeURIComponent(s)}`),z(`api/logs.php?type=absorption&gip_id=${encodeURIComponent(s)}`)]);n=i.success&&i.data?.success?i.data.logs:[],l=c.success&&c.data?.success?c.data.logs:[],p=g.success&&g.data?.success?g.data.logs:[];const f=h.success&&h.data?.success?h.data.logs:[];if(f.length>0){const y=f[0];r.absorbDate=y.absorption_datetime,r.absorb_where=y.where||y.absorb_where,r.absorb_position=y.position||y.absorb_position,r.absorb_agency=y.agency||y.absorb_agency}const k={arLogs:n,dtrLogs:l,docs:p};window.__doleDB&&window.__doleDB.setSecureCache&&await window.__doleDB.setSecureCache(t,k),r.arLogs=n,r.dtrLogs=l,r.docs=p,u?document.getElementById("beneficiary-drawer-container")&&pe(r,o):pe(r,o)}catch(n){console.error("Error fetching logs/docs:",n),u||(r.arLogs=[],r.dtrLogs=[],r.docs=[],pe(r,o))}},window.showAddDataModal=function(e){fe(e)},window.editBeneficiary=function(e){ze(e)},window.showExportConfigModal=function(e){Je(e)},window.showProfileModal=function(){Ye()}}async function Ye(){try{if(Ee()&&me){let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=a.id)}catch{}if(!e)throw new Error("User not logged in");const{data:o,error:s}=await me.from("users").select("*").eq("id",e).single();if(s)throw s;Se(o)}else{let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=`?user_id=${a.id}`)}catch{}const s=await(await fetch(`${W()}api/profile.php${e}`)).json();if(s.success){const a=s.profile;Se(a)}else m.fire({icon:"error",title:"Error",text:s.error||"Failed to load profile"})}}catch(e){console.error("Error fetching profile:",e)}}function Se(e){const o=e.profile_picture_path?`${W()}${e.profile_picture_path}`:null,s=e.full_name?e.full_name.split(" ").map(r=>r[0]).join("").substring(0,2).toUpperCase():"US",a=`
    ...
    `;m.fire({html:u,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:n=>{window.initFlowbite&&window.initFlowbite();const l=n.querySelector("#cancel-modal-btn");l&&l.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),m.close(),e?._isBulk&&ge.onCancel()});const p=n.querySelector("#bulk-add-btn");p&&p.addEventListener("click",()=>{m.close(),ge.init()});const i=(d,v)=>{d.addEventListener("input",b=>{const $=window.__maskDate(b.target.value);if(b.target.value!==$&&(b.target.value=$),$.length===10){const S=window.__parseFormattedDate($);S&&v&&(v(S),document.activeElement===d&&d.blur())}}),d.addEventListener("changeDate",b=>{if(b.detail&&b.detail.date&&v){v(b.detail.date);const $=d._datepicker||d.parentNode&&d.parentNode._datepicker;$&&typeof $.hide=="function"&&$.hide()}})},c=n.querySelector("#birthday-input"),g=n.querySelector("#age-display");if(c){i(c,v=>{g&&(g.value=window.calculateAge(v),g.classList.add("animate-pulse"),setTimeout(()=>g.classList.remove("animate-pulse"),400))});const d=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);d&&(c._datepicker=new d(c,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"}))}const h=n.querySelector("#name-input-field"),f=n.querySelector("#duplicate-warning");if(h&&f){let d;h.addEventListener("input",v=>{const b=v.target.value.trim();if(clearTimeout(d),b.length<3){f.classList.add("hidden");return}d=setTimeout(async()=>{try{const S=await(await fetch(`${W()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:b})})).json();S.success&&S.exists?(f.classList.remove("hidden"),oe()?f.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":f.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):f.classList.add("hidden")}catch($){console.error("Duplicate check error:",$)}},500)}),e?.name&&(f.classList.add("hidden"),(async()=>{const b=await(await fetch(`${W()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();b.success&&b.exists&&(f.classList.remove("hidden"),oe()?f.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":f.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const k=n.querySelector("#full-id-input"),y=n.querySelector("#series-no-input"),T=n.querySelector('input[name="startDate"]'),j=n.querySelector('input[name="endDate"]'),te=n.querySelectorAll('input[name="remarks"]'),Y=n.querySelector("#extension-log-container"),K=async d=>{if(!d)return;const v=`cache_next_id_${d}`,b=`cache_next_series_${d}`,$=`cache_expiry_${d}`,S=Date.now(),H=localStorage.getItem(v),_=localStorage.getItem(b),U=localStorage.getItem($);if(H&&_&&U&&S<parseInt(U,10)){k&&(k.value=H,k.classList.remove("placeholder:text-gray-300")),y&&(y.value=_);return}const V=[k,y].filter(Boolean);V.forEach(N=>{N.classList.add("animate-pulse"),N.placeholder="Syncing..."});try{const[N,P]=await Promise.all([z(`api/beneficiaries.php?next_id&year=${encodeURIComponent(d)}`),z(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(d)}`)]),Q=N.success&&N.data?.success?N.data.nextId:null,X=P.success&&P.data?.success?P.data.nextSeries:null;Q&&k&&(k.value=Q,localStorage.setItem(v,Q)),X&&y&&(y.value=X,localStorage.setItem(b,X)),localStorage.setItem($,(S+3e5).toString())}catch(N){console.error("ID Sync error:",N)}finally{V.forEach(N=>N.classList.remove("animate-pulse"))}},G=n.querySelector("#replacement-search-input"),ee=n.querySelector("#replacement-hidden"),F=n.querySelector("#replacement-suggestions"),L=d=>{const v=(d.name||"").toUpperCase().trim(),b=d.startDateFormatted||d.startDate||"N/A",$=d.endDateFormatted||d.endDate||"N/A";return`${v} (${b.toUpperCase()} - ${$.toUpperCase()})`},A=d=>{if(F){if(!d.length){F.innerHTML=`<div class="px-3 py-2 text-[10px] font-bold ${t.textCourseOpt}">No matching beneficiary found.</div>`,F.classList.remove("hidden");return}F.innerHTML=d.map(v=>{const b=L(v);return`
                    ...
                    `}).join(""),F.classList.remove("hidden"),F.querySelectorAll(".replacement-option").forEach(v=>{v.addEventListener("click",()=>{const b=v.getAttribute("data-replacement")||"";G&&(G.value=b),ee&&(ee.value=b),F.classList.add("hidden")})})}};let B=null;const D=async(d="")=>{const v=(d||"").trim(),b=`api/beneficiaries.php?replacement_candidates=1&limit=20${v?`&q=${encodeURIComponent(v)}`:""}`,$=await z(b);$.success&&$.data?.success&&Array.isArray($.data.candidates)&&A($.data.candidates)};G&&ee&&F&&(G.addEventListener("focus",()=>{D(G.value||"")}),G.addEventListener("input",()=>{ee.value=G.value.trim(),clearTimeout(B),B=setTimeout(()=>{D(G.value||"")},250)}),document.addEventListener("click",d=>{!G.contains(d.target)&&!F.contains(d.target)&&F.classList.add("hidden")}));const M=()=>{const d=n.querySelector('input[name="remarks"]:checked');return d?d.value:"ONGOING"},R=d=>{const v=n.querySelector(`input[name="remarks"][value="${d}"]`);v&&(v.checked=!0,E())},w=()=>{if(j&&j.value){const d=window.__parseFormattedDate(j.value);if(!d)return;const v=new Date;v.setHours(0,0,0,0);let b="ONGOING";d<v&&(b="EXPIRED"),R(b)}},E=()=>{if(!Y)return;const d=M();if(d==="ABSORBED"){const v=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});Y.innerHTML=`
                    ...
                    `}else if(d==="RESIGNED"){const v=e?.resignedDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});Y.innerHTML=`
                        <div class="mt-4 pt-4 border-t ${r?"border-slate-800":"border-gray-100"}">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                                <div class="${r?"bg-red-900/10 border-red-900/30":"bg-[#ffebee]/50 border-[#ffcdd2]"} rounded-lg p-2.5 flex items-center justify-between border">
                                    <div>
                                        ...
                                        <p class="text-[10px] font-black ${r?"text-red-400":"text-[#b71c1c]"} uppercase">${v}</p>
                                        <input type="hidden" name="resignedDate" value="${v}">
                                    </div>
                    ...
                    `}else Y.innerHTML=""};if(T){let d=null;i(T,S=>{const H=S.getFullYear();if(j){const _=new Date(S);_.setMonth(_.getMonth()+3),_.setDate(_.getDate()+1);const U=String(_.getMonth()+1).padStart(2,"0"),V=String(_.getDate()).padStart(2,"0"),N=_.getFullYear();j.value=`${U}/${V}/${N}`}w(),H>1900&&H!==d&&(d=H,K(H))}),j&&i(j,()=>w());const v=n.querySelector("#date-range-picker"),b=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null),$=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);if(b&&v&&T&&j){const S=new b(v,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0,orientation:"auto"});T._datepicker=S.datepickers?.[0]||null,j._datepicker=S.datepickers?.[1]||null}else $&&(T&&(T._datepicker=new $(T,{format:"mm/dd/yyyy",autohide:!0,orientation:"auto"})),j&&(j._datepicker=new $(j,{format:"mm/dd/yyyy",autohide:!0,orientation:"auto"})));if(!o){const S=new Date().getFullYear();K(S)}}te.forEach(d=>d.addEventListener("change",E));const x=n.querySelector("#resign-btn"),C=n.querySelector("#absorb-btn");x&&x.addEventListener("click",()=>R("RESIGNED")),C&&C.addEventListener("click",()=>R("ABSORBED")),n.querySelectorAll('input[type="text"], textarea').forEach(d=>{["id-number-input","full-id-input"].includes(d.id)||d.addEventListener("input",()=>{const v=d.selectionStart,b=d.selectionEnd;d.value=d.value.toUpperCase(),d.setSelectionRange(v,b)})}),w(),E(),Z("education-input","course-suggestions","course-option"),Z("designation-input","work-suggestions","work-option"),Z("office-input","office-suggestions","office-option");const I={publicSector:["DOLE","LGU","DEPED"],privateSector:["DICT","PCA"]},q=(d=[])=>{const v=n.querySelector("#office-suggestions");if(!v)return;const b=N=>String(N||"").trim().toUpperCase(),$=new Set(I.publicSector.map(b)),S=new Set(I.privateSector.map(b)),H=new Set([...$,...S]),U=[...new Set((d||[]).map(b).filter(Boolean))].filter(N=>!H.has(N)),V=(N,P)=>P.length?`
                        ...
                        `).join("")}
                    `:"";v.innerHTML=[V("Public Sector",[...$]),V("Private Sector",[...S]),V("Other Offices",U)].join("")};q(),(async()=>{try{const d=await z("api/beneficiaries.php?get_offices=1");d.success&&d.data?.success&&Array.isArray(d.data.offices)&&q(d.data.offices)}catch(d){console.error("Office suggestion sync failed:",d)}})();function Z(d,v,b){const $=n.querySelector(`#${d}`),S=n.querySelector(`#${v}`);if(!$||!S)return;let H=!1;$.addEventListener("focus",()=>S.classList.remove("hidden")),document.addEventListener("click",_=>{!$.contains(_.target)&&!S.contains(_.target)&&S.classList.add("hidden")}),$.addEventListener("input",()=>{if(H){H=!1;return}const _=$.value.toLowerCase(),U=S.querySelectorAll(`.${b}`);let V=!1;U.forEach(N=>{const P=N.querySelector(".option-text");(P?P.innerText:N.innerText).toLowerCase().includes(_)?(N.style.display="block",V=!0):N.style.display="none"}),V?S.classList.remove("hidden"):S.classList.add("hidden")}),S.addEventListener("click",_=>{const U=_.target.closest(`.${b}`);if(!U)return;const V=U.querySelector(".option-text");$.value=V?V.innerText.trim():U.innerText.trim(),H=!0,S.classList.add("hidden"),$.dispatchEvent(new Event("change"))})}const O=n.querySelector("#add-beneficiary-form"),J="add_beneficiary_draft";if(!o){const d=localStorage.getItem(J);if(d)try{const v=JSON.parse(d);Object.entries(v).forEach(([b,$])=>{const S=O.elements[b];S&&S.type!=="file"&&S.type!=="hidden"&&(S.value=$)})}catch(v){console.error("Error loading draft",v)}}O.addEventListener("input",d=>{if(!o){const v=new FormData(O),b={};v.forEach(($,S)=>b[S]=$),localStorage.setItem(J,JSON.stringify(b))}}),O&&O.addEventListener("submit",d=>{d.preventDefault(),O.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(X=>{X.classList.remove("ring-2","ring-red-500","!border-red-500")});const b=new FormData(O);let $=!1;const S=X=>{const re=O.querySelector(`[name="${X}"]`);re&&re.classList.add("ring-2","ring-red-500","!border-red-500"),$=!0},H=b.get("name"),_=b.get("contact"),U=b.get("startDate"),V=b.get("endDate"),N=(b.get("designation")||"").trim();if((!H||H.trim()===""||/[0-9]/.test(H))&&S("name"),_&&_.trim()!==""&&/[^0-9]/.test(_.replace(/[\s\-\+\(\)]/g,""))&&S("contact"),U||S("startDate"),V||S("endDate"),$)return;const P={};b.forEach((X,re)=>{if(["birthday","startDate","endDate"].includes(re)){const se=window.__parseFormattedDate(X);if(se){const be=se.getFullYear(),he=String(se.getMonth()+1).padStart(2,"0"),Me=String(se.getDate()).padStart(2,"0");P[re]=`${be}-${he}-${Me}`}else P[re]=X}else P[re]=X}),N||(P.designation="N/A"),P.replacement||(P.replacement="");const Q=n.querySelector("#full-id-input")?.value;o?(P.id=e?.id,Q&&(P.gip_id=Q)):(P.id=null,Q&&(P.gip_id=Q)),window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(P)){if(!o){const re="add_beneficiary_draft",se=O.querySelector('[name="office"]')?.value||"",be=O.querySelector('[name="designation"]')?.value||"",he=O.querySelector('[name="education"]')?.value||"";localStorage.setItem(re,JSON.stringify({office:se,designation:be,education:he}))}m.close(),setTimeout(()=>{m.fire({toast:!0,position:"top-end",icon:"success",title:`Record ${o?"Updated":"Added"} Successfully`,showConfirmButton:!1,timer:3e3,timerProgressBar:!0}),!o&&e?._isBulk&&ge.onSaveSuccess()},100)}else m.fire({icon:"error",title:"Save Failed",text:"There was an error saving the record to the database."})})()})}})}window.handleContactSubmit=async function(e){e.preventDefault();const o=e.target,s=o.querySelector('button[type="submit"]'),a=s.innerHTML;s.disabled=!0,s.innerHTML=`
        <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        Sending...
    `;try{const r=new FormData(o);if((await fetch(o.action,{method:"POST",body:r,headers:{Accept:"application/json"}})).ok)m.fire({toast:!0,position:"top-end",icon:"success",title:"Message Sent Successfully!",text:"Thank you for reaching out. I will get back to you soon!",showConfirmButton:!1,timer:4e3,timerProgressBar:!0,didOpen:u=>{u.addEventListener("mouseenter",m.stopTimer),u.addEventListener("mouseleave",m.resumeTimer)},customClass:{popup:"rounded-2xl border-l-4 border-teal-500 shadow-2xl"}}),o.reset();else throw new Error("Failed to send")}catch{m.fire({toast:!0,position:"top-end",icon:"error",title:"Oops! Something went wrong.",text:"Could not send your message. Please try again later.",showConfirmButton:!1,timer:4e3,customClass:{popup:"rounded-2xl border-l-4 border-red-500 shadow-2xl"}})}finally{s.disabled=!1,s.innerHTML=a}return!1};export{Ce as _,z as a,ne as b,rt as c,st as d,at as e,Ke as f,W as g,nt as h,Ee as i,Qe as j,et as k,ae as l,tt as m,it as n,Te as p,ot as r,me as s,Ze as u};
