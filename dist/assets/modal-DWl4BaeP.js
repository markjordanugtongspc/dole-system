...
import M from"./vendor-swal-BSk0fVSb.js";import{a as vt}from"./vendor-DHtNC8Ux.js";import{A as Ze}from"./vendor-charts-BjInCqFR.js";const yt="true".toLowerCase()==="true";function Te(){return yt}function he(){const e=window.location.pathname,t="/dole-system/",s=e.toLowerCase().indexOf(t.toLowerCase());if(s!==-1)return e.substring(0,s+t.length);const a=e.indexOf("/frontend/");if(a!==-1)return e.substring(0,a+1);const o=e.indexOf("/backend/");return o!==-1?e.substring(0,o+1):"/"}function et(e="Incorrect Username or Password"){M.fire({html:`
            <div class="p-6">
        ...
        `,timer:e?800:3e3,timerProgressBar:!0,showConfirmButton:!1,showCloseButton:!1,width:"350px",padding:"0",customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl overflow-hidden border border-gray-100",timerProgressBar:"bg-green-500 h-1.5"},backdrop:"rgba(0, 0, 0, 0.4)"})}function gr(){const e=localStorage.getItem("hasVisitedBefore"),t=document.getElementById("page-loader");e&&(document.documentElement.classList.add("loaded"),t&&(t.style.display="none")),window.addEventListener("load",()=>{const s=document.querySelector("body > *:not(.page-loader)");s&&s.classList.add("content-fade-in"),setTimeout(()=>{document.documentElement.classList.add("loaded"),t&&t.classList.add("hidden"),document.body.style.overflow="auto",localStorage.setItem("hasVisitedBefore","true")},300)})}const je={_key:"DOLE-GIP-SECURE-KEY-2026",encrypt:function(e){try{const t=o=>o.split("").map(r=>r.charCodeAt(0)),s=o=>("0"+Number(o).toString(16)).substr(-2),a=o=>t(this._key).reduce((r,l)=>r^l,o);return e.split("").map(t).map(a).map(s).join("")}catch(t){return console.error("Encryption Failed",t),null}},decrypt:function(e){try{const t=a=>a.split("").map(o=>o.charCodeAt(0)),s=a=>t(this._key).reduce((o,r)=>o^r,a);return e.match(/.{1,2}/g).map(a=>parseInt(a,16)).map(s).map(a=>String.fromCharCode(a)).join("")}catch(t){return console.error("Decryption Failed",t),null}}};function fr(){document.querySelectorAll(".login-form-shared").forEach(t=>{const s=t.querySelector('input[name="username"]'),a=t.querySelector('input[name="password"]'),o=t.querySelector('input[name="rememberMe"]');if(s&&a&&o){const r=localStorage.getItem("secure_user"),l=localStorage.getItem("secure_pass");if(r&&l){const i=je.decrypt(r),d=je.decrypt(l);i&&d&&(s.value=i,a.value=d,o.checked=!0)}}t.addEventListener("submit",async r=>{r.preventDefault();try{const i=await(await fetch(`${he()}api/auth.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:s.value,password:a.value})})).json();if(i.success){o.checked?(localStorage.setItem("secure_user",je.encrypt(s.value)),localStorage.setItem("secure_pass",je.encrypt(a.value))):(localStorage.removeItem("secure_user"),localStorage.removeItem("secure_pass"));const d=localStorage.getItem("hasLoggedInBefore")==="true";localStorage.setItem("isLoggedIn","true"),localStorage.setItem("hasLoggedInBefore","true"),localStorage.setItem("user",JSON.stringify(i.user));const g=document.getElementById("drawer-login");if(g){g.classList.add("translate-y-full");const p=g.querySelector("[data-drawer-hide]");p&&p.click()}await wt(d),kt(d)}else{const d=document.getElementById("drawer-login");d?(d.classList.add("translate-y-full"),setTimeout(()=>{et(),setTimeout(()=>{d.classList.remove("translate-y-full"),a.value="",a.focus()},600)},400)):(et(),a.value="",a.focus())}}catch(l){console.error("Login Error:",l),M.fire({icon:"error",title:"Login Error",text:"Unable to reach the server. Please check your connection."})}})})}function kt(e=!1){const t=document.getElementById("left-panel"),s=document.getElementById("right-panel"),a=document.getElementById("left-panel-content"),o=document.getElementById("right-panel-content");a&&(a.style.opacity="0"),o&&(o.style.opacity="0");const r=document.createElement("div");r.className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none";const l=e?"":"animate__delay-1s",i=e?"animation-duration: 0.8s;":"animation-duration: 2s;";r.innerHTML=`
        <img src="${he()}frontend/images/logo/doleiligan.png" class="w-64 h-64 md:w-96 md:h-96 object-contain bg-white/20 backdrop-blur-sm shadow-2xl rounded-full p-4 animate__animated animate__rotateOut ${l}" style="${i}" alt="DOLE Logo">
    `,document.body.appendChild(r);const d=e?0:1e3,g=e?600:1500;setTimeout(()=>{t&&t.classList.add("animate-slide-left"),s&&s.classList.add("animate-slide-right"),setTimeout(()=>{window.location.href=`${he()}frontend/dashboard/`},g)},d)}function br(){const e=document.getElementById("logoutBtn");e&&e.addEventListener("click",()=>{localStorage.removeItem("isLoggedIn"),window.location.href=`${he()}`})}function hr(){const e=document.getElementById("mobile-splash"),t=document.getElementById("show-login-btn"),s=document.getElementById("back-to-splash"),a=document.getElementById("mobile-bg-content"),o=document.getElementById("mobile-welcome-text"),r=document.getElementById("reopen-login-drawer"),l=document.getElementById("request-notifications-btn"),i=async()=>{if("Notification"in window){const h=await Notification.requestPermission();console.log("Notification permission:",h),h==="granted"&&l&&l.classList.add("hidden")}};Notification.permission==="default"&&l&&(l.classList.remove("hidden"),l.addEventListener("click",i));const d=()=>{e&&(e.style.transform="translateY(-100%)",setTimeout(()=>{e.style.visibility="hidden",e.style.pointerEvents="none",e.style.zIndex="-1",Notification.permission==="default"&&i();const h=document.getElementById("drawer-login");h&&h.classList.remove("translate-y-full")},800))},g=()=>{e&&(e.style.zIndex="9999",e.style.pointerEvents="auto",e.style.visibility="visible",e.style.transform="translateY(0)")};t&&t.addEventListener("click",d),document.querySelectorAll(".forgot-password-link").forEach(h=>{h.addEventListener("click",C=>{C.target.closest("#mobile-splash")&&d()})}),s&&s.addEventListener("click",()=>{const h=document.getElementById("drawer-login");if(h){h.classList.add("translate-y-full");const C=h.querySelector("[data-drawer-hide]");C&&C.click()}g()});const f=document.getElementById("drawer-login"),v=document.getElementById("curved-welcome"),c=document.getElementById("peoples-bg");f&&new MutationObserver(C=>{C.forEach(x=>{x.attributeName==="class"&&(f.classList.contains("translate-y-full")?(a&&(a.style.transform="translateY(0)"),o&&(o.style.opacity="1",o.style.transform="translateY(0) scale(1)"),v&&(v.style.opacity="0",v.style.transform="scale(0.5)"),r&&e&&e.style.visibility==="hidden"&&(r.style.opacity="1",r.style.transform="scale(1)"),c&&(c.classList.add("opacity-0","scale-0"),c.classList.remove("opacity-40","scale-[1.6]"))):(a&&(a.style.transform="translateY(-35%)"),o&&(o.style.opacity="0",o.style.transform="translateY(20px) scale(0.9)"),v&&(v.style.opacity="1",v.style.transform="scale(1)"),r&&(r.style.opacity="0",r.style.transform="scale(0)"),c&&(c.classList.remove("opacity-0","scale-0"),c.classList.add("opacity-40","scale-[1.6]"))))})}).observe(f,{attributes:!0})}const Be=()=>"false".toLowerCase()==="true";function St(e){try{return JSON.stringify(e)}catch{return"[unserializable]"}}const $e={debug(...e){Be()&&console.debug(...e)},info(...e){Be()&&console.info(...e)},warn(...e){Be()&&console.warn(...e)},error(...e){console.error(...e)},table(e){Be()&&console.table(e)},json(e,t){Be()&&console.debug(e,St(t))}},Pe=new Map;async function _e(e,t={}){const a=`${he()}${e}`;let o=null;try{const p=JSON.parse(localStorage.getItem("user"));p&&(o=p.user_id||p.id||null)}catch{}const r={headers:{"Content-Type":"application/json",...o?{"X-User-Id":o}:{},...t.headers},...t},i=(r.method||"GET").toUpperCase()==="GET"?2:1;let d=null;for(let p=1;p<=i;p++)try{if($e.debug("[API] Request",{url:a,method:r.method||"GET",hasUserId:!!o}),r.body)try{$e.json("[API] Payload",JSON.parse(r.body))}catch{$e.debug("[API] Payload (raw)",r.body)}const f=await fetch(a,r);if(!f.ok)throw new Error(`HTTP ${f.status}: ${f.statusText}`);const v=await f.json();return Pe.has(a)&&(Pe.delete(a),$e.info?.("[API] Recovered",{url:a})),$e.debug("[API] Response",{url:a,ok:!0}),{success:!0,data:v}}catch(f){if(d=f,f instanceof TypeError&&/fetch/i.test(f.message||"")&&p<i){await new Promise(h=>setTimeout(h,1200));continue}}return d instanceof TypeError&&/fetch/i.test(d.message||"")?Pe.get(a)||(Pe.set(a,!0),$e.error("API Request Network Error (suppressed for repeats):",{url:a,message:d.message})):$e.error("API Request Error:",d),{success:!1,error:d?.message||"Unknown request error"}}async function ce(e){return _e(e,{method:"GET"})}async function Ye(e,t){return _e(e,{method:"POST",body:JSON.stringify(t)})}async function Et(e,t){return _e(e,{method:"PUT",body:JSON.stringify(t)})}async function mr(e,t){const s=new URLSearchParams(t).toString();return _e(`${e}?${s}`,{method:"PATCH"})}class $t{constructor(){this.intervals=new Map,this.isPageVisible=!0,document.addEventListener("visibilitychange",()=>{this.isPageVisible=!document.hidden,this.isPageVisible?console.log("[Polling] Page visible - resuming normal activity"):console.log("[Polling] Page hidden - reducing activity")})}start(t,s,a=1e4){this.stop(t),s();const o=setInterval(async()=>{this.isPageVisible&&await s()},a);this.intervals.set(t,o),console.log(`[Polling] Started: ${t} (every ${a}ms)`)}stop(t){this.intervals.has(t)&&(clearInterval(this.intervals.get(t)),this.intervals.delete(t),console.log(`[Polling] Stopped: ${t}`))}stopAll(){this.intervals.forEach((t,s)=>this.stop(s)),console.log("[Polling] All intervals stopped")}getActivePolls(){return Array.from(this.intervals.keys())}}const Ct=new $t;function xr(){typeof window.initFlowbite=="function"?(window.initFlowbite(),console.log("[Flowbite] Components re-initialized")):console.warn("[Flowbite] initFlowbite() not available globally")}function vr(e){return JSON.stringify(e)}window.addEventListener("beforeunload",()=>{Ct.stopAll()});const tt="dole-gip-db",Lt=2,J={BENEFICIARIES:"beneficiaries",SYNC_QUEUE:"sync_queue",METADATA:"metadata",APP_CACHE:"app_cache"};let Fe=null;function xe(){return Fe?Promise.resolve(Fe):new Promise((e,t)=>{const s=indexedDB.open(tt,Lt);s.onupgradeneeded=a=>{const o=a.target.result;if(!o.objectStoreNames.contains(J.BENEFICIARIES)){const r=o.createObjectStore(J.BENEFICIARIES,{keyPath:"id"});r.createIndex("name","name",{unique:!1}),r.createIndex("office","office",{unique:!1}),r.createIndex("remarks","remarks",{unique:!1})}o.objectStoreNames.contains(J.SYNC_QUEUE)||o.createObjectStore(J.SYNC_QUEUE,{keyPath:"queueId",autoIncrement:!0}).createIndex("status","status",{unique:!1}),o.objectStoreNames.contains(J.METADATA)||o.createObjectStore(J.METADATA,{keyPath:"key"}),o.objectStoreNames.contains(J.APP_CACHE)||o.createObjectStore(J.APP_CACHE,{keyPath:"key"})},s.onsuccess=a=>{Fe=a.target.result,console.log("[DB] IndexedDB opened:",tt),e(Fe)},s.onerror=a=>{console.error("[DB] Failed to open IndexedDB:",a.target.error),t(a.target.error)}})}async function Ne(e){const t=await xe();return new Promise((s,a)=>{const o=t.transaction(J.BENEFICIARIES,"readwrite"),r=o.objectStore(J.BENEFICIARIES);r.clear(),e.forEach(l=>{const i={...l,id:l.id||l.gip_id};r.put(i)}),o.oncomplete=()=>{It("beneficiaries_last_sync",Date.now()),console.log(`[DB] Cached ${e.length} beneficiaries locally`),s(e.length)},o.onerror=()=>a(o.error)})}async function Ge(){const e=await xe();return new Promise((t,s)=>{const r=e.transaction(J.BENEFICIARIES,"readonly").objectStore(J.BENEFICIARIES).getAll();r.onsuccess=()=>t(r.result||[]),r.onerror=()=>s(r.error)})}async function yr(e){const t=await xe();return new Promise((s,a)=>{const r=t.transaction(J.BENEFICIARIES,"readwrite").objectStore(J.BENEFICIARIES),l={...e,id:e.id||e.gip_id},i=r.put(l);i.onsuccess=()=>s(i.result),i.onerror=()=>a(i.error)})}async function wr(e){const t=await xe();return new Promise((s,a)=>{const l=t.transaction(J.BENEFICIARIES,"readwrite").objectStore(J.BENEFICIARIES).delete(e);l.onsuccess=()=>s(),l.onerror=()=>a(l.error)})}async function Dt(){const e=await ct("beneficiaries_last_sync");return e?Date.now()-e:1/0}async function kr(e,t,s){const a=await xe();return new Promise((o,r)=>{const i=a.transaction(J.SYNC_QUEUE,"readwrite").objectStore(J.SYNC_QUEUE),d={method:e,endpoint:t,payload:s,status:"pending",attempts:0,createdAt:Date.now(),lastAttempt:null},g=i.add(d);g.onsuccess=()=>{console.log(`[SyncQueue] Enqueued ${e} ${t} (id: ${g.result})`),o(g.result)},g.onerror=()=>r(g.error)})}async function Qe(){const e=await xe();return new Promise((t,s)=>{const l=e.transaction(J.SYNC_QUEUE,"readonly").objectStore(J.SYNC_QUEUE).index("status").getAll("pending");l.onsuccess=()=>t(l.result||[]),l.onerror=()=>s(l.error)})}async function Sr(e,t,s={}){const a=await xe();return new Promise((o,r)=>{const i=a.transaction(J.SYNC_QUEUE,"readwrite").objectStore(J.SYNC_QUEUE),d=i.get(e);d.onsuccess=()=>{const g=d.result;if(!g)return o();const p={...g,status:t,lastAttempt:Date.now(),attempts:(g.attempts||0)+1,...s},f=i.put(p);f.onsuccess=()=>o(),f.onerror=()=>r(f.error)},d.onerror=()=>r(d.error)})}async function Er(e){const t=await xe();return new Promise((s,a)=>{const l=t.transaction(J.SYNC_QUEUE,"readwrite").objectStore(J.SYNC_QUEUE).delete(e);l.onsuccess=()=>s(),l.onerror=()=>a(l.error)})}async function $r(){return(await Qe()).length}async function It(e,t){const s=await xe();return new Promise((a,o)=>{const i=s.transaction(J.METADATA,"readwrite").objectStore(J.METADATA).put({key:e,value:t});i.onsuccess=()=>a(),i.onerror=()=>o(i.error)})}async function ct(e){const t=await xe();return new Promise((s,a)=>{const l=t.transaction(J.METADATA,"readonly").objectStore(J.METADATA).get(e);l.onsuccess=()=>s(l.result?.value??null),l.onerror=()=>a(l.error)})}function At(e){return e?btoa(encodeURIComponent(JSON.stringify(e))):null}function Bt(e){if(!e)return null;try{return JSON.parse(decodeURIComponent(atob(e)))}catch(t){return console.error("[DB] Failed to decrypt local cache",t),null}}async function Mt(e,t){const s=await xe();return new Promise((a,o)=>{const l=s.transaction(J.APP_CACHE,"readwrite").objectStore(J.APP_CACHE),i={key:e,data:At(t),updated_at:Date.now()},d=l.put(i);d.onsuccess=()=>{console.log(`[DB] Securely cached offline data for: ${e}`),a()},d.onerror=()=>o(d.error)})}async function Nt(e){const t=await xe();return new Promise((s,a)=>{const l=t.transaction(J.APP_CACHE,"readonly").objectStore(J.APP_CACHE).get(e);l.onsuccess=()=>{l.result&&l.result.data?s(Bt(l.result.data)):s(null)},l.onerror=()=>a(l.error)})}async function Ot(){const[e,t]=await Promise.all([Ge(),Qe()]),s=await ct("beneficiaries_last_sync");return{localBeneficiaries:e.length,pendingSync:t.length,lastSync:s?new Date(s).toLocaleString():"Never"}}window.__doleDB={getStats:Ot,getLocalBeneficiaries:Ge,getPendingSyncItems:Qe,setSecureCache:Mt,getSecureCache:Nt};const De=["Local Employment Unit (LEU)","Labor Standards Unit (LSU)","Internal Management Services Unit (IMSU)","Wellfare Workers Unit (WWU)","Labor Relation Unit (LRU)","Information Technology Unit (IT)"],de=()=>document.documentElement.classList.contains("dark"),Je=()=>de()?{text:"#ffffff",muted:"#94a3b8",grid:"#334155",cardBg:"#1e293b",tooltipBg:"#1e293b",tooltipBorder:"#334155"}:{text:"#0f172a",muted:"#64748b",grid:"#f1f5f9",cardBg:"#f8fafc",tooltipBg:"#ffffff",tooltipBorder:"#e2e8f0"},fe={royalBlue:()=>de()?"#60a5fa":"#0038A8",philippineRed:"#CE1126",goldenYellow:"#FCD116",successGreen:"#22c55e",mutedSlate:()=>(de(),"#94a3b8")};let Me=null;function ze(e){const s=`; ${document.cookie}`.match(new RegExp(`;\\s*${e}=([^;]+)`));return s?decodeURIComponent(s[1]):null}function Ue(e,t,s){let a=new Date;a.setTime(a.getTime()+s*24*60*60*1e3),document.cookie=`${e}=${encodeURIComponent(t)};expires=${a.toUTCString()};path=/`}let re=ze("user_workforce_filter")||"ALL",ut=ze("user_workforce_label")||"Overall Stats",Re=ze("user_gender_filter")||"ALL",pt=ze("user_gender_label")||"All Years";function me(e){if(!e)return null;if(e instanceof Date)return isNaN(e.getTime())?null:e;if(typeof e!="string")return null;const t=e.trim();if(!t)return null;if(/^\d{4}-\d{2}-\d{2}$/.test(t)){const a=new Date(`${t}T00:00:00`);return isNaN(a.getTime())?null:a}if(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/.test(t)){const a=t.replace(" ","T"),o=new Date(a);return isNaN(o.getTime())?null:o}const s=new Date(t);return isNaN(s.getTime())?null:s}function gt(e){const t=Number.parseInt(e?.age,10);if(Number.isInteger(t)&&t>=0)return t;const s=me(e?.birthday);if(!s)return null;const a=new Date;let o=a.getFullYear()-s.getFullYear();const r=a.getMonth()-s.getMonth();return(r<0||r===0&&a.getDate()<s.getDate())&&o--,o>=0?o:null}function Tt(e){const t=String(e||"").trim(),s=t.match(/\(([A-Z]{2,8})\)\s*$/i)?.[1];return s?s.toUpperCase():/information technology/i.test(t)?"IT":t.length>18?`${t.slice(0,16)}...`:t}function Rt(e){const t=String(e||"").trim().toUpperCase().split(" ").filter(Boolean).join(" ");if(!t||["N/A","NA","NONE","UNASSIGNED"].includes(t))return null;const s=De.find(r=>r.toUpperCase()===t);if(s)return s;const a=De.find(r=>{const l=r.lastIndexOf("("),i=r.lastIndexOf(")"),d=l>=0&&i>l?r.slice(l+1,i).toUpperCase():"";return d&&(t===d||t.endsWith("("+d+")"))});return a||{"INFORMATION TECHNOLOGY":"Information Technology Unit (IT)","WELFARE WORKERS UNIT":"Wellfare Workers Unit (WWU)","WELLFARE WORKERS UNIT":"Wellfare Workers Unit (WWU)"}[t]||null}function _t(e,t){const s=/^\d{4}$/.test(String(t||""))?Number(t):null,a=Array.from({length:12},()=>({total:0,ages:new Map,ageGroups:{"18-24":0,"25-30":0,"31-40":0,"41+":0}}));return e.forEach(o=>{const r=me(o.createdAt),l=gt(o);if(!r||!Number.isInteger(l)||l<18||s&&r.getFullYear()!==s)return;const i=a[r.getMonth()];i.total++,i.ages.set(l,(i.ages.get(l)||0)+1),l<=24?i.ageGroups["18-24"]++:l<=30?i.ageGroups["25-30"]++:l<=40?i.ageGroups["31-40"]++:i.ageGroups["41+"]++}),a.map((o,r)=>({month:new Intl.DateTimeFormat("en-US",{month:"long"}).format(new Date(2024,r,1)).toUpperCase(),totalAdded:o.total,ageGroups:o.ageGroups,exactAges:[...o.ages.entries()].sort((l,i)=>l[0]-i[0])}))}function rt(e){return e.reduce((t,s)=>{const a=me(s.createdAt);return a?Math.max(t,a.getFullYear()):t},0)}const ot={ILIGAN:120,KAUSWAGAN:50,BACOLOD:45,MAIGO:35,KOLAMBUGAN:50,TUBOD:65,BAROY:40,SALVADOR:35,KAPATAGAN:75,LALA:60,SAPAD:35,BALOI:40};async function Ve(e=!1){if(localStorage.getItem("isLoggedIn")!=="true"||!document.getElementById("workforce-chart"))return;let t=[];if(e&&(Me=null),Me)t=Me;else{const[u,w]=await Promise.all([Ge(),Dt()]);if(u.length>0&&w<6e4)t=u,Me=t,console.log(`[Charts] IndexedDB hit — ${t.length} records (${Math.round(w/1e3)}s old)`);else try{const S=await ce("api/beneficiaries.php?all=true");if(S.success&&S.data?.success&&S.data?.beneficiaries)t=S.data.beneficiaries,Me=t,Ne(t).catch(()=>{}),console.log(`[Charts] API fetch — cached ${t.length} records to IndexedDB`);else{console.debug("[CHARTS] Skipping chart render:",S.data?.error||S.error);return}}catch(S){console.debug("[CHARTS] Chart init skipped:",S?.message);return}}if(t.length===0)return;Vt();const s=Je();document.querySelectorAll('[id$="-chart"]').forEach(n=>n.innerHTML="");const a=[...new Set(t.map(n=>{const u=n.startDate||n.createdAt,w=me(u);return w?w.getFullYear().toString():null}).filter(n=>n))].sort((n,u)=>u-n);qt(a,t),Ut(a,t);const o=new Date;let r=t;re!=="ALL"&&(r=t.filter(n=>{if(re.includes("D")){const u=me(n.createdAt);if(!u)return!1;const w=parseInt(re),S=new Date;return S.setDate(o.getDate()-w),S.setHours(0,0,0,0),u>=S}else if(a.includes(re)){const u=me(n.startDate||n.createdAt);return u?u.getFullYear().toString()===re:!1}return!0}));const l=We(t),i=We(r);Ht(l,i);let d=[];if(re==="ALL"){const u=new Date().getFullYear();for(let w=2020;w<=u;w++)d.push(w.toString())}else if(a.includes(re))d=["Q1","Q2","Q3","Q4"];else{const n=parseInt(re)||7;d=Array.from({length:n},(u,w)=>{const S=new Date;return S.setDate(o.getDate()-(n-1-w)),new Date(S.getTime()-S.getTimezoneOffset()*6e4).toISOString().split("T")[0]})}const g={};d.forEach(n=>g[n]=0),r.forEach(n=>{const u=n.startDate||n.createdAt;if(u){const w=me(u);if(!w)return;const S=w.getFullYear().toString(),$=new Date(w.getTime()-w.getTimezoneOffset()*6e4).toISOString().split("T")[0];if(re==="ALL")g.hasOwnProperty(S)&&g[S]++;else if(re.includes("D"))g.hasOwnProperty($)&&g[$]++;else if(S===re){const B="Q"+(Math.floor(w.getMonth()/3)+1);g.hasOwnProperty(B)&&g[B]++}}});const p=Object.values(g),f=r.length,v=p[p.length-1]||0,c=p[p.length-2]||0;let h;if(re==="ALL"){const n=f/d.length;h=v>=n}else h=v>=c;let C=h?fe.successGreen:fe.philippineRed,x=h?"bg-green-500 shadow-green-500/30":"bg-red-500 shadow-red-500/30",E=h?"text-green-600 dark:text-green-400":"text-red-600 dark:text-red-400";re==="ALL"?(C=fe.successGreen,x="bg-green-500 shadow-green-500/30",E="text-green-600 dark:text-green-400"):re==="7D"?(C="#fb923c",x="bg-orange-500 shadow-orange-500/30",E="text-orange-500 dark:text-orange-400"):re==="30D"?(C="#eab308",x="bg-yellow-500 shadow-yellow-500/30",E="text-yellow-600 dark:text-yellow-400"):re==="90D"?(C="#2563eb",x="bg-blue-600 shadow-blue-600/30",E="text-blue-600 dark:text-blue-400"):a.includes(re)&&(C="#f87171",x="bg-red-400 shadow-red-400/30",E="text-red-500 dark:text-red-400"),document.querySelectorAll(".metric-added-count").forEach(n=>{n.textContent=f,n.className=`text-3xl sm:text-5xl font-black transition-colors duration-500 leading-none metric-added-count ${E}`});const L=c>0?Math.round((v-c)/c*100):v>0?100:0;document.querySelectorAll(".metric-added-rate").forEach(n=>n.textContent=(L>=0?"+":"")+(re==="ALL"?"Growth":L+"%"));const X=document.getElementById("added-metric-badge");X&&(X.className=`flex items-center px-3 py-1 text-[0.625rem] sm:text-xs font-black text-white rounded-full shadow-lg transition-all duration-500 border border-white/20 translate-y-1 ${x}`);const Q=document.getElementById("added-metric-icon");Q&&(Q.style.transform=h?"rotate(0deg)":"rotate(180deg)"),["dropdownDefaultButton","dropdownLastDaysEduButton","dropdownLastDays3Button"].forEach(n=>{const u=document.getElementById(n);u&&(u.innerHTML=`${ut} <svg class="w-3 h-3 ms-1.5" aria-hidden="true" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 9-7 7-7-7" /></svg>`)});const te={chart:{height:250,type:"area",fontFamily:"Montserrat, sans-serif",toolbar:{show:!1},sparkline:{enabled:!1},background:"transparent"},theme:{mode:de()?"dark":"light"},fill:{type:"gradient",gradient:{shadeIntensity:1,opacityFrom:.6,opacityTo:.1,stops:[0,90,100],colorStops:[{offset:0,color:C,opacity:.6},{offset:100,color:C,opacity:.1}]}},stroke:{curve:"smooth",width:3,colors:[C]},series:[{name:"New Beneficiaries",data:p}],xaxis:{categories:d,labels:{show:!0,style:{colors:s.muted,fontSize:"0.625rem",fontWeight:600}},axisBorder:{show:!1},axisTicks:{show:!1}},yaxis:{show:!0,labels:{show:!0,style:{colors:s.muted,fontSize:"0.625rem",fontWeight:600}}},grid:{show:!0,borderColor:s.grid,strokeDashArray:4,padding:{left:10,right:15,top:0,bottom:0}},colors:[C],markers:{size:d.length>20?0:4,colors:[C],strokeColors:s.cardBg,strokeWidth:2,hover:{size:6}},tooltip:{theme:de()?"dark":"light",y:{title:{formatter:()=>"Added:"}}}};Ce("workforce-chart",te);const ne=Re==="ALL"?t:t.filter(n=>{const u=me(n.startDate||n.createdAt);return u&&u.getFullYear().toString()===Re}),ge=We(ne),pe={series:[ge.genders.Female||0,ge.genders.Male||0],chart:{height:320,type:"donut",fontFamily:"Montserrat, sans-serif",background:s.cardBg},colors:[fe.philippineRed,fe.royalBlue()],labels:["Female","Male"],plotOptions:{pie:{donut:{size:"75%",labels:{show:!0,name:{show:!0,fontSize:"0.75rem",fontWeight:600,color:s.muted},value:{show:!0,fontSize:"1.5rem",fontWeight:900,color:s.text,formatter:n=>n},total:{show:!0,label:"TOTAL",fontSize:"0.625rem",fontWeight:800,color:s.muted,formatter:n=>n.globals.seriesTotals.reduce((u,w)=>u+w,0)}}}}},dataLabels:{enabled:!1},legend:{show:!1},stroke:{colors:[s.cardBg],width:4},theme:{mode:de()?"dark":"light"},responsive:[{breakpoint:640,options:{chart:{height:260},plotOptions:{pie:{donut:{size:"70%",labels:{value:{fontSize:"1.125rem"}}}}}}}]};Ce("gender-chart",pe);const be=[{key:"College Grad",label:"College Graduate",count:i.education["College Grad"]||0,color:fe.royalBlue()},{key:"College Lvl",label:"College Level",count:i.education["College Lvl"]||0,color:fe.goldenYellow},{key:"HS Grad",label:"High School Graduate",count:i.education["HS Grad"]||0,color:fe.philippineRed},{key:"Senior High",label:"Senior High",count:i.education["Senior High"]||0,color:fe.successGreen}],K=be.reduce((n,u)=>n+u.count,0),ue=[...be].sort((n,u)=>u.count-n.count||n.label.localeCompare(u.label)),ve=ue[0];Object.entries({"College Grad":".count-college-grad","College Lvl":".count-college-lvl","HS Grad":".count-hs-grad","Senior High":".count-senior-high"}).forEach(([n,u])=>{document.querySelectorAll(u).forEach(w=>{w.textContent=i.education[n]||0})});const N=document.getElementById("education-profile-total"),_=document.getElementById("education-profile-leading");if(N&&(N.textContent=K),_){const n=K>0?Math.round(ve.count/K*100):0;_.textContent=K>0?`${ve.label} · ${n}%`:"No data",_.title=_.textContent}const j={series:[{name:"Beneficiaries",data:ue.map(n=>({x:n.label,y:n.count,fillColor:n.color}))}],chart:{height:270,type:"bar",toolbar:{show:!1},fontFamily:"Montserrat, sans-serif",background:s.cardBg},plotOptions:{bar:{horizontal:!0,distributed:!0,barHeight:"48%",dataLabels:{position:"top"},borderRadius:3,borderRadiusApplication:"end"}},dataLabels:{enabled:!0,formatter:n=>Math.round(n),offsetX:10,textAnchor:"start",style:{fontSize:"0.625rem",fontWeight:900,colors:[s.text]},background:{enabled:!0,borderRadius:2,padding:3,opacity:.9,borderWidth:0}},xaxis:{min:0,forceNiceScale:!0,labels:{formatter:n=>Math.round(n),style:{colors:s.muted,fontSize:"0.5625rem",fontWeight:700}},axisBorder:{show:!1},axisTicks:{show:!1},title:{text:"TOTAL BENEFICIARIES",style:{color:s.muted,fontSize:"0.5625rem",fontWeight:800}}},yaxis:{labels:{maxWidth:150,style:{colors:s.text,fontSize:"0.625rem",fontWeight:800}}},grid:{borderColor:s.grid,strokeDashArray:6,xaxis:{lines:{show:!0}},yaxis:{lines:{show:!1}},padding:{top:0,right:40,bottom:-8,left:4}},legend:{show:!1},tooltip:{theme:de()?"dark":"light",y:{formatter:n=>{const u=K>0?Math.round(n/K*100):0;return`${n} beneficiaries (${u}% of recorded)`}}},noData:{text:"NO EDUCATION DATA",style:{color:s.muted,fontSize:"11px"}},theme:{mode:de()?"dark":"light"},responsive:[{breakpoint:640,options:{chart:{height:285},yaxis:{labels:{maxWidth:110,style:{fontSize:"0.5625rem"}}},dataLabels:{style:{fontSize:"0.5625rem"}}}}]};Ce("education-chart",j),document.querySelectorAll(".count-absorbed").forEach(n=>n.textContent=i.status.ABSORBED||0),document.querySelectorAll(".count-ongoing").forEach(n=>n.textContent=i.status.ONGOING||0);const z={series:[{name:"Beneficiaries",data:[{x:"Absorbed",y:i.status.ABSORBED||0,fillWeight:1},{x:"Ongoing",y:i.status.ONGOING||0},{x:"Expired",y:i.status.EXPIRED||0},{x:"Resigned",y:i.status.RESIGNED||0}]}],chart:{type:"bar",height:260,fontFamily:"Montserrat, sans-serif",toolbar:{show:!1},background:"transparent"},plotOptions:{bar:{horizontal:!1,columnWidth:"65%",borderRadius:10,distributed:!0,dataLabels:{position:"top"}}},colors:["#059669","#6ee7b7","#CE1126","#64748b"],dropShadow:{enabled:!0,top:3,left:0,blur:4,opacity:.22,color:"#64748b"},dataLabels:{enabled:!0,offsetY:-20,style:{fontSize:"0.75rem",fontWeight:"900",colors:[s.text]}},legend:{show:!1},xaxis:{categories:["Absorbed","Ongoing","Expired","Resigned"],axisBorder:{show:!1},axisTicks:{show:!1},labels:{style:{colors:s.muted,fontWeight:700}}},yaxis:{show:!1},grid:{show:!1},tooltip:{theme:de()?"dark":"light",y:{formatter:n=>n+" Beneficiaries"}},theme:{mode:de()?"dark":"light"}};Ce("status-chart",z);const G=new Map(Object.entries(l.designations).map(([n,u])=>[n.trim().toUpperCase(),u])),V=new Map(De.map((n,u)=>[n,u])),oe=De.map(n=>[n,G.get(n.toUpperCase())||0]).sort((n,u)=>u[1]-n[1]||V.get(n[0])-V.get(u[0])),ae=oe.map(([n])=>n),W={series:[{name:"Total GIP",data:oe.map(([,n])=>n)}],chart:{type:"bar",height:220,toolbar:{show:!1},fontFamily:"Montserrat, sans-serif",background:s.cardBg},colors:[fe.royalBlue()],plotOptions:{bar:{horizontal:!1,columnWidth:"34%",borderRadius:3,borderRadiusApplication:"end"}},dataLabels:{enabled:!0,offsetY:-14,formatter:n=>Math.round(n),style:{fontSize:"0.625rem",fontWeight:900,colors:[s.text]},background:{enabled:!0,borderRadius:2,padding:3,opacity:.86,borderWidth:0}},xaxis:{categories:ae,axisBorder:{show:!1},axisTicks:{show:!1},labels:{rotate:0,trim:!1,hideOverlappingLabels:!1,formatter:n=>Tt(n),style:{fontWeight:800,colors:s.muted,fontSize:"0.5625rem"}}},yaxis:{min:0,forceNiceScale:!0,labels:{formatter:n=>Math.round(n),style:{fontWeight:700,colors:s.muted,fontSize:"0.5625rem"}},title:{text:"TOTAL COUNT",style:{color:s.muted,fontSize:"0.5625rem",fontWeight:800}}},grid:{show:!0,borderColor:s.grid,strokeDashArray:7,xaxis:{lines:{show:!1}},yaxis:{lines:{show:!0}},padding:{top:18,left:2,right:4,bottom:-4}},legend:{show:!1},tooltip:{theme:de()?"dark":"light",x:{formatter:(n,u)=>ae[u.dataPointIndex]||"Assigned Unit"},y:{formatter:n=>`${n} Beneficiaries`}},theme:{mode:de()?"dark":"light"},responsive:[{breakpoint:640,options:{chart:{height:210},plotOptions:{bar:{columnWidth:"46%"}},dataLabels:{style:{fontSize:"0.5rem"}},xaxis:{labels:{style:{fontSize:"0.5rem"}}}}}]};Ce("assigned-units-chart",W),jt(oe);const ee=(/^\d{4}$/.test(re)?Number(re):0)||rt(r)||rt(t)||new Date().getFullYear(),b=_t(r,ee),k=document.getElementById("age-chart-year");k&&(k.textContent=ee);const y=[{key:"18-24",label:"AGE 18–24"},{key:"25-30",label:"AGE 25–30"},{key:"31-40",label:"AGE 31–40"},{key:"41+",label:"AGE 41+"}],I={series:y.map(n=>({name:n.label,data:b.map(u=>u.ageGroups[n.key])})),chart:{type:"bar",stacked:!0,height:330,toolbar:{show:!1},zoom:{enabled:!1},fontFamily:"Montserrat, sans-serif",background:s.cardBg},colors:["#0038A8","#2563EB","#60A5FA","#93C5FD"],plotOptions:{bar:{horizontal:!1,columnWidth:"54%",borderRadius:2,borderRadiusApplication:"end",dataLabels:{total:{enabled:!0,offsetY:-8,style:{fontSize:"0.625rem",fontWeight:900,color:s.text}}}}},dataLabels:{enabled:!0,formatter:n=>n>0?Math.round(n):"",style:{fontSize:"0.5625rem",fontWeight:900,colors:["#ffffff","#ffffff","#0f172a","#0f172a"]},dropShadow:{enabled:!1}},xaxis:{categories:b.map(n=>n.month),axisBorder:{show:!0,color:s.grid},axisTicks:{show:!1},title:{text:"MONTH ADDED",style:{color:s.muted,fontSize:"0.5625rem",fontWeight:800}},labels:{rotate:-40,trim:!1,hideOverlappingLabels:!1,style:{fontWeight:800,colors:s.muted,fontSize:"0.5625rem"}}},yaxis:{min:0,forceNiceScale:!0,labels:{formatter:n=>Math.round(n),style:{fontWeight:700,colors:s.muted,fontSize:"0.625rem"}},title:{text:"TOTAL BENEFICIARIES",style:{color:s.muted,fontSize:"0.5625rem",fontWeight:800}}},grid:{show:!0,borderColor:s.grid,strokeDashArray:7,xaxis:{lines:{show:!1}},yaxis:{lines:{show:!0}},padding:{top:18,left:8,right:12,bottom:4}},legend:{show:!0,position:"top",horizontalAlign:"left",fontSize:"10px",fontWeight:800,labels:{colors:s.muted},markers:{size:5,shape:"square"},itemMargin:{horizontal:8,vertical:3}},tooltip:{shared:!0,intersect:!1,theme:de()?"dark":"light",custom:({dataPointIndex:n})=>{const u=b[n],w=y.map($=>`${$.label}: <strong>${u?.ageGroups[$.key]||0}</strong>`).join("<br>"),S=u?.exactAges?.length?u.exactAges.map(([$,B])=>`Age ${$}: ${B}`).join(" · "):"No recorded ages";return`<div class="px-3 py-2 text-xs leading-5"><strong>${u?.month||""} ${ee}</strong><br>Total: <strong>${u?.totalAdded||0}</strong><br>${w}<div class="mt-1 border-t border-slate-200 pt-1 text-[10px] dark:border-slate-600">${S}</div></div>`}},noData:{text:"NO AGE DATA",align:"center",verticalAlign:"middle",style:{color:s.muted,fontSize:"11px"}},theme:{mode:de()?"dark":"light"},responsive:[{breakpoint:640,options:{chart:{height:360},plotOptions:{bar:{columnWidth:"66%"}},dataLabels:{enabled:!1},xaxis:{labels:{rotate:-55,style:{fontSize:"0.5rem"}}},legend:{fontSize:"9px",itemMargin:{horizontal:5,vertical:2}}}}]};Ce("age-chart",I);const D=Pt(t);Ft(D);const m={series:[{name:"Actual Beneficiaries",data:Object.values(D.municipalityData).map(n=>n.actual)},{name:"Target Slots",data:Object.values(D.municipalityData).map(n=>n.target)}],chart:{type:"bar",height:350,fontFamily:"Montserrat, sans-serif",toolbar:{show:!1},background:s.cardBg},theme:{mode:de()?"dark":"light"},colors:[fe.royalBlue(),de()?"#334155":"#E2E8F0"],plotOptions:{bar:{horizontal:!1,columnWidth:"55%",borderRadius:8}},dataLabels:{enabled:!1},stroke:{show:!0,width:2,colors:["transparent"]},xaxis:{categories:Object.keys(D.municipalityData),labels:{style:{fontWeight:600,colors:s.muted,fontSize:"0.5625rem"}},axisBorder:{show:!1},axisTicks:{show:!1}},yaxis:{labels:{style:{fontWeight:600,colors:s.muted}}},legend:{show:!1},fill:{opacity:1},grid:{borderColor:s.grid,strokeDashArray:4,yaxis:{lines:{show:!0}}}};Ce("performance-gap-chart",m)}function jt(e){const t=document.getElementById("assigned-units-summary");t&&(t.innerHTML=e.map(([s,a],o)=>`
    ...
    `).join(""))}function Ce(e,t){const s=document.getElementById(e);if(!s)return;s.innerHTML="",new Ze(s,t).render()}function We(e){const t={offices:{},genders:{Female:0,Male:0,Unknown:0},education:{"College Grad":0,"College Lvl":0,"HS Grad":0,"Senior High":0},designations:{},ages:{"18-24":0,"25-30":0,"31-40":0,"41+":0},exactAges:{},totalAge:0,ageCount:0,status:{ABSORBED:0,ONGOING:0,EXPIRED:0,RESIGNED:0}},s=new Date;return s.setHours(0,0,0,0),e.forEach(a=>{const o=a.office||"Unassigned";t.offices[o]=(t.offices[o]||0)+1;const r=(a.gender||"Unknown").trim(),l=r==="F"||r==="Female"?"Female":r==="M"||r==="Male"?"Male":"Unknown";t.genders[l]++;const i=String(a.education||"").trim().toUpperCase().replace(/\s+/g," "),d=i.includes("COLLEGE")&&i.includes("GRADUATE")||i.includes("BACHELOR")||i.includes("DEGREE")||/(^|\s)(BS|AB)(\s|$)/.test(i);i.includes("SENIOR HIGH")?t.education["Senior High"]++:d?t.education["College Grad"]++:i.includes("COLLEGE")?t.education["College Lvl"]++:(i.includes("HIGH SCHOOL")||/(^|\s)HS(\s|$)/.test(i))&&t.education["HS Grad"]++;const g=Rt(a.designation);g&&(t.designations[g]=(t.designations[g]||0)+1);const p=(a.remarks||a.status_name||"").trim().replace(/\s+/g,"").toUpperCase(),f=!!a.absorbDate;if(p.includes("ABSORBED")||f)t.status.ABSORBED++;else if(p.includes("RESIGNED"))t.status.RESIGNED++;else if(p==="ONGOING"||p.includes("ONGOING")||p.includes("ACTIVE")||a.status_id==1)t.status.ONGOING++;else if(p.includes("EXPIRED"))t.status.EXPIRED++;else{let c=!1;if(a.endDate){const h=me(a.endDate);h&&h<s&&(c=!0)}c?t.status.EXPIRED++:t.status.ONGOING++}const v=gt(a);Number.isInteger(v)&&(t.totalAge+=v,t.ageCount++,t.exactAges[v]=(t.exactAges[v]||0)+1,v>=18&&v<=24?t.ages["18-24"]++:v>=25&&v<=30?t.ages["25-30"]++:v>=31&&v<=40?t.ages["31-40"]++:v>=41&&t.ages["41+"]++)}),t}function Pt(e){const t={municipalityData:{},totalTarget:0,totalActual:0,retention:{count:0,resign:0},velocity:{totalDays:0,count:0}};return Object.entries(ot).forEach(([s,a])=>{t.municipalityData[s]={actual:0,target:a},t.totalTarget+=a}),e.forEach(s=>{const a=(s.office||"").toUpperCase();let o="OTHER";for(const l in ot)if(a.includes(l)){o=l;break}if(t.municipalityData[o]&&(t.municipalityData[o].actual++,t.totalActual++),(s.remarks||"ONGOING").toUpperCase()==="RESIGNED"?t.retention.resign++:t.retention.count++,s.createdAt&&s.startDate){const l=new Date(s.createdAt),i=new Date(s.startDate),d=Math.ceil((i-l)/(1e3*60*60*24));d>=0&&d<180&&(t.velocity.totalDays+=d,t.velocity.count++)}}),t}function Ft(e){const t=e.totalTarget>0?(e.totalActual/e.totalTarget*100).toFixed(1):0;document.querySelectorAll(".metric-utilization-rate").forEach(r=>{r.textContent=t+"%";const l=r.parentElement?.nextElementSibling?.firstElementChild;l&&(l.style.width=t+"%")});const s=e.velocity.count>0?(e.velocity.totalDays/e.velocity.count).toFixed(1):"14.2";document.querySelectorAll(".metric-velocity-avg").forEach(r=>r.textContent=s);const a=e.retention.count+e.retention.resign,o=a>0?(e.retention.count/a*100).toFixed(1):"100";document.querySelectorAll(".metric-retention-rate").forEach(r=>r.textContent=o+"%")}function Ht(e,t){const s=Object.values(e.offices).reduce((v,c)=>v+c,0);document.querySelectorAll(".metric-total-beneficiaries").forEach(v=>v.textContent=s);const a=e.genders.Female||0,o=e.genders.Male||0,r=a+o,l=r>0?Math.round(a/r*100)+"%":"0%",i=r>0?Math.round(o/r*100)+"%":"0%";document.querySelectorAll(".metric-female-ratio").forEach(v=>v.textContent=l),document.querySelectorAll(".metric-male-ratio").forEach(v=>v.textContent=i);const d=v=>{const[c,h]=Object.entries(v.exactAges||{}).sort((x,E)=>E[1]-x[1]||Number(x[0])-Number(E[0]))[0]||[null,0],C=v.ageCount>0?h/v.ageCount*100:0;return{age:c,count:h,percentage:C}},g=v=>`${Number.isInteger(v)?v.toFixed(0):v.toFixed(1)}%`,p=d(e);document.querySelectorAll(".metric-top-age-label").forEach(v=>{v.textContent=p.age===null?"N/A":`${p.age} YRS`}),document.querySelectorAll(".metric-top-age-share").forEach(v=>{v.textContent=`${g(p.percentage)} of recorded ages`});const f=d(t);document.querySelectorAll(".metric-top-age").forEach(v=>{v.textContent=f.age===null?"N/A":f.age}),document.querySelectorAll(".metric-filtered-top-age-share").forEach(v=>{v.textContent=`${g(f.percentage)} of filtered ages`})}function qt(e,t){const s=document.querySelector("#lastDaysdropdown ul");if(!s)return;const a=t.length,o=new Date,r=d=>{const g=new Date;return g.setDate(o.getDate()-d),g.setHours(0,0,0,0),t.filter(p=>{const f=me(p.createdAt);return f&&f>=g}).length},l=d=>t.filter(g=>{const p=me(g.startDate||g.createdAt);return p&&p.getFullYear().toString()===d}).length;let i=`
        <li>
    ...
    `;e.forEach(d=>{const g=l(d);i+=`
        ...
        </li>`}),s.innerHTML=i}function Ut(e,t){const s=document.getElementById("gender-filter-options"),a=document.getElementById("gender-filter-button");if(!s||!a)return;const o=t.length,r=i=>t.filter(d=>{const g=me(d.startDate||d.createdAt);return g&&g.getFullYear().toString()===i}).length;let l=`
        <li>
    ...
    `;e.forEach(i=>{const d=r(i);l+=`
        ...
        `}),s.innerHTML=l,a.innerHTML=`${pt} <svg class="w-3 h-3 ms-1.5" aria-hidden="true" fill="none" viewBox="0 0 24 24"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m19 9-7 7-7-7" /></svg>`}function Gt(e,t){re=e,ut=t,Ue("user_workforce_filter",e,30),Ue("user_workforce_label",t,30),["lastDaysdropdown"].forEach(a=>{const o=document.getElementById(a);if(o&&window.FlowbiteInstances){const r=window.FlowbiteInstances.getInstance("Dropdown",a);r&&r.hide()}else o&&o.classList.add("hidden")}),Ve()}function zt(e,t){Re=e,pt=t,Ue("user_gender_filter",e,30),Ue("user_gender_label",t,30);const s=document.getElementById("gender-filter-dropdown");if(s&&window.FlowbiteInstances){const a=window.FlowbiteInstances.getInstance("Dropdown","gender-filter-dropdown");a&&a.hide()}else s&&s.classList.add("hidden");Ve()}function Vt(){const e=localStorage.getItem("user");if(e)try{const t=JSON.parse(e),s=t.full_name||t.username||"System User",a=t.email||(t.username?`${t.username}@dole.gov.ph`:"user@dole.gov.ph"),o=t.profile_picture_path,r=s.trim().split(" ").map(l=>l[0]).join("").substring(0,2).toUpperCase()||"??";document.querySelectorAll(".sidebar-user-name").forEach(l=>l.textContent=s),document.querySelectorAll(".sidebar-user-email").forEach(l=>l.textContent=a),document.querySelectorAll(".sidebar-user-avatar").forEach(l=>{const i=l.querySelector(".sidebar-avatar-initials"),d=l.querySelector(".sidebar-avatar-img");if(o&&d){const g=he(),p=o.startsWith("http")?o:g+o.replace(/^\//,"");d.src=p,d.classList.remove("hidden"),i&&i.classList.add("hidden")}else i&&(i.textContent=r,i.classList.remove("hidden"),d&&d.classList.add("hidden"))})}catch(t){console.error("Failed to parse user data for sidebar:",t)}}window.updateWorkforceFilter=Gt;window.updateGenderFilter=zt;document.addEventListener("themeChanged",()=>{setTimeout(()=>Ve(),50)});window.addEventListener("dataSynced",()=>{console.log("[Charts] Data synced detected, refreshing analytics..."),Ve(!0)});let Le=null;function Yt(e,t){const s=document.getElementById(t);if(!s)return;if(Le&&(Le.destroy(),Le=null),e.length===0){const g=Je(),p={series:[1],chart:{type:"donut",height:250,fontFamily:"Montserrat, sans-serif",background:"transparent",animations:{enabled:!1}},labels:["No Data"],colors:[g.grid],plotOptions:{pie:{donut:{size:"65%",labels:{show:!0,name:{show:!1},value:{show:!0,fontSize:"24px",fontWeight:900,color:g.muted,formatter:()=>"0"},total:{show:!0,showAlways:!0,label:"TOTAL",fontSize:"10px",fontWeight:900,color:g.muted,formatter:()=>"0"}}}}},dataLabels:{enabled:!1},stroke:{show:!0,colors:de()?["#1e293b"]:["#ffffff"],width:2},tooltip:{enabled:!1},legend:{show:!1}};Le=new Ze(s,p),Le.render();return}const a={ongoing:0,expired:0,absorbed:0,resigned:0,other:0};e.forEach(g=>{const p=(g.remarks||"").toUpperCase();p==="ONGOING"?a.ongoing++:p==="EXPIRED"?a.expired++:p==="ABSORBED"?a.absorbed++:p==="RESIGNED"?a.resigned++:a.other++});const o=[],r=[],l=[];a.ongoing>0&&(o.push(a.ongoing),r.push("Ongoing"),l.push(fe.successGreen)),a.expired>0&&(o.push(a.expired),r.push("Expired"),l.push(fe.philippineRed)),a.absorbed>0&&(o.push(a.absorbed),r.push("Absorbed"),l.push("#059669")),a.resigned>0&&(o.push(a.resigned),r.push("Resigned"),l.push("#b91c1c")),a.other>0&&(o.push(a.other),r.push("Other"),l.push(fe.mutedSlate()));const i=Je(),d={series:o,chart:{type:"donut",height:250,fontFamily:"Montserrat, sans-serif",background:"transparent",animations:{enabled:!0,easing:"easeinout",speed:800,dynamicAnimation:{enabled:!0,speed:350}}},labels:r,colors:l,plotOptions:{pie:{donut:{size:"65%",labels:{show:!0,name:{show:!0,fontSize:"10px",fontWeight:800,color:i.muted,offsetY:-5},value:{show:!0,fontSize:"24px",fontWeight:900,color:i.text,offsetY:5},total:{show:!0,showAlways:!0,label:"TOTAL",fontSize:"10px",fontWeight:900,color:i.muted}}}}},dataLabels:{enabled:!1},stroke:{show:!0,colors:de()?["#1e293b"]:["#ffffff"],width:2},tooltip:{theme:de()?"dark":"light",style:{fontSize:"12px"}},legend:{show:!1}};Le=new Ze(s,d),Le.render()}const Wt="modulepreload",Zt=function(e,t){return new URL(e,t).href},at={},ft=function(t,s,a){let o=Promise.resolve();if(s&&s.length>0){let g=function(p){return Promise.all(p.map(f=>Promise.resolve(f).then(v=>({status:"fulfilled",value:v}),v=>({status:"rejected",reason:v}))))};const l=document.getElementsByTagName("link"),i=document.querySelector("meta[property=csp-nonce]"),d=i?.nonce||i?.getAttribute("nonce");o=g(s.map(p=>{if(p=Zt(p,a),p in at)return;at[p]=!0;const f=p.endsWith(".css"),v=f?'[rel="stylesheet"]':"";if(a)for(let h=l.length-1;h>=0;h--){const C=l[h];if(C.href===p&&(!f||C.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${p}"]${v}`))return;const c=document.createElement("link");if(c.rel=f?"stylesheet":Wt,f||(c.as="script"),c.crossOrigin="",c.href=p,d&&c.setAttribute("nonce",d),document.head.appendChild(c),f)return new Promise((h,C)=>{c.addEventListener("load",h),c.addEventListener("error",()=>C(new Error(`Unable to preload CSS for ${p}`)))})}))}function r(l){const i=new Event("vite:preloadError",{cancelable:!0});if(i.payload=l,window.dispatchEvent(i),!i.defaultPrevented)throw l}return o.then(l=>{for(const i of l||[])i.status==="rejected"&&r(i.reason);return t().catch(r)})};let Se=null;if(Te()){const e="https://llnddycvbcetztzwbdpx.supabase.co",t="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxsbmRkeWN2YmNldHp0endiZHB4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ1MDMwMTEsImV4cCI6MjA5MDA3OTAxMX0.apIpDlZYja7SNuA-joXgFSvu_wtbBcVVP3OQSPrh7fA";try{Se=vt(e,t),console.log("[SUPABASE SDK] Client Initialized Successfully")}catch(s){console.error("[SUPABASE SDK] Failed to initialize client:",s)}}else console.log("[SUPABASE SDK] Supabase mode is disabled (Localhost PHP mode active).");function Jt(e=new Date().getFullYear()){const t=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"],s=[];for(let a=0;a<12;a++){const o=new Date(e,a+1,0).getDate();s.push(`${t[a]} 1-15, ${e}`),s.push(`${t[a]} 16-${o}, ${e}`)}return s}function Xt(e,t,s){if(s==="ar")return(e.period||"").toUpperCase().trim()===t.toUpperCase().trim();{const a=t.match(/([A-Z]+)\s+(\d+)-(\d+),\s+(\d+)/i);if(!a)return!1;const r=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"].indexOf(a[1].toUpperCase());if(r===-1)return!1;const l=parseInt(a[4]),i=parseInt(a[2]),d=parseInt(a[3]),g=(e.date||"").substring(0,10),p=new Date(g+"T00:00:00");return isNaN(p)?!1:p.getFullYear()===l&&p.getMonth()===r&&p.getDate()>=i&&p.getDate()<=d}}function Kt(e){if(!e)return"-";const t=e.toUpperCase();return t==="VERIFIED"||t==="COMPLETED"?"✓":t==="REJECTED"||t==="DECLINED"?"X":t==="PENDING"?"?":t}function st(e,t,s,a){const o=e.map(r=>{const l=t[r.id]||[],i=a.map(d=>{const g=l.find(p=>Xt(p,d,s));return g?Kt(g.status):"-"});return{name:r.name||r.id,cells:i}});return{periods:a,rows:o}}function nt(e,t,s){const{periods:a,rows:o}=t,r=a.length+1;let l='<table border="1" style="border-collapse:collapse;font-family:Arial,sans-serif;font-size:10px;">';return l+=`<tr><td colspan="${r}" style="background:${s};color:#fff;font-weight:bold;font-size:13px;padding:8px 12px;text-transform:uppercase;letter-spacing:1px;">${e}</td></tr>`,l+=`<tr><th style="background:${s};color:#fff;padding:6px 10px;min-width:200px;text-align:left;">BENEFICIARY NAME</th>`,a.forEach(i=>{l+=`<th style="background:${s};color:#fff;padding:6px 8px;text-align:center;white-space:nowrap;">${i}</th>`}),l+="</tr>",o.forEach((i,d)=>{const g=d%2===0?"#ffffff":"#f5f5f5";l+="<tr>",l+=`<td style="background:${g};padding:5px 10px;font-weight:bold;text-transform:uppercase;">${i.name}</td>`,i.cells.forEach(p=>{l+=`<td style="background:${g};padding:5px 8px;text-align:center;font-weight:bold;color:${p==="✓"?"#15803d":p==="X"?"#dc2626":"#9ca3af"};">${p}</td>`}),l+="</tr>"}),l+="</table>",l}async function Cr(e){const t="flex flex-col items-center justify-center p-3 rounded-2xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[0.625rem] gap-1.5",s=e.length,a=await M.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Export DTR/AR',html:`
        ...
        `,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Generate Excel</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Cancel</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-emerald-600 text-white text-xs px-6 py-2.5 rounded-xl shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1,didOpen:()=>{document.querySelectorAll('input[name="exp-count"]').forEach(d=>{d.addEventListener("change",()=>{const g=document.getElementById("custom-count-wrap");g.classList.toggle("hidden",d.value!=="custom"||!d.checked);const p=document.querySelector('input[name="exp-count"]:checked');g.classList.toggle("hidden",p?.value!=="custom")})})},preConfirm:()=>{const d=document.querySelector('input[name="exp-type"]:checked')?.value||"dtr",g=document.querySelector('input[name="exp-count"]:checked')?.value||"10";let p=parseInt(g==="custom"?document.getElementById("exp-custom-count")?.value||s:g,10);(isNaN(p)||p<1)&&(p=10),p=Math.min(p,s);const f=parseInt(document.getElementById("exp-year")?.value||new Date().getFullYear(),10);return{type:d,count:p,year:f}}});if(!a.isConfirmed||!a.value)return;const{type:o,count:r,year:l}=a.value,i=e.slice(0,r);await bt(i,o,l)}async function bt(e,t,s){M.fire({title:'<span class="text-lg font-black text-emerald-600 uppercase tracking-tight">Building Excel File</span>',html:`<div class="flex flex-col items-center p-4">
        ...
        </div>`,showConfirmButton:!1,allowOutsideClick:!1,customClass:{popup:"rounded-[1.5rem] shadow-2xl font-montserrat dark:bg-slate-900 border border-gray-100 dark:border-slate-800"}});try{const a=Jt(s),o=he();async function r(C){const E=await(await fetch(`${o}api/logs.php?type=${C}`,{headers:{"X-User-Id":(()=>{try{return JSON.parse(localStorage.getItem("user"))?.id||""}catch{return""}})()}})).json();return E.success?E.logs||[]:[]}let l={},i={};(t==="dtr"||t==="both")&&(await r("dtr")).forEach(x=>{const E=String(x.gip_id||x.beneficiary_id||x.id||"");l[E]||(l[E]=[]),l[E].push(x)}),(t==="ar"||t==="both")&&(await r("ar")).forEach(x=>{const E=String(x.gip_id||x.beneficiary_id||x.id||"");i[E]||(i[E]=[]),i[E].push(x)});const d=e.map(C=>({...C,mapKey:String(C.id||C.gip_id||C.beneficiary_id)}));let g="";const p=new Intl.DateTimeFormat("en-PH",{timeZone:"Asia/Manila",dateStyle:"long",timeStyle:"short"}).format(new Date);if(t==="dtr"||t==="both"){const C=d.map(E=>({...E,id:E.mapKey})),x=st(C,l,"dtr",a);g+="<br>"+nt(`DTR – Daily Time Records (${s})`,x,"#1d4ed8")}if(t==="ar"||t==="both"){const C=d.map(E=>({...E,id:E.mapKey})),x=st(C,i,"ar",a);g+="<br><br>"+nt(`AR – Accomplishment Reports (${s})`,x,"#d97706")}const f=`
            ...
            <body>
                <p style="font-size:9px;color:#6b7280;">Generated: ${p} | DOLE LDNPFO – GIP Monitoring System</p>
                ${g}
            </body>
            </html>
        `,v=new Blob([f],{type:"application/vnd.ms-excel"}),c=URL.createObjectURL(v),h=document.createElement("a");h.href=c,h.download=`GIP_LOGS_${t.toUpperCase()}_${s}.xls`,document.body.appendChild(h),h.click(),URL.revokeObjectURL(c),document.body.removeChild(h),M.fire({toast:!0,position:"top-end",icon:"success",title:"Excel file downloaded!",showConfirmButton:!1,timer:2500})}catch(a){console.error("[LogsExport] Error:",a),M.fire("Error",a.message||"Failed to generate export.","error")}}const Ie="color-theme",Qt=3600*24*365;function er(e,t,s){document.cookie=`${e}=${t}; max-age=${s}; path=/; SameSite=Lax`}function ht(e){const t=document.cookie.match(new RegExp("(?:^|; )"+e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+"=([^;]*)"));return t?decodeURIComponent(t[1]):null}function mt(){const e=localStorage.getItem(Ie)||ht(Ie);return e==="dark"||e==="light"?e:"light"}function He(e){const t=document.documentElement;e==="dark"?t.classList.add("dark"):t.classList.remove("dark"),localStorage.setItem(Ie,e),er(Ie,e,Qt),tr(e),document.dispatchEvent(new CustomEvent("themeChanged",{detail:{theme:e}}))}function Xe(){const e=mt();He(e==="dark"?"light":"dark")}function tr(e){const t=e==="dark",s=document.getElementById("pref-dark-mode");s&&(s.checked=t);const a=document.getElementById("theme-toggle-dark-icon"),o=document.getElementById("theme-toggle-light-icon");a&&o&&(a.classList.toggle("hidden",t),o.classList.toggle("hidden",!t));const r=document.getElementById("sidebar-theme-label");r&&(r.textContent=t?"LIGHT MODE":"DARK MODE")}function Lr(){const e=mt();He(e);const t=document.getElementById("pref-dark-mode");t&&t.addEventListener("change",()=>{He(t.checked?"dark":"light")});const s=document.getElementById("theme-toggle-btn");s&&s.addEventListener("click",Xe),document.querySelectorAll("[data-theme-toggle]").forEach(a=>{a.addEventListener("click",Xe)}),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",a=>{localStorage.getItem(Ie)||ht(Ie)||He(a.matches?"dark":"light")})}function ye(){return document.documentElement.classList.contains("dark")}window.toggleTheme=Xe;window.isDarkMode=ye;const qe={queue:[],currentIndex:0,isActive:!1,isAutoSave:!1,lastInteractionTime:0,init(){this.showUploadModal()},showUploadModal(){const e=ye(),t={bgCard:e?"bg-slate-900/40":"bg-gray-50/40",borderCard:e?"border-slate-800":"border-gray-100",textHeading:e?"text-green-500":"text-[#2e7d32]",textSubtitle:e?"text-slate-500":"text-gray-400 dark:text-gray-300",bgUpload:e?"bg-slate-800":"bg-white",borderUpload:e?"border-slate-700":"border-gray-200",textUpload:e?"text-slate-400":"text-gray-500",hoverUpload:e?"hover:bg-slate-800/80 hover:border-blue-500":"hover:bg-blue-50/50 hover:border-royal-blue",iconText:e?"text-green-400":"text-[#2e7d32]",iconBg:e?"bg-green-900/20":"bg-[#e8f5e9]",iconBorder:e?"border-green-800/30":"border-[#c8e6c9]"},s=`
        ...
        `;M.fire({html:s,width:"600px",showConfirmButton:!1,showCloseButton:!0,customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:a=>{const o=a.querySelector("#csv-upload"),r=a.querySelector('label[for="csv-upload"]');["dragenter","dragover","dragleave","drop"].forEach(i=>{r.addEventListener(i,l,!1)});function l(i){i.preventDefault(),i.stopPropagation()}["dragenter","dragover"].forEach(i=>{r.addEventListener(i,()=>{r.classList.add("border-blue-500","bg-blue-50/50"),e&&r.classList.add("dark:bg-slate-800/80")},!1)}),["dragleave","drop"].forEach(i=>{r.addEventListener(i,()=>{r.classList.remove("border-blue-500","bg-blue-50/50"),e&&r.classList.remove("dark:bg-slate-800/80")},!1)}),o.addEventListener("change",i=>{const d=i.target.files[0];if(d){const g=a.querySelector("#auto-save-toggle");this.isAutoSave=g?g.checked:!1,this.handleFile(d)}}),r.addEventListener("drop",i=>{const g=i.dataTransfer.files[0];if(g){const p=a.querySelector("#auto-save-toggle");this.isAutoSave=p?p.checked:!1,this.handleFile(g)}},!1)}})},handleFile(e){if(!e.name.toLowerCase().endsWith(".csv")&&!e.name.toLowerCase().endsWith(".txt")){M.fire("Invalid File","Please upload a valid .csv or .txt file.","error");return}const t=new FileReader;t.onload=s=>{const a=s.target.result;this.parseCSV(a)},t.readAsText(e)},async parseCSV(e){let t=[],s="",a=!1;for(let o=0;o<e.length;o++){let r=e[o];r==='"'&&(a=!a),!a&&(r===`
`||r==="\r")?(r==="\r"&&e[o+1]===`
`&&o++,s.trim()!==""&&t.push(s),s=""):s+=r}s.trim()!==""&&t.push(s),this.queue=[];for(let o=0;o<t.length;o++){let r=t[o].trim();if(!r)continue;let l=[],i="",d=!1;for(let g=0;g<r.length;g++){let p=r[g];p==='"'?d=!d:p===","&&!d?(l.push(i.replace(/(^"|"$)/g,"").trim()),i=""):i+=p}if(l.push(i.replace(/(^"|"$)/g,"").trim()),l.length>=2){const g=l[3];if(!g||isNaN(parseInt(g)))continue;const p=l[1];if(!p||p.toLowerCase()==="name"||p.toLowerCase()==="full name")continue;const f=l[2];let v=l[4]?l[4].toUpperCase().trim():"",c="";(v==="F"||v.includes("FEMALE"))&&(c="Female"),(v==="M"||v.includes("MALE"))&&(c="Male");const h=l[5],C=l[6],x=l[7],E=this.formatDate(l[8]),L=this.formatDate(l[9]);this.queue.push({name:p,address:f,age:g,gender:c,education:h,startDate:E,endDate:L,office:C,designation:x})}}if(this.queue.length>0){try{M.fire({title:"Checking duplicates...",html:'<p class="text-sm">Please wait while we cross-reference your data.</p>',allowOutsideClick:!1,didOpen:()=>{M.showLoading()}});const o=this.queue.map(d=>d.name);let r=null;try{r=JSON.parse(localStorage.getItem("user")||"{}")?.id||null}catch{r=null}const i=await(await fetch(`${he()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json",...r?{"X-User-Id":String(r)}:{}},body:JSON.stringify({names:o,user_id:r})})).json();if(i.success&&i.duplicates&&i.duplicates.length>0){const d=new Set(i.duplicates.map(p=>p.toLowerCase().trim())),g=this.queue.length;this.queue=this.queue.filter(p=>{const f=d.has(p.name.toLowerCase().trim());return f&&console.warn(`%c[Bulk Add] SKIPPED: ${p.name} already exists in database.`,"color: #ff9800; font-weight: bold;"),!f}),console.log(`[Bulk Add] Removed ${g-this.queue.length} duplicates ahead of time.`)}}catch(o){console.error("Bulk duplicate check failed:",o)}if(this.queue.length===0){M.fire("All Skipped","All beneficiaries in the CSV already exist in the database.","info");return}this.isActive=!0,this.currentIndex=0,M.close(),this.processNext()}else M.fire("Error","No valid beneficiary data found. Please ensure your CSV formatting matches the requirements.","error")},showProgressModal(){ye();const e=Math.round(this.currentIndex/this.queue.length*100),t=`
        ...
        <div class="col-span-7 flex justify-end overflow-hidden w-full">
            <div class="flex flex-col gap-1 items-start w-auto">
                ...
                <div class="flex items-center gap-2 min-h-[30px] font-montserrat flex-nowrap">
                    <span class="${or(e.remarks)} text-[0.5625rem] sm:text-[0.625rem] font-black px-2.5 sm:px-4 py-2 rounded-lg border uppercase tracking-widest shadow-sm border-l-4 ${e.remarks==="ONGOING"?"border-l-green-600":"border-l-red-600"} min-w-[80px] sm:min-w-[100px] text-center inline-block whitespace-nowrap">${e.remarks}</span>
                    <span class="${rr(e.office)} inline-block text-[0.5625rem] sm:text-[0.625rem] font-black px-2.5 sm:px-4 py-2 rounded-lg border shadow-sm min-w-[80px] sm:min-w-[100px] text-center whitespace-nowrap max-w-full truncate" title="${e.office}">${e.office}</span>
                </div>
            </div>
        </div>
    </div>
...
<div id="personal-profile-section" class="transition-all duration-300">
    <div class="flex flex-col gap-4 text-sm mt-3 px-1 mb-8">
        ...
        <div class="flex justify-between items-center group">
            ...
            <span class="${e.age||it(e.birthday)?"font-black text-heading":"font-bold text-gray-300 italic"} text-right">${e.age||it(e.birthday)||"N/A"}</span>
        </div>
        <div class="flex justify-between items-center group">
...
<div class="relative">
    ...
    <div id="drawer-page-1" class="hidden flex-1 flex flex-col gap-6">
         ...
         <div class="flex flex-col gap-5">
            <div>
                ...
                <div class="space-y-2">
                    ${r.length?r.map(c=>{const h=c.status||"PENDING";let C=h==="VERIFIED"||h==="COMPLETED"?"text-green-500":h==="REJECTED"||h==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500",x=c.date||c.createdAt,E=x;if(x){const L=/^\d{4}-\d{2}-\d{2}$/.test(x)?new Date(x+"T00:00:00Z"):new Date(x);isNaN(L)||(E=L.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric",timeZone:"Asia/Manila"}).toUpperCase())}return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-blue-100 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors edit-log-btn" data-type="dtr" data-id="${c.id}" data-val="${c.day||x}" data-status="${h}">
                            <span class="text-xs font-black text-royal-blue dark:text-blue-400 capitalize whitespace-nowrap pointer-events-none">${c.day||E}</span>
                            <span class="text-[0.6875rem] font-bold ${C} uppercase tracking-widest truncate max-w-[50%] text-right pr-6 group-hover:pr-12 pointer-events-none transition-all">${h}</span>
            ...
            <div>
                ...
                <div class="space-y-2">
                    ${o.length?o.map(c=>{const h=c.status||"PENDING";let C=h==="VERIFIED"||h==="COMPLETED"?"text-green-500":h==="REJECTED"||h==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500",x=c.period||c.createdAt,E=x;if(x){const L=/^\d{4}-\d{2}-\d{2}$/.test(x)?new Date(x+"T00:00:00Z"):new Date(x);isNaN(L)||(E=L.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric",timeZone:"Asia/Manila"}).toUpperCase())}return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-orange-100 dark:border-orange-900/50 bg-orange-50/50 dark:bg-orange-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-orange-100 dark:hover:bg-orange-900/40 transition-colors edit-log-btn" data-type="ar" data-id="${c.id}" data-val="${x}" data-status="${h}">
                            <span class="text-xs font-black text-orange-600 dark:text-orange-400 capitalize whitespace-nowrap pointer-events-none">${x||E}</span>
                            <span class="text-[0.6875rem] font-bold ${C} uppercase tracking-widest truncate max-w-[50%] text-right pr-6 group-hover:pr-12 pointer-events-none transition-all">${h}</span>
    ...
    <div id="drawer-page-2" class="hidden flex-1 flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-3">
            ${d.map(c=>{const h=c.status.toUpperCase(),x={VERIFIED:"COMPLETED",DECLINED:"REJECTED",PENDING:"PENDING"}[h]||h,E=x==="COMPLETED",L=x==="REJECTED",X=E?"text-green-500":L?"text-red-500":"text-gray-400 dark:text-gray-500",Q=E?"bg-green-50/50 dark:bg-green-900/10":L?"bg-red-50/50 dark:bg-red-900/10":"bg-gray-50/50 dark:bg-slate-800/50",R=E?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800 hover:bg-green-200 cursor-pointer":L?"bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800 hover:bg-red-200 cursor-pointer":"bg-white text-gray-500 border-gray-200 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer";let te='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';return E?te='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>':L&&(te='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>'),`
                <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-0.5 ${Q}">
                    <div class="flex items-center gap-3 w-full">
                        ...
                        <span class="text-xs sm:text-sm font-black ${E?"text-heading":"text-gray-500 dark:text-gray-400"} uppercase tracking-tight flex-1">${c.name}</span>
                    </div>
...
<style>
    ...
    `,p=!!e._noAnimation;let f=document.getElementById("beneficiary-drawer-container");const v=p&&!!f&&f.dataset.beneficiaryId===String(e.id||"");if(v){const c=f.scrollTop;f.innerHTML=g,f.scrollTop=c}else f&&(f.remove(),document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden")),f=document.createElement("div"),f.id="beneficiary-drawer-container",f.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-neutral-primary-soft dark:bg-slate-900 w-full sm:w-[500px] lg:w-[560px] shadow-2xl",f.setAttribute("tabindex","-1"),f.setAttribute("data-drawer-backdrop","true"),f.innerHTML=g,document.body.appendChild(f),document.documentElement.classList.add("overflow-hidden"),document.body.classList.add("overflow-hidden");f.dataset.beneficiaryId=String(e.id||""),ft(async()=>{const{Drawer:c}=await import("./vendor-flowbite-B7rSfpuT.js").then(h=>h.b);return{Drawer:c}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:c})=>{let h=v?f.__drawerInstance:null;if(!h){const N={placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden"),setTimeout(()=>{f&&f.parentNode&&f.remove()},300)}};h=new c(f,N),f.__drawerInstance=h,h.show()}f.querySelector("#close-drawer-btn").addEventListener("click",()=>h.hide());const x=f.querySelector("#drawer-prev-btn"),E=f.querySelector("#drawer-next-btn"),L=2,X=()=>{f.querySelectorAll("[id^=drawer-page-]").forEach((j,z)=>{j.classList.toggle("hidden",z!==a)});const N=f.querySelector("#drawer-section-title");N&&N.classList.toggle("invisible",a!==0);const _=f.querySelector("#personal-profile-section");_&&_.classList.toggle("hidden",a!==0),x.disabled=a===0,E.disabled=a===L,x.classList.toggle("opacity-50",a===0),E.classList.toggle("opacity-50",a===L)};x.addEventListener("click",()=>{a>0&&a--,X()}),E.addEventListener("click",()=>{a<L&&a++,X()}),X(),f.querySelectorAll(".drawer-doc-btn").forEach(N=>{N.addEventListener("click",async()=>{const _=N.dataset.name,j=N.dataset.status,z="flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[0.5625rem] gap-2 transition-all duration-300 ",G=await M.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Update Document</span>',html:`
                        <div class="font-montserrat text-left">
                ...
                `,focusConfirm:!1,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">SAVE REVISIONS</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">CANCEL</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2"},buttonsStyling:!1,preConfirm:()=>{const ee=document.querySelector('input[name="swal-log-status"]:checked');return{val:document.getElementById("swal-log-val").value.trim().toUpperCase(),status:ee?ee.value:"PENDING"}}});if(O&&(O.val!==z||O.status!==G))try{const ee={type:_,id:j,status:O.status};if(_==="dtr"){const y=O.val.toUpperCase().match(/([A-Z]{3})\s+(\d+)-\d+,\s*(\d{4})/);if(y){const I=R.indexOf(y[1]),D=parseInt(y[2]),m=parseInt(y[3]);let n=new Date(m,I,D);for(;!te(n.toISOString().split("T")[0]);)n.setDate(n.getDate()+1);ee.record_date=n.toISOString().split("T")[0]}else ee.record_date=new Date().toISOString().split("T")[0];ee.weekday=O.val}_==="ar"&&(ee.period=O.val);const b=await Et("api/logs.php",ee),k=b.success?b.data:{success:!1,error:b.error};k.success?(M.fire({toast:!0,position:s,icon:"success",title:"Log Updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):M.fire("Error",k.error||"Failed to update log.","error")}catch(ee){M.fire("Error",ee.message,"error")}},ue=f.querySelector("#add-dtr-log-btn");ue&&ue.addEventListener("click",()=>be("dtr",ge()));const ve=f.querySelector("#add-ar-log-btn");ve&&ve.addEventListener("click",()=>be("ar",pe()));const we=f.querySelector("#export-log-btn");we&&we.addEventListener("click",async()=>{const N="flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[0.6875rem] gap-2 ",_=await M.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Export DTR/AR',html:`
                    ...
                    `,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Generate Export</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Cancel</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-emerald-600 text-white hover:bg-emerald-700 text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1,preConfirm:()=>{const j=document.querySelector('input[name="swal-export-type"]:checked');return j?j.value:null}});if(_.isConfirmed&&_.value){const j=_.value,z=new Date().getFullYear();await bt([e],j,z)}}),f.querySelectorAll(".edit-log-btn").forEach(N=>{N.addEventListener("click",_=>{if(_.target.closest(".delete-log-btn"))return;const j=N.dataset.type,z=N.dataset.id,G=N.dataset.val,V=N.dataset.status;K(j.toUpperCase(),j,z,G,V)})}),f.querySelectorAll(".delete-log-btn").forEach(N=>{N.addEventListener("click",async()=>{const _=N.dataset.id,j=N.dataset.type;if((await M.fire({title:'<span class="text-xl font-black text-philippine-red uppercase tracking-tight">Delete item?</span>',text:"This action cannot be undone.",icon:"warning",showCancelButton:!0,confirmButtonText:'<span class="font-black tracking-widest uppercase">Delete</span>',cancelButtonText:'<span class="font-black tracking-widest uppercase">Wait</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100",confirmButton:"bg-philippine-red text-white hover:bg-red-700 text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs px-6 py-2.5 rounded-xl border border-gray-200 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1})).isConfirmed)try{const G=await Ye(`api/logs.php?type=${j}`,{log_id:_,action:"delete"});(G.success?G.data:{success:!1,error:G.error}).success?(M.fire({toast:!0,position:s,icon:"success",title:"Deleted",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):M.fire("Error","Failed to delete data.","error")}catch(G){M.fire("Error",G.message,"error")}})})}).catch(console.error)}function ar(e){const t=ye(),s="w-full rounded-none border border-slate-300 bg-white px-3 py-2.5 text-sm font-bold text-slate-900 placeholder-slate-400 outline-none transition-colors focus:border-brand focus:ring-1 focus:ring-brand dark:border-slate-700 dark:bg-slate-950 dark:text-white dark:placeholder-slate-600",a="w-full resize-none overflow-hidden rounded-none border-0 border-b-2 border-blue-200 bg-transparent px-0 py-2 text-xl font-black leading-tight tracking-tight text-royal-blue placeholder-gray-300 outline-none focus:border-brand focus:ring-0 sm:text-2xl dark:border-slate-700 dark:text-white",o="mb-1.5 block text-[0.625rem] font-black uppercase tracking-[0.14em] text-slate-500 dark:text-slate-400";function r(x){const E=String(x||"").trim();if(!E)return{month:"",day:"",year:"",iso:""};let L=E.match(/^(\d{4})-(\d{2})-(\d{2})/);if(L)return{year:L[1],month:L[2],day:L[3],iso:`${L[1]}-${L[2]}-${L[3]}`};if(L=E.match(/^(\d{1,2})[\/.\-](\d{1,2})[\/.\-](\d{4})$/),L){const X=L[1].padStart(2,"0"),Q=L[2].padStart(2,"0");return{year:L[3],month:X,day:Q,iso:`${L[3]}-${X}-${Q}`}}return{month:"",day:"",year:"",iso:""}}function l(x,E,L){const X=Number.parseInt(x,10),Q=Number.parseInt(E,10),R=Number.parseInt(L,10);if(!Number.isInteger(X)||!Number.isInteger(Q)||!Number.isInteger(R)||R<1900||R>new Date().getFullYear())return"";const te=new Date(R,X,0).getDate();return X<1||X>12||Q<1||Q>te?"":`${String(R).padStart(4,"0")}-${String(X).padStart(2,"0")}-${String(Q).padStart(2,"0")}`}function i(x){const E=r(x);if(!E.iso)return"";const L=Number.parseInt(E.year,10),X=Number.parseInt(E.month,10),Q=Number.parseInt(E.day,10),R=new Date;let te=R.getFullYear()-L;return(R.getMonth()+1<X||R.getMonth()+1===X&&R.getDate()<Q)&&te--,te>=0?te:""}const d=r(e.birthday),g=Array.from({length:12},(x,E)=>{const L=String(E+1).padStart(2,"0");return`<option value="${L}" ${d.month===L?"selected":""}>${L}</option>`}).join(""),p=Array.from({length:31},(x,E)=>{const L=String(E+1).padStart(2,"0");return`<option value="${L}" ${d.day===L?"selected":""}>${L}</option>`}).join("");function f(x){if(!x)return"bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-white border-gray-200 dark:border-slate-700";const E=String(x).toUpperCase();return E==="ONGOING"?"bg-green-100 text-green-700 border-green-200":E==="EXPIRED"?"bg-red-400 text-white border-red-400":E==="RESIGNED"?"bg-[#ce1126] text-white border-[#ce1126]":E==="ABSORBED"?"bg-[#2e7d32] text-white border-[#2e7d32]":"bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-white border-gray-200 dark:border-slate-700"}const v=`
        ...
        <div class="grid grid-cols-1 gap-4 p-4 sm:grid-cols-2">
            ...
            <div class="relative">
                ...
                <div id="edit-education-suggestions-box" class="absolute left-0 right-0 z-[70] mt-1 hidden max-h-48 overflow-y-auto border border-slate-300 bg-white shadow-xl dark:border-slate-700 dark:bg-slate-900">
                    ${Ke.map(x=>`<button type="button" class="edit-education-option w-full border-b border-slate-100 px-3 py-2 text-left text-[0.6875rem] font-bold text-slate-700 hover:bg-blue-50 dark:border-slate-800 dark:text-slate-200 dark:hover:bg-slate-800"><span class="option-text">${x.name}</span></button>`).join("")}
                </div>
            </div>
            ...
            <label class="block">
                ...
                <select name="relationshipToAssured" class="${s} cursor-pointer appearance-none uppercase">
                    ...
                    ${xt.map(x=>`<option value="${x}" ${String(e.relationshipToAssured||"").toUpperCase()===x.toUpperCase()?"selected":""}>${x}</option>`).join("")}
                </select>
            </label>
        </div>
    </section>
...
.pulse-highlight {
    ...
    `;let c=document.getElementById("edit-drawer-container");c&&(c.remove(),document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden")),c=document.createElement("div"),c.id="edit-drawer-container",c.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full border-l border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",c.setAttribute("tabindex","-1"),c.innerHTML=v,document.body.appendChild(c),document.documentElement.classList.add("overflow-hidden"),document.body.classList.add("overflow-hidden"),setTimeout(()=>{const x=c.querySelector('textarea[name="name"]');x&&(x.style.height="auto",x.style.height=x.scrollHeight+"px")},10);const h=c.querySelector("#edit-education-suggestions-box");h&&(h.innerHTML=Ke.map(x=>`
        ...
        `).join(""));const C=c.querySelector("#edit-designation-suggestions-box");C&&(C.innerHTML=De.map(x=>`
        ...
        `).join("")),ft(async()=>{const{Drawer:x}=await import("./vendor-flowbite-B7rSfpuT.js").then(E=>E.b);return{Drawer:x}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:x})=>{const E=new x(c,{placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden"),setTimeout(()=>{c&&c.parentNode&&c.remove()},400)}});E.show(),window.initFlowbite&&window.initFlowbite();const L=()=>{document.activeElement&&typeof document.activeElement.blur=="function"&&document.activeElement.blur(),E.hide()};c.querySelector("#close-edit-drawer-btn").addEventListener("click",L),c.querySelector("#edit-drawer-cancel-btn").addEventListener("click",L);const X=c.querySelector("#edit-beneficiary-drawer-form"),Q=c.querySelector("#edit-bday-input"),R=c.querySelector("#edit-birth-month"),te=c.querySelector("#edit-birth-day"),ne=c.querySelector("#edit-birth-year"),ge=c.querySelector("#edit-birthday-calendar"),pe=c.querySelector("#edit-birthday-error"),be=c.querySelector("#edit-age-display"),K=c.querySelector("#edit-startDate-input"),ue=c.querySelector("#edit-endDate-input"),ve=c.querySelector('input[name="seriesNo"]'),we=c.querySelector('input[name="gip_id"]'),N=(m=te?.value||"")=>{if(!te)return;const n=Number.parseInt(R?.value||"",10),u=Number.parseInt(ne?.value||"",10),w=Number.isInteger(n)&&n>=1&&n<=12?new Date(Number.isInteger(u)&&u>=1900?u:2e3,n,0).getDate():31,S=document.createDocumentFragment(),$=document.createElement("option");$.value="",$.textContent="DD",S.append($);for(let B=1;B<=w;B++){const A=document.createElement("option");A.value=String(B).padStart(2,"0"),A.textContent=A.value,A.selected=A.value===String(m).padStart(2,"0"),S.append(A)}te.replaceChildren(S)},_=(m=!1)=>{const n=!!(R?.value||te?.value||ne?.value),u=n?l(R?.value,te?.value,ne?.value):"";return Q&&(Q.value=u),ge&&ge.value!==u&&(ge.value=u),be&&(be.value=u?i(u):""),pe&&pe.classList.toggle("hidden",!!u||!n||!m),{isoBirthday:u,hasBirthdayInput:n}},j=m=>{const n=r(m);return n.iso?(R&&(R.value=n.month),ne&&(ne.value=n.year),N(n.day),te&&(te.value=n.day),_(!1),!0):!1};R&&R.addEventListener("change",()=>{N(),_(!1)}),te&&te.addEventListener("change",()=>_(!1)),ne&&ne.addEventListener("input",()=>{ne.value=ne.value.replace(/\D/g,"").slice(0,4),N(),_(!1)}),ge&&ge.addEventListener("change",()=>{ge.value&&j(ge.value)}),N(d.day),_(!1);const z=c.querySelector("#edit-drawer-remarks"),G=c.querySelector("#edit-extension-log-container"),V=()=>{if(!G)return;const m=z.value,n=ye();if(m==="ABSORBED"){const u=e.absorbDate&&!String(e.absorbDate).includes("0000-00-00")?new Date(e.absorbDate):new Date,w=u.getTimezoneOffset()*6e4,S=new Date(u.getTime()-w).toISOString().slice(0,16);G.innerHTML=`
                ...
                `}else if(m==="RESIGNED"){const u=e.resignedDate&&!String(e.resignedDate).includes("0000-00-00")?new Date(e.resignedDate):new Date,w=u.getTimezoneOffset()*6e4,S=new Date(u.getTime()-w).toISOString().slice(0,16);G.innerHTML=`
                    <div class="mt-4 pt-4 border-t ${n?"border-slate-800":"border-gray-100"}">
                ...
                `}};z&&z.addEventListener("change",m=>{const n="text-[0.625rem] sm:text-[0.6875rem] font-black px-2.5 py-2.5 rounded-none border uppercase tracking-widest shadow-sm outline-none focus:ring-2 focus:ring-brand w-full cursor-pointer transition-colors duration-300 h-[42px]";z.className=`${f(m.target.value)} ${n} editable-indicator`,V(),(z.value==="ABSORBED"||z.value==="RESIGNED")&&setTimeout(()=>{G.scrollIntoView({behavior:"smooth",block:"center"}),G.classList.add("pulse-highlight"),setTimeout(()=>G.classList.remove("pulse-highlight"),1500)},50)}),V();let oe=!1;const ae=(m,n)=>{m.addEventListener("paste",u=>{u.preventDefault();let w=(u.clipboardData||window.clipboardData).getData("text");if(w){w=w.replace(/[-.\s]/g,"/");const S=w.split("/");if(S.length===3){const $=S[0].padStart(2,"0"),B=S[1].padStart(2,"0");let A=S[2];if(A.length===2){const H=new Date().getFullYear(),T=Math.floor(H/100)*100;A=String(T+parseInt(A))}else A=A.padStart(4,"0");const P=`${$}/${B}/${A}`;m.value=P;const q=new Event("input",{bubbles:!0});m.dispatchEvent(q);const U=window.__parseFormattedDate(P);if(U&&n&&(oe||n(U),document.activeElement===m&&m.blur()),m._datepicker)m._datepicker.hide();else{const H=m.parentNode&&m.parentNode._datepicker;H&&typeof H.hide=="function"&&H.hide()}}}}),m.addEventListener("input",u=>{const w=u.target.value,S=window.__maskDate(w);if(w!==S&&(u.target.value=S),S.length===10){const $=window.__parseFormattedDate(S);if($&&n)if(oe||n($),document.activeElement===m&&m.blur(),m._datepicker)m._datepicker.hide();else{const B=m.parentNode&&m.parentNode._datepicker;B&&typeof B.hide=="function"&&B.hide()}}}),m.addEventListener("changeDate",u=>{u.detail&&u.detail.date&&n&&(oe||n(u.detail.date),m._datepicker&&m._datepicker.hide())})};K&&ae(K,m=>{if(ue){const u=new Date(m);u.setDate(u.getDate()+243);const w=String(u.getMonth()+1).padStart(2,"0"),S=String(u.getDate()).padStart(2,"0"),$=u.getFullYear();ue.value=`${w}/${S}/${$}`}const n=m.getFullYear();n>1900&&we&&ve&&Promise.all([ce(`api/beneficiaries.php?next_id&year=${encodeURIComponent(n)}`),ce(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(n)}`)]).then(([u,w])=>{const S=u.success&&u.data?.success?u.data.nextId:null,$=w.success&&w.data?.success?w.data.nextSeries:null,B=String(we.value||"").match(/^ROX-RD-ESIG-(\d{4})-\d{4}$/),A=String(ve.value||"").match(/^(\d{4})-\d{2}-\d{3}$/),P=B?Number(B[1]):null,q=A?Number(A[1]):null;S&&(P===null||P!==n)&&(we.value=S),$&&(q===null||q!==n)&&(ve.value=$)}).catch(u=>{console.error("Edit drawer identifier sync error:",u)})}),ue&&ae(ue),window.Datepicker||typeof Datepicker<"u"&&Datepicker;const W=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null),O=c.querySelector("#edit-date-range-picker");if(W&&O){const m=new W(O,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0,orientation:"auto"});K&&(K._datepicker=m.datepickers[0]),ue&&(ue._datepicker=m.datepickers[1])}e.id&&(oe=!0,ce(`api/beneficiaries.php?id=${encodeURIComponent(e.id)}`).then(m=>{if(m.success&&m.data&&m.data.beneficiary){const n=m.data.beneficiary;if(n.birthday&&j(n.birthday),K&&n.startDate){const u=new Date(n.startDate);isNaN(u)||(K.value=n.startDateFormatted||"",K._datepicker&&K._datepicker.setDate(u))}if(ue&&n.endDate){const u=new Date(n.endDate);isNaN(u)||(ue.value=n.endDateFormatted||"",ue._datepicker&&ue._datepicker.setDate(u))}}setTimeout(()=>{oe=!1},100)}).catch(m=>{console.error("Error fetching accurate beneficiary dates:",m),oe=!1}));const ee=(m,n,u)=>{const w=c.querySelector(m),S=c.querySelector(n);if(!w||!S)return;const $=()=>S.classList.add("hidden"),B=()=>S.classList.remove("hidden");w.addEventListener("focus",B),w.addEventListener("input",()=>{const A=w.value.toLowerCase().trim();let P=0;S.querySelectorAll(u).forEach(q=>{const H=(q.querySelector(".option-text")?.textContent||q.textContent||"").toLowerCase().includes(A);q.style.display=H?"block":"none",H&&P++}),P>0?B():$()}),S.addEventListener("click",A=>{const P=A.target.closest(u);P&&(w.value=(P.querySelector(".option-text")?.textContent||P.textContent||"").trim(),$(),w.dispatchEvent(new Event("change")))}),document.addEventListener("click",A=>{!w.contains(A.target)&&!S.contains(A.target)&&$()})};ee("#edit-education-input","#edit-education-suggestions-box",".edit-education-option"),ee("#edit-designation-input","#edit-designation-suggestions-box",".edit-designation-option"),(()=>{const m=c.querySelector("#edit-office-input"),n=c.querySelector("#edit-office-suggestions-box");if(!m||!n)return;n.classList.add("mt-[52px]");let u="OFFICES",w=null,S=[];const $={textLabel:t?"text-slate-400":"text-slate-500",borderDivide:t?"border-slate-800":"border-slate-100",courseHover:t?"hover:bg-slate-800/80":"hover:bg-blue-50",textCourseOpt:t?"text-slate-300":"text-slate-700"},B=async()=>{const P="dole_offices_cache",q=async()=>{let H=[];try{const T=await ce("api/beneficiaries.php?get_offices_advanced=1");T.success&&T.data?.success&&Array.isArray(T.data.offices)&&(H=T.data.offices)}catch(T){console.error("Office fetch failed:",T)}return H.length>0&&(S=H,localStorage.setItem(P,JSON.stringify({data:H,timestamp:Date.now()}))),H},U=localStorage.getItem(P);if(U)try{const{data:H,timestamp:T}=JSON.parse(U);return S=H,Date.now()-T>1800*1e3&&q().then(()=>{u==="OFFICES"&&A("OFFICES",w,m.value)}),H}catch{localStorage.removeItem(P)}return S.length===0?await q():S},A=async(P="OFFICES",q=null,U="")=>{if(u=P,w=q,P==="OFFICES"){const T=(await B()).filter(Y=>Y.office.toLowerCase().includes(U.toLowerCase()));n.innerHTML=`
                        ...
                        <div class="max-h-64 overflow-y-auto scrollbar-hide">
                            ${T.length>0?T.map(Y=>{const le=parseInt(Y.location_count||0)>0;return`
                                    <div class="office-code-option group/opt px-3 py-2 text-[0.5625rem] font-bold ${$.textCourseOpt} ${$.courseHover} rounded-none ${le?"cursor-pointer":"cursor-default opacity-60"} transition-all flex items-center justify-between group active:scale-[0.98] mx-1 mb-0.5"
                                        data-id="${Y.id}" data-name="${Y.office}" data-has-locations="${le}">
                                ...
                                <div class="px-2 pb-2 flex flex-col gap-1.5">
                                    <div class="text-[0.4375rem] font-black uppercase tracking-widest ${$.textLabel} opacity-50 px-1">New office: "${U.trim()}"</div>
                    ...
                    `;const F=U.trim(),Z=n.querySelector("#add-office-location-row-edit"),se=n.querySelector("#new-office-loc-input-edit"),Ae=n.querySelector("#confirm-office-with-loc-edit"),Ee=n.querySelector("#add-office-with-loc-btn-edit"),ie=n.querySelector("#skip-office-loc-btn-edit");if(Ee&&Z&&Ee.addEventListener("click",Y=>{Y.stopPropagation(),Z.classList.remove("hidden"),Z.classList.add("flex"),setTimeout(()=>se?.focus(),50)}),Ae&&se){const Y=le=>{le.stopPropagation();const ke=se.value.trim();m.value=ke?`${F} - ${ke}`:F,n.classList.add("hidden"),m.dispatchEvent(new Event("change"))};Ae.addEventListener("click",Y),se.addEventListener("keydown",le=>{le.key==="Enter"&&Y(le)}),se.addEventListener("click",le=>le.stopPropagation())}ie&&ie.addEventListener("click",Y=>{Y.stopPropagation(),m.value=F,n.classList.add("hidden"),m.dispatchEvent(new Event("change"))}),n.querySelectorAll(".office-code-option").forEach(Y=>{Y.addEventListener("click",le=>{le.stopPropagation(),Y.dataset.hasLocations==="true"?A("LOCATIONS",{id:Y.dataset.id,name:Y.dataset.name}):(m.value=Y.dataset.name,n.classList.add("hidden"),m.dispatchEvent(new Event("change")))})})}else{n.innerHTML=`
                    ...
                    `;const H=n.querySelector("#loc-list-edit"),T=n.querySelector("#location-search-edit"),F=`dole_locs_cache_${q.id}`;let Z=[];const se=localStorage.getItem(F);if(se)try{const{data:ie,timestamp:Y}=JSON.parse(se);Z=ie,Date.now()-Y<3600*1e3}catch{localStorage.removeItem(F)}const Ae=async()=>{let ie=[];if(Se&&Te()){const{data:Y,error:le}=await Se.from("office_locations").select("location").eq("office_id",q.id).order("location");!le&&Y&&(ie=Y)}if(ie.length===0)try{const Y=await ce(`api/beneficiaries.php?get_office_locations=1&office_id=${q.id}`);Y.success&&Y.data?.success&&Array.isArray(Y.data.locations)&&(ie=Y.data.locations)}catch(Y){console.error("Office locations fetch failed:",Y)}ie.length>0&&(Z=ie,localStorage.setItem(F,JSON.stringify({data:ie,timestamp:Date.now()})),Ee(T.value))},Ee=(ie="")=>{const Y=Z.filter(ke=>ke.location.toLowerCase().includes(ie.toLowerCase())),le=ie.trim();Y.length>0?H.innerHTML=Y.map(ke=>`
                                <div class="location-option-edit group/loc px-3 py-1.5 text-[0.5625rem] font-bold ${$.textCourseOpt} ${$.courseHover} rounded-none cursor-pointer transition-all flex items-center gap-3 active:scale-[0.98] mb-0.5" data-location="${ke.location}">
                            ...
                            `).join(""):Z.length===0?H.innerHTML=`<div class="px-3 py-4 text-center text-[0.5625rem] font-bold ${$.textLabel} animate-pulse">Fetching...</div>`:(H.innerHTML=`
                            ...
                            `,le&&H.querySelector("#add-new-location-edit")?.addEventListener("click",()=>{m.value=`${q.name} - ${le}`,n.classList.add("hidden"),m.dispatchEvent(new Event("change"))})),H.querySelectorAll(".location-option-edit").forEach(ke=>{ke.addEventListener("click",()=>{m.value=`${q.name} - ${ke.dataset.location}`,n.classList.add("hidden"),m.dispatchEvent(new Event("change"))})})};Ee(T.value),Ae(),setTimeout(()=>T.focus(),50),T.addEventListener("input",()=>Ee(T.value)),T.addEventListener("click",ie=>ie.stopPropagation()),n.querySelector("#back-to-offices-edit").addEventListener("click",ie=>{ie.stopPropagation(),A("OFFICES")})}};m.addEventListener("focus",()=>{n.classList.remove("hidden"),A(u,w,m.value)}),m.addEventListener("input",()=>{u==="OFFICES"&&A("OFFICES",null,m.value)}),document.addEventListener("click",P=>{!m.contains(P.target)&&!n.contains(P.target)&&n.classList.add("hidden")})})();const k=c.querySelector("#edit-replacement-input"),y=c.querySelector("#edit-replacement-suggestions-box"),I=c.querySelector("#edit-replacement-loading");let D=null;k&&y&&(k.addEventListener("input",m=>{const n=m.target.value.trim();clearTimeout(D),y.classList.add("hidden"),!(n.length<2)&&(I&&I.classList.remove("hidden"),D=setTimeout(async()=>{try{const u=await _e(`api/beneficiaries.php?replacement_candidates=1&q=${encodeURIComponent(n)}&limit=10`);u.success&&u.data&&u.data.candidates&&u.data.candidates.length>0?(y.innerHTML=u.data.candidates.map(w=>`
                            ...
                            `).join(""),y.classList.remove("hidden")):(y.innerHTML='<div class="px-4 py-3 text-xs text-gray-500 italic text-center font-medium">No active beneficiaries found.</div>',y.classList.remove("hidden"))}catch(u){console.error("Replacement fetch error:",u)}finally{I&&I.classList.add("hidden")}},400))}),y.addEventListener("click",m=>{const n=m.target.closest("button");n&&(k.value=n.dataset.name,y.classList.add("hidden"))}),document.addEventListener("click",m=>{!k.contains(m.target)&&!y.contains(m.target)&&y.classList.add("hidden")})),X.querySelectorAll("input, select, textarea").forEach(m=>{const n=(m.getAttribute("type")||"").toLowerCase(),u=!m.disabled&&!m.readOnly&&n!=="hidden";m.classList.remove("editable-indicator"),u&&m.classList.add("editable-indicator")}),X.addEventListener("submit",m=>{m.preventDefault();const{isoBirthday:n,hasBirthdayInput:u}=_(!0);if(u&&!n){R?.focus(),M.fire({toast:!0,position:"top-end",icon:"error",title:"Enter a valid birthday",text:"Complete the MM, DD, and YYYY fields.",showConfirmButton:!1,timer:3500});return}const w=B=>{const A=String(B||"").trim();if(!A)return"";const P=A.match(/^(\d{4})-(\d{2})-(\d{2})/);if(P)return`${P[1]}-${P[2]}-${P[3]}`;const q=window.__parseFormattedDate?.(A);if(!q)return A;const U=q.getFullYear(),H=String(q.getMonth()+1).padStart(2,"0"),T=String(q.getDate()).padStart(2,"0");return`${U}-${H}-${T}`},S=new FormData(X),$={};S.forEach((B,A)=>{$[A]=["birthday","startDate","endDate"].includes(A)?w(B):B}),$.birthday=n,$.id=e.id,$.gip_id=$.gip_id||e.id,window.addBeneficiaryData&&(async()=>await window.addBeneficiaryData($,!0,!1)&&(L(),window.viewBeneficiary&&setTimeout(()=>window.viewBeneficiary({id:e.id},0),100),M.fire({toast:!0,position:"top-end",icon:"success",title:"Record Updated",showConfirmButton:!1,timer:3e3})))()})})}function Dr(){window.showAddDataModal=Oe,window.__maskDate=function(e){let t=e.replace(/\D/g,"").slice(0,8);return t.length>2&&t.length<=4?t=t.slice(0,2)+"/"+t.slice(2):t.length>4&&(t=t.slice(0,2)+"/"+t.slice(2,4)+"/"+t.slice(4)),t},window.__parseFormattedDate=function(e){if(!e)return null;const t=e.split("/");if(t.length===3){const s=parseInt(t[0])-1,a=parseInt(t[1]),o=parseInt(t[2]);if(o>1e3&&s>=0&&s<12&&a>0&&a<=31)return new Date(o,s,a)}return null},window.calculateAge=function(e){if(!e)return"";const t=e instanceof Date?e:new Date(e),s=new Date;let a=s.getFullYear()-t.getFullYear();const o=s.getMonth()-t.getMonth();return(o<0||o===0&&s.getDate()<t.getDate())&&a--,a>=0?a:0},window.viewBeneficiary=async function(e,t=0){const s=e?.id||e?.gip_id||null;if(!s)return;const a=!!(e?.name&&e?.office&&e?.remarks);let o={...e,id:s};if(!a){const d=await ce(`api/beneficiaries.php?id=${encodeURIComponent(s)}`);d.success&&d.data?.success&&d.data?.beneficiary&&(o={...d.data.beneficiary,...o,id:s})}const r=`logs_cache_${s}`,l=window.__doleDB?.getSecureCache?await window.__doleDB.getSecureCache(r):null,i=!!l;o.arLogs=l?.arLogs||[],o.dtrLogs=l?.dtrLogs||[],o.docs=l?.docs||[],lt(o,t);try{const[d,g,p,f]=await Promise.all([ce(`api/logs.php?type=ar&gip_id=${encodeURIComponent(s)}`),ce(`api/logs.php?type=dtr&gip_id=${encodeURIComponent(s)}`),ce(`api/logs.php?type=docs&gip_id=${encodeURIComponent(s)}`),ce(`api/logs.php?type=absorption&gip_id=${encodeURIComponent(s)}`)]),v=d.success&&d.data?.success?d.data.logs:[],c=g.success&&g.data?.success?g.data.logs:[],h=p.success&&p.data?.success?p.data.logs:[],C=f.success&&f.data?.success?f.data.logs:[];if(C.length>0){const L=C[0];o.absorbDate=L.absorption_datetime,o.absorb_where=L.where||L.absorb_where,o.absorb_position=L.position||L.absorb_position,o.absorb_agency=L.agency||L.absorb_agency}window.__doleDB?.setSecureCache&&await window.__doleDB.setSecureCache(r,{arLogs:v,dtrLogs:c,docs:h});const x=JSON.stringify({ar:l?.arLogs||[],dtr:l?.dtrLogs||[],docs:l?.docs||[]}),E=JSON.stringify({ar:v,dtr:c,docs:h});if(!i||x!==E){const L=document.getElementById("beneficiary-drawer-container");L&&L.dataset.beneficiaryId===String(s)&&(o.arLogs=v,o.dtrLogs=c,o.docs=h,lt({...o,_noAnimation:!0},t))}}catch(d){console.error("Error fetching logs/docs:",d)}},window.showAddDataModal=function(e){Oe(e)},window.editBeneficiary=function(e){ar(e)},window.showExportConfigModal=function(e){ir(e)},window.showProfileModal=function(){sr()},window.showSearchExtraStatsModal=function(){lr()}}async function sr(){try{if(Te()&&Se){let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=a.id)}catch{}if(!e)throw new Error("User not logged in");const{data:t,error:s}=await Se.from("users").select("*").eq("user_id",e).single();if(s)throw s;dt(t)}else{let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=`?user_id=${a.id}`)}catch{}const s=await(await fetch(`${he()}api/profile.php${e}`)).json();if(s.success){const a=s.profile;dt(a)}else M.fire({icon:"error",title:"Error",text:s.error||"Failed to load profile"})}}catch(e){console.error("Error fetching profile:",e)}}function dt(e){const t=e.profile_picture_path?`${he()}${e.profile_picture_path}`:null,s=e.full_name?e.full_name.split(" ").map(o=>o[0]).join("").substring(0,2).toUpperCase():"US",a=`
    ...
    `;M.fire({html:a,width:"450px",showConfirmButton:!1,showCloseButton:!0,padding:"1.5rem",customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl overflow-visible ldn-modal-popup",closeButton:"focus:outline-none bg-gray-50 border-none swal2-custom-close cursor-pointer"},didOpen:o=>{const r=o.querySelector("#profile-edit-form"),l=o.querySelector("#profile-pic-input"),i=o.querySelector("#profile-avatar-preview");l.addEventListener("change",d=>{const g=d.target.files[0];if(g){const p=new FileReader;p.onload=f=>{i.innerHTML=`<img src="${f.target.result}" class="w-full h-full object-cover" />`},p.readAsDataURL(g)}}),r.addEventListener("submit",async d=>{d.preventDefault();const g=new FormData(r);try{const p=JSON.parse(localStorage.getItem("user"));p&&p.id&&g.append("user_id",p.id)}catch{}try{const f=await(await fetch(`${he()}api/profile.php`,{method:"POST",body:g})).json();f.success?(f.profile&&(localStorage.setItem("user",JSON.stringify(f.profile)),nr(f.profile)),M.close(),M.fire({toast:!0,position:"top-end",icon:"success",title:"Profile Updated",showConfirmButton:!1,timer:3e3,timerProgressBar:!0})):M.fire({icon:"error",title:"Update Failed",text:f.error||"Something went wrong"})}catch(p){console.error("Error saving profile:",p)}})}})}function nr(e){const t=e.profile_picture_path?`${he()}${e.profile_picture_path}`:null,s=e.full_name?e.full_name.split(" ").map(l=>l[0]).join("").substring(0,2).toUpperCase():"US",a=document.querySelectorAll(".sidebar-user-name"),o=document.querySelectorAll(".sidebar-user-email"),r=document.querySelectorAll(".sidebar-user-avatar");a.forEach(l=>l.textContent=e.full_name),o.forEach(l=>l.textContent=e.email||"No email set"),r.forEach(l=>{t?l.innerHTML=`<img src="${t}" class="w-full h-full object-cover" />`:l.textContent=s}),localStorage.setItem("user_full_name",e.full_name),t&&localStorage.setItem("user_avatar",t)}function ir(e){const t=window.getExportFilters?window.getExportFilters():{office:"ALL",remarks:"ALL",gender:"ALL",ageGroup:"ALL",search:"",sort:"name",section:"ALL",columns:["id","name","age","office","position","startdate","enddate","status"],preparedBy:localStorage.getItem("ldn_export_prepared")||"",approvedBy:localStorage.getItem("ldn_export_approved")||""},s=`
        <div class="text-left font-montserrat p-1 overflow-visible">
            <div class="flex items-center gap-3 mb-6">
                <div class="w-12 h-12 bg-royal-blue/10 rounded-2xl flex items-center justify-center">
            ...
            <form id="export-config-form" class="space-y-6">
                ...
                <div class="bg-gray-50/50 rounded-2xl p-4 border border-gray-100 mt-4">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                        ${["ID","Name","Age","Office","Position","Start Date","End Date","Status"].map(a=>{const o=a.toLowerCase().replace(" ",""),r=t.columns.includes(o),l=`col-switch-${o}`;return`
    ...
    `;M.fire({html:l,width:window.innerWidth<640?"96vw":window.innerWidth<1024?"90vw":"1120px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<640?"0.75rem":window.innerWidth<1024?"1.25rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},willOpen:()=>{document.documentElement.classList.add("overflow-hidden"),document.body.classList.add("overflow-hidden")},didClose:()=>{document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden")},didOpen:i=>{window.initFlowbite&&window.initFlowbite();const d=i.querySelector("#cancel-modal-btn");d&&d.addEventListener("click",()=>{!t&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),M.close(),e?._isBulk&&qe.onCancel()});const g=i.querySelector("#bulk-add-btn");g&&g.addEventListener("click",()=>{M.close(),qe.init()});const p=(b,k)=>{b.addEventListener("paste",y=>{y.preventDefault();let I=(y.clipboardData||window.clipboardData).getData("text");if(I){I=I.replace(/[-.\s]/g,"/");const D=I.split("/");if(D.length===3){const m=D[0].padStart(2,"0"),n=D[1].padStart(2,"0");let u=D[2];if(u.length===2){const A=new Date().getFullYear(),P=Math.floor(A/100)*100;u=String(P+parseInt(u))}else u=u.padStart(4,"0");const w=`${m}/${n}/${u}`;b.value=w;const S=new Event("input",{bubbles:!0});b.dispatchEvent(S);const $=window.__parseFormattedDate(w);$&&k&&(k($),document.activeElement===b&&b.blur());const B=b._datepicker||b.parentNode&&b.parentNode._datepicker;B&&typeof B.hide=="function"&&B.hide()}}}),b.addEventListener("input",y=>{const I=window.__maskDate(y.target.value);if(y.target.value!==I&&(y.target.value=I),I.length===10){const D=window.__parseFormattedDate(I);if(D&&k){k(D),document.activeElement===b&&b.blur();const m=b._datepicker||b.parentNode&&b.parentNode._datepicker;m&&typeof m.hide=="function"&&m.hide()}}}),b.addEventListener("changeDate",y=>{if(y.detail&&y.detail.date&&k){k(y.detail.date);const I=b._datepicker||b.parentNode&&b.parentNode._datepicker;I&&typeof I.hide=="function"&&I.hide()}})},f=i.querySelector("#birthday-input"),v=i.querySelector("#age-display"),c=i.querySelector("#age-warning"),h=i.querySelector("#submit-beneficiary-btn"),C=b=>{if(!b)return c&&c.classList.add("hidden"),h&&(h.disabled=!1,h.classList.remove("opacity-50","cursor-not-allowed","grayscale"),h.classList.add("cursor-pointer")),!0;const k=parseInt(b),y=!isNaN(k)&&k>=18&&k<=29;return c&&(c.className=`mt-1 text-[0.625rem] font-bold ${y?"hidden":"flex"} items-center gap-1.5 animate-pulse ${ye()?"text-red-400":"text-red-600"}`),h&&(y?(h.disabled=!1,h.classList.remove("opacity-50","cursor-not-allowed","grayscale"),h.classList.add("cursor-pointer","active:scale-[0.98]")):(h.disabled=!0,h.classList.add("opacity-50","cursor-not-allowed","grayscale"),h.classList.remove("cursor-pointer","active:scale-[0.98]"))),y};if(v&&(v.addEventListener("input",b=>{C(b.target.value)}),v.value&&C(v.value)),f){p(f,k=>{v&&(v.value=window.calculateAge(k),C(v.value),v.classList.add("animate-pulse"),setTimeout(()=>v.classList.remove("animate-pulse"),400))});const b=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);b&&(f._datepicker=new b(f,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"}))}const x=i.querySelector("#name-input-field"),E=i.querySelector("#duplicate-warning");if(x&&E){let b;const k=()=>{try{return JSON.parse(localStorage.getItem("user")||"{}")?.id||null}catch{return null}},y=(D,m="")=>{E.className=`mt-1 text-[0.625rem] font-bold ${D?"flex":"hidden"} items-center gap-1.5 animate-pulse ${ye()?"text-red-400":"text-red-600"}`;const n=E.querySelector("span");n&&(n.textContent=m?`Beneficiary already exists: ${m}`:"Beneficiary already exists")},I=async D=>{const m=k(),n=await fetch(`${he()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json",...m?{"X-User-Id":String(m)}:{}},body:JSON.stringify({name:D,user_id:m,exclude_id:t?e?.id:null})});if(!n.ok)throw new Error(`Duplicate check failed (${n.status})`);return n.json()};x.addEventListener("input",D=>{const m=D.target.value.trim();if(clearTimeout(b),m.length<3){y(!1);return}b=setTimeout(async()=>{try{const n=await I(m);n.success&&n.exists?y(!0,n.match||n.name):y(!1)}catch(n){console.error("Duplicate check error:",n)}},500)}),e?.name&&(y(!1),(async()=>{const D=await I(e.name);D.success&&D.exists&&y(!0,D.match||D.name)})())}const L=i.querySelector("#full-id-input"),X=i.querySelector("#series-no-input"),Q=i.querySelector('input[name="startDate"]'),R=i.querySelector('input[name="endDate"]'),te=i.querySelectorAll('input[name="remarks"]'),ne=i.querySelector("#extension-log-container"),ge=async b=>{if(!b)return;const k=[L,X].filter(Boolean);k.forEach(y=>{y.classList.add("animate-pulse"),y.placeholder="Syncing..."});try{const[y,I]=await Promise.all([ce(`api/beneficiaries.php?next_id&year=${encodeURIComponent(b)}`),ce(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(b)}`)]),D=y.success&&y.data?.success?y.data.nextId:null,m=I.success&&I.data?.success?I.data.nextSeries:null;D&&L&&(L.value=D),m&&X&&(X.value=m)}catch(y){console.error("ID Sync error:",y)}finally{k.forEach(y=>y.classList.remove("animate-pulse"))}},pe=i.querySelector("#replacement-search-input"),be=i.querySelector("#replacement-hidden"),K=i.querySelector("#replacement-suggestions"),ue=b=>{const k=(b.name||"").toUpperCase().trim(),y=b.startDateFormatted||b.startDate||"N/A",I=b.endDateFormatted||b.endDate||"N/A";return`${k} (${y.toUpperCase()} - ${I.toUpperCase()})`},ve=b=>{if(K){if(!b.length){K.innerHTML=`<div class="px-3 py-2 text-[0.625rem] font-bold ${r.textCourseOpt}">No matching beneficiary found.</div>`,K.classList.remove("hidden");return}K.innerHTML=b.map(k=>{const y=ue(k);return`
                    ...
                    `}).join(""),K.classList.remove("hidden"),K.querySelectorAll(".replacement-option").forEach(k=>{k.addEventListener("click",()=>{const y=k.getAttribute("data-replacement")||"";pe&&(pe.value=y),be&&(be.value=y),K.classList.add("hidden")})})}};let we=null;const N=async(b="")=>{const k=(b||"").trim(),y=`api/beneficiaries.php?replacement_candidates=1&limit=20${k?`&q=${encodeURIComponent(k)}`:""}`,I=await ce(y);I.success&&I.data?.success&&Array.isArray(I.data.candidates)&&ve(I.data.candidates)};pe&&be&&K&&(pe.addEventListener("focus",()=>{N(pe.value||"")}),pe.addEventListener("input",()=>{be.value=pe.value.trim(),clearTimeout(we),we=setTimeout(()=>{N(pe.value||"")},250)}),document.addEventListener("click",b=>{pe&&K&&!pe.contains(b.target)&&!K.contains(b.target)&&K.classList.add("hidden")}));const _=()=>{const b=i.querySelector('input[name="remarks"]:checked');return b?b.value:"ONGOING"},j=b=>{const k=i.querySelector(`input[name="remarks"][value="${b}"]`);k&&(k.checked=!0,G())},z=()=>{if(R&&R.value){const b=window.__parseFormattedDate(R.value);if(!b)return;const k=new Date;k.setHours(0,0,0,0);let y="ONGOING";b<k&&(y="EXPIRED"),j(y)}},G=()=>{if(!ne)return;const b=_();if(b==="ABSORBED"){const k=e?.absorbDate&&!String(e.absorbDate).includes("0000-00-00")?new Date(e.absorbDate):new Date,y=k.getTimezoneOffset()*6e4,I=new Date(k.getTime()-y).toISOString().slice(0,16);ne.innerHTML=`
                    ...
                    `}else if(b==="RESIGNED"){const k=e?.resignedDate&&!String(e.resignedDate).includes("0000-00-00")?new Date(e.resignedDate):new Date,y=k.getTimezoneOffset()*6e4,I=new Date(k.getTime()-y).toISOString().slice(0,16);ne.innerHTML=`
                        <div class="mt-4 pt-4 border-t ${o?"border-slate-800":"border-gray-100"}">
                    ...
                    `}else ne.innerHTML=""};if(Q){let b=null;p(Q,D=>{const m=D.getFullYear();if(R){const n=new Date(D);n.setDate(n.getDate()+243);const u=String(n.getMonth()+1).padStart(2,"0"),w=String(n.getDate()).padStart(2,"0"),S=n.getFullYear();R.value=`${u}/${w}/${S}`}z(),m>1900&&m!==b&&(b=m,ge(m))}),R&&p(R,()=>z());const k=i.querySelector("#date-range-picker"),y=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null),I=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);if(y&&k&&Q&&R){const D=new y(k,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0,orientation:"auto"});Q._datepicker=D.datepickers?.[0]||null,R._datepicker=D.datepickers?.[1]||null}else I&&(Q&&(Q._datepicker=new I(Q,{format:"mm/dd/yyyy",autohide:!0,orientation:"auto"})),R&&(R._datepicker=new I(R,{format:"mm/dd/yyyy",autohide:!0,orientation:"auto"})));if(!t){const D=new Date().getFullYear();ge(D)}}te.forEach(b=>b.addEventListener("change",G));const V=i.querySelector("#resign-btn"),oe=i.querySelector("#absorb-btn");V&&V.addEventListener("click",()=>j("RESIGNED")),oe&&oe.addEventListener("click",()=>j("ABSORBED")),i.querySelectorAll('input[type="text"], textarea').forEach(b=>{["id-number-input","full-id-input"].includes(b.id)||b.addEventListener("input",()=>{const k=b.selectionStart,y=b.selectionEnd;b.value=b.value.toUpperCase(),b.setSelectionRange(k,y)})}),z(),G(),W("education-input","course-suggestions","course-option"),W("designation-input","work-suggestions","work-option"),(()=>{const b=i.querySelector("#office-input"),k=i.querySelector("#office-suggestions");if(!b||!k)return;let y="OFFICES",I=null,D=[];const m=async()=>{const u="dole_offices_cache",w=async()=>{let $=[];try{if(Se&&Te()){const[{data:B,error:A},{data:P}]=await Promise.all([Se.from("offices").select("*").order("office"),Se.from("office_locations").select("office_id")]);if(!A&&B?.length){const q={};P&&P.forEach(U=>{q[U.office_id]=(q[U.office_id]||0)+1}),$=B.map(U=>({id:U.id??U.office_id,office:U.office||U.office_name||"",location_count:q[U.id??U.office_id]||0})).filter(U=>U.office)}}}catch{}if(!$.length)try{const B=await ce("api/beneficiaries.php?get_offices_advanced=1");B.success&&B.data?.success&&Array.isArray(B.data.offices)&&($=B.data.offices)}catch(B){console.error("Office fetch failed:",B)}return $.length>0&&(D=$,localStorage.setItem(u,JSON.stringify({data:$,timestamp:Date.now()}))),$},S=localStorage.getItem(u);if(S)try{const{data:$,timestamp:B}=JSON.parse(S);return D=$,Date.now()-B>300*1e3&&w().then(()=>{y==="OFFICES"&&n("OFFICES",I,b.value)}),$}catch{localStorage.removeItem(u)}return D.length===0?await w():D},n=async(u="OFFICES",w=null,S="")=>{if(y=u,I=w,u==="OFFICES"){const B=(await m()).filter(F=>F.office.toLowerCase().includes(S.toLowerCase()));k.innerHTML=`
                            ...
                            <div class="max-h-64 overflow-y-auto scrollbar-hide">
                                ${B.length>0?B.map(F=>{const Z=parseInt(F.location_count||0)>0;return`
                                        <div class="office-code-option group/opt px-3 py-2 text-[0.5625rem] font-bold ${r.textCourseOpt} ${r.courseHover} rounded-lg ${Z?"cursor-pointer":"cursor-default opacity-60"} transition-all flex items-center justify-between group active:scale-[0.98] mx-1 mb-0.5" 
                                            data-id="${F.id}" data-name="${F.office}" data-has-locations="${Z}">
                                            <div class="flex items-center gap-2.5">
                                    ...
                                    ${S.trim()?`
                                    <div class="px-2 pb-2 flex flex-col gap-1.5">
                                        <div class="text-[0.4375rem] font-black uppercase tracking-widest ${r.textLabel} opacity-50 px-1">New office: "${S.trim()}"</div>
                        ...
                        `;const A=S.trim(),P=k.querySelector("#add-office-location-row-modal"),q=k.querySelector("#new-office-loc-input-modal"),U=k.querySelector("#confirm-office-with-loc-modal"),H=k.querySelector("#add-office-with-loc-btn-modal"),T=k.querySelector("#skip-office-loc-btn-modal");if(H&&P&&H.addEventListener("click",F=>{F.stopPropagation(),P.classList.remove("hidden"),P.classList.add("flex"),setTimeout(()=>q?.focus(),50)}),U&&q){const F=Z=>{Z.stopPropagation();const se=q.value.trim();b.value=se?`${A} - ${se}`:A,b.dataset.locationName=se||"",k.classList.add("hidden"),b.dispatchEvent(new Event("change"))};U.addEventListener("click",F),q.addEventListener("keydown",Z=>{Z.key==="Enter"&&F(Z)}),q.addEventListener("click",Z=>Z.stopPropagation())}T&&T.addEventListener("click",F=>{F.stopPropagation(),b.value=A,b.dataset.locationName="",k.classList.add("hidden"),b.dispatchEvent(new Event("change"))}),k.querySelectorAll(".office-code-option").forEach(F=>{F.addEventListener("click",Z=>{Z.stopPropagation(),F.dataset.hasLocations==="true"?n("LOCATIONS",{id:F.dataset.id,name:F.dataset.name}):(b.value=F.dataset.name,b.dataset.officeId=F.dataset.id,delete b.dataset.locationName,k.classList.add("hidden"),b.dispatchEvent(new Event("change")))})})}else{k.innerHTML=`
                            ...
                            <div class="p-2 border-b ${r.borderDivide}">
                                <div class="relative group">
                                    ...
                                    <input type="text" id="location-search-internal" placeholder="Search in ${w.name}..." 
                        ...
                        `;const $=k.querySelector("#locations-list-container"),B=k.querySelector("#location-search-internal"),A=`dole_locs_cache_${w.id}`;let P=[];const q=localStorage.getItem(A);if(q)try{const{data:T,timestamp:F}=JSON.parse(q);P=T}catch{localStorage.removeItem(A)}const U=async()=>{let T=[];if(Se&&Te()){const{data:F,error:Z}=await Se.from("office_locations").select("location").eq("office_id",w.id).order("location");!Z&&F&&(T=F)}if(T.length===0)try{const F=await ce(`api/beneficiaries.php?get_office_locations=1&office_id=${w.id}`);F.success&&F.data?.success&&Array.isArray(F.data.locations)&&(T=F.data.locations)}catch(F){console.error("Office locations fetch failed:",F)}T.length>0&&(P=T,localStorage.setItem(A,JSON.stringify({data:T,timestamp:Date.now()})),H(B.value))},H=(T="")=>{const F=P.filter(se=>se.location.toLowerCase().includes(T.toLowerCase())),Z=T.trim();F.length>0?$.innerHTML=F.map(se=>`
                                ...
                                `,Z&&$.querySelector("#add-new-location-modal")?.addEventListener("click",()=>{b.value=`${w.name} - ${Z}`,b.dataset.officeId=w.id,b.dataset.locationName=Z,k.classList.add("hidden"),b.dispatchEvent(new Event("change"))})),$.querySelectorAll(".location-option").forEach(se=>{se.addEventListener("click",Ae=>{const Ee=se.dataset.location;b.value=`${w.name} - ${Ee}`,b.dataset.officeId=w.id,b.dataset.locationName=Ee,k.classList.add("hidden"),b.dispatchEvent(new Event("change"))})})};H(B.value),U(),setTimeout(()=>B.focus(),50),B.addEventListener("input",()=>H(B.value)),B.addEventListener("click",T=>T.stopPropagation()),k.querySelector("#back-to-offices").addEventListener("click",T=>{T.stopPropagation(),n("OFFICES")})}};b.addEventListener("focus",()=>{k.classList.remove("hidden"),n(y,I,b.value)}),b.addEventListener("input",()=>{delete b.dataset.officeId,delete b.dataset.locationName,y="OFFICES",I=null,k.classList.remove("hidden"),n("OFFICES",null,b.value)}),document.addEventListener("click",u=>{!b.contains(u.target)&&!k.contains(u.target)&&(k.classList.add("hidden"),b.value||(y="OFFICES",I=null))})})();function W(b,k,y){const I=i.querySelector(`#${b}`),D=i.querySelector(`#${k}`);if(!I||!D)return;let m=!1;I.addEventListener("focus",()=>D.classList.remove("hidden")),document.addEventListener("click",n=>{!I.contains(n.target)&&!D.contains(n.target)&&D.classList.add("hidden")}),I.addEventListener("input",()=>{if(m){m=!1;return}const n=I.value.toLowerCase(),u=D.querySelectorAll(`.${y}`);let w=!1;u.forEach(S=>{const $=S.querySelector(".option-text");($?$.innerText:S.innerText).toLowerCase().includes(n)?(S.style.display="block",w=!0):S.style.display="none"}),w?D.classList.remove("hidden"):D.classList.add("hidden")}),D.addEventListener("click",n=>{const u=n.target.closest(`.${y}`);if(!u)return;const w=u.querySelector(".option-text");I.value=w?w.innerText.trim():u.innerText.trim(),m=!0,D.classList.add("hidden"),I.dispatchEvent(new Event("change"))})}const O=i.querySelector("#add-beneficiary-form"),ee="add_beneficiary_draft";if(!t){const b=localStorage.getItem(ee);if(b)try{const k=JSON.parse(b);Object.entries(k).forEach(([y,I])=>{const D=O.elements[y];D&&D.type!=="file"&&D.type!=="hidden"&&(D.value=I)})}catch(k){console.error("Error loading draft",k)}}O.addEventListener("input",b=>{if(!t){const k=new FormData(O),y={};k.forEach((I,D)=>y[D]=I),localStorage.setItem(ee,JSON.stringify(y))}}),O&&O.addEventListener("submit",b=>{b.preventDefault(),O.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(U=>{U.classList.remove("ring-2","ring-red-500","!border-red-500")});const y=new FormData(O);let I=!1;const D=U=>{const H=O.querySelector(`[name="${U}"]`);H&&H.classList.add("ring-2","ring-red-500","!border-red-500"),I=!0},m=y.get("name"),n=y.get("contact"),u=y.get("startDate"),w=y.get("endDate"),S=(y.get("designation")||"").trim();(!m||m.trim()===""||/[0-9]/.test(m))&&D("name"),n&&n.trim()!==""&&/[^0-9]/.test(n.replace(/[\s\-\+\(\)]/g,""))&&D("contact"),u||D("startDate"),w||D("endDate");const $=y.get("age"),B=parseInt($);if((!$||isNaN(B)||B<18||B>29)&&(I=!0,c&&(c.className=`mt-1 text-[0.625rem] font-bold flex items-center gap-1.5 animate-pulse ${typeof ye=="function"&&ye()?"text-red-400":"text-red-600"}`),h&&(h.disabled=!0,h.classList.add("opacity-50","cursor-not-allowed","grayscale"),h.classList.remove("cursor-pointer","active:scale-[0.98]"))),I)return;const A={};y.forEach((U,H)=>{if(["birthday","startDate","endDate"].includes(H)){const T=window.__parseFormattedDate(U);if(T){const F=T.getFullYear(),Z=String(T.getMonth()+1).padStart(2,"0"),se=String(T.getDate()).padStart(2,"0");A[H]=`${F}-${Z}-${se}`}else A[H]=U}else A[H]=U}),S||(A.designation="N/A"),A.replacement||(A.replacement="");const P=i.querySelector("#office-input");P?.dataset.officeId&&(A.officeId=P.dataset.officeId),P?.dataset.locationName&&(A.locationName=P.dataset.locationName);const q=i.querySelector("#full-id-input")?.value;t?(A.id=e?.id,q&&(A.gip_id=q)):(A.id=null,q&&(A.gip_id=q)),window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(A)){if(!t){const H="add_beneficiary_draft",T=O.querySelector('[name="office"]')?.value||"",F=O.querySelector('[name="designation"]')?.value||"",Z=O.querySelector('[name="education"]')?.value||"";localStorage.setItem(H,JSON.stringify({office:T,designation:F,education:Z}))}M.close(),setTimeout(()=>{M.fire({toast:!0,position:"top-end",icon:"success",title:`Record ${t?"Updated":"Added"} Successfully`,showConfirmButton:!1,timer:3e3,timerProgressBar:!0}),!t&&e?._isBulk&&qe.onSaveSuccess()},100)}else M.fire({icon:"error",title:"Save Failed",text:"There was an error saving the record to the database."})})()})}})}window.handleContactSubmit=async function(e){e.preventDefault();const t=e.target,s=t.querySelector('button[type="submit"]'),a=s.innerHTML;s.disabled=!0,s.innerHTML=`
        <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        Sending...
    `;try{const o=new FormData(t);if((await fetch(t.action,{method:"POST",body:o,headers:{Accept:"application/json"}})).ok)M.fire({toast:!0,position:"top-end",icon:"success",title:"Message Sent Successfully!",text:"Thank you for reaching out. I will get back to you soon!",showConfirmButton:!1,timer:4e3,timerProgressBar:!0,didOpen:l=>{l.addEventListener("mouseenter",M.stopTimer),l.addEventListener("mouseleave",M.resumeTimer)},customClass:{popup:"rounded-2xl border-l-4 border-teal-500 shadow-2xl"}}),t.reset();else throw new Error("Failed to send")}catch{M.fire({toast:!0,position:"top-end",icon:"error",title:"Oops! Something went wrong.",text:"Could not send your message. Please try again later.",showConfirmButton:!1,timer:4e3,customClass:{popup:"rounded-2xl border-l-4 border-red-500 shadow-2xl"}})}finally{s.disabled=!1,s.innerHTML=a}return!1};function lr(){M.fire({html:`
        ...
        `,width:"800px",showConfirmButton:!1,showCloseButton:!1,backdrop:!0,position:"top",scrollbarPadding:!1,customClass:{container:"font-montserrat !backdrop-blur-md !bg-slate-900/70",popup:"!bg-transparent border-0 !shadow-none p-0 !overflow-visible mt-24",htmlContainer:"!overflow-visible",closeButton:"hidden"},didOpen:()=>{const e=document.getElementById("extraStatsSearchForm"),t=document.getElementById("statsSearchInput"),s=document.getElementById("statsDatePickerContainer"),a=document.getElementById("datepicker-range-start"),o=document.getElementById("datepicker-range-end"),r=document.getElementById("statsSortDropdownBtn"),l=document.getElementById("statsSortDropdown"),i=document.getElementById("statsSortDropdownLabel");let d="keyword";r&&l&&(r.addEventListener("click",g=>{g.stopPropagation(),l.classList.toggle("hidden")}),document.querySelectorAll(".stats-sort-option").forEach(g=>{g.addEventListener("click",p=>{const f=p.target.getAttribute("data-sort");d=f,i.textContent=p.target.textContent,l.classList.add("hidden"),f==="date"?(t.classList.add("hidden"),t.required=!1,s.classList.remove("hidden"),s.classList.add("flex"),t.value=""):(s.classList.add("hidden"),s.classList.remove("flex"),t.classList.remove("hidden"),t.required=!1,a.value="",o.value="",f==="offices"?t.placeholder="Search by Office name (e.g. Iligan)...":f==="education"?t.placeholder="Search by Education level (e.g. College)...":f==="ages"?t.placeholder="Search by Age (e.g. 24)...":t.placeholder="Search by name, office, status...")})}),document.addEventListener("click",g=>{!r.contains(g.target)&&!l.contains(g.target)&&l.classList.add("hidden")})),setTimeout(()=>t?.focus(),100),e.addEventListener("submit",async g=>{g.preventDefault();const p={mode:d,query:t.value.trim().toLowerCase(),startDate:a.value,endDate:o.value};await dr(p)})}})}async function dr(e){const t=document.getElementById("statsSearchLoader"),s=document.getElementById("statsSearchResult");t.classList.remove("hidden"),s.classList.add("hidden"),s.classList.remove("grid");let a=await Ge();if(!a||a.length===0){const o=await ce("api/beneficiaries.php?all=true");o&&o.status==="success"&&o.data?(a=o.data,typeof Ne=="function"&&Ne(a)):o&&o.data&&(a=Array.isArray(o.data)?o.data:Array.isArray(o)?o:[],typeof Ne=="function"&&Ne(a))}setTimeout(()=>{const{mode:o,query:r,startDate:l,endDate:i}=e,d=a.filter(c=>{if(o==="date"){const h=c.startDate||c.createdAt;if(!h)return!1;const C=new Date(h);if(isNaN(C.getTime()))return!1;if(C.setHours(0,0,0,0),l){const x=new Date(l);if(x.setHours(0,0,0,0),C<x)return!1}if(i){const x=new Date(i);if(x.setHours(0,0,0,0),C>x)return!1}return!0}else return o==="offices"?c.office?.toLowerCase().includes(r)||!1:o==="education"?c.education?.toLowerCase().includes(r)||!1:o==="ages"?c.age==r:r?c.name?.toLowerCase().includes(r)||!1||c.id?.toLowerCase().includes(r)||!1||c.office?.toLowerCase().includes(r)||!1||c.remarks?.toLowerCase().includes(r)||!1||c.designation?.toLowerCase().includes(r)||!1:!0});let g="";o==="date"?l&&i?g=`Date: ${l} to ${i}`:l?g=`Date: From ${l}`:i?g=`Date: Until ${i}`:g="Date: All Time":g=`${o.charAt(0).toUpperCase()+o.slice(1)}: "${r||"ALL"}"`,document.getElementById("statsSearchTermDisplay").textContent=g;const p=document.getElementById("statsTopResults");p&&(p.innerHTML="",d.length>0?d.slice(0,3).forEach(h=>{const C=(h.remarks||"No Status").toUpperCase();let x="text-gray-500";C==="ONGOING"?x="text-green-500":C==="EXPIRED"?x="text-red-500":C==="ABSORBED"?x="text-emerald-600":C==="RESIGNED"?x="text-[#ce1126]":x="text-royal-blue",p.innerHTML+=`
                        <div class="flex flex-col border-b border-gray-200 dark:border-slate-700 pb-2 mb-2 last:border-0 last:pb-0 last:mb-0">
                            ...
                            <div class="flex justify-between items-center text-xs mt-1">
                                ...
                                <span class="${x} font-bold text-[10px] uppercase tracking-wider">${C}</span>
                            </div>
                        </div>
                    `}):p.innerHTML='<div class="text-center text-gray-400 font-bold text-xs mt-6 uppercase tracking-widest">No matching records found.</div>'),d.length,d.filter(c=>(c.remarks||"").toUpperCase()==="ONGOING").length,d.filter(c=>(c.remarks||"").toUpperCase()==="EXPIRED").length,d.filter(c=>(c.remarks||"").toUpperCase()==="ABSORBED").length,d.filter(c=>(c.remarks||"").toUpperCase()==="RESIGNED").length;const f=new Date,v={month:"short",day:"numeric",year:"numeric",hour:"numeric",minute:"numeric",hour12:!0};document.getElementById("statsCurrentDateTime").textContent=f.toLocaleString("en-US",v),Yt(d,"statsModalChartContainer"),t.classList.add("hidden"),s.classList.remove("hidden"),s.classList.add("grid")},400)}export{Dr as A,nr as B,ft as _,_e as a,Qe as b,yr as c,wr as d,Er as e,$r as f,he as g,Ge as h,Ne as i,vr as j,Dt as k,$e as l,Te as m,ce as n,mr as o,kr as p,Cr as q,xr as r,Se as s,gr as t,Sr as u,Lr as v,fr as w,br as x,hr as y,Ve as z};
