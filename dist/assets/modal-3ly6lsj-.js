...
import f from"./vendor-swal-BSk0fVSb.js";import{a as Ie}from"./vendor-DHtNC8Ux.js";const De="true".toLowerCase()==="true";function we(){return De}function _(){const e=window.location.pathname,o="/dole-system/",s=e.toLowerCase().indexOf(o.toLowerCase());if(s!==-1)return e.substring(0,s+o.length);const a=e.indexOf("/frontend/");if(a!==-1)return e.substring(0,a+1);const r=e.indexOf("/backend/");return r!==-1?e.substring(0,r+1):"/"}function fe(e="Incorrect Username or Password"){f.fire({html:`
            <div class="p-6">
        ...
        `,timer:e?800:3e3,timerProgressBar:!0,showConfirmButton:!1,showCloseButton:!1,width:"350px",padding:"0",customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl overflow-hidden border border-gray-100",timerProgressBar:"bg-green-500 h-1.5"},backdrop:"rgba(0, 0, 0, 0.4)"})}function We(){const e=localStorage.getItem("hasVisitedBefore"),o=document.getElementById("page-loader");e&&(document.documentElement.classList.add("loaded"),o&&(o.style.display="none")),window.addEventListener("load",()=>{const s=document.querySelector("body > *:not(.page-loader)");s&&s.classList.add("content-fade-in"),setTimeout(()=>{document.documentElement.classList.add("loaded"),o&&o.classList.add("hidden"),document.body.style.overflow="auto",localStorage.setItem("hasVisitedBefore","true")},300)})}const se={_key:"DOLE-GIP-SECURE-KEY-2026",encrypt:function(e){try{const o=r=>r.split("").map(t=>t.charCodeAt(0)),s=r=>("0"+Number(r).toString(16)).substr(-2),a=r=>o(this._key).reduce((t,l)=>t^l,r);return e.split("").map(o).map(a).map(s).join("")}catch(o){return console.error("Encryption Failed",o),null}},decrypt:function(e){try{const o=a=>a.split("").map(r=>r.charCodeAt(0)),s=a=>o(this._key).reduce((r,t)=>r^t,a);return e.match(/.{1,2}/g).map(a=>parseInt(a,16)).map(s).map(a=>String.fromCharCode(a)).join("")}catch(o){return console.error("Decryption Failed",o),null}}};function Ye(){document.querySelectorAll(".login-form-shared").forEach(o=>{const s=o.querySelector('input[name="username"]'),a=o.querySelector('input[name="password"]'),r=o.querySelector('input[name="rememberMe"]');if(s&&a&&r){const t=localStorage.getItem("secure_user"),l=localStorage.getItem("secure_pass");if(t&&l){const n=se.decrypt(t),d=se.decrypt(l);n&&d&&(s.value=n,a.value=d,r.checked=!0)}}o.addEventListener("submit",async t=>{t.preventDefault();try{const n=await(await fetch(`${_()}api/auth.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:s.value,password:a.value})})).json();if(n.success){r.checked?(localStorage.setItem("secure_user",se.encrypt(s.value)),localStorage.setItem("secure_pass",se.encrypt(a.value))):(localStorage.removeItem("secure_user"),localStorage.removeItem("secure_pass"));const d=localStorage.getItem("hasLoggedInBefore")==="true";localStorage.setItem("isLoggedIn","true"),localStorage.setItem("hasLoggedInBefore","true"),localStorage.setItem("user",JSON.stringify(n.user));const p=document.getElementById("drawer-login");if(p){p.classList.add("translate-y-full");const i=p.querySelector("[data-drawer-hide]");i&&i.click()}await Ae(d),Me(d)}else{const d=document.getElementById("drawer-login");d?(d.classList.add("translate-y-full"),setTimeout(()=>{fe(),setTimeout(()=>{d.classList.remove("translate-y-full"),a.value="",a.focus()},600)},400)):(fe(),a.value="",a.focus())}}catch(l){console.error("Login Error:",l),f.fire({icon:"error",title:"Login Error",text:"Unable to reach the server. Please check your connection."})}})})}function Me(e=!1){const o=document.getElementById("left-panel"),s=document.getElementById("right-panel"),a=document.getElementById("left-panel-content"),r=document.getElementById("right-panel-content");a&&(a.style.opacity="0"),r&&(r.style.opacity="0");const t=document.createElement("div");t.className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none";const l=e?"":"animate__delay-1s",n=e?"animation-duration: 0.8s;":"animation-duration: 2s;";t.innerHTML=`
        <img src="${_()}frontend/images/logo/doleiligan.png" class="w-64 h-64 md:w-96 md:h-96 object-contain bg-white/20 backdrop-blur-sm shadow-2xl rounded-full p-4 animate__animated animate__rotateOut ${l}" style="${n}" alt="DOLE Logo">
    `,document.body.appendChild(t);const d=e?0:1e3,p=e?600:1500;setTimeout(()=>{o&&o.classList.add("animate-slide-left"),s&&s.classList.add("animate-slide-right"),setTimeout(()=>{window.location.href=`${_()}frontend/dashboard/`},p)},d)}function Ke(){const e=document.getElementById("logoutBtn");e&&e.addEventListener("click",()=>{localStorage.removeItem("isLoggedIn"),window.location.href=`${_()}`})}function Ze(){const e=document.getElementById("mobile-splash"),o=document.getElementById("show-login-btn"),s=document.getElementById("back-to-splash"),a=document.getElementById("mobile-bg-content"),r=document.getElementById("mobile-welcome-text"),t=document.getElementById("reopen-login-drawer"),l=document.getElementById("request-notifications-btn"),n=async()=>{if("Notification"in window){const m=await Notification.requestPermission();console.log("Notification permission:",m),m==="granted"&&l&&l.classList.add("hidden")}};Notification.permission==="default"&&l&&(l.classList.remove("hidden"),l.addEventListener("click",n));const d=()=>{e&&(e.style.transform="translateY(-100%)",setTimeout(()=>{e.style.visibility="hidden",e.style.pointerEvents="none",e.style.zIndex="-1",Notification.permission==="default"&&n();const m=document.getElementById("drawer-login");m&&m.classList.remove("translate-y-full")},800))},p=()=>{e&&(e.style.zIndex="9999",e.style.pointerEvents="auto",e.style.visibility="visible",e.style.transform="translateY(0)")};o&&o.addEventListener("click",d),document.querySelectorAll(".forgot-password-link").forEach(m=>{m.addEventListener("click",x=>{x.target.closest("#mobile-splash")&&d()})}),s&&s.addEventListener("click",()=>{const m=document.getElementById("drawer-login");if(m){m.classList.add("translate-y-full");const x=m.querySelector("[data-drawer-hide]");x&&x.click()}p()});const c=document.getElementById("drawer-login"),u=document.getElementById("curved-welcome"),b=document.getElementById("peoples-bg");c&&new MutationObserver(x=>{x.forEach(v=>{v.attributeName==="class"&&(c.classList.contains("translate-y-full")?(a&&(a.style.transform="translateY(0)"),r&&(r.style.opacity="1",r.style.transform="translateY(0) scale(1)"),u&&(u.style.opacity="0",u.style.transform="scale(0.5)"),t&&e&&e.style.visibility==="hidden"&&(t.style.opacity="1",t.style.transform="scale(1)"),b&&(b.classList.add("opacity-0","scale-0"),b.classList.remove("opacity-40","scale-[1.6]"))):(a&&(a.style.transform="translateY(-35%)"),r&&(r.style.opacity="0",r.style.transform="translateY(20px) scale(0.9)"),u&&(u.style.opacity="1",u.style.transform="scale(1)"),t&&(t.style.opacity="0",t.style.transform="scale(0)"),b&&(b.classList.remove("opacity-0","scale-0"),b.classList.add("opacity-40","scale-[1.6]"))))})}).observe(c,{attributes:!0})}const re=()=>"false".toLowerCase()==="true";function Ne(e){try{return JSON.stringify(e)}catch{return"[unserializable]"}}const X={debug(...e){re()&&console.debug(...e)},info(...e){re()&&console.info(...e)},warn(...e){re()&&console.warn(...e)},error(...e){console.error(...e)},table(e){re()&&console.table(e)},json(e,o){re()&&console.debug(e,Ne(o))}},ne=new Map;async function oe(e,o={}){const a=`${_()}${e}`;let r=null;try{const l=JSON.parse(localStorage.getItem("user"));l&&(r=l.user_id||l.id||null)}catch{}const t={headers:{"Content-Type":"application/json",...r?{"X-User-Id":r}:{},...o.headers},...o};try{if(X.debug("[API] Request",{url:a,method:t.method||"GET",hasUserId:!!r}),t.body)try{X.json("[API] Payload",JSON.parse(t.body))}catch{X.debug("[API] Payload (raw)",t.body)}const l=await fetch(a,t);if(!l.ok)throw new Error(`HTTP ${l.status}: ${l.statusText}`);const n=await l.json();return ne.has(a)&&(ne.delete(a),X.info?.("[API] Recovered",{url:a})),X.debug("[API] Response",{url:a,ok:!0}),{success:!0,data:n}}catch(l){return l instanceof TypeError&&/fetch/i.test(l.message||"")?ne.get(a)||(ne.set(a,!0),X.error("API Request Network Error (suppressed for repeats):",{url:a,message:l.message})):X.error("API Request Error:",l),{success:!1,error:l.message}}}async function F(e){return oe(e,{method:"GET"})}async function ue(e,o){return oe(e,{method:"POST",body:JSON.stringify(o)})}async function je(e,o){return oe(e,{method:"PUT",body:JSON.stringify(o)})}async function Qe(e,o){const s=new URLSearchParams(o).toString();return oe(`${e}?${s}`,{method:"PATCH"})}class Oe{constructor(){this.intervals=new Map,this.isPageVisible=!0,document.addEventListener("visibilitychange",()=>{this.isPageVisible=!document.hidden,this.isPageVisible?console.log("[Polling] Page visible - resuming normal activity"):console.log("[Polling] Page hidden - reducing activity")})}start(o,s,a=1e4){this.stop(o),s();const r=setInterval(async()=>{this.isPageVisible&&await s()},a);this.intervals.set(o,r),console.log(`[Polling] Started: ${o} (every ${a}ms)`)}stop(o){this.intervals.has(o)&&(clearInterval(this.intervals.get(o)),this.intervals.delete(o),console.log(`[Polling] Stopped: ${o}`))}stopAll(){this.intervals.forEach((o,s)=>this.stop(s)),console.log("[Polling] All intervals stopped")}getActivePolls(){return Array.from(this.intervals.keys())}}const Pe=new Oe;function et(){typeof window.initFlowbite=="function"?(window.initFlowbite(),console.log("[Flowbite] Components re-initialized")):console.warn("[Flowbite] initFlowbite() not available globally")}function tt(e,o="",s="info"){f.fire({toast:!0,position:"top-end",icon:s,title:e,text:o,showConfirmButton:!1,timer:3e3,timerProgressBar:!0})}function rt(e){return JSON.stringify(e)}window.addEventListener("beforeunload",()=>{Pe.stopAll()});const Re="modulepreload",Te=function(e,o){return new URL(e,o).href},be={},ye=function(o,s,a){let r=Promise.resolve();if(s&&s.length>0){let p=function(i){return Promise.all(i.map(c=>Promise.resolve(c).then(u=>({status:"fulfilled",value:u}),u=>({status:"rejected",reason:u}))))};const l=document.getElementsByTagName("link"),n=document.querySelector("meta[property=csp-nonce]"),d=n?.nonce||n?.getAttribute("nonce");r=p(s.map(i=>{if(i=Te(i,a),i in be)return;be[i]=!0;const c=i.endsWith(".css"),u=c?'[rel="stylesheet"]':"";if(a)for(let m=l.length-1;m>=0;m--){const x=l[m];if(x.href===i&&(!c||x.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${i}"]${u}`))return;const b=document.createElement("link");if(b.rel=c?"stylesheet":Re,c||(b.as="script"),b.crossOrigin="",b.href=i,d&&b.setAttribute("nonce",d),document.head.appendChild(b),c)return new Promise((m,x)=>{b.addEventListener("load",m),b.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${i}`)))})}))}function t(l){const n=new Event("vite:preloadError",{cancelable:!0});if(n.payload=l,window.dispatchEvent(n),!n.defaultPrevented)throw l}return r.then(l=>{for(const n of l||[])n.status==="rejected"&&t(n.reason);return o().catch(t)})};let pe=null;if(we()){const e="https://llnddycvbcetztzwbdpx.supabase.co",o="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxsbmRkeWN2YmNldHp0endiZHB4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ1MDMwMTEsImV4cCI6MjA5MDA3OTAxMX0.apIpDlZYja7SNuA-joXgFSvu_wtbBcVVP3OQSPrh7fA";try{pe=Ie(e,o),console.log("[SUPABASE SDK] Client Initialized Successfully")}catch(s){console.error("[SUPABASE SDK] Failed to initialize client:",s)}}else console.log("[SUPABASE SDK] Supabase mode is disabled (Localhost PHP mode active).");const ee="color-theme",_e=3600*24*365;function qe(e,o,s){document.cookie=`${e}=${o}; max-age=${s}; path=/; SameSite=Lax`}function ke(e){const o=document.cookie.match(new RegExp("(?:^|; )"+e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+"=([^;]*)"));return o?decodeURIComponent(o[1]):null}function $e(){const e=localStorage.getItem(ee)||ke(ee);return e==="dark"||e==="light"?e:"light"}function le(e){const o=document.documentElement;e==="dark"?o.classList.add("dark"):o.classList.remove("dark"),localStorage.setItem(ee,e),qe(ee,e,_e),Fe(e),document.dispatchEvent(new CustomEvent("themeChanged",{detail:{theme:e}}))}function ge(){const e=$e();le(e==="dark"?"light":"dark")}function Fe(e){const o=e==="dark",s=document.getElementById("pref-dark-mode");s&&(s.checked=o);const a=document.getElementById("theme-toggle-dark-icon"),r=document.getElementById("theme-toggle-light-icon");a&&r&&(a.classList.toggle("hidden",o),r.classList.toggle("hidden",!o));const t=document.getElementById("sidebar-theme-label");t&&(t.textContent=o?"Light Mode":"Dark Mode")}function ot(){const e=$e();le(e);const o=document.getElementById("pref-dark-mode");o&&o.addEventListener("change",()=>{le(o.checked?"dark":"light")});const s=document.getElementById("theme-toggle-btn");s&&s.addEventListener("click",ge),document.querySelectorAll("[data-theme-toggle]").forEach(a=>{a.addEventListener("click",ge)}),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",a=>{localStorage.getItem(ee)||ke(ee)||le(a.matches?"dark":"light")})}function z(){return document.documentElement.classList.contains("dark")}window.toggleTheme=ge;window.isDarkMode=z;const de={queue:[],currentIndex:0,isActive:!1,isAutoSave:!1,lastInteractionTime:0,init(){this.showUploadModal()},showUploadModal(){const e=z(),o={bgCard:e?"bg-slate-900/40":"bg-gray-50/40",borderCard:e?"border-slate-800":"border-gray-100",textHeading:e?"text-green-500":"text-[#2e7d32]",textSubtitle:e?"text-slate-500":"text-gray-400 dark:text-gray-300",bgUpload:e?"bg-slate-800":"bg-white",borderUpload:e?"border-slate-700":"border-gray-200",textUpload:e?"text-slate-400":"text-gray-500",hoverUpload:e?"hover:bg-slate-800/80 hover:border-blue-500":"hover:bg-blue-50/50 hover:border-royal-blue",iconText:e?"text-green-400":"text-[#2e7d32]",iconBg:e?"bg-green-900/20":"bg-[#e8f5e9]",iconBorder:e?"border-green-800/30":"border-[#c8e6c9]"},s=`
        ...
        `;f.fire({html:s,width:"600px",showConfirmButton:!1,showCloseButton:!0,customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:a=>{const r=a.querySelector("#csv-upload"),t=a.querySelector('label[for="csv-upload"]');["dragenter","dragover","dragleave","drop"].forEach(n=>{t.addEventListener(n,l,!1)});function l(n){n.preventDefault(),n.stopPropagation()}["dragenter","dragover"].forEach(n=>{t.addEventListener(n,()=>{t.classList.add("border-blue-500","bg-blue-50/50"),e&&t.classList.add("dark:bg-slate-800/80")},!1)}),["dragleave","drop"].forEach(n=>{t.addEventListener(n,()=>{t.classList.remove("border-blue-500","bg-blue-50/50"),e&&t.classList.remove("dark:bg-slate-800/80")},!1)}),r.addEventListener("change",n=>{const d=n.target.files[0];if(d){const p=a.querySelector("#auto-save-toggle");this.isAutoSave=p?p.checked:!1,this.handleFile(d)}}),t.addEventListener("drop",n=>{const p=n.dataTransfer.files[0];if(p){const i=a.querySelector("#auto-save-toggle");this.isAutoSave=i?i.checked:!1,this.handleFile(p)}},!1)}})},handleFile(e){if(!e.name.toLowerCase().endsWith(".csv")&&!e.name.toLowerCase().endsWith(".txt")){f.fire("Invalid File","Please upload a valid .csv or .txt file.","error");return}const o=new FileReader;o.onload=s=>{const a=s.target.result;this.parseCSV(a)},o.readAsText(e)},async parseCSV(e){let o=[],s="",a=!1;for(let r=0;r<e.length;r++){let t=e[r];t==='"'&&(a=!a),!a&&(t===`
`||t==="\r")?(t==="\r"&&e[r+1]===`
`&&r++,s.trim()!==""&&o.push(s),s=""):s+=t}s.trim()!==""&&o.push(s),this.queue=[];for(let r=0;r<o.length;r++){let t=o[r].trim();if(!t)continue;let l=[],n="",d=!1;for(let p=0;p<t.length;p++){let i=t[p];i==='"'?d=!d:i===","&&!d?(l.push(n.replace(/(^"|"$)/g,"").trim()),n=""):n+=i}if(l.push(n.replace(/(^"|"$)/g,"").trim()),l.length>=2){const p=l[3];if(!p||isNaN(parseInt(p)))continue;const i=l[1];if(!i||i.toLowerCase()==="name"||i.toLowerCase()==="full name")continue;const c=l[2];let u=l[4]?l[4].toUpperCase().trim():"",b="";(u==="F"||u.includes("FEMALE"))&&(b="Female"),(u==="M"||u.includes("MALE"))&&(b="Male");const m=l[5],x=l[6],v=l[7],$=this.formatDate(l[8]),I=this.formatDate(l[9]);this.queue.push({name:i,address:c,age:p,gender:b,education:m,startDate:$,endDate:I,office:x,designation:v})}}if(this.queue.length>0){try{f.fire({title:"Checking duplicates...",html:'<p class="text-sm">Please wait while we cross-reference your data.</p>',allowOutsideClick:!1,didOpen:()=>{f.showLoading()}});const r=this.queue.map(n=>n.name),l=await(await fetch(`${_()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({names:r})})).json();if(l.success&&l.duplicates&&l.duplicates.length>0){const n=new Set(l.duplicates.map(p=>p.toLowerCase().trim())),d=this.queue.length;this.queue=this.queue.filter(p=>{const i=n.has(p.name.toLowerCase().trim());return i&&console.warn(`%c[Bulk Add] SKIPPED: ${p.name} already exists in database.`,"color: #ff9800; font-weight: bold;"),!i}),console.log(`[Bulk Add] Removed ${d-this.queue.length} duplicates ahead of time.`)}}catch(r){console.error("Bulk duplicate check failed:",r)}if(this.queue.length===0){f.fire("All Skipped","All beneficiaries in the CSV already exist in the database.","info");return}this.isActive=!0,this.currentIndex=0,f.close(),this.processNext()}else f.fire("Error","No valid beneficiary data found. Please ensure your CSV formatting matches the requirements.","error")},showProgressModal(){z();const e=Math.round(this.currentIndex/this.queue.length*100),o=`
...
<div class="mt-8 pt-6 border-t border-default relative">
    ...
    <div id="drawer-page-1" class="hidden flex-1 flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-3">
            ${d.map(c=>{const u=c.status.toUpperCase(),m={VERIFIED:"COMPLETED",DECLINED:"REJECTED",PENDING:"PENDING"}[u]||u,x=m==="COMPLETED",v=m==="REJECTED",$=x?"text-green-500":v?"text-red-500":"text-gray-400 dark:text-gray-500",I=x?"bg-green-50/50 dark:bg-green-900/10":v?"bg-red-50/50 dark:bg-red-900/10":"bg-gray-50/50 dark:bg-slate-800/50",M=x?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800 hover:bg-green-200 cursor-pointer":v?"bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800 hover:bg-red-200 cursor-pointer":"bg-white text-gray-500 border-gray-200 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer";let P='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';return x?P='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>':v&&(P='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>'),`
                <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-0.5 ${I}">
                    <div class="flex items-center gap-3 w-full">
                        <div class="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center bg-white dark:bg-slate-800 shadow-sm border border-gray-100 dark:border-slate-700 ${$}">
                            ${P}
                        </div>
                        <span class="text-xs sm:text-sm font-black ${x?"text-heading":"text-gray-500 dark:text-gray-400"} uppercase tracking-tight flex-1">${c.name}</span>
                    </div>
                    <button type="button" class="ml-3 ${M} text-[10px] font-black px-3 py-1.5 rounded-full border uppercase tracking-widest transition-colors flex-shrink-0 drawer-doc-btn" data-id="${c.id}" data-name="${c.name}" data-status="${m} cursosr-pointer">
                        ${m}
                    </button>
                </div>
                `}).join("")}
    ...
    <div id="drawer-page-2" class="hidden flex-1 flex flex-col gap-6">
         ...
         <div class="flex flex-col gap-5">
            <div>
                ...
                <div class="space-y-2">
                    ${t.length?t.map(c=>{const u=c.status||"PENDING";let b=u==="VERIFIED"||u==="COMPLETED"?"text-green-500":u==="REJECTED"||u==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500";return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-blue-100 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors edit-log-btn" data-type="dtr" data-id="${c.id}" data-val="${c.date||c.createdAt}" data-status="${u}">
                            <span class="text-xs font-black text-royal-blue dark:text-blue-400 capitalize whitespace-nowrap pointer-events-none">${c.date||c.createdAt}</span>
            ...
            <div>
                ...
                <div class="space-y-2">
                    ${r.length?r.map(c=>{const u=c.status||"PENDING";let b=u==="VERIFIED"||u==="COMPLETED"?"text-green-500":u==="REJECTED"||u==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500";return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-orange-100 dark:border-orange-900/50 bg-orange-50/50 dark:bg-orange-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-orange-100 dark:hover:bg-orange-900/40 transition-colors edit-log-btn" data-type="ar" data-id="${c.id}" data-val="${c.period||c.createdAt}" data-status="${u}">
                            <span class="text-xs font-black text-orange-600 dark:text-orange-400 capitalize whitespace-nowrap pointer-events-none">${c.period||c.createdAt}</span>
                            <span class="text-[11px] font-bold ${b} uppercase tracking-widest truncate max-w-[50%] text-right pr-6 group-hover:pr-12 pointer-events-none transition-all">${u}</span>
...
<style>
    ...
    `;let i=document.getElementById("beneficiary-drawer-container");i&&i.remove(),i=document.createElement("div"),i.id="beneficiary-drawer-container",i.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-neutral-primary-soft dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl",i.setAttribute("tabindex","-1"),i.setAttribute("data-drawer-backdrop","true"),i.innerHTML=p,document.body.appendChild(i),ye(async()=>{const{Drawer:c}=await import("./vendor-flowbite-bBKPL--D.js");return{Drawer:c}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:c})=>{const u={placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{i&&i.parentNode&&i.remove()},300)}},b=new c(i,u);b.show(),i.querySelector("#close-drawer-btn").addEventListener("click",()=>b.hide());const x=i.querySelector("#drawer-prev-btn"),v=i.querySelector("#drawer-next-btn"),$=2,I=()=>{i.querySelectorAll("[id^=drawer-page-]").forEach((k,S)=>{k.classList.toggle("hidden",S!==a)}),x.disabled=a===0,v.disabled=a===$,x.classList.toggle("opacity-50",a===0),v.classList.toggle("opacity-50",a===$)};x.addEventListener("click",()=>{a>0&&a--,I()}),v.addEventListener("click",()=>{a<$&&a++,I()}),I(),i.querySelectorAll(".drawer-doc-btn").forEach(k=>{k.addEventListener("click",async()=>{const S=k.dataset.name,C=k.dataset.status,L=await f.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Update Document</span>',html:`
                    ...
                    `,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest">Update</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest">Cancel</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs px-6 py-2.5 rounded-xl border border-gray-200 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1,preConfirm:()=>document.getElementById("swal-doc-status").value});if(L.isConfirmed){const E=L.value;if(E===C)return;try{const G={COMPLETED:"VERIFIED",REJECTED:"DECLINED",PENDING:"PENDING"}[E]||E,H=await ue("api/logs.php?type=docs",{gip_id:e.id,doc_name:S,status:G}),V=H.success?H.data:{success:!1,error:H.error};V.success?(f.fire({toast:!0,position:s,icon:"success",title:"Status updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):f.fire("Error",V.error||"Failed to update","error")}catch(j){f.fire("Error",j.message,"error")}}})});const M=()=>{if(!t.length)return new Date().toISOString().split("T")[0];const k=new Date(t[0].date);return k.setDate(k.getDate()+1),k.toISOString().split("T")[0]},P=()=>{if(!r.length){const L=new Date,E=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"][L.getMonth()];return L.getDate()<=15?`${E} 1-15`:`${E} 16-${new Date(L.getFullYear(),L.getMonth()+1,0).getDate()}`}const k=r[0].period||"",S=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"],C=k.toUpperCase().match(/([A-Z]{3})\s+(\d+)-(\d+)/);if(C){let L=C[1],E=parseInt(C[2]),j=S.indexOf(L);if(j!==-1)return E===1?`${L} 16-${new Date(new Date().getFullYear(),j+1,0).getDate()}`:`${S[(j+1)%12]} 1-15`}return"NEW PERIOD"},N=async(k,S)=>{f.fire({title:"Adding...",allowOutsideClick:!1,showConfirmButton:!1}),f.showLoading();try{const C={gip_id:e.id};k==="dtr"&&(C.record_date=S),k==="ar"&&(C.period=S);const L=await ue(`api/logs.php?type=${k}`,C);(L.success?L.data:{success:!1,error:L.error}).success?(f.fire({toast:!0,position:s,icon:"success",title:"Auto-Added!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):f.fire("Error","Failed to add log.","error")}catch(C){f.fire("Error",C.message,"error")}},O=async(k,S,C,L,E)=>{const j=S==="dtr"?"Record Date":"Period";z();const G="flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[10px] sm:text-xs gap-2 ",H='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>',V='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>',{value:q}=await f.fire({title:`<span class="text-xl font-black text-heading uppercase tracking-tight">Edit ${k} Log</span>`,html:`
                    <div class="flex flex-col gap-5 text-left font-montserrat mt-2">
                        <div>
                            <label class="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-1">${j}</label>
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-log-status" value="VERIFIED" class="peer sr-only" ${E==="VERIFIED"?"checked":""}>
                                ...
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-log-status" value="REJECTED" class="peer sr-only" ${E==="REJECTED"||E==="DECLINED"?"checked":""}>
                ...
                `,focusConfirm:!1,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">SAVE REVISIONS</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">CANCEL</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2"},buttonsStyling:!1,preConfirm:()=>{const A=document.querySelector('input[name="swal-log-status"]:checked');return{val:document.getElementById("swal-log-val").value.trim().toUpperCase(),status:A?A.value:"PENDING"}}});if(q&&(q.val!==L||q.status!==E))try{const A={type:S,id:C,status:q.status};S==="dtr"&&(A.record_date=q.val),S==="ar"&&(A.period=q.val);const W=await je("api/logs.php",A),g=W.success?W.data:{success:!1,error:W.error};g.success?(f.fire({toast:!0,position:s,icon:"success",title:"Log Updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):f.fire("Error",g.error||"Failed to update log.","error")}catch(A){f.fire("Error",A.message,"error")}},D=i.querySelector("#add-dtr-log-btn");D&&D.addEventListener("click",()=>N("dtr",M()));const U=i.querySelector("#add-ar-log-btn");U&&U.addEventListener("click",()=>N("ar",P())),i.querySelectorAll(".edit-log-btn").forEach(k=>{k.addEventListener("click",S=>{if(S.target.closest(".delete-log-btn"))return;const C=k.dataset.type,L=k.dataset.id,E=k.dataset.val,j=k.dataset.status;O(C.toUpperCase(),C,L,E,j)})}),i.querySelectorAll(".delete-log-btn").forEach(k=>{k.addEventListener("click",async()=>{const S=k.dataset.id,C=k.dataset.type;if((await f.fire({title:'<span class="text-xl font-black text-philippine-red uppercase tracking-tight">Delete item?</span>',text:"This action cannot be undone.",icon:"warning",showCancelButton:!0,confirmButtonText:'<span class="font-black tracking-widest uppercase">Delete</span>',cancelButtonText:'<span class="font-black tracking-widest uppercase">Wait</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100",confirmButton:"bg-philippine-red text-white hover:bg-red-700 text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs px-6 py-2.5 rounded-xl border border-gray-200 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1})).isConfirmed)try{const E=await ue(`api/logs.php?type=${C}`,{log_id:S,action:"delete"});(E.success?E.data:{success:!1,error:E.error}).success?(f.fire({toast:!0,position:s,icon:"success",title:"Deleted",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):f.fire("Error","Failed to delete data.","error")}catch(E){f.fire("Error",E.message,"error")}})})}).catch(console.error)}function Ue(e){const o=z(),s=`w-full bg-transparent border-b-2 ${o?"border-slate-700 text-white focus:border-brand placeholder-slate-600":"border-gray-200 text-gray-900 focus:border-brand placeholder-gray-300"} px-1 py-1 text-sm font-black outline-none transition-all focus:ring-0`,a=`w-full bg-transparent border-none ${o?"text-white":"text-royal-blue"} px-0 py-0 text-xl sm:text-2xl font-black leading-tight tracking-tight focus:ring-0 outline-none placeholder-gray-300 resize-none overflow-hidden`,r=["DOLE Field Office","LGU","DEPED","DICT","PCA"];function t(d){if(!d)return"";const p=new Date(d),i=new Date;let c=i.getFullYear()-p.getFullYear();const u=i.getMonth()-p.getMonth();return(u<0||u===0&&i.getDate()<p.getDate())&&c--,c>=0?c:0}const l=`
...
.editable-indicator {
    ...
    `;let n=document.getElementById("edit-drawer-container");n&&n.remove(),n=document.createElement("div"),n.id="edit-drawer-container",n.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-white dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",n.setAttribute("tabindex","-1"),n.innerHTML=l,document.body.appendChild(n),setTimeout(()=>{const d=n.querySelector('textarea[name="name"]');d&&(d.style.height="auto",d.style.height=d.scrollHeight+"px")},10),(async()=>{try{const d=await oe("api/beneficiaries.php?get_offices=1");if(d.success&&d.data.offices){const p=n.querySelector("#edit-office-suggestions-box");if(p){const i=["DOLE Field Office","LGU","DEPED","DICT","PCA"],c=[...new Set([...i,...d.data.offices])];p.innerHTML=c.map(u=>`
                    ...
                    `).join("")}}}catch(d){console.error("Error fetching office suggestions:",d)}})(),ye(async()=>{const{Drawer:d}=await import("./vendor-flowbite-bBKPL--D.js");return{Drawer:d}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:d})=>{const p=new d(n,{placement:"right",backdrop:!0,bodyScrolling:!1,backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{n.parentNode&&n.remove()},300)}});p.show(),n.querySelector("#close-edit-drawer-btn").addEventListener("click",()=>p.hide()),n.querySelector("#edit-drawer-cancel-btn").addEventListener("click",()=>p.hide());const i=n.querySelector("#edit-beneficiary-drawer-form"),c=n.querySelector("#edit-bday-input"),u=n.querySelector("#edit-age-display");let b=!1;c&&u&&(u.addEventListener("input",()=>{b=!0}),c.addEventListener("change",x=>{x.target.value&&(!b||!u.value)&&(u.value=t(x.target.value))}));const m=(x,v,$)=>{const I=n.querySelector(x),M=n.querySelector(v);if(!I||!M)return;const P=()=>M.classList.add("hidden"),N=()=>M.classList.remove("hidden");I.addEventListener("focus",N),I.addEventListener("input",()=>{const O=I.value.toLowerCase().trim();let D=0;M.querySelectorAll($).forEach(U=>{const S=(U.querySelector(".option-text")?.textContent||U.textContent||"").toLowerCase().includes(O);U.style.display=S?"block":"none",S&&D++}),D>0?N():P()}),M.addEventListener("click",O=>{const D=O.target.closest($);D&&(I.value=(D.querySelector(".option-text")?.textContent||D.textContent||"").trim(),P(),I.dispatchEvent(new Event("change")))}),document.addEventListener("click",O=>{!I.contains(O.target)&&!M.contains(O.target)&&P()})};m("#edit-education-input","#edit-education-suggestions-box",".edit-education-option"),m("#edit-designation-input","#edit-designation-suggestions-box",".edit-designation-option"),m("#edit-office-input","#edit-office-suggestions-box",".edit-office-option"),i.querySelectorAll("input, select, textarea").forEach(x=>{const v=(x.getAttribute("type")||"").toLowerCase(),$=!x.disabled&&!x.readOnly&&v!=="hidden";x.classList.remove("editable-indicator"),$&&x.classList.add("editable-indicator")}),i.addEventListener("submit",x=>{x.preventDefault();const v=new FormData(i),$={};v.forEach((I,M)=>$[M]=I),$.id=e.id,$.gip_id=e.id,window.addBeneficiaryData&&(async()=>await window.addBeneficiaryData($,!0,!1)&&(p.hide(),window.viewBeneficiary&&setTimeout(()=>window.viewBeneficiary({id:e.id},0),100),f.fire({toast:!0,position:"top-end",icon:"success",title:"Record Updated",showConfirmButton:!1,timer:3e3})))()})})}function at(){window.viewBeneficiary=async function(e,o=0){const s=e?.id||e?.gip_id||null;if(!s)return;const a=!!(e?.name&&e?.office&&e?.remarks);let r={...e,id:s};if(!a){const n=await F(`api/beneficiaries.php?id=${encodeURIComponent(s)}`);n.success&&n.data?.success&&n.data?.beneficiary&&(r={...n.data.beneficiary,...r,id:s})}const t=`logs_cache_${s}`;let l=!1;try{if(window.__doleDB&&window.__doleDB.getSecureCache){const v=await window.__doleDB.getSecureCache(t);v&&(r.arLogs=v.arLogs||[],r.dtrLogs=v.dtrLogs||[],r.docs=v.docs||[],ie(r,o),l=!0)}let n=[],d=[],p=[];const[i,c,u,b]=await Promise.all([F(`api/logs.php?type=ar&gip_id=${encodeURIComponent(s)}`),F(`api/logs.php?type=dtr&gip_id=${encodeURIComponent(s)}`),F(`api/logs.php?type=docs&gip_id=${encodeURIComponent(s)}`),F(`api/logs.php?type=absorption&gip_id=${encodeURIComponent(s)}`)]);n=i.success&&i.data?.success?i.data.logs:[],d=c.success&&c.data?.success?c.data.logs:[],p=u.success&&u.data?.success?u.data.logs:[];const m=b.success&&b.data?.success?b.data.logs:[];if(m.length>0){const v=m[0];r.absorbDate=v.absorption_datetime,r.absorb_where=v.where||v.absorb_where,r.absorb_position=v.position||v.absorb_position,r.absorb_agency=v.agency||v.absorb_agency}const x={arLogs:n,dtrLogs:d,docs:p};window.__doleDB&&window.__doleDB.setSecureCache&&await window.__doleDB.setSecureCache(t,x),r.arLogs=n,r.dtrLogs=d,r.docs=p,l?document.getElementById("beneficiary-drawer-container")&&ie(r,o):ie(r,o)}catch(n){console.error("Error fetching logs/docs:",n),l||(r.arLogs=[],r.dtrLogs=[],r.docs=[],ie(r,o))}},window.showAddDataModal=function(e){ce(e)},window.editBeneficiary=function(e){Ue(e)},window.showExportConfigModal=function(e){Ve(e)},window.showProfileModal=function(){He()}}async function He(){try{if(we()&&pe){let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=a.id)}catch{}if(!e)throw new Error("User not logged in");const{data:o,error:s}=await pe.from("users").select("*").eq("id",e).single();if(s)throw s;me(o)}else{let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=`?user_id=${a.id}`)}catch{}const s=await(await fetch(`${_()}api/profile.php${e}`)).json();if(s.success){const a=s.profile;me(a)}else f.fire({icon:"error",title:"Error",text:s.error||"Failed to load profile"})}}catch(e){console.error("Error fetching profile:",e)}}function me(e){const o=e.profile_picture_path?`${_()}${e.profile_picture_path}`:null,s=e.full_name?e.full_name.split(" ").map(r=>r[0]).join("").substring(0,2).toUpperCase():"US",a=`
            ...
            <form id="export-config-form" class="space-y-6">
                ...
                <div class="bg-gray-50/50 rounded-2xl p-4 border border-gray-100 mt-4">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                        ${["ID","Name","Office","Position","Start Date","End Date","Status"].map(a=>{const r=a.toLowerCase().replace(" ",""),t=o.columns.includes(r),l=`col-switch-${r}`;return`
    ...
    `;f.fire({html:l,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:n=>{const d=n.querySelector("#cancel-modal-btn");d&&d.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),f.close(),e?._isBulk&&de.onCancel()});const p=n.querySelector("#bulk-add-btn");p&&p.addEventListener("click",()=>{f.close(),de.init()});const i=n.querySelector("#birthday-input"),c=n.querySelector("#age-display");i&&c&&(i.addEventListener("change",g=>{g.target.value&&(c.value=ve(g.target.value))}),i.value&&!e?.age&&(c.value=ve(i.value)));const u=n.querySelector("#name-input-field"),b=n.querySelector("#duplicate-warning");if(u&&b){let g;u.addEventListener("input",h=>{const w=h.target.value.trim();if(clearTimeout(g),w.length<3){b.classList.add("hidden");return}g=setTimeout(async()=>{try{const B=await(await fetch(`${_()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:w})})).json();B.success&&B.exists?(b.classList.remove("hidden"),z()?b.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":b.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):b.classList.add("hidden")}catch(y){console.error("Duplicate check error:",y)}},500)}),e?.name&&(b.classList.add("hidden"),(async()=>{const w=await(await fetch(`${_()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();w.success&&w.exists&&(b.classList.remove("hidden"),z()?b.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":b.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const m=n.querySelector("#full-id-input"),x=n.querySelector("#series-no-input"),v=n.querySelector('input[name="startDate"]'),$=n.querySelector('input[name="endDate"]'),I=n.querySelectorAll('input[name="remarks"]'),M=n.querySelector("#extension-log-container"),P=async g=>{if(!o){if(m){m.classList.add("animate-pulse"),m.placeholder="Loading...";try{const h=await F(`api/beneficiaries.php?next_id&year=${encodeURIComponent(g)}`);h.success&&h.data?.success&&h.data?.nextId&&(m.value=h.data.nextId)}finally{m.classList.remove("animate-pulse")}}if(x){x.classList.add("animate-pulse"),x.placeholder="Loading...";try{const h=await F(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(g)}`);h.success&&h.data?.success&&h.data?.nextSeries&&(x.value=h.data.nextSeries)}finally{x.classList.remove("animate-pulse")}}}},N=n.querySelector("#replacement-search-input"),O=n.querySelector("#replacement-hidden"),D=n.querySelector("#replacement-suggestions"),U=g=>{const h=(g.name||"").toUpperCase().trim(),w=g.startDateFormatted||g.startDate||"N/A",y=g.endDateFormatted||g.endDate||"N/A";return`${h} (${w.toUpperCase()} - ${y.toUpperCase()})`},k=g=>{if(D){if(!g.length){D.innerHTML=`<div class="px-3 py-2 text-[10px] font-bold ${t.textCourseOpt}">No matching beneficiary found.</div>`,D.classList.remove("hidden");return}D.innerHTML=g.map(h=>{const w=U(h);return`
                        <button type="button" class="replacement-option w-full text-left px-3 py-2 text-[10px] font-bold ${t.textCourseOpt} ${t.courseHover} rounded-md cursor-pointer transition-colors"
                            data-replacement="${w.replace(/"/g,"&quot;")}">
                            ${w}
                        </button>
                    `}).join(""),D.classList.remove("hidden"),D.querySelectorAll(".replacement-option").forEach(h=>{h.addEventListener("click",()=>{const w=h.getAttribute("data-replacement")||"";N&&(N.value=w),O&&(O.value=w),D.classList.add("hidden")})})}};let S=null;const C=async(g="")=>{const h=(g||"").trim(),w=`api/beneficiaries.php?replacement_candidates=1&limit=20${h?`&q=${encodeURIComponent(h)}`:""}`,y=await F(w);y.success&&y.data?.success&&Array.isArray(y.data.candidates)&&k(y.data.candidates)};N&&O&&D&&(N.addEventListener("focus",()=>{C(N.value||"")}),N.addEventListener("input",()=>{O.value=N.value.trim(),clearTimeout(S),S=setTimeout(()=>{C(N.value||"")},250)}),document.addEventListener("click",g=>{!N.contains(g.target)&&!D.contains(g.target)&&D.classList.add("hidden")}));const L=()=>{const g=n.querySelector('input[name="remarks"]:checked');return g?g.value:"ONGOING"},E=g=>{const h=n.querySelector(`input[name="remarks"][value="${g}"]`);h&&(h.checked=!0,G())},j=()=>{if($&&$.value){const g=$.value.split("-"),h=new Date(g[0],g[1]-1,g[2]),w=new Date;w.setHours(0,0,0,0);let y="ONGOING";h<w&&(y="EXPIRED"),E(y)}},G=()=>{if(!M)return;if(L()==="ABSORBED"){const h=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});M.innerHTML=`
                    ...
                    `}else M.innerHTML=""};if(v&&(v.addEventListener("change",g=>{const h=g.target.value;if(h){const w=new Date(h).getFullYear();if(P(w),$&&!$.value){const y=new Date(h);y.setMonth(y.getMonth()+1),$.value=y.toISOString().split("T")[0]}j()}}),!o)){const g=v.value?new Date(v.value).getFullYear():new Date().getFullYear();P(g)}$&&$.addEventListener("change",j),I.forEach(g=>g.addEventListener("change",G));const H=n.querySelector("#resign-btn"),V=n.querySelector("#absorb-btn");H&&H.addEventListener("click",()=>E("RESIGNED")),V&&V.addEventListener("click",()=>E("ABSORBED")),n.querySelectorAll('input[type="text"], textarea').forEach(g=>{["id-number-input","full-id-input"].includes(g.id)||g.addEventListener("input",()=>{const h=g.selectionStart,w=g.selectionEnd;g.value=g.value.toUpperCase(),g.setSelectionRange(h,w)})}),j(),G(),q("education-input","course-suggestions","course-option"),q("designation-input","work-suggestions","work-option"),q("office-input","office-suggestions","office-option");function q(g,h,w){const y=n.querySelector(`#${g}`),B=n.querySelector(`#${h}`);if(!y||!B)return;let J=!1;y.addEventListener("focus",()=>B.classList.remove("hidden")),document.addEventListener("click",R=>{!y.contains(R.target)&&!B.contains(R.target)&&B.classList.add("hidden")}),y.addEventListener("input",()=>{if(J){J=!1;return}const R=y.value.toLowerCase(),Y=B.querySelectorAll(`.${w}`);let ae=!1;Y.forEach(K=>{const T=K.querySelector(".option-text");(T?T.innerText:K.innerText).toLowerCase().includes(R)?(K.style.display="block",ae=!0):K.style.display="none"}),ae?B.classList.remove("hidden"):B.classList.add("hidden")}),B.querySelectorAll(`.${w}`).forEach(R=>{R.addEventListener("click",()=>{const Y=R.querySelector(".option-text");y.value=Y?Y.innerText.trim():R.innerText.trim(),J=!0,B.classList.add("hidden"),y.dispatchEvent(new Event("change"))})})}const A=n.querySelector("#add-beneficiary-form"),W="add_beneficiary_draft";if(!o){const g=localStorage.getItem(W);if(g)try{const h=JSON.parse(g);Object.entries(h).forEach(([w,y])=>{const B=A.elements[w];B&&B.type!=="file"&&B.type!=="hidden"&&(B.value=y)})}catch(h){console.error("Error loading draft",h)}}A.addEventListener("input",g=>{if(!o){const h=new FormData(A),w={};h.forEach((y,B)=>w[B]=y),localStorage.setItem(W,JSON.stringify(w))}}),A&&A.addEventListener("submit",g=>{g.preventDefault(),A.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(Z=>{Z.classList.remove("ring-2","ring-red-500","!border-red-500")});const w=new FormData(A);let y=!1;const B=Z=>{const Q=A.querySelector(`[name="${Z}"]`);Q&&Q.classList.add("ring-2","ring-red-500","!border-red-500"),y=!0},J=w.get("name"),R=w.get("contact"),Y=w.get("startDate"),ae=w.get("endDate"),K=(w.get("designation")||"").trim();if((!J||J.trim()===""||/[0-9]/.test(J))&&B("name"),R&&R.trim()!==""&&/[^0-9]/.test(R.replace(/[\s\-\+\(\)]/g,""))&&B("contact"),Y||B("startDate"),ae||B("endDate"),y)return;const T={};w.forEach((Z,Q)=>{T[Q]=Z}),K||(T.designation="N/A"),T.replacement||(T.replacement="");const te=n.querySelector("#full-id-input")?.value;o?(T.id=e?.id,te&&(T.gip_id=te)):(T.id=null,te&&(T.gip_id=te)),window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(T)){if(!o){const Q="add_beneficiary_draft",Ce=A.querySelector('[name="office"]')?.value||"",Be=A.querySelector('[name="designation"]')?.value||"",Le=A.querySelector('[name="education"]')?.value||"";localStorage.setItem(Q,JSON.stringify({office:Ce,designation:Be,education:Le}))}f.close(),setTimeout(()=>{f.fire({toast:!0,position:"top-end",icon:"success",title:`Record ${o?"Updated":"Added"} Successfully`,showConfirmButton:!1,timer:3e3,timerProgressBar:!0}),!o&&e?._isBulk&&de.onSaveSuccess()},100)}else f.fire({icon:"error",title:"Save Failed",text:"There was an error saving the record to the database."})})()})}})}window.handleContactSubmit=async function(e){e.preventDefault();const o=e.target,s=o.querySelector('button[type="submit"]'),a=s.innerHTML;s.disabled=!0,s.innerHTML=`
        <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        Sending...
    `;try{const r=new FormData(o);if((await fetch(o.action,{method:"POST",body:r,headers:{Accept:"application/json"}})).ok)f.fire({toast:!0,position:"top-end",icon:"success",title:"Message Sent Successfully!",text:"Thank you for reaching out. I will get back to you soon!",showConfirmButton:!1,timer:4e3,timerProgressBar:!0,didOpen:l=>{l.addEventListener("mouseenter",f.stopTimer),l.addEventListener("mouseleave",f.resumeTimer)},customClass:{popup:"rounded-2xl border-l-4 border-teal-500 shadow-2xl"}}),o.reset();else throw new Error("Failed to send")}catch{f.fire({toast:!0,position:"top-end",icon:"error",title:"Oops! Something went wrong.",text:"Could not send your message. Please try again later.",showConfirmButton:!1,timer:4e3,customClass:{popup:"rounded-2xl border-l-4 border-red-500 shadow-2xl"}})}finally{s.disabled=!1,s.innerHTML=a}return!1};export{ye as _,F as a,oe as b,Qe as c,rt as d,je as e,tt as f,_ as g,We as h,we as i,ot as j,Ye as k,X as l,Ke as m,Ze as n,at as o,Pe as p,et as r,pe as s,ze as u};
