...
import p from"./vendor-swal-BSk0fVSb.js";const G="color-theme",fe=3600*24*365;function be(e,o,a){document.cookie=`${e}=${o}; max-age=${a}; path=/; SameSite=Lax`}function se(e){const o=document.cookie.match(new RegExp("(?:^|; )"+e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+"=([^;]*)"));return o?decodeURIComponent(o[1]):null}function ne(){const e=localStorage.getItem(G)||se(G);return e==="dark"||e==="light"?e:"light"}function J(e){const o=document.documentElement;e==="dark"?o.classList.add("dark"):o.classList.remove("dark"),localStorage.setItem(G,e),be(G,e,fe),xe(e),document.dispatchEvent(new CustomEvent("themeChanged",{detail:{theme:e}}))}function Q(){const e=ne();J(e==="dark"?"light":"dark")}function xe(e){const o=e==="dark",a=document.getElementById("pref-dark-mode");a&&(a.checked=o);const s=document.getElementById("theme-toggle-dark-icon"),r=document.getElementById("theme-toggle-light-icon");s&&r&&(s.classList.toggle("hidden",o),r.classList.toggle("hidden",!o));const t=document.getElementById("sidebar-theme-label");t&&(t.textContent=o?"Light Mode":"Dark Mode")}function Ie(){const e=ne();J(e);const o=document.getElementById("pref-dark-mode");o&&o.addEventListener("change",()=>{J(o.checked?"dark":"light")});const a=document.getElementById("theme-toggle-btn");a&&a.addEventListener("click",Q),document.querySelectorAll("[data-theme-toggle]").forEach(s=>{s.addEventListener("click",Q)}),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",s=>{localStorage.getItem(G)||se(G)||J(s.matches?"dark":"light")})}function T(){return document.documentElement.classList.contains("dark")}window.toggleTheme=Q;window.isDarkMode=T;const X={queue:[],currentIndex:0,isActive:!1,isAutoSave:!1,lastInteractionTime:0,init(){this.showUploadModal()},showUploadModal(){const e=T(),o={bgCard:e?"bg-slate-900/40":"bg-gray-50/40",borderCard:e?"border-slate-800":"border-gray-100",textHeading:e?"text-green-500":"text-[#2e7d32]",textSubtitle:e?"text-slate-500":"text-gray-400 dark:text-gray-300",bgUpload:e?"bg-slate-800":"bg-white",borderUpload:e?"border-slate-700":"border-gray-200",textUpload:e?"text-slate-400":"text-gray-500",hoverUpload:e?"hover:bg-slate-800/80 hover:border-blue-500":"hover:bg-blue-50/50 hover:border-royal-blue",iconText:e?"text-green-400":"text-[#2e7d32]",iconBg:e?"bg-green-900/20":"bg-[#e8f5e9]",iconBorder:e?"border-green-800/30":"border-[#c8e6c9]"},a=`
...
.text-right-select { text-align-last: right; }
    ...
    `;p.fire({html:i,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:n=>{const c=n.querySelector("#cancel-modal-btn");c&&c.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),p.close(),e?._isBulk&&X.onCancel()});const d=n.querySelector("#bulk-add-btn");d&&d.addEventListener("click",()=>{p.close(),X.init()});const l=n.querySelector("#birthday-input"),g=n.querySelector("#age-display");l&&g&&(l.addEventListener("change",u=>{u.target.value&&(g.value=ae(u.target.value))}),l.value&&!e?.age&&(g.value=ae(l.value)));const b=n.querySelector("#name-input-field"),h=n.querySelector("#duplicate-warning");if(b&&h){let u;b.addEventListener("input",f=>{const m=f.target.value.trim();if(clearTimeout(u),m.length<3){h.classList.add("hidden");return}u=setTimeout(async()=>{try{const y=await(await fetch(`${B()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:m})})).json();y.success&&y.exists?(h.classList.remove("hidden"),T()?h.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":h.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):h.classList.add("hidden")}catch(k){console.error("Duplicate check error:",k)}},500)}),e?.name&&(h.classList.add("hidden"),(async()=>{const m=await(await fetch(`${B()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();m.success&&m.exists&&(h.classList.remove("hidden"),T()?h.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":h.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const x=n.querySelector("#full-id-input"),$=n.querySelector("#series-no-input"),D=n.querySelector('input[name="startDate"]'),I=n.querySelector('input[name="endDate"]'),N=n.querySelectorAll('input[name="remarks"]'),A=n.querySelector("#extension-log-container"),H=u=>{o||(x&&(x.classList.add("animate-pulse"),x.placeholder="Loading...",fetch(`${B()}api/beneficiaries.php?next_id&year=${u}`).then(f=>f.json()).then(f=>{x.classList.remove("animate-pulse"),f.success&&f.nextId&&(x.value=f.nextId)})),$&&($.classList.add("animate-pulse"),$.placeholder="Loading...",fetch(`${B()}api/beneficiaries.php?next_series_no&year=${u}`).then(f=>f.json()).then(f=>{$.classList.remove("animate-pulse"),f.success&&f.nextSeries&&($.value=f.nextSeries)})))},K=()=>{const u=n.querySelector('input[name="remarks"]:checked');return u?u.value:"ONGOING"},P=u=>{const f=n.querySelector(`input[name="remarks"][value="${u}"]`);f&&(f.checked=!0,w())},R=()=>{if(I&&I.value){const u=I.value.split("-"),f=new Date(u[0],u[1]-1,u[2]),m=new Date;m.setHours(0,0,0,0);let k="ONGOING";f<m&&(k="EXPIRED"),P(k)}},w=()=>{if(!A)return;if(K()==="ABSORBED"){const f=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});A.innerHTML=`
