                        ...
                        <div class="font-montserrat text-left">
                            ...
                            <div class="grid grid-cols-3 gap-3">
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-doc-status" value="PENDING" class="peer sr-only" ${C==="PENDING"?"checked":""}>
                                ...
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-doc-status" value="COMPLETED" class="peer sr-only" ${C==="COMPLETED"?"checked":""}>
                                ...
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-doc-status" value="REJECTED" class="peer sr-only" ${C==="REJECTED"?"checked":""}>
...
.editable-indicator {
    ...
    `;let l=document.getElementById("edit-drawer-container");l&&l.remove(),l=document.createElement("div"),l.id="edit-drawer-container",l.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-white dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",l.setAttribute("tabindex","-1"),l.innerHTML=n,document.body.appendChild(l),setTimeout(()=>{const c=l.querySelector('textarea[name="name"]');c&&(c.style.height="auto",c.style.height=c.scrollHeight+"px")},10),(async()=>{try{const c=await ne("api/beneficiaries.php?get_offices=1");if(c.success&&c.data.offices){const i=l.querySelector("#edit-office-suggestions-box");if(i){const d=["DOLE Field Office","LGU","DEPED","DICT","PCA"],g=[...new Set([...d,...c.data.offices])];i.innerHTML=g.map(b=>`
                    ...
                    `).join("")}}}catch(c){console.error("Error fetching office suggestions:",c)}})(),Ce(async()=>{const{Drawer:c}=await import("./vendor-flowbite-B7rSfpuT.js").then(i=>i.b);return{Drawer:c}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:c})=>{const i=new c(l,{placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{l&&l.parentNode&&l.remove()},400)}});i.show(),window.initFlowbite&&window.initFlowbite();const d=()=>{document.activeElement&&typeof document.activeElement.blur=="function"&&document.activeElement.blur(),i.hide()};l.querySelector("#close-edit-drawer-btn").addEventListener("click",d),l.querySelector("#edit-drawer-cancel-btn").addEventListener("click",d);const g=l.querySelector("#edit-beneficiary-drawer-form"),b=l.querySelector("#edit-bday-input"),v=l.querySelector("#edit-age-display"),k=l.querySelector("#edit-startDate-input"),y=l.querySelector("#edit-endDate-input"),T=l.querySelector('input[name="seriesNo"]'),R=l.querySelector('input[name="gip_id"]'),Q=l.querySelector("#edit-drawer-remarks");Q&&Q.addEventListener("change",f=>{const E="text-[10px] sm:text-[11px] font-black px-2.5 py-2.5 rounded-lg border uppercase tracking-widest shadow-sm outline-none focus:ring-2 focus:ring-brand w-full cursor-pointer transition-colors duration-300";Q.className=`${u(f.target.value)} ${E} editable-indicator`});let Y=!1;const ee=(f,E)=>{f.addEventListener("input",w=>{const L=w.target.value,I=window.__maskDate(L);if(L!==I&&(w.target.value=I),I.length===10){const j=window.__parseFormattedDate(I);j&&E&&(E(j),document.activeElement===f&&f.blur())}}),f.addEventListener("changeDate",w=>{w.detail&&w.detail.date&&E&&(E(w.detail.date),f._datepicker&&f._datepicker.hide())})};b&&ee(b,f=>{v&&(!Y||!v.value)&&(v.value=window.calculateAge(f))}),k&&ee(k,f=>{if(y){const w=new Date(f);w.setMonth(w.getMonth()+3),w.setDate(w.getDate()+1);const L=String(w.getMonth()+1).padStart(2,"0"),I=String(w.getDate()).padStart(2,"0"),j=w.getFullYear();y.value=`${L}/${I}/${j}`}const E=f.getFullYear();E>1900&&R&&T&&Promise.all([z(`api/beneficiaries.php?next_id&year=${encodeURIComponent(E)}`),z(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(E)}`)]).then(([w,L])=>{const I=w.success&&w.data?.success?w.data.nextId:null,j=L.success&&L.data?.success?L.data.nextSeries:null,Z=String(R.value||"").match(/^ROX-RD-ESIG-(\d{4})-\d{4}$/),F=String(T.value||"").match(/^(\d{4})-\d{2}-\d{3}$/),_=Z?Number(Z[1]):null,te=F?Number(F[1]):null;I&&(_===null||_!==E)&&(R.value=I),j&&(te===null||te!==E)&&(T.value=j)}).catch(w=>{console.error("Edit drawer identifier sync error:",w)})}),y&&ee(y);const G=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null),K=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null);G&&b&&(b._datepicker=new G(b,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"}));const H=l.querySelector("#edit-date-range-picker");if(K&&H){const f=new K(H,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0,orientation:"bottom right"});k&&(k._datepicker=f.datepickers[0]),y&&(y._datepicker=f.datepickers[1])}e.id&&z(`api/beneficiaries.php?id=${encodeURIComponent(e.id)}`).then(f=>{if(f.success&&f.data&&f.data.beneficiary){const E=f.data.beneficiary;k&&E.startDateFormatted&&(k.value=E.startDateFormatted,k._datepicker&&k._datepicker.setDate(E.startDateFormatted)),y&&E.endDateFormatted&&(y.value=E.endDateFormatted,y._datepicker&&y._datepicker.setDate(E.endDateFormatted))}}).catch(f=>console.error("Error fetching accurate beneficiary dates:",f)),v&&v.addEventListener("input",()=>Y=!0);const D=(f,E,w)=>{const L=l.querySelector(f),I=l.querySelector(E);if(!L||!I)return;const j=()=>I.classList.add("hidden"),Z=()=>I.classList.remove("hidden");L.addEventListener("focus",Z),L.addEventListener("input",()=>{const F=L.value.toLowerCase().trim();let _=0;I.querySelectorAll(w).forEach(te=>{const m=(te.querySelector(".option-text")?.textContent||te.textContent||"").toLowerCase().includes(F);te.style.display=m?"block":"none",m&&_++}),_>0?Z():j()}),I.addEventListener("click",F=>{const _=F.target.closest(w);_&&(L.value=(_.querySelector(".option-text")?.textContent||_.textContent||"").trim(),j(),L.dispatchEvent(new Event("change")))}),document.addEventListener("click",F=>{!L.contains(F.target)&&!I.contains(F.target)&&j()})};D("#edit-education-input","#edit-education-suggestions-box",".edit-education-option"),D("#edit-designation-input","#edit-designation-suggestions-box",".edit-designation-option"),D("#edit-office-input","#edit-office-suggestions-box",".edit-office-option");const M=l.querySelector("#edit-replacement-input"),C=l.querySelector("#edit-replacement-suggestions-box"),B=l.querySelector("#edit-replacement-loading");let A=null;M&&C&&(M.addEventListener("input",f=>{const E=f.target.value.trim();clearTimeout(A),C.classList.add("hidden"),!(E.length<2)&&(B&&B.classList.remove("hidden"),A=setTimeout(async()=>{try{const w=await ne(`api/beneficiaries.php?replacement_candidates=1&q=${encodeURIComponent(E)}&limit=10`);w.success&&w.data&&w.data.candidates&&w.data.candidates.length>0?(C.innerHTML=w.data.candidates.map(L=>`
    ...
    `;x.fire({html:u,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:n=>{window.initFlowbite&&window.initFlowbite();const l=n.querySelector("#cancel-modal-btn");l&&l.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),x.close(),e?._isBulk&&ge.onCancel()});const c=n.querySelector("#bulk-add-btn");c&&c.addEventListener("click",()=>{x.close(),ge.init()});const i=(p,m)=>{p.addEventListener("input",h=>{const $=window.__maskDate(h.target.value);if(h.target.value!==$&&(h.target.value=$),$.length===10){const S=window.__parseFormattedDate($);S&&m&&(m(S),document.activeElement===p&&p.blur())}}),p.addEventListener("changeDate",h=>{if(h.detail&&h.detail.date&&m){m(h.detail.date);const $=p._datepicker||p.parentNode&&p.parentNode._datepicker;$&&typeof $.hide=="function"&&$.hide()}})},d=n.querySelector("#birthday-input"),g=n.querySelector("#age-display");if(d){i(d,m=>{g&&(g.value=window.calculateAge(m),g.classList.add("animate-pulse"),setTimeout(()=>g.classList.remove("animate-pulse"),400))});const p=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);p&&(d._datepicker=new p(d,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"}))}const b=n.querySelector("#name-input-field"),v=n.querySelector("#duplicate-warning");if(b&&v){let p;b.addEventListener("input",m=>{const h=m.target.value.trim();if(clearTimeout(p),h.length<3){v.classList.add("hidden");return}p=setTimeout(async()=>{try{const S=await(await fetch(`${X()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:h})})).json();S.success&&S.exists?(v.classList.remove("hidden"),oe()?v.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":v.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):v.classList.add("hidden")}catch($){console.error("Duplicate check error:",$)}},500)}),e?.name&&(v.classList.add("hidden"),(async()=>{const h=await(await fetch(`${X()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();h.success&&h.exists&&(v.classList.remove("hidden"),oe()?v.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":v.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const k=n.querySelector("#full-id-input"),y=n.querySelector("#series-no-input"),T=n.querySelector('input[name="startDate"]'),R=n.querySelector('input[name="endDate"]'),Q=n.querySelectorAll('input[name="remarks"]'),Y=n.querySelector("#extension-log-container"),ee=async p=>{if(!p)return;const m=`cache_next_id_${p}`,h=`cache_next_series_${p}`,$=`cache_expiry_${p}`,S=Date.now(),q=localStorage.getItem(m),P=localStorage.getItem(h),U=localStorage.getItem($);if(q&&P&&U&&S<parseInt(U,10)){k&&(k.value=q,k.classList.remove("placeholder:text-gray-300")),y&&(y.value=P);return}const V=[k,y].filter(Boolean);V.forEach(N=>{N.classList.add("animate-pulse"),N.placeholder="Syncing..."});try{const[N,O]=await Promise.all([z(`api/beneficiaries.php?next_id&year=${encodeURIComponent(p)}`),z(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(p)}`)]),W=N.success&&N.data?.success?N.data.nextId:null,J=O.success&&O.data?.success?O.data.nextSeries:null;W&&k&&(k.value=W,localStorage.setItem(m,W)),J&&y&&(y.value=J,localStorage.setItem(h,J)),localStorage.setItem($,(S+3e5).toString())}catch(N){console.error("ID Sync error:",N)}finally{V.forEach(N=>N.classList.remove("animate-pulse"))}},G=n.querySelector("#replacement-search-input"),K=n.querySelector("#replacement-hidden"),H=n.querySelector("#replacement-suggestions"),D=p=>{const m=(p.name||"").toUpperCase().trim(),h=p.startDateFormatted||p.startDate||"N/A",$=p.endDateFormatted||p.endDate||"N/A";return`${m} (${h.toUpperCase()} - ${$.toUpperCase()})`},M=p=>{if(H){if(!p.length){H.innerHTML=`<div class="px-3 py-2 text-[10px] font-bold ${t.textCourseOpt}">No matching beneficiary found.</div>`,H.classList.remove("hidden");return}H.innerHTML=p.map(m=>{const h=D(m);return`
                    ...
                    `}).join(""),H.classList.remove("hidden"),H.querySelectorAll(".replacement-option").forEach(m=>{m.addEventListener("click",()=>{const h=m.getAttribute("data-replacement")||"";G&&(G.value=h),K&&(K.value=h),H.classList.add("hidden")})})}};let C=null;const B=async(p="")=>{const m=(p||"").trim(),h=`api/beneficiaries.php?replacement_candidates=1&limit=20${m?`&q=${encodeURIComponent(m)}`:""}`,$=await z(h);$.success&&$.data?.success&&Array.isArray($.data.candidates)&&M($.data.candidates)};G&&K&&H&&(G.addEventListener("focus",()=>{B(G.value||"")}),G.addEventListener("input",()=>{K.value=G.value.trim(),clearTimeout(C),C=setTimeout(()=>{B(G.value||"")},250)}),document.addEventListener("click",p=>{!G.contains(p.target)&&!H.contains(p.target)&&H.classList.add("hidden")}));const A=()=>{const p=n.querySelector('input[name="remarks"]:checked');return p?p.value:"ONGOING"},f=p=>{const m=n.querySelector(`input[name="remarks"][value="${p}"]`);m&&(m.checked=!0,w())},E=()=>{if(R&&R.value){const p=window.__parseFormattedDate(R.value);if(!p)return;const m=new Date;m.setHours(0,0,0,0);let h="ONGOING";p<m&&(h="EXPIRED"),f(h)}},w=()=>{if(!Y)return;if(A()==="ABSORBED"){const m=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});Y.innerHTML=`
                    ...
                    `}else Y.innerHTML=""};if(T){let p=null;i(T,S=>{const q=S.getFullYear();if(R){const P=new Date(S);P.setMonth(P.getMonth()+3),P.setDate(P.getDate()+1);const U=String(P.getMonth()+1).padStart(2,"0"),V=String(P.getDate()).padStart(2,"0"),N=P.getFullYear();R.value=`${U}/${V}/${N}`}E(),q>1900&&q!==p&&(p=q,ee(q))}),R&&i(R,()=>E());const m=n.querySelector("#date-range-picker"),h=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null),$=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);if(h&&m&&T&&R){const S=new h(m,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0,orientation:"bottom right"});T._datepicker=S.datepickers?.[0]||null,R._datepicker=S.datepickers?.[1]||null}else $&&(T&&(T._datepicker=new $(T,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"})),R&&(R._datepicker=new $(R,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"})));if(!o){const S=new Date().getFullYear();ee(S)}}Q.forEach(p=>p.addEventListener("change",w));const L=n.querySelector("#resign-btn"),I=n.querySelector("#absorb-btn");L&&L.addEventListener("click",()=>f("RESIGNED")),I&&I.addEventListener("click",()=>f("ABSORBED")),n.querySelectorAll('input[type="text"], textarea').forEach(p=>{["id-number-input","full-id-input"].includes(p.id)||p.addEventListener("input",()=>{const m=p.selectionStart,h=p.selectionEnd;p.value=p.value.toUpperCase(),p.setSelectionRange(m,h)})}),E(),w(),F("education-input","course-suggestions","course-option"),F("designation-input","work-suggestions","work-option"),F("office-input","office-suggestions","office-option");const j={publicSector:["DOLE","LGU","DEPED"],privateSector:["DICT","PCA"]},Z=(p=[])=>{const m=n.querySelector("#office-suggestions");if(!m)return;const h=N=>String(N||"").trim().toUpperCase(),$=new Set(j.publicSector.map(h)),S=new Set(j.privateSector.map(h)),q=new Set([...$,...S]),U=[...new Set((p||[]).map(h).filter(Boolean))].filter(N=>!q.has(N)),V=(N,O)=>O.length?`
                        ...
                        `).join("")}
                    `:"";m.innerHTML=[V("Public Sector",[...$]),V("Private Sector",[...S]),V("Other Offices",U)].join("")};Z(),(async()=>{try{const p=await z("api/beneficiaries.php?get_offices=1");p.success&&p.data?.success&&Array.isArray(p.data.offices)&&Z(p.data.offices)}catch(p){console.error("Office suggestion sync failed:",p)}})();function F(p,m,h){const $=n.querySelector(`#${p}`),S=n.querySelector(`#${m}`);if(!$||!S)return;let q=!1;$.addEventListener("focus",()=>S.classList.remove("hidden")),document.addEventListener("click",P=>{!$.contains(P.target)&&!S.contains(P.target)&&S.classList.add("hidden")}),$.addEventListener("input",()=>{if(q){q=!1;return}const P=$.value.toLowerCase(),U=S.querySelectorAll(`.${h}`);let V=!1;U.forEach(N=>{const O=N.querySelector(".option-text");(O?O.innerText:N.innerText).toLowerCase().includes(P)?(N.style.display="block",V=!0):N.style.display="none"}),V?S.classList.remove("hidden"):S.classList.add("hidden")}),S.addEventListener("click",P=>{const U=P.target.closest(`.${h}`);if(!U)return;const V=U.querySelector(".option-text");$.value=V?V.innerText.trim():U.innerText.trim(),q=!0,S.classList.add("hidden"),$.dispatchEvent(new Event("change"))})}const _=n.querySelector("#add-beneficiary-form"),te="add_beneficiary_draft";if(!o){const p=localStorage.getItem(te);if(p)try{const m=JSON.parse(p);Object.entries(m).forEach(([h,$])=>{const S=_.elements[h];S&&S.type!=="file"&&S.type!=="hidden"&&(S.value=$)})}catch(m){console.error("Error loading draft",m)}}_.addEventListener("input",p=>{if(!o){const m=new FormData(_),h={};m.forEach(($,S)=>h[S]=$),localStorage.setItem(te,JSON.stringify(h))}}),_&&_.addEventListener("submit",p=>{p.preventDefault(),_.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(J=>{J.classList.remove("ring-2","ring-red-500","!border-red-500")});const h=new FormData(_);let $=!1;const S=J=>{const re=_.querySelector(`[name="${J}"]`);re&&re.classList.add("ring-2","ring-red-500","!border-red-500"),$=!0},q=h.get("name"),P=h.get("contact"),U=h.get("startDate"),V=h.get("endDate"),N=(h.get("designation")||"").trim();if((!q||q.trim()===""||/[0-9]/.test(q))&&S("name"),P&&P.trim()!==""&&/[^0-9]/.test(P.replace(/[\s\-\+\(\)]/g,""))&&S("contact"),U||S("startDate"),V||S("endDate"),$)return;const O={};h.forEach((J,re)=>{if(["birthday","startDate","endDate"].includes(re)){const se=window.__parseFormattedDate(J);if(se){const be=se.getFullYear(),he=String(se.getMonth()+1).padStart(2,"0"),Me=String(se.getDate()).padStart(2,"0");O[re]=`${be}-${he}-${Me}`}else O[re]=J}else O[re]=J}),N||(O.designation="N/A"),O.replacement||(O.replacement="");const W=n.querySelector("#full-id-input")?.value;o?(O.id=e?.id,W&&(O.gip_id=W)):(O.id=null,W&&(O.gip_id=W)),window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(O)){if(!o){const re="add_beneficiary_draft",se=_.querySelector('[name="office"]')?.value||"",be=_.querySelector('[name="designation"]')?.value||"",he=_.querySelector('[name="education"]')?.value||"";localStorage.setItem(re,JSON.stringify({office:se,designation:be,education:he}))}x.close(),setTimeout(()=>{x.fire({toast:!0,position:"top-end",icon:"success",title:`Record ${o?"Updated":"Added"} Successfully`,showConfirmButton:!1,timer:3e3,timerProgressBar:!0}),!o&&e?._isBulk&&ge.onSaveSuccess()},100)}else x.fire({icon:"error",title:"Save Failed",text:"There was an error saving the record to the database."})})()})}})}window.handleContactSubmit=async function(e){e.preventDefault();const o=e.target,s=o.querySelector('button[type="submit"]'),a=s.innerHTML;s.disabled=!0,s.innerHTML=`
