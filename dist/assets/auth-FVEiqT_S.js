...
import p from"./vendor-swal-BSk0fVSb.js";const G="color-theme",fe=3600*24*365;function be(e,o,a){document.cookie=`${e}=${o}; max-age=${a}; path=/; SameSite=Lax`}function se(e){const o=document.cookie.match(new RegExp("(?:^|; )"+e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+"=([^;]*)"));return o?decodeURIComponent(o[1]):null}function ne(){const e=localStorage.getItem(G)||se(G);return e==="dark"||e==="light"?e:"light"}function X(e){const o=document.documentElement;e==="dark"?o.classList.add("dark"):o.classList.remove("dark"),localStorage.setItem(G,e),be(G,e,fe),xe(e),document.dispatchEvent(new CustomEvent("themeChanged",{detail:{theme:e}}))}function Z(){const e=ne();X(e==="dark"?"light":"dark")}function xe(e){const o=e==="dark",a=document.getElementById("pref-dark-mode");a&&(a.checked=o);const s=document.getElementById("theme-toggle-dark-icon"),r=document.getElementById("theme-toggle-light-icon");s&&r&&(s.classList.toggle("hidden",o),r.classList.toggle("hidden",!o));const t=document.getElementById("sidebar-theme-label");t&&(t.textContent=o?"Light Mode":"Dark Mode")}function Ie(){const e=ne();X(e);const o=document.getElementById("pref-dark-mode");o&&o.addEventListener("change",()=>{X(o.checked?"dark":"light")});const a=document.getElementById("theme-toggle-btn");a&&a.addEventListener("click",Z),document.querySelectorAll("[data-theme-toggle]").forEach(s=>{s.addEventListener("click",Z)}),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",s=>{localStorage.getItem(G)||se(G)||X(s.matches?"dark":"light")})}function T(){return document.documentElement.classList.contains("dark")}window.toggleTheme=Z;window.isDarkMode=T;const Y={queue:[],currentIndex:0,isActive:!1,isAutoSave:!1,lastInteractionTime:0,init(){this.showUploadModal()},showUploadModal(){const e=T(),o={bgCard:e?"bg-slate-900/40":"bg-gray-50/40",borderCard:e?"border-slate-800":"border-gray-100",textHeading:e?"text-green-500":"text-[#2e7d32]",textSubtitle:e?"text-slate-500":"text-gray-400 dark:text-gray-300",bgUpload:e?"bg-slate-800":"bg-white",borderUpload:e?"border-slate-700":"border-gray-200",textUpload:e?"text-slate-400":"text-gray-500",hoverUpload:e?"hover:bg-slate-800/80 hover:border-blue-500":"hover:bg-blue-50/50 hover:border-royal-blue",iconText:e?"text-green-400":"text-[#2e7d32]",iconBg:e?"bg-green-900/20":"bg-[#e8f5e9]",iconBorder:e?"border-green-800/30":"border-[#c8e6c9]"},a=`
...
<div class="border-b border-default pb-4 mb-5 flex flex-col relative w-full pt-4 font-montserrat user-select-none">
    ...
    <div class="grid grid-cols-2 gap-4 mt-6 w-full">
        ...
        <div class="flex flex-col gap-1 text-left overflow-hidden">
            ...
            <div class="flex items-center flex-wrap gap-1.5 min-h-[30px]">
                ...
                <span class="${re(e.office)} hidden sm:inline-block text-[9px] font-black px-2.5 py-1 rounded-full border shadow-sm truncate max-w-[120px] lg:max-w-none" title="${e.office}">${e.office}</span>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col gap-1 text-left mt-4 sm:hidden">
        ...
        <span class="${re(e.office)} text-[10px] font-black px-2.5 py-2 rounded border border-gray-200 dark:border-slate-700 uppercase tracking-widest shadow-sm border-l-4 border-l-indigo-500 w-full truncate" title="${e.office}">${e.office}</span>
    </div>
</div>
...
<div class="flex flex-col gap-4 text-sm mt-3 px-1">
    ...
    <div class="flex justify-between items-center group">
        ...
        <span class="${e.age||te(e.birthday)?"font-black text-heading":"font-bold text-gray-300 italic"} text-right">${e.age||te(e.birthday)||"N/A"}</span>
    </div>
    <div class="flex justify-between items-center group">
            ...
            ${n.map(l=>{const g=l.status.toUpperCase(),h={VERIFIED:"COMPLETED",DECLINED:"REJECTED",PENDING:"PENDING"}[g]||g,b=h==="COMPLETED",$=h==="REJECTED",D=b?"text-green-500":$?"text-red-500":"text-gray-400 dark:text-gray-500",I=b?"bg-green-50/50 dark:bg-green-900/10":$?"bg-red-50/50 dark:bg-red-900/10":"bg-gray-50/50 dark:bg-slate-800/50",N=b?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800 hover:bg-green-200 cursor-pointer":$?"bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800 hover:bg-red-200 cursor-pointer":"bg-white text-gray-500 border-gray-200 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer";let A='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';return b?A='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>':$&&(A='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>'),`
                <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-0.5 ${I}">
                    <div class="flex items-center gap-3 w-full">
                        ...
                        <span class="text-xs sm:text-sm font-black ${b?"text-heading":"text-gray-500 dark:text-gray-400"} uppercase tracking-tight flex-1">${l.name}</span>
                    </div>
...
.text-right-select { text-align-last: right; }
    ...
    `;let i=document.getElementById("edit-drawer-container");i&&i.remove(),i=document.createElement("div"),i.id="edit-drawer-container",i.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-white dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",i.setAttribute("tabindex","-1"),i.innerHTML=t,document.body.appendChild(i),setTimeout(()=>{const n=i.querySelector('textarea[name="name"]');n&&(n.style.height="auto",n.style.height=n.scrollHeight+"px")},10),(async()=>{try{const n=await le("api/beneficiaries.php?get_offices=1");if(n.success&&n.data.offices){const c=i.querySelector("#edit-office-suggestions");if(c){const d=["DOLE Field Office","LGU","DEPED","DICT","PCA"],l=[...new Set([...d,...n.data.offices])];c.innerHTML=l.map(g=>`<option value="${g}">`).join("")}}}catch(n){console.error("Error fetching office suggestions:",n)}})(),ie(async()=>{const{Drawer:n}=await import("./vendor-flowbite-B2TFTxm5.js");return{Drawer:n}},__vite__mapDeps([0,1])).then(({Drawer:n})=>{const c=new n(i,{placement:"right",backdrop:!0,bodyScrolling:!1,backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{i.parentNode&&i.remove()},300)}});c.show(),i.querySelector("#close-edit-drawer-btn").addEventListener("click",()=>c.hide()),i.querySelector("#edit-drawer-cancel-btn").addEventListener("click",()=>c.hide());const d=i.querySelector("#edit-bday-input"),l=i.querySelector("#edit-age-display");d&&l&&d.addEventListener("change",f=>{f.target.value&&(l.value=r(f.target.value))});const g=i.querySelector("#edit-beneficiary-drawer-form");g.addEventListener("submit",f=>{f.preventDefault();const h=new FormData(g),b={};h.forEach(($,D)=>b[D]=$),b.id=e.id,b.gip_id=e.id,window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(b,!0,!1)){c.hide();const D=document.querySelector("#close-drawer-btn");D&&D.click(),p.fire({toast:!0,position:"top-end",icon:"success",title:"Record Updated",showConfirmButton:!1,timer:3e3})}})()})})}function oe(e="Incorrect Username or Password"){p.fire({html:`
            <div class="p-6">
        ...
        `,timer:e?800:3e3,timerProgressBar:!0,showConfirmButton:!1,showCloseButton:!1,width:"350px",padding:"0",customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl overflow-hidden border border-gray-100",timerProgressBar:"bg-green-500 h-1.5"},backdrop:"rgba(0, 0, 0, 0.4)"})}function Oe(){window.viewBeneficiary=async function(e,o=0){const a=`logs_cache_${e.id}`;let s=!1;try{if(window.__doleDB&&window.__doleDB.getSecureCache){const b=await window.__doleDB.getSecureCache(a);b&&(e.arLogs=b.arLogs||[],e.dtrLogs=b.dtrLogs||[],e.docs=b.docs||[],W(e,o),s=!0)}const[r,t,i]=await Promise.all([fetch(`${B()}api/logs.php?type=ar&gip_id=${encodeURIComponent(e.id)}`),fetch(`${B()}api/logs.php?type=dtr&gip_id=${encodeURIComponent(e.id)}`),fetch(`${B()}api/logs.php?type=docs&gip_id=${encodeURIComponent(e.id)}`)]),n=await r.json(),c=await t.json(),d=await i.json(),l=n.success?n.logs:[],g=c.success?c.logs:[],f=d.success?d.logs:[],h={arLogs:l,dtrLogs:g,docs:f};window.__doleDB&&window.__doleDB.setSecureCache&&await window.__doleDB.setSecureCache(a,h),e.arLogs=l,e.dtrLogs=g,e.docs=f,s?document.getElementById("beneficiary-drawer-container")&&W(e,o):W(e,o)}catch(r){console.error("Error fetching logs/docs:",r),s||(e.arLogs=[],e.dtrLogs=[],e.docs=[],W(e,o))}},window.showAddDataModal=function(e){K(e)},window.editBeneficiary=function(e){ke(e)},window.showExportConfigModal=function(e){Be(e)},window.showProfileModal=function(){Ee()}}async function Ee(){try{let e="";try{const s=JSON.parse(localStorage.getItem("user"));s&&s.id&&(e=`?user_id=${s.id}`)}catch{}const a=await(await fetch(`${B()}api/profile.php${e}`)).json();if(a.success){const s=a.profile;Se(s)}else p.fire({icon:"error",title:"Error",text:a.error||"Failed to load profile"})}catch(e){console.error("Error fetching profile:",e)}}function Se(e){const o=e.profile_picture_path?`${B()}${e.profile_picture_path}`:null,a=e.full_name?e.full_name.split(" ").map(r=>r[0]).join("").substring(0,2).toUpperCase():"US",s=`
            ...
            <form id="export-config-form" class="space-y-6">
                ...
                <div class="bg-gray-50/50 rounded-2xl p-4 border border-gray-100 mt-4">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                        ${["ID","Name","Office","Position","Start Date","End Date","Status"].map(s=>{const r=s.toLowerCase().replace(" ",""),t=o.columns.includes(r),i=`col-switch-${r}`;return`
    ...
    `;p.fire({html:i,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:n=>{const c=n.querySelector("#cancel-modal-btn");c&&c.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),p.close(),e?._isBulk&&Y.onCancel()});const d=n.querySelector("#bulk-add-btn");d&&d.addEventListener("click",()=>{p.close(),Y.init()});const l=n.querySelector("#birthday-input"),g=n.querySelector("#age-display");l&&g&&(l.addEventListener("change",u=>{u.target.value&&(g.value=ae(u.target.value))}),l.value&&!e?.age&&(g.value=ae(l.value)));const f=n.querySelector("#name-input-field"),h=n.querySelector("#duplicate-warning");if(f&&h){let u;f.addEventListener("input",x=>{const m=x.target.value.trim();if(clearTimeout(u),m.length<3){h.classList.add("hidden");return}u=setTimeout(async()=>{try{const y=await(await fetch(`${B()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:m})})).json();y.success&&y.exists?(h.classList.remove("hidden"),T()?h.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":h.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):h.classList.add("hidden")}catch(k){console.error("Duplicate check error:",k)}},500)}),e?.name&&(h.classList.add("hidden"),(async()=>{const m=await(await fetch(`${B()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();m.success&&m.exists&&(h.classList.remove("hidden"),T()?h.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":h.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const b=n.querySelector("#full-id-input"),$=n.querySelector("#series-no-input"),D=n.querySelector('input[name="startDate"]'),I=n.querySelector('input[name="endDate"]'),N=n.querySelectorAll('input[name="remarks"]'),A=n.querySelector("#extension-log-container"),H=u=>{o||(b&&(b.classList.add("animate-pulse"),b.placeholder="Loading...",fetch(`${B()}api/beneficiaries.php?next_id&year=${u}`).then(x=>x.json()).then(x=>{b.classList.remove("animate-pulse"),x.success&&x.nextId&&(b.value=x.nextId)})),$&&($.classList.add("animate-pulse"),$.placeholder="Loading...",fetch(`${B()}api/beneficiaries.php?next_series_no&year=${u}`).then(x=>x.json()).then(x=>{$.classList.remove("animate-pulse"),x.success&&x.nextSeries&&($.value=x.nextSeries)})))},Q=()=>{const u=n.querySelector('input[name="remarks"]:checked');return u?u.value:"ONGOING"},P=u=>{const x=n.querySelector(`input[name="remarks"][value="${u}"]`);x&&(x.checked=!0,w())},R=()=>{if(I&&I.value){const u=I.value.split("-"),x=new Date(u[0],u[1]-1,u[2]),m=new Date;m.setHours(0,0,0,0);let k="ONGOING";x<m&&(k="EXPIRED"),P(k)}},w=()=>{if(!A)return;if(Q()==="ABSORBED"){const x=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});A.innerHTML=`
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                                <div class="${r?"bg-green-900/10 border-green-900/30":"bg-[#e8f5e9]/50 border-[#c8e6c9]"} rounded-lg p-2.5 flex items-center justify-between border">
                                    <div>
                                        ...
                                        <p class="text-[10px] font-black ${r?"text-green-400":"text-[#1b5e20]"} uppercase">${x}</p>
                                        <input type="hidden" name="absorbDate" value="${x}">
                                    </div>
                    ...
                    `}else A.innerHTML=""};if(D&&(D.addEventListener("change",u=>{const x=u.target.value;if(x){const m=new Date(x).getFullYear();if(H(m),I&&!I.value){const k=new Date(x);k.setMonth(k.getMonth()+1),I.value=k.toISOString().split("T")[0]}R()}}),!o)){const u=D.value?new Date(D.value).getFullYear():new Date().getFullYear();H(u)}I&&I.addEventListener("change",R),N.forEach(u=>u.addEventListener("change",w));const S=n.querySelector("#resign-btn"),E=n.querySelector("#absorb-btn");S&&S.addEventListener("click",()=>P("RESIGNED")),E&&E.addEventListener("click",()=>P("ABSORBED")),n.querySelectorAll('input[type="text"], textarea').forEach(u=>{["id-number-input","full-id-input"].includes(u.id)||u.addEventListener("input",()=>{const x=u.selectionStart,m=u.selectionEnd;u.value=u.value.toUpperCase(),u.setSelectionRange(x,m)})}),R(),w(),C("education-input","course-suggestions","course-option"),C("designation-input","work-suggestions","work-option"),C("office-input","office-suggestions","office-option");function C(u,x,m){const k=n.querySelector(`#${u}`),y=n.querySelector(`#${x}`);!k||!y||(k.addEventListener("focus",()=>y.classList.remove("hidden")),document.addEventListener("click",M=>{!k.contains(M.target)&&!y.contains(M.target)&&y.classList.add("hidden")}),k.addEventListener("input",()=>{const M=k.value.toLowerCase(),j=y.querySelectorAll(`.${m}`);let z=!1;j.forEach(_=>{const U=_.querySelector(".option-text");(U?U.innerText:_.innerText).toLowerCase().includes(M)?(_.style.display="block",z=!0):_.style.display="none"}),z?y.classList.remove("hidden"):y.classList.add("hidden")}),y.querySelectorAll(`.${m}`).forEach(M=>{M.addEventListener("click",()=>{const j=M.querySelector(".option-text");k.value=j?j.innerText.trim():M.innerText.trim(),y.classList.add("hidden"),k.dispatchEvent(new Event("change")),k.dispatchEvent(new Event("input"))})}))}const v=n.querySelector("#add-beneficiary-form"),L="add_beneficiary_draft";if(!o){const u=localStorage.getItem(L);if(u)try{const x=JSON.parse(u);Object.entries(x).forEach(([m,k])=>{const y=v.elements[m];y&&y.type!=="file"&&y.type!=="hidden"&&(y.value=k)})}catch(x){console.error("Error loading draft",x)}}v.addEventListener("input",u=>{if(!o){const x=new FormData(v),m={};x.forEach((k,y)=>m[y]=k),localStorage.setItem(L,JSON.stringify(m))}}),v&&v.addEventListener("submit",u=>{u.preventDefault(),v.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(q=>{q.classList.remove("ring-2","ring-red-500","!border-red-500")});const m=new FormData(v);let k=!1;const y=q=>{const F=v.querySelector(`[name="${q}"]`);F&&F.classList.add("ring-2","ring-red-500","!border-red-500"),k=!0},M=m.get("name"),j=m.get("contact"),z=m.get("startDate"),_=m.get("endDate"),U=m.get("designation");if((!M||M.trim()===""||/[0-9]/.test(M))&&y("name"),j&&j.trim()!==""&&/[^0-9]/.test(j.replace(/[\s\-\+\(\)]/g,""))&&y("contact"),z||y("startDate"),_||y("endDate"),(!U||U.trim()==="")&&y("designation"),k)return;const O={};m.forEach((q,F)=>{O[F]=q});const V=n.querySelector("#full-id-input")?.value;o?(O.id=e?.id,V&&(O.gip_id=V)):(O.id=null,V&&(O.gip_id=V)),window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(O)){if(!o){const F="add_beneficiary_draft",ue=v.querySelector('[name="office"]')?.value||"",pe=v.querySelector('[name="designation"]')?.value||"",ge=v.querySelector('[name="education"]')?.value||"";localStorage.setItem(F,JSON.stringify({office:ue,designation:pe,education:ge}))}p.close(),setTimeout(()=>{p.fire({toast:!0,position:"top-end",icon:"success",title:`Record ${o?"Updated":"Added"} Successfully`,showConfirmButton:!1,timer:3e3,timerProgressBar:!0}),!o&&e?._isBulk&&Y.onSaveSuccess()},100)}else p.fire({icon:"error",title:"Save Failed",text:"There was an error saving the record to the database."})})()})}})}window.handleContactSubmit=async function(e){e.preventDefault();const o=e.target,a=o.querySelector('button[type="submit"]'),s=a.innerHTML;a.disabled=!0,a.innerHTML=`
        <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        Sending...
    `;try{const r=new FormData(o);if((await fetch(o.action,{method:"POST",body:r,headers:{Accept:"application/json"}})).ok)p.fire({toast:!0,position:"top-end",icon:"success",title:"Message Sent Successfully!",text:"Thank you for reaching out. I will get back to you soon!",showConfirmButton:!1,timer:4e3,timerProgressBar:!0,didOpen:i=>{i.addEventListener("mouseenter",p.stopTimer),i.addEventListener("mouseleave",p.resumeTimer)},customClass:{popup:"rounded-2xl border-l-4 border-teal-500 shadow-2xl"}}),o.reset();else throw new Error("Failed to send")}catch{p.fire({toast:!0,position:"top-end",icon:"error",title:"Oops! Something went wrong.",text:"Could not send your message. Please try again later.",showConfirmButton:!1,timer:4e3,customClass:{popup:"rounded-2xl border-l-4 border-red-500 shadow-2xl"}})}finally{a.disabled=!1,a.innerHTML=s}return!1};function B(){const e=window.location.pathname,o="/dole-system/",a=e.toLowerCase().indexOf(o.toLowerCase());return a!==-1?e.substring(0,a+o.length):"/"}function Te(){const e=localStorage.getItem("hasVisitedBefore"),o=document.getElementById("page-loader");e&&(document.documentElement.classList.add("loaded"),o&&(o.style.display="none")),window.addEventListener("load",()=>{const a=document.querySelector("body > *:not(.page-loader)");a&&a.classList.add("content-fade-in"),setTimeout(()=>{document.documentElement.classList.add("loaded"),o&&o.classList.add("hidden"),document.body.style.overflow="auto",localStorage.setItem("hasVisitedBefore","true")},300)})}const J={_key:"DOLE-GIP-SECURE-KEY-2026",encrypt:function(e){try{const o=r=>r.split("").map(t=>t.charCodeAt(0)),a=r=>("0"+Number(r).toString(16)).substr(-2),s=r=>o(this._key).reduce((t,i)=>t^i,r);return e.split("").map(o).map(s).map(a).join("")}catch(o){return console.error("Encryption Failed",o),null}},decrypt:function(e){try{const o=s=>s.split("").map(r=>r.charCodeAt(0)),a=s=>o(this._key).reduce((r,t)=>r^t,s);return e.match(/.{1,2}/g).map(s=>parseInt(s,16)).map(a).map(s=>String.fromCharCode(s)).join("")}catch(o){return console.error("Decryption Failed",o),null}}};function Pe(){document.querySelectorAll(".login-form-shared").forEach(o=>{const a=o.querySelector('input[name="username"]'),s=o.querySelector('input[name="password"]'),r=o.querySelector('input[name="rememberMe"]');if(a&&s&&r){const t=localStorage.getItem("secure_user"),i=localStorage.getItem("secure_pass");if(t&&i){const n=J.decrypt(t),c=J.decrypt(i);n&&c&&(a.value=n,s.value=c,r.checked=!0)}}o.addEventListener("submit",async t=>{t.preventDefault();try{const n=await(await fetch(`${B()}api/auth.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:a.value,password:s.value})})).json();if(n.success){r.checked?(localStorage.setItem("secure_user",J.encrypt(a.value)),localStorage.setItem("secure_pass",J.encrypt(s.value))):(localStorage.removeItem("secure_user"),localStorage.removeItem("secure_pass"));const c=localStorage.getItem("hasLoggedInBefore")==="true";localStorage.setItem("isLoggedIn","true"),localStorage.setItem("hasLoggedInBefore","true"),localStorage.setItem("user",JSON.stringify(n.user));const d=document.getElementById("drawer-login");if(d){d.classList.add("translate-y-full");const l=d.querySelector("[data-drawer-hide]");l&&l.click()}await $e(c),De(c)}else{const c=document.getElementById("drawer-login");c?(c.classList.add("translate-y-full"),setTimeout(()=>{oe(),setTimeout(()=>{c.classList.remove("translate-y-full"),s.value="",s.focus()},600)},400)):(oe(),s.value="",s.focus())}}catch(i){console.error("Login Error:",i),p.fire({icon:"error",title:"Login Error",text:"Unable to reach the server. Please check your connection."})}})})}function De(e=!1){const o=document.getElementById("left-panel"),a=document.getElementById("right-panel"),s=document.getElementById("left-panel-content"),r=document.getElementById("right-panel-content");s&&(s.style.opacity="0"),r&&(r.style.opacity="0");const t=document.createElement("div");t.className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none";const i=e?"":"animate__delay-1s",n=e?"animation-duration: 0.8s;":"animation-duration: 2s;";t.innerHTML=`
        <img src="${B()}frontend/images/logo/doleiligan.png" class="w-64 h-64 md:w-96 md:h-96 object-contain bg-white/20 backdrop-blur-sm shadow-2xl rounded-full p-4 animate__animated animate__rotateOut ${i}" style="${n}" alt="DOLE Logo">
    `,document.body.appendChild(t);const c=e?0:1e3,d=e?600:1500;setTimeout(()=>{o&&o.classList.add("animate-slide-left"),a&&a.classList.add("animate-slide-right"),setTimeout(()=>{window.location.href=`${B()}frontend/dashboard/`},d)},c)}function Re(){const e=document.getElementById("logoutBtn");e&&e.addEventListener("click",()=>{localStorage.removeItem("isLoggedIn"),window.location.href=`${B()}`})}function _e(){const e=document.getElementById("mobile-splash"),o=document.getElementById("show-login-btn"),a=document.getElementById("back-to-splash"),s=document.getElementById("mobile-bg-content"),r=document.getElementById("mobile-welcome-text"),t=document.getElementById("reopen-login-drawer"),i=document.getElementById("request-notifications-btn"),n=async()=>{if("Notification"in window){const b=await Notification.requestPermission();console.log("Notification permission:",b),b==="granted"&&i&&i.classList.add("hidden")}};Notification.permission==="default"&&i&&(i.classList.remove("hidden"),i.addEventListener("click",n));const c=()=>{e&&(e.style.transform="translateY(-100%)",setTimeout(()=>{e.style.visibility="hidden",e.style.pointerEvents="none",e.style.zIndex="-1",Notification.permission==="default"&&n();const b=document.getElementById("drawer-login");b&&b.classList.remove("translate-y-full")},800))},d=()=>{e&&(e.style.zIndex="9999",e.style.pointerEvents="auto",e.style.visibility="visible",e.style.transform="translateY(0)")};o&&o.addEventListener("click",c),document.querySelectorAll(".forgot-password-link").forEach(b=>{b.addEventListener("click",$=>{$.target.closest("#mobile-splash")&&c()})}),a&&a.addEventListener("click",()=>{const b=document.getElementById("drawer-login");if(b){b.classList.add("translate-y-full");const $=b.querySelector("[data-drawer-hide]");$&&$.click()}d()});const g=document.getElementById("drawer-login"),f=document.getElementById("curved-welcome"),h=document.getElementById("peoples-bg");g&&new MutationObserver($=>{$.forEach(D=>{D.attributeName==="class"&&(g.classList.contains("translate-y-full")?(s&&(s.style.transform="translateY(0)"),r&&(r.style.opacity="1",r.style.transform="translateY(0) scale(1)"),f&&(f.style.opacity="0",f.style.transform="scale(0.5)"),t&&e&&e.style.visibility==="hidden"&&(t.style.opacity="1",t.style.transform="scale(1)"),h&&(h.classList.add("opacity-0","scale-0"),h.classList.remove("opacity-40","scale-[1.6]"))):(s&&(s.style.transform="translateY(-35%)"),r&&(r.style.opacity="0",r.style.transform="translateY(20px) scale(0.9)"),f&&(f.style.opacity="1",f.style.transform="scale(1)"),t&&(t.style.opacity="0",t.style.transform="scale(0)"),h&&(h.classList.remove("opacity-0","scale-0"),h.classList.add("opacity-40","scale-[1.6]"))))})}).observe(g,{attributes:!0})}export{ie as _,le as a,Ne as b,Me as c,Ie as d,Pe as e,Re as f,B as g,_e as h,Te as i,Oe as j,ye as p,Ae as r,je as s,Ce as u};
