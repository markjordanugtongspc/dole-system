...
import b from"./vendor-swal-BSk0fVSb.js";import{a as Ae}from"./vendor-DHtNC8Ux.js";const Ne="true".toLowerCase()==="true";function Se(){return Ne}function Y(){const e=window.location.pathname,o="/dole-system/",n=e.toLowerCase().indexOf(o.toLowerCase());if(n!==-1)return e.substring(0,n+o.length);const a=e.indexOf("/frontend/");if(a!==-1)return e.substring(0,a+1);const r=e.indexOf("/backend/");return r!==-1?e.substring(0,r+1):"/"}function we(e="Incorrect Username or Password"){b.fire({html:`
            <div class="p-6">
        ...
        `,timer:e?800:3e3,timerProgressBar:!0,showConfirmButton:!1,showCloseButton:!1,width:"350px",padding:"0",customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl overflow-hidden border border-gray-100",timerProgressBar:"bg-green-500 h-1.5"},backdrop:"rgba(0, 0, 0, 0.4)"})}function Ke(){const e=localStorage.getItem("hasVisitedBefore"),o=document.getElementById("page-loader");e&&(document.documentElement.classList.add("loaded"),o&&(o.style.display="none")),window.addEventListener("load",()=>{const n=document.querySelector("body > *:not(.page-loader)");n&&n.classList.add("content-fade-in"),setTimeout(()=>{document.documentElement.classList.add("loaded"),o&&o.classList.add("hidden"),document.body.style.overflow="auto",localStorage.setItem("hasVisitedBefore","true")},300)})}const le={_key:"DOLE-GIP-SECURE-KEY-2026",encrypt:function(e){try{const o=r=>r.split("").map(t=>t.charCodeAt(0)),n=r=>("0"+Number(r).toString(16)).substr(-2),a=r=>o(this._key).reduce((t,u)=>t^u,r);return e.split("").map(o).map(a).map(n).join("")}catch(o){return console.error("Encryption Failed",o),null}},decrypt:function(e){try{const o=a=>a.split("").map(r=>r.charCodeAt(0)),n=a=>o(this._key).reduce((r,t)=>r^t,a);return e.match(/.{1,2}/g).map(a=>parseInt(a,16)).map(n).map(a=>String.fromCharCode(a)).join("")}catch(o){return console.error("Decryption Failed",o),null}}};function Qe(){document.querySelectorAll(".login-form-shared").forEach(o=>{const n=o.querySelector('input[name="username"]'),a=o.querySelector('input[name="password"]'),r=o.querySelector('input[name="rememberMe"]');if(n&&a&&r){const t=localStorage.getItem("secure_user"),u=localStorage.getItem("secure_pass");if(t&&u){const s=le.decrypt(t),d=le.decrypt(u);s&&d&&(n.value=s,a.value=d,r.checked=!0)}}o.addEventListener("submit",async t=>{t.preventDefault();try{const s=await(await fetch(`${Y()}api/auth.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:n.value,password:a.value})})).json();if(s.success){r.checked?(localStorage.setItem("secure_user",le.encrypt(n.value)),localStorage.setItem("secure_pass",le.encrypt(a.value))):(localStorage.removeItem("secure_user"),localStorage.removeItem("secure_pass"));const d=localStorage.getItem("hasLoggedInBefore")==="true";localStorage.setItem("isLoggedIn","true"),localStorage.setItem("hasLoggedInBefore","true"),localStorage.setItem("user",JSON.stringify(s.user));const g=document.getElementById("drawer-login");if(g){g.classList.add("translate-y-full");const i=g.querySelector("[data-drawer-hide]");i&&i.click()}await je(d),Pe(d)}else{const d=document.getElementById("drawer-login");d?(d.classList.add("translate-y-full"),setTimeout(()=>{we(),setTimeout(()=>{d.classList.remove("translate-y-full"),a.value="",a.focus()},600)},400)):(we(),a.value="",a.focus())}}catch(u){console.error("Login Error:",u),b.fire({icon:"error",title:"Login Error",text:"Unable to reach the server. Please check your connection."})}})})}function Pe(e=!1){const o=document.getElementById("left-panel"),n=document.getElementById("right-panel"),a=document.getElementById("left-panel-content"),r=document.getElementById("right-panel-content");a&&(a.style.opacity="0"),r&&(r.style.opacity="0");const t=document.createElement("div");t.className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none";const u=e?"":"animate__delay-1s",s=e?"animation-duration: 0.8s;":"animation-duration: 2s;";t.innerHTML=`
        <img src="${Y()}frontend/images/logo/doleiligan.png" class="w-64 h-64 md:w-96 md:h-96 object-contain bg-white/20 backdrop-blur-sm shadow-2xl rounded-full p-4 animate__animated animate__rotateOut ${u}" style="${s}" alt="DOLE Logo">
    `,document.body.appendChild(t);const d=e?0:1e3,g=e?600:1500;setTimeout(()=>{o&&o.classList.add("animate-slide-left"),n&&n.classList.add("animate-slide-right"),setTimeout(()=>{window.location.href=`${Y()}frontend/dashboard/`},g)},d)}function et(){const e=document.getElementById("logoutBtn");e&&e.addEventListener("click",()=>{localStorage.removeItem("isLoggedIn"),window.location.href=`${Y()}`})}function tt(){const e=document.getElementById("mobile-splash"),o=document.getElementById("show-login-btn"),n=document.getElementById("back-to-splash"),a=document.getElementById("mobile-bg-content"),r=document.getElementById("mobile-welcome-text"),t=document.getElementById("reopen-login-drawer"),u=document.getElementById("request-notifications-btn"),s=async()=>{if("Notification"in window){const x=await Notification.requestPermission();console.log("Notification permission:",x),x==="granted"&&u&&u.classList.add("hidden")}};Notification.permission==="default"&&u&&(u.classList.remove("hidden"),u.addEventListener("click",s));const d=()=>{e&&(e.style.transform="translateY(-100%)",setTimeout(()=>{e.style.visibility="hidden",e.style.pointerEvents="none",e.style.zIndex="-1",Notification.permission==="default"&&s();const x=document.getElementById("drawer-login");x&&x.classList.remove("translate-y-full")},800))},g=()=>{e&&(e.style.zIndex="9999",e.style.pointerEvents="auto",e.style.visibility="visible",e.style.transform="translateY(0)")};o&&o.addEventListener("click",d),document.querySelectorAll(".forgot-password-link").forEach(x=>{x.addEventListener("click",E=>{E.target.closest("#mobile-splash")&&d()})}),n&&n.addEventListener("click",()=>{const x=document.getElementById("drawer-login");if(x){x.classList.add("translate-y-full");const E=x.querySelector("[data-drawer-hide]");E&&E.click()}g()});const l=document.getElementById("drawer-login"),p=document.getElementById("curved-welcome"),m=document.getElementById("peoples-bg");l&&new MutationObserver(E=>{E.forEach(D=>{D.attributeName==="class"&&(l.classList.contains("translate-y-full")?(a&&(a.style.transform="translateY(0)"),r&&(r.style.opacity="1",r.style.transform="translateY(0) scale(1)"),p&&(p.style.opacity="0",p.style.transform="scale(0.5)"),t&&e&&e.style.visibility==="hidden"&&(t.style.opacity="1",t.style.transform="scale(1)"),m&&(m.classList.add("opacity-0","scale-0"),m.classList.remove("opacity-40","scale-[1.6]"))):(a&&(a.style.transform="translateY(-35%)"),r&&(r.style.opacity="0",r.style.transform="translateY(20px) scale(0.9)"),p&&(p.style.opacity="1",p.style.transform="scale(1)"),t&&(t.style.opacity="0",t.style.transform="scale(0)"),m&&(m.classList.remove("opacity-0","scale-0"),m.classList.add("opacity-40","scale-[1.6]"))))})}).observe(l,{attributes:!0})}const ne=()=>"false".toLowerCase()==="true";function _e(e){try{return JSON.stringify(e)}catch{return"[unserializable]"}}const te={debug(...e){ne()&&console.debug(...e)},info(...e){ne()&&console.info(...e)},warn(...e){ne()&&console.warn(...e)},error(...e){console.error(...e)},table(e){ne()&&console.table(e)},json(e,o){ne()&&console.debug(e,_e(o))}},de=new Map;async function ie(e,o={}){const a=`${Y()}${e}`;let r=null;try{const i=JSON.parse(localStorage.getItem("user"));i&&(r=i.user_id||i.id||null)}catch{}const t={headers:{"Content-Type":"application/json",...r?{"X-User-Id":r}:{},...o.headers},...o},s=(t.method||"GET").toUpperCase()==="GET"?2:1;let d=null;for(let i=1;i<=s;i++)try{if(te.debug("[API] Request",{url:a,method:t.method||"GET",hasUserId:!!r}),t.body)try{te.json("[API] Payload",JSON.parse(t.body))}catch{te.debug("[API] Payload (raw)",t.body)}const l=await fetch(a,t);if(!l.ok)throw new Error(`HTTP ${l.status}: ${l.statusText}`);const p=await l.json();return de.has(a)&&(de.delete(a),te.info?.("[API] Recovered",{url:a})),te.debug("[API] Response",{url:a,ok:!0}),{success:!0,data:p}}catch(l){if(d=l,l instanceof TypeError&&/fetch/i.test(l.message||"")&&i<s){await new Promise(x=>setTimeout(x,1200));continue}}return d instanceof TypeError&&/fetch/i.test(d.message||"")?de.get(a)||(de.set(a,!0),te.error("API Request Network Error (suppressed for repeats):",{url:a,message:d.message})):te.error("API Request Error:",d),{success:!1,error:d?.message||"Unknown request error"}}async function G(e){return ie(e,{method:"GET"})}async function he(e,o){return ie(e,{method:"POST",body:JSON.stringify(o)})}async function Oe(e,o){return ie(e,{method:"PUT",body:JSON.stringify(o)})}async function rt(e,o){const n=new URLSearchParams(o).toString();return ie(`${e}?${n}`,{method:"PATCH"})}class Re{constructor(){this.intervals=new Map,this.isPageVisible=!0,document.addEventListener("visibilitychange",()=>{this.isPageVisible=!document.hidden,this.isPageVisible?console.log("[Polling] Page visible - resuming normal activity"):console.log("[Polling] Page hidden - reducing activity")})}start(o,n,a=1e4){this.stop(o),n();const r=setInterval(async()=>{this.isPageVisible&&await n()},a);this.intervals.set(o,r),console.log(`[Polling] Started: ${o} (every ${a}ms)`)}stop(o){this.intervals.has(o)&&(clearInterval(this.intervals.get(o)),this.intervals.delete(o),console.log(`[Polling] Stopped: ${o}`))}stopAll(){this.intervals.forEach((o,n)=>this.stop(n)),console.log("[Polling] All intervals stopped")}getActivePolls(){return Array.from(this.intervals.keys())}}const Te=new Re;function ot(){typeof window.initFlowbite=="function"?(window.initFlowbite(),console.log("[Flowbite] Components re-initialized")):console.warn("[Flowbite] initFlowbite() not available globally")}function at(e,o="",n="info"){b.fire({toast:!0,position:"top-end",icon:n,title:e,text:o,showConfirmButton:!1,timer:3e3,timerProgressBar:!0})}function st(e){return JSON.stringify(e)}window.addEventListener("beforeunload",()=>{Te.stopAll()});const qe="modulepreload",He=function(e,o){return new URL(e,o).href},ye={},Ce=function(o,n,a){let r=Promise.resolve();if(n&&n.length>0){let g=function(i){return Promise.all(i.map(l=>Promise.resolve(l).then(p=>({status:"fulfilled",value:p}),p=>({status:"rejected",reason:p}))))};const u=document.getElementsByTagName("link"),s=document.querySelector("meta[property=csp-nonce]"),d=s?.nonce||s?.getAttribute("nonce");r=g(n.map(i=>{if(i=He(i,a),i in ye)return;ye[i]=!0;const l=i.endsWith(".css"),p=l?'[rel="stylesheet"]':"";if(a)for(let x=u.length-1;x>=0;x--){const E=u[x];if(E.href===i&&(!l||E.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${i}"]${p}`))return;const m=document.createElement("link");if(m.rel=l?"stylesheet":qe,l||(m.as="script"),m.crossOrigin="",m.href=i,d&&m.setAttribute("nonce",d),document.head.appendChild(m),l)return new Promise((x,E)=>{m.addEventListener("load",x),m.addEventListener("error",()=>E(new Error(`Unable to preload CSS for ${i}`)))})}))}function t(u){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=u,window.dispatchEvent(s),!s.defaultPrevented)throw u}return r.then(u=>{for(const s of u||[])s.status==="rejected"&&t(s.reason);return o().catch(t)})};let xe=null;if(Se()){const e="https://llnddycvbcetztzwbdpx.supabase.co",o="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxsbmRkeWN2YmNldHp0endiZHB4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ1MDMwMTEsImV4cCI6MjA5MDA3OTAxMX0.apIpDlZYja7SNuA-joXgFSvu_wtbBcVVP3OQSPrh7fA";try{xe=Ae(e,o),console.log("[SUPABASE SDK] Client Initialized Successfully")}catch(n){console.error("[SUPABASE SDK] Failed to initialize client:",n)}}else console.log("[SUPABASE SDK] Supabase mode is disabled (Localhost PHP mode active).");const se="color-theme",Ve=3600*24*365;function Fe(e,o,n){document.cookie=`${e}=${o}; max-age=${n}; path=/; SameSite=Lax`}function De(e){const o=document.cookie.match(new RegExp("(?:^|; )"+e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+"=([^;]*)"));return o?decodeURIComponent(o[1]):null}function Le(){const e=localStorage.getItem(se)||De(se);return e==="dark"||e==="light"?e:"light"}function ue(e){const o=document.documentElement;e==="dark"?o.classList.add("dark"):o.classList.remove("dark"),localStorage.setItem(se,e),Fe(se,e,Ve),Ge(e),document.dispatchEvent(new CustomEvent("themeChanged",{detail:{theme:e}}))}function me(){const e=Le();ue(e==="dark"?"light":"dark")}function Ge(e){const o=e==="dark",n=document.getElementById("pref-dark-mode");n&&(n.checked=o);const a=document.getElementById("theme-toggle-dark-icon"),r=document.getElementById("theme-toggle-light-icon");a&&r&&(a.classList.toggle("hidden",o),r.classList.toggle("hidden",!o));const t=document.getElementById("sidebar-theme-label");t&&(t.textContent=o?"Light Mode":"Dark Mode")}function nt(){const e=Le();ue(e);const o=document.getElementById("pref-dark-mode");o&&o.addEventListener("change",()=>{ue(o.checked?"dark":"light")});const n=document.getElementById("theme-toggle-btn");n&&n.addEventListener("click",me),document.querySelectorAll("[data-theme-toggle]").forEach(a=>{a.addEventListener("click",me)}),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",a=>{localStorage.getItem(se)||De(se)||ue(a.matches?"dark":"light")})}function ee(){return document.documentElement.classList.contains("dark")}window.toggleTheme=me;window.isDarkMode=ee;const pe={queue:[],currentIndex:0,isActive:!1,isAutoSave:!1,lastInteractionTime:0,init(){this.showUploadModal()},showUploadModal(){const e=ee(),o={bgCard:e?"bg-slate-900/40":"bg-gray-50/40",borderCard:e?"border-slate-800":"border-gray-100",textHeading:e?"text-green-500":"text-[#2e7d32]",textSubtitle:e?"text-slate-500":"text-gray-400 dark:text-gray-300",bgUpload:e?"bg-slate-800":"bg-white",borderUpload:e?"border-slate-700":"border-gray-200",textUpload:e?"text-slate-400":"text-gray-500",hoverUpload:e?"hover:bg-slate-800/80 hover:border-blue-500":"hover:bg-blue-50/50 hover:border-royal-blue",iconText:e?"text-green-400":"text-[#2e7d32]",iconBg:e?"bg-green-900/20":"bg-[#e8f5e9]",iconBorder:e?"border-green-800/30":"border-[#c8e6c9]"},n=`
        ...
        `;b.fire({html:n,width:"600px",showConfirmButton:!1,showCloseButton:!0,customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:a=>{const r=a.querySelector("#csv-upload"),t=a.querySelector('label[for="csv-upload"]');["dragenter","dragover","dragleave","drop"].forEach(s=>{t.addEventListener(s,u,!1)});function u(s){s.preventDefault(),s.stopPropagation()}["dragenter","dragover"].forEach(s=>{t.addEventListener(s,()=>{t.classList.add("border-blue-500","bg-blue-50/50"),e&&t.classList.add("dark:bg-slate-800/80")},!1)}),["dragleave","drop"].forEach(s=>{t.addEventListener(s,()=>{t.classList.remove("border-blue-500","bg-blue-50/50"),e&&t.classList.remove("dark:bg-slate-800/80")},!1)}),r.addEventListener("change",s=>{const d=s.target.files[0];if(d){const g=a.querySelector("#auto-save-toggle");this.isAutoSave=g?g.checked:!1,this.handleFile(d)}}),t.addEventListener("drop",s=>{const g=s.dataTransfer.files[0];if(g){const i=a.querySelector("#auto-save-toggle");this.isAutoSave=i?i.checked:!1,this.handleFile(g)}},!1)}})},handleFile(e){if(!e.name.toLowerCase().endsWith(".csv")&&!e.name.toLowerCase().endsWith(".txt")){b.fire("Invalid File","Please upload a valid .csv or .txt file.","error");return}const o=new FileReader;o.onload=n=>{const a=n.target.result;this.parseCSV(a)},o.readAsText(e)},async parseCSV(e){let o=[],n="",a=!1;for(let r=0;r<e.length;r++){let t=e[r];t==='"'&&(a=!a),!a&&(t===`
`||t==="\r")?(t==="\r"&&e[r+1]===`
`&&r++,n.trim()!==""&&o.push(n),n=""):n+=t}n.trim()!==""&&o.push(n),this.queue=[];for(let r=0;r<o.length;r++){let t=o[r].trim();if(!t)continue;let u=[],s="",d=!1;for(let g=0;g<t.length;g++){let i=t[g];i==='"'?d=!d:i===","&&!d?(u.push(s.replace(/(^"|"$)/g,"").trim()),s=""):s+=i}if(u.push(s.replace(/(^"|"$)/g,"").trim()),u.length>=2){const g=u[3];if(!g||isNaN(parseInt(g)))continue;const i=u[1];if(!i||i.toLowerCase()==="name"||i.toLowerCase()==="full name")continue;const l=u[2];let p=u[4]?u[4].toUpperCase().trim():"",m="";(p==="F"||p.includes("FEMALE"))&&(m="Female"),(p==="M"||p.includes("MALE"))&&(m="Male");const x=u[5],E=u[6],D=u[7],j=this.formatDate(u[8]),M=this.formatDate(u[9]);this.queue.push({name:i,address:l,age:g,gender:m,education:x,startDate:j,endDate:M,office:E,designation:D})}}if(this.queue.length>0){try{b.fire({title:"Checking duplicates...",html:'<p class="text-sm">Please wait while we cross-reference your data.</p>',allowOutsideClick:!1,didOpen:()=>{b.showLoading()}});const r=this.queue.map(s=>s.name),u=await(await fetch(`${Y()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({names:r})})).json();if(u.success&&u.duplicates&&u.duplicates.length>0){const s=new Set(u.duplicates.map(g=>g.toLowerCase().trim())),d=this.queue.length;this.queue=this.queue.filter(g=>{const i=s.has(g.name.toLowerCase().trim());return i&&console.warn(`%c[Bulk Add] SKIPPED: ${g.name} already exists in database.`,"color: #ff9800; font-weight: bold;"),!i}),console.log(`[Bulk Add] Removed ${d-this.queue.length} duplicates ahead of time.`)}}catch(r){console.error("Bulk duplicate check failed:",r)}if(this.queue.length===0){b.fire("All Skipped","All beneficiaries in the CSV already exist in the database.","info");return}this.isActive=!0,this.currentIndex=0,b.close(),this.processNext()}else b.fire("Error","No valid beneficiary data found. Please ensure your CSV formatting matches the requirements.","error")},showProgressModal(){ee();const e=Math.round(this.currentIndex/this.queue.length*100),o=`
        ...
        `;if(b.isVisible()&&b.getPopup().querySelector("#bulk-progress-bar")){const n=document.getElementById("bulk-progress-bar"),a=b.getPopup().querySelector("span.text-\\[10px\\]"),r=document.getElementById("bulk-current-name");n&&(n.style.width=`${e}%`),a&&(a.textContent=`${this.currentIndex} / ${this.queue.length}`),r&&(r.textContent=this.queue[this.currentIndex]?.name||"...")}else b.fire({html:o,showConfirmButton:!1,allowOutsideClick:!1,allowEscapeKey:!1,width:"450px",customClass:{container:"font-montserrat",popup:"rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-800"},didOpen:n=>{n.querySelector("#stop-bulk-btn").onclick=()=>this.onCancel()}})},formatDate(e){if(!e||e.trim()==="")return"";const o=new Date(e);if(isNaN(o.getTime())){const t=e.split("/");return t.length===3?`${t[2]}-${t[1].padStart(2,"0")}-${t[0].padStart(2,"0")}`:""}const n=o.getFullYear(),a=String(o.getMonth()+1).padStart(2,"0"),r=String(o.getDate()).padStart(2,"0");return`${n}-${a}-${r}`},async processNext(){if(this.currentIndex<this.queue.length){const e=this.queue[this.currentIndex];e._isBulk=!0,e._bulkCurrent=this.currentIndex+1,e._bulkTotal=this.queue.length,this.isAutoSave?(this.showProgressModal(),window.addBeneficiaryData?(async()=>{try{const n=e.startDate?new Date(e.startDate).getFullYear():new Date().getFullYear(),[a,r]=await Promise.all([G(`api/beneficiaries.php?next_id&year=${n}`),G(`api/beneficiaries.php?next_series_no&year=${n}`)]);a.success&&a.data?.success&&a.data?.nextId&&(e.gip_id=a.data.nextId,e.id=null),r.success&&r.data?.success&&r.data?.nextSeries&&(e.seriesNo=r.data.nextSeries)}catch(n){console.warn("[Bulk Add] Identifier fetch failed, continuing:",n?.message||n)}const o=await window.addBeneficiaryData(e);this.isActive&&(o?this.onSaveSuccess():ge(e))})():ge(e)):ge(e)}else this.isActive=!1,this.lastInteractionTime=Date.now(),b.fire({icon:"success",title:"Bulk Add Complete!",text:`Successfully processed ${this.queue.length} beneficiaries.`,confirmButtonColor:"#2e7d32"})},onSaveSuccess(){if(this.isActive){this.currentIndex++;const e=this.isAutoSave?100:1500;setTimeout(()=>{this.processNext()},e)}},onCancel(){this.isActive&&(this.isActive=!1,this.lastInteractionTime=Date.now(),this.queue=[],this.currentIndex=0,b.fire({icon:"info",title:"Bulk Add Cancelled",text:"Stopped processing the remaining beneficiaries and cleared the queue.",confirmButtonColor:"#3085d6"}).then(()=>{this.showUploadModal()}))}};window.BulkApp=pe;function ke(e){if(!e)return"";const o=new Date(e),n=new Date;let a=n.getFullYear()-o.getFullYear();const r=n.getMonth()-o.getMonth();return(r<0||r===0&&n.getDate()<o.getDate())&&a--,a>=0?a:0}function $e(e){return e?e.includes("DOLE")?"bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/40 dark:text-blue-300 dark:border-blue-800":e.includes("DepEd")?"bg-orange-100 text-orange-700 border-orange-200 dark:bg-orange-900/40 dark:text-orange-300 dark:border-orange-800":e.includes("LGU")?"bg-purple-100 text-purple-700 border-purple-200 dark:bg-purple-900/40 dark:text-purple-300 dark:border-purple-800":e.includes("DICT")?"bg-cyan-100 text-cyan-700 border-cyan-200 dark:bg-cyan-900/40 dark:text-cyan-300 dark:border-cyan-800":"bg-gray-100 text-gray-700 border-gray-200 dark:text-gray-300":"bg-gray-100 text-gray-700 border-gray-200 dark:text-gray-300"}function Ue(e){if(!e)return"bg-gray-100 text-gray-600 border-gray-200 dark:text-gray-300";const o=e.toUpperCase();return o==="ONGOING"?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800":o==="EXPIRED"?"bg-red-400 text-white border-red-400 dark:bg-red-900/60 dark:border-red-800":o==="RESIGNED"?"bg-[#ce1126] text-white border-[#ce1126] dark:bg-red-900/80 dark:border-red-900":o==="ABSORBED"?"bg-[#2e7d32] text-white border-[#2e7d32] dark:bg-green-900/80 dark:border-green-900":"bg-gray-100 text-gray-600 border-gray-200 dark:text-gray-300"}function ce(e,o=0){e={...e,id:e?.id||e?.gip_id||"N/A",name:e?.name||"N/A",office:e?.office||"N/A",remarks:e?.remarks||"N/A",designation:e?.designation||"N/A"};const n=window.innerWidth<640?"top":"top-start";let a=o;const r=e.arLogs||[],t=e.dtrLogs||[],u=e.docs||[],s=["GIP FORM","BIRTH CERTIFICATE","DIPLOMA","TOR","VALID ID"],d=s.map(l=>{const p=u.find(m=>m.name.toUpperCase()===l.toUpperCase());return p||{name:l,status:"PENDING",id:null}});u.forEach(l=>{s.some(m=>m.toUpperCase()===l.name.toUpperCase())||d.push(l)});const g=`
...
<div class="mt-8 pt-2 border-t border-default relative">
    ...
    <div id="drawer-page-1" class="hidden flex-1 flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-3">
            ${d.map(l=>{const p=l.status.toUpperCase(),x={VERIFIED:"COMPLETED",DECLINED:"REJECTED",PENDING:"PENDING"}[p]||p,E=x==="COMPLETED",D=x==="REJECTED",j=E?"text-green-500":D?"text-red-500":"text-gray-400 dark:text-gray-500",M=E?"bg-green-50/50 dark:bg-green-900/10":D?"bg-red-50/50 dark:bg-red-900/10":"bg-gray-50/50 dark:bg-slate-800/50",W=E?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800 hover:bg-green-200 cursor-pointer":D?"bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800 hover:bg-red-200 cursor-pointer":"bg-white text-gray-500 border-gray-200 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer";let U='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';return E?U='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>':D&&(U='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>'),`
                <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-0.5 ${M}">
                    <div class="flex items-center gap-3 w-full">
                        <div class="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center bg-white dark:bg-slate-800 shadow-sm border border-gray-100 dark:border-slate-700 ${j}">
                            ${U}
                        </div>
                        <span class="text-xs sm:text-sm font-black ${E?"text-heading":"text-gray-500 dark:text-gray-400"} uppercase tracking-tight flex-1">${l.name}</span>
                    </div>
                    <button type="button" class="ml-3 ${W} text-[10px] font-black px-3 py-1.5 rounded-full border uppercase tracking-widest transition-colors flex-shrink-0 drawer-doc-btn" data-id="${l.id}" data-name="${l.name}" data-status="${x} cursosr-pointer">
                        ${x}
                    </button>
                </div>
                `}).join("")}
    ...
    <div id="drawer-page-2" class="hidden flex-1 flex flex-col gap-6">
         ...
         <div class="flex flex-col gap-5">
            <div>
                ...
                <div class="space-y-2">
                    ${t.length?t.map(l=>{const p=l.status||"PENDING";let m=p==="VERIFIED"||p==="COMPLETED"?"text-green-500":p==="REJECTED"||p==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500";return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-blue-100 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors edit-log-btn" data-type="dtr" data-id="${l.id}" data-val="${l.date||l.createdAt}" data-status="${p}">
                            ...
                            <span class="text-[11px] font-bold ${m} uppercase tracking-widest truncate max-w-[50%] text-right pr-6 group-hover:pr-12 pointer-events-none transition-all">${p}</span>
...
<style>
    ...
    `;let i=document.getElementById("beneficiary-drawer-container");i&&i.remove(),i=document.createElement("div"),i.id="beneficiary-drawer-container",i.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-neutral-primary-soft dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl",i.setAttribute("tabindex","-1"),i.setAttribute("data-drawer-backdrop","true"),i.innerHTML=g,document.body.appendChild(i),Ce(async()=>{const{Drawer:l}=await import("./vendor-flowbite-bBKPL--D.js");return{Drawer:l}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:l})=>{const p={placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{i&&i.parentNode&&i.remove()},300)}},m=new l(i,p);m.show(),i.querySelector("#close-drawer-btn").addEventListener("click",()=>m.hide());const E=i.querySelector("#drawer-prev-btn"),D=i.querySelector("#drawer-next-btn"),j=2,M=()=>{i.querySelectorAll("[id^=drawer-page-]").forEach((w,h)=>{w.classList.toggle("hidden",h!==a)}),E.disabled=a===0,D.disabled=a===j,E.classList.toggle("opacity-50",a===0),D.classList.toggle("opacity-50",a===j)};E.addEventListener("click",()=>{a>0&&a--,M()}),D.addEventListener("click",()=>{a<j&&a++,M()}),M(),i.querySelectorAll(".drawer-doc-btn").forEach(w=>{w.addEventListener("click",async()=>{const h=w.dataset.name,y=w.dataset.status,k="flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[9px] gap-2 transition-all duration-300 ",L=await b.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Update Document</span>',html:`
                        <div class="font-montserrat text-left">
                            <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-4 ps-1">Set status for <span class="text-brand font-black">${h}</span></label>
                            
                            <div class="grid grid-cols-3 gap-3">
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-doc-status" value="PENDING" class="peer sr-only" ${y==="PENDING"?"checked":""}>
                                ...
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-doc-status" value="COMPLETED" class="peer sr-only" ${y==="COMPLETED"?"checked":""}>
                                ...
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-doc-status" value="REJECTED" class="peer sr-only" ${y==="REJECTED"?"checked":""}>
                                    <div class="${k} border-gray-100 bg-gray-50 text-gray-400 dark:border-slate-700 dark:bg-slate-800/50 dark:text-gray-500 peer-checked:border-red-500 peer-checked:bg-red-50 peer-checked:text-red-600 dark:peer-checked:bg-red-900/20 dark:peer-checked:text-red-400 hover:bg-red-50/50 dark:hover:bg-slate-700">
                    ...
                    `,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Update Status</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Cancel</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2"},buttonsStyling:!1,preConfirm:()=>{const I=document.querySelector('input[name="swal-doc-status"]:checked');return I?I.value:null}});if(L.isConfirmed){const I=L.value;if(I===y)return;try{const _={COMPLETED:"VERIFIED",REJECTED:"DECLINED",PENDING:"PENDING"}[I]||I,T=await he("api/logs.php?type=docs",{gip_id:e.id,doc_name:h,status:_}),F=T.success?T.data:{success:!1,error:T.error};F.success?(b.fire({toast:!0,position:n,icon:"success",title:"Status updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):b.fire("Error",F.error||"Failed to update","error")}catch(P){b.fire("Error",P.message,"error")}}})});const W=()=>{if(!t.length)return new Date().toISOString().split("T")[0];const w=new Date(t[0].date);return w.setDate(w.getDate()+1),w.toISOString().split("T")[0]},U=()=>{if(!r.length){const k=new Date,L=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"][k.getMonth()];return k.getDate()<=15?`${L} 1-15`:`${L} 16-${new Date(k.getFullYear(),k.getMonth()+1,0).getDate()}`}const w=r[0].period||"",h=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"],y=w.toUpperCase().match(/([A-Z]{3})\s+(\d+)-(\d+)/);if(y){let k=y[1],L=parseInt(y[2]),I=h.indexOf(k);if(I!==-1)return L===1?`${k} 16-${new Date(new Date().getFullYear(),I+1,0).getDate()}`:`${h[(I+1)%12]} 1-15`}return"NEW PERIOD"},Q=async(w,h)=>{b.fire({title:"Adding...",allowOutsideClick:!1,showConfirmButton:!1}),b.showLoading();try{const y={gip_id:e.id};w==="dtr"&&(y.record_date=h),w==="ar"&&(y.period=h);const k=await he(`api/logs.php?type=${w}`,y);(k.success?k.data:{success:!1,error:k.error}).success?(b.fire({toast:!0,position:n,icon:"success",title:"Auto-Added!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):b.fire("Error","Failed to add log.","error")}catch(y){b.fire("Error",y.message,"error")}},q=async(w,h,y,k,L)=>{const I=h==="dtr"?"Record Date":"Period";ee();const P="flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[10px] sm:text-xs gap-2 ",_='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>',T='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>',{value:F}=await b.fire({title:`<span class="text-xl font-black text-heading uppercase tracking-tight">Edit ${w} Log</span>`,html:`
                    <div class="flex flex-col gap-5 text-left font-montserrat mt-2">
                        <div>
                            ...
                            <input id="swal-log-val" value="${k}" class="bg-gray-50 dark:bg-slate-800/80 border border-gray-200 dark:border-slate-600 text-heading text-sm rounded-xl focus:ring-brand focus:border-brand block w-full p-2.5 font-bold uppercase transition-all" placeholder="Enter value">
                        </div>
                        <div>
                ...
                `,focusConfirm:!1,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">SAVE REVISIONS</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">CANCEL</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2"},buttonsStyling:!1,preConfirm:()=>{const H=document.querySelector('input[name="swal-log-status"]:checked');return{val:document.getElementById("swal-log-val").value.trim().toUpperCase(),status:H?H.value:"PENDING"}}});if(F&&(F.val!==k||F.status!==L))try{const H={type:h,id:y,status:F.status};h==="dtr"&&(H.record_date=F.val),h==="ar"&&(H.period=F.val);const re=await Oe("api/logs.php",H),oe=re.success?re.data:{success:!1,error:re.error};oe.success?(b.fire({toast:!0,position:n,icon:"success",title:"Log Updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):b.fire("Error",oe.error||"Failed to update log.","error")}catch(H){b.fire("Error",H.message,"error")}},Z=i.querySelector("#add-dtr-log-btn");Z&&Z.addEventListener("click",()=>Q("dtr",W()));const C=i.querySelector("#add-ar-log-btn");C&&C.addEventListener("click",()=>Q("ar",U())),i.querySelectorAll(".edit-log-btn").forEach(w=>{w.addEventListener("click",h=>{if(h.target.closest(".delete-log-btn"))return;const y=w.dataset.type,k=w.dataset.id,L=w.dataset.val,I=w.dataset.status;q(y.toUpperCase(),y,k,L,I)})}),i.querySelectorAll(".delete-log-btn").forEach(w=>{w.addEventListener("click",async()=>{const h=w.dataset.id,y=w.dataset.type;if((await b.fire({title:'<span class="text-xl font-black text-philippine-red uppercase tracking-tight">Delete item?</span>',text:"This action cannot be undone.",icon:"warning",showCancelButton:!0,confirmButtonText:'<span class="font-black tracking-widest uppercase">Delete</span>',cancelButtonText:'<span class="font-black tracking-widest uppercase">Wait</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100",confirmButton:"bg-philippine-red text-white hover:bg-red-700 text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs px-6 py-2.5 rounded-xl border border-gray-200 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1})).isConfirmed)try{const L=await he(`api/logs.php?type=${y}`,{log_id:h,action:"delete"});(L.success?L.data:{success:!1,error:L.error}).success?(b.fire({toast:!0,position:n,icon:"success",title:"Deleted",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):b.fire("Error","Failed to delete data.","error")}catch(L){b.fire("Error",L.message,"error")}})})}).catch(console.error)}function ze(e){const o=ee(),n=`w-full bg-transparent border-b-2 ${o?"border-slate-700 text-white focus:border-brand placeholder-slate-600":"border-gray-200 text-gray-900 focus:border-brand placeholder-gray-300"} px-1 py-1 text-sm font-black outline-none transition-all focus:ring-0`,a=`w-full bg-transparent border-none ${o?"text-white":"text-royal-blue"} px-0 py-0 text-xl sm:text-2xl font-black leading-tight tracking-tight focus:ring-0 outline-none placeholder-gray-300 resize-none overflow-hidden`,r=["DOLE Field Office","LGU","DEPED","DICT","PCA"];function t(d){if(!d)return"";const g=new Date(d),i=new Date;let l=i.getFullYear()-g.getFullYear();const p=i.getMonth()-g.getMonth();return(p<0||p===0&&i.getDate()<g.getDate())&&l--,l>=0?l:0}const u=`
<form id="edit-beneficiary-drawer-form" class="h-full flex flex-col pt-4 font-montserrat relative pb-20 overflow-y-auto">
    ...
    <input type="hidden" name="gip_id" value="${e.gip_id||e.id}">
    ...
    <div class="flex flex-col gap-4 text-sm mt-3 px-1 pb-24">
        ...
        <div class="flex justify-between items-center group">
            ...
            <div class="relative max-w-[180px]">
                ...
                <input type="text" name="birthday" id="edit-bday-input" value="${e.birthday||""}" class="${n} !pl-9 text-right uppercase" placeholder="MM/DD/YYYY" datepicker>
            </div>
        </div>
        
        <div class="flex justify-between items-center group">
            <span class="text-gray-500 font-medium whitespace-nowrap mr-4 shrink-0">Age</span>
            <input type="text" name="age" id="edit-age-display" value="${e.age||t(e.birthday)||""}" class="${n} text-right max-w-[80px]" placeholder="Auto">
        </div>
        ...
        <div class="flex flex-col gap-2 pt-3 mt-1 border-t border-gray-50 dark:border-slate-800/60">
            ...
            <div id="edit-education-suggestions-box" class="hidden mt-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                ${Ie.map(d=>`<button type="button" class="edit-education-option w-full text-left px-3 py-2 text-[11px] font-bold text-slate-700 dark:text-slate-200 hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer"><span class="option-text">${d.name}</span></button>`).join("")}
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col gap-4 text-sm mt-4 px-1">
        ...
        <div class="flex flex-col gap-2">
            ...
            <div id="edit-designation-suggestions-box" class="hidden mt-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                ${Be.map(d=>`<button type="button" class="edit-designation-option w-full text-left px-3 py-2 text-[11px] font-bold text-slate-700 dark:text-slate-200 hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer"><span class="option-text">${d}</span></button>`).join("")}
            </div>
        </div>
...
.editable-indicator {
    ...
    `;let s=document.getElementById("edit-drawer-container");s&&s.remove(),s=document.createElement("div"),s.id="edit-drawer-container",s.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-white dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",s.setAttribute("tabindex","-1"),s.innerHTML=u,document.body.appendChild(s),setTimeout(()=>{const d=s.querySelector('textarea[name="name"]');d&&(d.style.height="auto",d.style.height=d.scrollHeight+"px")},10),(async()=>{try{const d=await ie("api/beneficiaries.php?get_offices=1");if(d.success&&d.data.offices){const g=s.querySelector("#edit-office-suggestions-box");if(g){const i=["DOLE Field Office","LGU","DEPED","DICT","PCA"],l=[...new Set([...i,...d.data.offices])];g.innerHTML=l.map(p=>`
                    ...
                    `).join("")}}}catch(d){console.error("Error fetching office suggestions:",d)}})(),Ce(async()=>{const{Drawer:d}=await import("./vendor-flowbite-bBKPL--D.js");return{Drawer:d}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:d})=>{const g=new d(s,{placement:"right",backdrop:!0,bodyScrolling:!1,backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{s.parentNode&&s.remove()},300)}});g.show();const i=()=>{document.activeElement&&typeof document.activeElement.blur=="function"&&document.activeElement.blur(),g.hide()};s.querySelector("#close-edit-drawer-btn").addEventListener("click",i),s.querySelector("#edit-drawer-cancel-btn").addEventListener("click",i);const l=s.querySelector("#edit-beneficiary-drawer-form"),p=s.querySelector("#edit-bday-input"),m=s.querySelector("#edit-age-display"),x=s.querySelector("#edit-startDate-input"),E=s.querySelector("#edit-endDate-input"),D=s.querySelector('input[name="seriesNo"]'),j=s.querySelector('input[name="gip_id"]');let M=!1;const W=(C,w)=>{C.addEventListener("input",h=>{const y=h.target.value,k=window.__maskDate(y);if(y!==k&&(h.target.value=k),k.length===10){const L=window.__parseFormattedDate(k);L&&w&&(w(L),document.activeElement===C&&C.blur())}}),C.addEventListener("changeDate",h=>{h.detail&&h.detail.date&&w&&(w(h.detail.date),C._datepicker&&C._datepicker.hide())})};p&&W(p,C=>{m&&(!M||!m.value)&&(m.value=window.calculateAge(C))}),x&&W(x,C=>{if(E){const h=new Date(C);h.setMonth(h.getMonth()+3),h.setDate(h.getDate()+1);const y=String(h.getMonth()+1).padStart(2,"0"),k=String(h.getDate()).padStart(2,"0"),L=h.getFullYear();E.value=`${y}/${k}/${L}`}const w=C.getFullYear();w>1900&&j&&D&&Promise.all([G(`api/beneficiaries.php?next_id&year=${encodeURIComponent(w)}`),G(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(w)}`)]).then(([h,y])=>{const k=h.success&&h.data?.success?h.data.nextId:null,L=y.success&&y.data?.success?y.data.nextSeries:null,I=String(j.value||"").match(/^ROX-RD-ESIG-(\d{4})-\d{4}$/),P=String(D.value||"").match(/^(\d{4})-\d{2}-\d{3}$/),_=I?Number(I[1]):null,T=P?Number(P[1]):null;k&&(_===null||_!==w)&&(j.value=k),L&&(T===null||T!==w)&&(D.value=L)}).catch(h=>{console.error("Edit drawer identifier sync error:",h)})}),E&&W(E);const U=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null),Q=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null);U&&p&&(p._datepicker=new U(p,{format:"mm/dd/yyyy",autohide:!0}));const q=s.querySelector("#edit-date-range-picker");if(Q&&q){const C=new Q(q,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0});x&&(x._datepicker=C.datepickers[0]),E&&(E._datepicker=C.datepickers[1])}m&&m.addEventListener("input",()=>M=!0);const Z=(C,w,h)=>{const y=s.querySelector(C),k=s.querySelector(w);if(!y||!k)return;const L=()=>k.classList.add("hidden"),I=()=>k.classList.remove("hidden");y.addEventListener("focus",I),y.addEventListener("input",()=>{const P=y.value.toLowerCase().trim();let _=0;k.querySelectorAll(h).forEach(T=>{const H=(T.querySelector(".option-text")?.textContent||T.textContent||"").toLowerCase().includes(P);T.style.display=H?"block":"none",H&&_++}),_>0?I():L()}),k.addEventListener("click",P=>{const _=P.target.closest(h);_&&(y.value=(_.querySelector(".option-text")?.textContent||_.textContent||"").trim(),L(),y.dispatchEvent(new Event("change")))}),document.addEventListener("click",P=>{!y.contains(P.target)&&!k.contains(P.target)&&L()})};Z("#edit-education-input","#edit-education-suggestions-box",".edit-education-option"),Z("#edit-designation-input","#edit-designation-suggestions-box",".edit-designation-option"),Z("#edit-office-input","#edit-office-suggestions-box",".edit-office-option"),l.querySelectorAll("input, select, textarea").forEach(C=>{const w=(C.getAttribute("type")||"").toLowerCase(),h=!C.disabled&&!C.readOnly&&w!=="hidden";C.classList.remove("editable-indicator"),h&&C.classList.add("editable-indicator")}),l.addEventListener("submit",C=>{C.preventDefault();const w=new FormData(l),h={};w.forEach((y,k)=>h[k]=y),h.id=e.id,h.gip_id=e.id,window.addBeneficiaryData&&(async()=>await window.addBeneficiaryData(h,!0,!1)&&(i(),window.viewBeneficiary&&setTimeout(()=>window.viewBeneficiary({id:e.id},0),100),b.fire({toast:!0,position:"top-end",icon:"success",title:"Record Updated",showConfirmButton:!1,timer:3e3})))()})})}function it(){window.__maskDate=function(e){let o=e.replace(/\D/g,"").slice(0,8);return o.length>2&&o.length<=4?o=o.slice(0,2)+"/"+o.slice(2):o.length>4&&(o=o.slice(0,2)+"/"+o.slice(2,4)+"/"+o.slice(4)),o},window.__parseFormattedDate=function(e){if(!e)return null;const o=e.split("/");if(o.length===3){const n=parseInt(o[0])-1,a=parseInt(o[1]),r=parseInt(o[2]);if(r>1e3&&n>=0&&n<12&&a>0&&a<=31)return new Date(r,n,a)}return null},window.calculateAge=function(e){if(!e)return"";const o=e instanceof Date?e:new Date(e),n=new Date;let a=n.getFullYear()-o.getFullYear();const r=n.getMonth()-o.getMonth();return(r<0||r===0&&n.getDate()<o.getDate())&&a--,a>=0?a:0},window.viewBeneficiary=async function(e,o=0){const n=e?.id||e?.gip_id||null;if(!n)return;const a=!!(e?.name&&e?.office&&e?.remarks);let r={...e,id:n};if(!a){const s=await G(`api/beneficiaries.php?id=${encodeURIComponent(n)}`);s.success&&s.data?.success&&s.data?.beneficiary&&(r={...s.data.beneficiary,...r,id:n})}const t=`logs_cache_${n}`;let u=!1;try{if(window.__doleDB&&window.__doleDB.getSecureCache){const D=await window.__doleDB.getSecureCache(t);D&&(r.arLogs=D.arLogs||[],r.dtrLogs=D.dtrLogs||[],r.docs=D.docs||[],ce(r,o),u=!0)}let s=[],d=[],g=[];const[i,l,p,m]=await Promise.all([G(`api/logs.php?type=ar&gip_id=${encodeURIComponent(n)}`),G(`api/logs.php?type=dtr&gip_id=${encodeURIComponent(n)}`),G(`api/logs.php?type=docs&gip_id=${encodeURIComponent(n)}`),G(`api/logs.php?type=absorption&gip_id=${encodeURIComponent(n)}`)]);s=i.success&&i.data?.success?i.data.logs:[],d=l.success&&l.data?.success?l.data.logs:[],g=p.success&&p.data?.success?p.data.logs:[];const x=m.success&&m.data?.success?m.data.logs:[];if(x.length>0){const D=x[0];r.absorbDate=D.absorption_datetime,r.absorb_where=D.where||D.absorb_where,r.absorb_position=D.position||D.absorb_position,r.absorb_agency=D.agency||D.absorb_agency}const E={arLogs:s,dtrLogs:d,docs:g};window.__doleDB&&window.__doleDB.setSecureCache&&await window.__doleDB.setSecureCache(t,E),r.arLogs=s,r.dtrLogs=d,r.docs=g,u?document.getElementById("beneficiary-drawer-container")&&ce(r,o):ce(r,o)}catch(s){console.error("Error fetching logs/docs:",s),u||(r.arLogs=[],r.dtrLogs=[],r.docs=[],ce(r,o))}},window.showAddDataModal=function(e){ge(e)},window.editBeneficiary=function(e){ze(e)},window.showExportConfigModal=function(e){Je(e)},window.showProfileModal=function(){Ye()}}async function Ye(){try{if(Se()&&xe){let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=a.id)}catch{}if(!e)throw new Error("User not logged in");const{data:o,error:n}=await xe.from("users").select("*").eq("id",e).single();if(n)throw n;Ee(o)}else{let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=`?user_id=${a.id}`)}catch{}const n=await(await fetch(`${Y()}api/profile.php${e}`)).json();if(n.success){const a=n.profile;Ee(a)}else b.fire({icon:"error",title:"Error",text:n.error||"Failed to load profile"})}}catch(e){console.error("Error fetching profile:",e)}}function Ee(e){const o=e.profile_picture_path?`${Y()}${e.profile_picture_path}`:null,n=e.full_name?e.full_name.split(" ").map(r=>r[0]).join("").substring(0,2).toUpperCase():"US",a=`
    ...
    `;b.fire({html:a,width:"450px",showConfirmButton:!1,showCloseButton:!0,padding:"1.5rem",customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl overflow-visible ldn-modal-popup",closeButton:"focus:outline-none bg-gray-50 border-none swal2-custom-close cursor-pointer"},didOpen:r=>{const t=r.querySelector("#profile-edit-form"),u=r.querySelector("#profile-pic-input"),s=r.querySelector("#profile-avatar-preview");u.addEventListener("change",d=>{const g=d.target.files[0];if(g){const i=new FileReader;i.onload=l=>{s.innerHTML=`<img src="${l.target.result}" class="w-full h-full object-cover" />`},i.readAsDataURL(g)}}),t.addEventListener("submit",async d=>{d.preventDefault();const g=new FormData(t);try{const i=JSON.parse(localStorage.getItem("user"));i&&i.id&&g.append("user_id",i.id)}catch{}try{const l=await(await fetch(`${Y()}api/profile.php`,{method:"POST",body:g})).json();l.success?(l.profile&&(localStorage.setItem("user",JSON.stringify(l.profile)),Ze(l.profile)),b.close(),b.fire({toast:!0,position:"top-end",icon:"success",title:"Profile Updated",showConfirmButton:!1,timer:3e3,timerProgressBar:!0})):b.fire({icon:"error",title:"Update Failed",text:l.error||"Something went wrong"})}catch(i){console.error("Error saving profile:",i)}})}})}function Ze(e){const o=e.profile_picture_path?`${Y()}${e.profile_picture_path}`:null,n=e.full_name?e.full_name.split(" ").map(t=>t[0]).join("").substring(0,2).toUpperCase():"US",a=document.querySelectorAll(".sidebar-user-name"),r=document.querySelectorAll(".sidebar-user-avatar");a.forEach(t=>t.textContent=e.full_name),r.forEach(t=>{o?t.innerHTML=`<img src="${o}" class="w-full h-full object-cover" />`:t.textContent=n}),localStorage.setItem("user_full_name",e.full_name),o&&localStorage.setItem("user_avatar",o)}function Je(e){const o=window.getExportFilters?window.getExportFilters():{office:"ALL",remarks:"ALL",gender:"ALL",ageGroup:"ALL",search:"",sort:"name",section:"ALL",columns:["id","name","age","office","position","startdate","enddate","status"],preparedBy:localStorage.getItem("ldn_export_prepared")||"",approvedBy:localStorage.getItem("ldn_export_approved")||""},n=`
        <div class="text-left font-montserrat p-1 overflow-visible">
            <div class="flex items-center gap-3 mb-6">
                <div class="w-12 h-12 bg-royal-blue/10 rounded-2xl flex items-center justify-center">
                                ...
                                <div class="flex flex-wrap gap-2 p-1.5 ${t.bgStatusWrap} border ${t.borderStatus} rounded-xl shadow-inner flex-1">
                                    ${(()=>{const s={ONGOING:"peer-checked:bg-green-400 peer-checked:text-white peer-checked:border-green-400",EXPIRED:"peer-checked:bg-red-400 peer-checked:text-white peer-checked:border-red-400",RESIGNED:"peer-checked:bg-[#ce1126] peer-checked:text-white peer-checked:border-[#ce1126]",ABSORBED:"peer-checked:bg-[#2e7d32] peer-checked:text-white peer-checked:border-[#2e7d32]"};return["ONGOING","EXPIRED","RESIGNED","ABSORBED"].map(d=>`
                                            <label class="cursor-pointer relative min-w-[80px]">
                                                ...
                                                <span class="px-3 py-1.5 rounded-lg border border-transparent text-[10px] font-black text-gray-400 dark:text-white! uppercase tracking-widest ${s[d]||""} transition-all block text-center cursor-pointer shadow-sm">
                                                    ${d}
                                                </span>
                                            </label>
                                        `).join("")})()}
    ...
    `;b.fire({html:u,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:s=>{const d=s.querySelector("#cancel-modal-btn");d&&d.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),b.close(),e?._isBulk&&pe.onCancel()});const g=s.querySelector("#bulk-add-btn");g&&g.addEventListener("click",()=>{b.close(),pe.init()});const i=(c,v)=>{c.addEventListener("input",f=>{const $=window.__maskDate(f.target.value);if(f.target.value!==$&&(f.target.value=$),$.length===10){const S=window.__parseFormattedDate($);S&&v&&(v(S),document.activeElement===c&&c.blur())}}),c.addEventListener("changeDate",f=>{if(f.detail&&f.detail.date&&v){v(f.detail.date);const $=c._datepicker||c.parentNode&&c.parentNode._datepicker;$&&typeof $.hide=="function"&&$.hide()}})},l=s.querySelector("#birthday-input"),p=s.querySelector("#age-display");if(l){i(l,v=>{p&&(p.value=window.calculateAge(v),p.classList.add("animate-pulse"),setTimeout(()=>p.classList.remove("animate-pulse"),400))});const c=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);c&&(l._datepicker=new c(l,{format:"mm/dd/yyyy",autohide:!0}))}const m=s.querySelector("#name-input-field"),x=s.querySelector("#duplicate-warning");if(m&&x){let c;m.addEventListener("input",v=>{const f=v.target.value.trim();if(clearTimeout(c),f.length<3){x.classList.add("hidden");return}c=setTimeout(async()=>{try{const S=await(await fetch(`${Y()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:f})})).json();S.success&&S.exists?(x.classList.remove("hidden"),ee()?x.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":x.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):x.classList.add("hidden")}catch($){console.error("Duplicate check error:",$)}},500)}),e?.name&&(x.classList.add("hidden"),(async()=>{const f=await(await fetch(`${Y()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();f.success&&f.exists&&(x.classList.remove("hidden"),ee()?x.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":x.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const E=s.querySelector("#full-id-input"),D=s.querySelector("#series-no-input"),j=s.querySelector('input[name="startDate"]'),M=s.querySelector('input[name="endDate"]'),W=s.querySelectorAll('input[name="remarks"]'),U=s.querySelector("#extension-log-container"),Q=async c=>{if(!c)return;const v=`cache_next_id_${c}`,f=`cache_next_series_${c}`,$=`cache_expiry_${c}`,S=Date.now(),O=localStorage.getItem(v),A=localStorage.getItem(f),V=localStorage.getItem($);if(O&&A&&V&&S<parseInt(V,10)){E&&(E.value=O,E.classList.remove("placeholder:text-gray-300")),D&&(D.value=A);return}const R=[E,D].filter(Boolean);R.forEach(B=>{B.classList.add("animate-pulse"),B.placeholder="Syncing..."});try{const[B,N]=await Promise.all([G(`api/beneficiaries.php?next_id&year=${encodeURIComponent(c)}`),G(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(c)}`)]),X=B.success&&B.data?.success?B.data.nextId:null,z=N.success&&N.data?.success?N.data.nextSeries:null;X&&E&&(E.value=X,localStorage.setItem(v,X)),z&&D&&(D.value=z,localStorage.setItem(f,z)),localStorage.setItem($,(S+3e5).toString())}catch(B){console.error("ID Sync error:",B)}finally{R.forEach(B=>B.classList.remove("animate-pulse"))}},q=s.querySelector("#replacement-search-input"),Z=s.querySelector("#replacement-hidden"),C=s.querySelector("#replacement-suggestions"),w=c=>{const v=(c.name||"").toUpperCase().trim(),f=c.startDateFormatted||c.startDate||"N/A",$=c.endDateFormatted||c.endDate||"N/A";return`${v} (${f.toUpperCase()} - ${$.toUpperCase()})`},h=c=>{if(C){if(!c.length){C.innerHTML=`<div class="px-3 py-2 text-[10px] font-bold ${t.textCourseOpt}">No matching beneficiary found.</div>`,C.classList.remove("hidden");return}C.innerHTML=c.map(v=>{const f=w(v);return`
                    ...
                    `}).join(""),C.classList.remove("hidden"),C.querySelectorAll(".replacement-option").forEach(v=>{v.addEventListener("click",()=>{const f=v.getAttribute("data-replacement")||"";q&&(q.value=f),Z&&(Z.value=f),C.classList.add("hidden")})})}};let y=null;const k=async(c="")=>{const v=(c||"").trim(),f=`api/beneficiaries.php?replacement_candidates=1&limit=20${v?`&q=${encodeURIComponent(v)}`:""}`,$=await G(f);$.success&&$.data?.success&&Array.isArray($.data.candidates)&&h($.data.candidates)};q&&Z&&C&&(q.addEventListener("focus",()=>{k(q.value||"")}),q.addEventListener("input",()=>{Z.value=q.value.trim(),clearTimeout(y),y=setTimeout(()=>{k(q.value||"")},250)}),document.addEventListener("click",c=>{!q.contains(c.target)&&!C.contains(c.target)&&C.classList.add("hidden")}));const L=()=>{const c=s.querySelector('input[name="remarks"]:checked');return c?c.value:"ONGOING"},I=c=>{const v=s.querySelector(`input[name="remarks"][value="${c}"]`);v&&(v.checked=!0,_())},P=()=>{if(M&&M.value){const c=window.__parseFormattedDate(M.value);if(!c)return;const v=new Date;v.setHours(0,0,0,0);let f="ONGOING";c<v&&(f="EXPIRED"),I(f)}},_=()=>{if(!U)return;if(L()==="ABSORBED"){const v=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});U.innerHTML=`
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                                <div class="${r?"bg-green-900/10 border-green-900/30":"bg-[#e8f5e9]/50 border-[#c8e6c9]"} rounded-lg p-2.5 flex items-center justify-between border">
                                    <div>
                                        ...
                                        <p class="text-[10px] font-black ${r?"text-green-400":"text-[#1b5e20]"} uppercase">${v}</p>
                                        <input type="hidden" name="absorbDate" value="${v}">
                                    </div>
                    ...
                    `}else U.innerHTML=""};if(j){let c=null;i(j,S=>{const O=S.getFullYear();if(M){const A=new Date(S);A.setMonth(A.getMonth()+3),A.setDate(A.getDate()+1);const V=String(A.getMonth()+1).padStart(2,"0"),R=String(A.getDate()).padStart(2,"0"),B=A.getFullYear();M.value=`${V}/${R}/${B}`}P(),O>1900&&O!==c&&(c=O,Q(O))}),M&&i(M,()=>P());const v=s.querySelector("#date-range-picker"),f=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null),$=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);if(f&&v&&j&&M){const S=new f(v,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0});j._datepicker=S.datepickers?.[0]||null,M._datepicker=S.datepickers?.[1]||null}else $&&(j&&(j._datepicker=new $(j,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom left"})),M&&(M._datepicker=new $(M,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom left"})));if(!o){const S=new Date().getFullYear();Q(S)}}W.forEach(c=>c.addEventListener("change",_));const T=s.querySelector("#resign-btn"),F=s.querySelector("#absorb-btn");T&&T.addEventListener("click",()=>I("RESIGNED")),F&&F.addEventListener("click",()=>I("ABSORBED")),s.querySelectorAll('input[type="text"], textarea').forEach(c=>{["id-number-input","full-id-input"].includes(c.id)||c.addEventListener("input",()=>{const v=c.selectionStart,f=c.selectionEnd;c.value=c.value.toUpperCase(),c.setSelectionRange(v,f)})}),P(),_(),oe("education-input","course-suggestions","course-option"),oe("designation-input","work-suggestions","work-option"),oe("office-input","office-suggestions","office-option");const H={publicSector:["DOLE","LGU","DEPED"],privateSector:["DICT","PCA"]},re=(c=[])=>{const v=s.querySelector("#office-suggestions");if(!v)return;const f=B=>String(B||"").trim().toUpperCase(),$=new Set(H.publicSector.map(f)),S=new Set(H.privateSector.map(f)),O=new Set([...$,...S]),V=[...new Set((c||[]).map(f).filter(Boolean))].filter(B=>!O.has(B)),R=(B,N)=>N.length?`
                        <div class="px-2.5 py-1 text-[8px] font-black uppercase tracking-widest ${t.textLabel} opacity-70">${B}</div>
                        ${N.map(X=>`
                            <div class="office-option px-3 py-2 text-[10px] font-bold ${t.textCourseOpt} ${t.courseHover} rounded-md cursor-pointer transition-colors flex items-center gap-2.5 active:scale-[0.98]">
                                <span class="option-text">${X}</span>
                            </div>
                        `).join("")}
                    `:"";v.innerHTML=[R("Public Sector",[...$]),R("Private Sector",[...S]),R("Other Offices",V)].join("")};re(),(async()=>{try{const c=await G("api/beneficiaries.php?get_offices=1");c.success&&c.data?.success&&Array.isArray(c.data.offices)&&re(c.data.offices)}catch(c){console.error("Office suggestion sync failed:",c)}})();function oe(c,v,f){const $=s.querySelector(`#${c}`),S=s.querySelector(`#${v}`);if(!$||!S)return;let O=!1;$.addEventListener("focus",()=>S.classList.remove("hidden")),document.addEventListener("click",A=>{!$.contains(A.target)&&!S.contains(A.target)&&S.classList.add("hidden")}),$.addEventListener("input",()=>{if(O){O=!1;return}const A=$.value.toLowerCase(),V=S.querySelectorAll(`.${f}`);let R=!1;V.forEach(B=>{const N=B.querySelector(".option-text");(N?N.innerText:B.innerText).toLowerCase().includes(A)?(B.style.display="block",R=!0):B.style.display="none"}),R?S.classList.remove("hidden"):S.classList.add("hidden")}),S.addEventListener("click",A=>{const V=A.target.closest(`.${f}`);if(!V)return;const R=V.querySelector(".option-text");$.value=R?R.innerText.trim():V.innerText.trim(),O=!0,S.classList.add("hidden"),$.dispatchEvent(new Event("change"))})}const J=s.querySelector("#add-beneficiary-form"),ve="add_beneficiary_draft";if(!o){const c=localStorage.getItem(ve);if(c)try{const v=JSON.parse(c);Object.entries(v).forEach(([f,$])=>{const S=J.elements[f];S&&S.type!=="file"&&S.type!=="hidden"&&(S.value=$)})}catch(v){console.error("Error loading draft",v)}}J.addEventListener("input",c=>{if(!o){const v=new FormData(J),f={};v.forEach(($,S)=>f[S]=$),localStorage.setItem(ve,JSON.stringify(f))}}),J&&J.addEventListener("submit",c=>{c.preventDefault(),J.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(z=>{z.classList.remove("ring-2","ring-red-500","!border-red-500")});const f=new FormData(J);let $=!1;const S=z=>{const K=J.querySelector(`[name="${z}"]`);K&&K.classList.add("ring-2","ring-red-500","!border-red-500"),$=!0},O=f.get("name"),A=f.get("contact"),V=f.get("startDate"),R=f.get("endDate"),B=(f.get("designation")||"").trim();if((!O||O.trim()===""||/[0-9]/.test(O))&&S("name"),A&&A.trim()!==""&&/[^0-9]/.test(A.replace(/[\s\-\+\(\)]/g,""))&&S("contact"),V||S("startDate"),R||S("endDate"),$)return;const N={};f.forEach((z,K)=>{if(["birthday","startDate","endDate"].includes(K)){const ae=window.__parseFormattedDate(z);if(ae){const fe=ae.getFullYear(),be=String(ae.getMonth()+1).padStart(2,"0"),Me=String(ae.getDate()).padStart(2,"0");N[K]=`${fe}-${be}-${Me}`}else N[K]=z}else N[K]=z}),B||(N.designation="N/A"),N.replacement||(N.replacement="");const X=s.querySelector("#full-id-input")?.value;o?(N.id=e?.id,X&&(N.gip_id=X)):(N.id=null,X&&(N.gip_id=X)),window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(N)){if(!o){const K="add_beneficiary_draft",ae=J.querySelector('[name="office"]')?.value||"",fe=J.querySelector('[name="designation"]')?.value||"",be=J.querySelector('[name="education"]')?.value||"";localStorage.setItem(K,JSON.stringify({office:ae,designation:fe,education:be}))}b.close(),setTimeout(()=>{b.fire({toast:!0,position:"top-end",icon:"success",title:`Record ${o?"Updated":"Added"} Successfully`,showConfirmButton:!1,timer:3e3,timerProgressBar:!0}),!o&&e?._isBulk&&pe.onSaveSuccess()},100)}else b.fire({icon:"error",title:"Save Failed",text:"There was an error saving the record to the database."})})()})}})}window.handleContactSubmit=async function(e){e.preventDefault();const o=e.target,n=o.querySelector('button[type="submit"]'),a=n.innerHTML;n.disabled=!0,n.innerHTML=`
        <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        Sending...
    `;try{const r=new FormData(o);if((await fetch(o.action,{method:"POST",body:r,headers:{Accept:"application/json"}})).ok)b.fire({toast:!0,position:"top-end",icon:"success",title:"Message Sent Successfully!",text:"Thank you for reaching out. I will get back to you soon!",showConfirmButton:!1,timer:4e3,timerProgressBar:!0,didOpen:u=>{u.addEventListener("mouseenter",b.stopTimer),u.addEventListener("mouseleave",b.resumeTimer)},customClass:{popup:"rounded-2xl border-l-4 border-teal-500 shadow-2xl"}}),o.reset();else throw new Error("Failed to send")}catch{b.fire({toast:!0,position:"top-end",icon:"error",title:"Oops! Something went wrong.",text:"Could not send your message. Please try again later.",showConfirmButton:!1,timer:4e3,customClass:{popup:"rounded-2xl border-l-4 border-red-500 shadow-2xl"}})}finally{n.disabled=!1,n.innerHTML=a}return!1};export{Ce as _,G as a,ie as b,rt as c,st as d,at as e,Ke as f,Y as g,nt as h,Se as i,Qe as j,et as k,te as l,tt as m,it as n,Te as p,ot as r,xe as s,Ze as u};
