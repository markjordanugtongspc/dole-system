...
import p from"./vendor-swal-BSk0fVSb.js";import{a as ye}from"./vendor-DHtNC8Ux.js";let F=null;if(ae()){const e="https://llnddycvbcetztzwbdpx.supabase.co",o="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxsbmRkeWN2YmNldHp0endiZHB4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ1MDMwMTEsImV4cCI6MjA5MDA3OTAxMX0.apIpDlZYja7SNuA-joXgFSvu_wtbBcVVP3OQSPrh7fA";try{F=ye(e,o),console.log("[SUPABASE SDK] Client Initialized Successfully")}catch(a){console.error("[SUPABASE SDK] Failed to initialize client:",a)}}else console.log("[SUPABASE SDK] Supabase mode is disabled (Localhost PHP mode active).");const V="color-theme",ke=3600*24*365;function $e(e,o,a){document.cookie=`${e}=${o}; max-age=${a}; path=/; SameSite=Lax`}function ge(e){const o=document.cookie.match(new RegExp("(?:^|; )"+e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+"=([^;]*)"));return o?decodeURIComponent(o[1]):null}function fe(){const e=localStorage.getItem(V)||ge(V);return e==="dark"||e==="light"?e:"light"}function Q(e){const o=document.documentElement;e==="dark"?o.classList.add("dark"):o.classList.remove("dark"),localStorage.setItem(V,e),$e(V,e,ke),Ee(e),document.dispatchEvent(new CustomEvent("themeChanged",{detail:{theme:e}}))}function re(){const e=fe();Q(e==="dark"?"light":"dark")}function Ee(e){const o=e==="dark",a=document.getElementById("pref-dark-mode");a&&(a.checked=o);const s=document.getElementById("theme-toggle-dark-icon"),r=document.getElementById("theme-toggle-light-icon");s&&r&&(s.classList.toggle("hidden",o),r.classList.toggle("hidden",!o));const t=document.getElementById("sidebar-theme-label");t&&(t.textContent=o?"Light Mode":"Dark Mode")}function _e(){const e=fe();Q(e);const o=document.getElementById("pref-dark-mode");o&&o.addEventListener("change",()=>{Q(o.checked?"dark":"light")});const a=document.getElementById("theme-toggle-btn");a&&a.addEventListener("click",re),document.querySelectorAll("[data-theme-toggle]").forEach(s=>{s.addEventListener("click",re)}),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",s=>{localStorage.getItem(V)||ge(V)||Q(s.matches?"dark":"light")})}function _(){return document.documentElement.classList.contains("dark")}window.toggleTheme=re;window.isDarkMode=_;const X=()=>"false".toLowerCase()==="true";function Se(e){try{return JSON.stringify(e)}catch{return"[unserializable]"}}const W={debug(...e){X()&&console.debug(...e)},info(...e){X()&&console.info(...e)},warn(...e){X()&&console.warn(...e)},error(...e){console.error(...e)},table(e){X()&&console.table(e)},json(e,o){X()&&console.debug(e,Se(o))}};async function oe(e,o={}){const s=`${D()}${e}`;let r=null;try{const l=JSON.parse(localStorage.getItem("user"));l&&l.id&&(r=l.id)}catch{}const t={headers:{"Content-Type":"application/json",...r?{"X-User-Id":r}:{},...o.headers},...o};try{if(W.debug("[API] Request",{url:s,method:t.method||"GET",hasUserId:!!r}),t.body)try{W.json("[API] Payload",JSON.parse(t.body))}catch{W.debug("[API] Payload (raw)",t.body)}const l=await fetch(s,t);if(!l.ok)throw new Error(`HTTP ${l.status}: ${l.statusText}`);const n=await l.json();return W.debug("[API] Response",{url:s,ok:!0}),{success:!0,data:n}}catch(l){return W.error("API Request Error:",l),{success:!1,error:l.message}}}async function se(e){return oe(e,{method:"GET"})}async function qe(e,o){return oe(e,{method:"PUT",body:JSON.stringify(o)})}class Ce{constructor(){this.intervals=new Map,this.isPageVisible=!0,document.addEventListener("visibilitychange",()=>{this.isPageVisible=!document.hidden,this.isPageVisible?console.log("[Polling] Page visible - resuming normal activity"):console.log("[Polling] Page hidden - reducing activity")})}start(o,a,s=1e4){this.stop(o),a();const r=setInterval(async()=>{this.isPageVisible&&await a()},s);this.intervals.set(o,r),console.log(`[Polling] Started: ${o} (every ${s}ms)`)}stop(o){this.intervals.has(o)&&(clearInterval(this.intervals.get(o)),this.intervals.delete(o),console.log(`[Polling] Stopped: ${o}`))}stopAll(){this.intervals.forEach((o,a)=>this.stop(a)),console.log("[Polling] All intervals stopped")}getActivePolls(){return Array.from(this.intervals.keys())}}const Be=new Ce;function Fe(){typeof window.initFlowbite=="function"?(window.initFlowbite(),console.log("[Flowbite] Components re-initialized")):console.warn("[Flowbite] initFlowbite() not available globally")}function Ge(e,o="",a="info"){p.fire({toast:!0,position:"top-end",icon:a,title:e,text:o,showConfirmButton:!1,timer:3e3,timerProgressBar:!0})}function Ue(e){return JSON.stringify(e)}window.addEventListener("beforeunload",()=>{Be.stopAll()});const ee={queue:[],currentIndex:0,isActive:!1,isAutoSave:!1,lastInteractionTime:0,init(){this.showUploadModal()},showUploadModal(){const e=_(),o={bgCard:e?"bg-slate-900/40":"bg-gray-50/40",borderCard:e?"border-slate-800":"border-gray-100",textHeading:e?"text-green-500":"text-[#2e7d32]",textSubtitle:e?"text-slate-500":"text-gray-400 dark:text-gray-300",bgUpload:e?"bg-slate-800":"bg-white",borderUpload:e?"border-slate-700":"border-gray-200",textUpload:e?"text-slate-400":"text-gray-500",hoverUpload:e?"hover:bg-slate-800/80 hover:border-blue-500":"hover:bg-blue-50/50 hover:border-royal-blue",iconText:e?"text-green-400":"text-[#2e7d32]",iconBg:e?"bg-green-900/20":"bg-[#e8f5e9]",iconBorder:e?"border-green-800/30":"border-[#c8e6c9]"},a=`
...
<div class="border-b border-default pb-4 mb-5 flex flex-col relative w-full pt-4 font-montserrat user-select-none">
    ...
    <div class="grid grid-cols-2 gap-4 mt-6 w-full">
        ...
        <div class="flex flex-col gap-1 text-left overflow-hidden">
            ...
            <div class="flex items-center flex-wrap gap-1.5 min-h-[30px]">
                <span class="${Ie(e.remarks)} text-[10px] font-black px-2 py-1 rounded border uppercase tracking-widest shadow-sm border-l-4 ${e.remarks==="ONGOING"?"border-l-green-600":"border-l-red-600"}">${e.remarks}</span>
                <!-- Desktop Only Office Badge (Pill Style) -->
                <span class="${le(e.office)} hidden sm:inline-block text-[9px] font-black px-2.5 py-1 rounded-full border shadow-sm truncate max-w-[120px] lg:max-w-none" title="${e.office}">${e.office}</span>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col gap-1 text-left mt-4 sm:hidden">
        ...
        <span class="${le(e.office)} text-[10px] font-black px-2.5 py-2 rounded border border-gray-200 dark:border-slate-700 uppercase tracking-widest shadow-sm border-l-4 border-l-indigo-500 w-full truncate" title="${e.office}">${e.office}</span>
    </div>
</div>
...
<div class="flex flex-col gap-4 text-sm mt-3 px-1">
    ...
    <div class="flex justify-between items-center group">
        ...
        <span class="${e.age||ie(e.birthday)?"font-black text-heading":"font-bold text-gray-300 italic"} text-right">${e.age||ie(e.birthday)||"N/A"}</span>
    </div>
    <div class="flex justify-between items-center group">
