...
import b from"./vendor-swal-BSk0fVSb.js";import{a as Ie}from"./vendor-DHtNC8Ux.js";const De="true".toLowerCase()==="true";function we(){return De}function T(){const e=window.location.pathname,o="/dole-system/",s=e.toLowerCase().indexOf(o.toLowerCase());if(s!==-1)return e.substring(0,s+o.length);const a=e.indexOf("/frontend/");if(a!==-1)return e.substring(0,a+1);const r=e.indexOf("/backend/");return r!==-1?e.substring(0,r+1):"/"}function fe(e="Incorrect Username or Password"){b.fire({html:`
            <div class="p-6">
        ...
        `,timer:e?800:3e3,timerProgressBar:!0,showConfirmButton:!1,showCloseButton:!1,width:"350px",padding:"0",customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl overflow-hidden border border-gray-100",timerProgressBar:"bg-green-500 h-1.5"},backdrop:"rgba(0, 0, 0, 0.4)"})}function We(){const e=localStorage.getItem("hasVisitedBefore"),o=document.getElementById("page-loader");e&&(document.documentElement.classList.add("loaded"),o&&(o.style.display="none")),window.addEventListener("load",()=>{const s=document.querySelector("body > *:not(.page-loader)");s&&s.classList.add("content-fade-in"),setTimeout(()=>{document.documentElement.classList.add("loaded"),o&&o.classList.add("hidden"),document.body.style.overflow="auto",localStorage.setItem("hasVisitedBefore","true")},300)})}const se={_key:"DOLE-GIP-SECURE-KEY-2026",encrypt:function(e){try{const o=r=>r.split("").map(t=>t.charCodeAt(0)),s=r=>("0"+Number(r).toString(16)).substr(-2),a=r=>o(this._key).reduce((t,c)=>t^c,r);return e.split("").map(o).map(a).map(s).join("")}catch(o){return console.error("Encryption Failed",o),null}},decrypt:function(e){try{const o=a=>a.split("").map(r=>r.charCodeAt(0)),s=a=>o(this._key).reduce((r,t)=>r^t,a);return e.match(/.{1,2}/g).map(a=>parseInt(a,16)).map(s).map(a=>String.fromCharCode(a)).join("")}catch(o){return console.error("Decryption Failed",o),null}}};function Ye(){document.querySelectorAll(".login-form-shared").forEach(o=>{const s=o.querySelector('input[name="username"]'),a=o.querySelector('input[name="password"]'),r=o.querySelector('input[name="rememberMe"]');if(s&&a&&r){const t=localStorage.getItem("secure_user"),c=localStorage.getItem("secure_pass");if(t&&c){const n=se.decrypt(t),d=se.decrypt(c);n&&d&&(s.value=n,a.value=d,r.checked=!0)}}o.addEventListener("submit",async t=>{t.preventDefault();try{const n=await(await fetch(`${T()}api/auth.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:s.value,password:a.value})})).json();if(n.success){r.checked?(localStorage.setItem("secure_user",se.encrypt(s.value)),localStorage.setItem("secure_pass",se.encrypt(a.value))):(localStorage.removeItem("secure_user"),localStorage.removeItem("secure_pass"));const d=localStorage.getItem("hasLoggedInBefore")==="true";localStorage.setItem("isLoggedIn","true"),localStorage.setItem("hasLoggedInBefore","true"),localStorage.setItem("user",JSON.stringify(n.user));const p=document.getElementById("drawer-login");if(p){p.classList.add("translate-y-full");const i=p.querySelector("[data-drawer-hide]");i&&i.click()}await Ae(d),Me(d)}else{const d=document.getElementById("drawer-login");d?(d.classList.add("translate-y-full"),setTimeout(()=>{fe(),setTimeout(()=>{d.classList.remove("translate-y-full"),a.value="",a.focus()},600)},400)):(fe(),a.value="",a.focus())}}catch(c){console.error("Login Error:",c),b.fire({icon:"error",title:"Login Error",text:"Unable to reach the server. Please check your connection."})}})})}function Me(e=!1){const o=document.getElementById("left-panel"),s=document.getElementById("right-panel"),a=document.getElementById("left-panel-content"),r=document.getElementById("right-panel-content");a&&(a.style.opacity="0"),r&&(r.style.opacity="0");const t=document.createElement("div");t.className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none";const c=e?"":"animate__delay-1s",n=e?"animation-duration: 0.8s;":"animation-duration: 2s;";t.innerHTML=`
        <img src="${T()}frontend/images/logo/doleiligan.png" class="w-64 h-64 md:w-96 md:h-96 object-contain bg-white/20 backdrop-blur-sm shadow-2xl rounded-full p-4 animate__animated animate__rotateOut ${c}" style="${n}" alt="DOLE Logo">
    `,document.body.appendChild(t);const d=e?0:1e3,p=e?600:1500;setTimeout(()=>{o&&o.classList.add("animate-slide-left"),s&&s.classList.add("animate-slide-right"),setTimeout(()=>{window.location.href=`${T()}frontend/dashboard/`},p)},d)}function Ke(){const e=document.getElementById("logoutBtn");e&&e.addEventListener("click",()=>{localStorage.removeItem("isLoggedIn"),window.location.href=`${T()}`})}function Ze(){const e=document.getElementById("mobile-splash"),o=document.getElementById("show-login-btn"),s=document.getElementById("back-to-splash"),a=document.getElementById("mobile-bg-content"),r=document.getElementById("mobile-welcome-text"),t=document.getElementById("reopen-login-drawer"),c=document.getElementById("request-notifications-btn"),n=async()=>{if("Notification"in window){const m=await Notification.requestPermission();console.log("Notification permission:",m),m==="granted"&&c&&c.classList.add("hidden")}};Notification.permission==="default"&&c&&(c.classList.remove("hidden"),c.addEventListener("click",n));const d=()=>{e&&(e.style.transform="translateY(-100%)",setTimeout(()=>{e.style.visibility="hidden",e.style.pointerEvents="none",e.style.zIndex="-1",Notification.permission==="default"&&n();const m=document.getElementById("drawer-login");m&&m.classList.remove("translate-y-full")},800))},p=()=>{e&&(e.style.zIndex="9999",e.style.pointerEvents="auto",e.style.visibility="visible",e.style.transform="translateY(0)")};o&&o.addEventListener("click",d),document.querySelectorAll(".forgot-password-link").forEach(m=>{m.addEventListener("click",w=>{w.target.closest("#mobile-splash")&&d()})}),s&&s.addEventListener("click",()=>{const m=document.getElementById("drawer-login");if(m){m.classList.add("translate-y-full");const w=m.querySelector("[data-drawer-hide]");w&&w.click()}p()});const l=document.getElementById("drawer-login"),u=document.getElementById("curved-welcome"),f=document.getElementById("peoples-bg");l&&new MutationObserver(w=>{w.forEach(x=>{x.attributeName==="class"&&(l.classList.contains("translate-y-full")?(a&&(a.style.transform="translateY(0)"),r&&(r.style.opacity="1",r.style.transform="translateY(0) scale(1)"),u&&(u.style.opacity="0",u.style.transform="scale(0.5)"),t&&e&&e.style.visibility==="hidden"&&(t.style.opacity="1",t.style.transform="scale(1)"),f&&(f.classList.add("opacity-0","scale-0"),f.classList.remove("opacity-40","scale-[1.6]"))):(a&&(a.style.transform="translateY(-35%)"),r&&(r.style.opacity="0",r.style.transform="translateY(20px) scale(0.9)"),u&&(u.style.opacity="1",u.style.transform="scale(1)"),t&&(t.style.opacity="0",t.style.transform="scale(0)"),f&&(f.classList.remove("opacity-0","scale-0"),f.classList.add("opacity-40","scale-[1.6]"))))})}).observe(l,{attributes:!0})}const re=()=>"false".toLowerCase()==="true";function Ne(e){try{return JSON.stringify(e)}catch{return"[unserializable]"}}const X={debug(...e){re()&&console.debug(...e)},info(...e){re()&&console.info(...e)},warn(...e){re()&&console.warn(...e)},error(...e){console.error(...e)},table(e){re()&&console.table(e)},json(e,o){re()&&console.debug(e,Ne(o))}},ne=new Map;async function oe(e,o={}){const a=`${T()}${e}`;let r=null;try{const i=JSON.parse(localStorage.getItem("user"));i&&(r=i.user_id||i.id||null)}catch{}const t={headers:{"Content-Type":"application/json",...r?{"X-User-Id":r}:{},...o.headers},...o},n=(t.method||"GET").toUpperCase()==="GET"?2:1;let d=null;for(let i=1;i<=n;i++)try{if(X.debug("[API] Request",{url:a,method:t.method||"GET",hasUserId:!!r}),t.body)try{X.json("[API] Payload",JSON.parse(t.body))}catch{X.debug("[API] Payload (raw)",t.body)}const l=await fetch(a,t);if(!l.ok)throw new Error(`HTTP ${l.status}: ${l.statusText}`);const u=await l.json();return ne.has(a)&&(ne.delete(a),X.info?.("[API] Recovered",{url:a})),X.debug("[API] Response",{url:a,ok:!0}),{success:!0,data:u}}catch(l){if(d=l,l instanceof TypeError&&/fetch/i.test(l.message||"")&&i<n){await new Promise(m=>setTimeout(m,1200));continue}}return d instanceof TypeError&&/fetch/i.test(d.message||"")?ne.get(a)||(ne.set(a,!0),X.error("API Request Network Error (suppressed for repeats):",{url:a,message:d.message})):X.error("API Request Error:",d),{success:!1,error:d?.message||"Unknown request error"}}async function F(e){return oe(e,{method:"GET"})}async function ue(e,o){return oe(e,{method:"POST",body:JSON.stringify(o)})}async function je(e,o){return oe(e,{method:"PUT",body:JSON.stringify(o)})}async function Qe(e,o){const s=new URLSearchParams(o).toString();return oe(`${e}?${s}`,{method:"PATCH"})}class Oe{constructor(){this.intervals=new Map,this.isPageVisible=!0,document.addEventListener("visibilitychange",()=>{this.isPageVisible=!document.hidden,this.isPageVisible?console.log("[Polling] Page visible - resuming normal activity"):console.log("[Polling] Page hidden - reducing activity")})}start(o,s,a=1e4){this.stop(o),s();const r=setInterval(async()=>{this.isPageVisible&&await s()},a);this.intervals.set(o,r),console.log(`[Polling] Started: ${o} (every ${a}ms)`)}stop(o){this.intervals.has(o)&&(clearInterval(this.intervals.get(o)),this.intervals.delete(o),console.log(`[Polling] Stopped: ${o}`))}stopAll(){this.intervals.forEach((o,s)=>this.stop(s)),console.log("[Polling] All intervals stopped")}getActivePolls(){return Array.from(this.intervals.keys())}}const Pe=new Oe;function et(){typeof window.initFlowbite=="function"?(window.initFlowbite(),console.log("[Flowbite] Components re-initialized")):console.warn("[Flowbite] initFlowbite() not available globally")}function tt(e,o="",s="info"){b.fire({toast:!0,position:"top-end",icon:s,title:e,text:o,showConfirmButton:!1,timer:3e3,timerProgressBar:!0})}function rt(e){return JSON.stringify(e)}window.addEventListener("beforeunload",()=>{Pe.stopAll()});const Re="modulepreload",Te=function(e,o){return new URL(e,o).href},be={},ye=function(o,s,a){let r=Promise.resolve();if(s&&s.length>0){let p=function(i){return Promise.all(i.map(l=>Promise.resolve(l).then(u=>({status:"fulfilled",value:u}),u=>({status:"rejected",reason:u}))))};const c=document.getElementsByTagName("link"),n=document.querySelector("meta[property=csp-nonce]"),d=n?.nonce||n?.getAttribute("nonce");r=p(s.map(i=>{if(i=Te(i,a),i in be)return;be[i]=!0;const l=i.endsWith(".css"),u=l?'[rel="stylesheet"]':"";if(a)for(let m=c.length-1;m>=0;m--){const w=c[m];if(w.href===i&&(!l||w.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${i}"]${u}`))return;const f=document.createElement("link");if(f.rel=l?"stylesheet":Re,l||(f.as="script"),f.crossOrigin="",f.href=i,d&&f.setAttribute("nonce",d),document.head.appendChild(f),l)return new Promise((m,w)=>{f.addEventListener("load",m),f.addEventListener("error",()=>w(new Error(`Unable to preload CSS for ${i}`)))})}))}function t(c){const n=new Event("vite:preloadError",{cancelable:!0});if(n.payload=c,window.dispatchEvent(n),!n.defaultPrevented)throw c}return r.then(c=>{for(const n of c||[])n.status==="rejected"&&t(n.reason);return o().catch(t)})};let pe=null;if(we()){const e="https://llnddycvbcetztzwbdpx.supabase.co",o="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxsbmRkeWN2YmNldHp0endiZHB4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ1MDMwMTEsImV4cCI6MjA5MDA3OTAxMX0.apIpDlZYja7SNuA-joXgFSvu_wtbBcVVP3OQSPrh7fA";try{pe=Ie(e,o),console.log("[SUPABASE SDK] Client Initialized Successfully")}catch(s){console.error("[SUPABASE SDK] Failed to initialize client:",s)}}else console.log("[SUPABASE SDK] Supabase mode is disabled (Localhost PHP mode active).");const ee="color-theme",_e=3600*24*365;function qe(e,o,s){document.cookie=`${e}=${o}; max-age=${s}; path=/; SameSite=Lax`}function ke(e){const o=document.cookie.match(new RegExp("(?:^|; )"+e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+"=([^;]*)"));return o?decodeURIComponent(o[1]):null}function Ee(){const e=localStorage.getItem(ee)||ke(ee);return e==="dark"||e==="light"?e:"light"}function le(e){const o=document.documentElement;e==="dark"?o.classList.add("dark"):o.classList.remove("dark"),localStorage.setItem(ee,e),qe(ee,e,_e),Fe(e),document.dispatchEvent(new CustomEvent("themeChanged",{detail:{theme:e}}))}function ge(){const e=Ee();le(e==="dark"?"light":"dark")}function Fe(e){const o=e==="dark",s=document.getElementById("pref-dark-mode");s&&(s.checked=o);const a=document.getElementById("theme-toggle-dark-icon"),r=document.getElementById("theme-toggle-light-icon");a&&r&&(a.classList.toggle("hidden",o),r.classList.toggle("hidden",!o));const t=document.getElementById("sidebar-theme-label");t&&(t.textContent=o?"Light Mode":"Dark Mode")}function ot(){const e=Ee();le(e);const o=document.getElementById("pref-dark-mode");o&&o.addEventListener("change",()=>{le(o.checked?"dark":"light")});const s=document.getElementById("theme-toggle-btn");s&&s.addEventListener("click",ge),document.querySelectorAll("[data-theme-toggle]").forEach(a=>{a.addEventListener("click",ge)}),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",a=>{localStorage.getItem(ee)||ke(ee)||le(a.matches?"dark":"light")})}function z(){return document.documentElement.classList.contains("dark")}window.toggleTheme=ge;window.isDarkMode=z;const de={queue:[],currentIndex:0,isActive:!1,isAutoSave:!1,lastInteractionTime:0,init(){this.showUploadModal()},showUploadModal(){const e=z(),o={bgCard:e?"bg-slate-900/40":"bg-gray-50/40",borderCard:e?"border-slate-800":"border-gray-100",textHeading:e?"text-green-500":"text-[#2e7d32]",textSubtitle:e?"text-slate-500":"text-gray-400 dark:text-gray-300",bgUpload:e?"bg-slate-800":"bg-white",borderUpload:e?"border-slate-700":"border-gray-200",textUpload:e?"text-slate-400":"text-gray-500",hoverUpload:e?"hover:bg-slate-800/80 hover:border-blue-500":"hover:bg-blue-50/50 hover:border-royal-blue",iconText:e?"text-green-400":"text-[#2e7d32]",iconBg:e?"bg-green-900/20":"bg-[#e8f5e9]",iconBorder:e?"border-green-800/30":"border-[#c8e6c9]"},s=`
        ...
        `;b.fire({html:s,width:"600px",showConfirmButton:!1,showCloseButton:!0,customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:a=>{const r=a.querySelector("#csv-upload"),t=a.querySelector('label[for="csv-upload"]');["dragenter","dragover","dragleave","drop"].forEach(n=>{t.addEventListener(n,c,!1)});function c(n){n.preventDefault(),n.stopPropagation()}["dragenter","dragover"].forEach(n=>{t.addEventListener(n,()=>{t.classList.add("border-blue-500","bg-blue-50/50"),e&&t.classList.add("dark:bg-slate-800/80")},!1)}),["dragleave","drop"].forEach(n=>{t.addEventListener(n,()=>{t.classList.remove("border-blue-500","bg-blue-50/50"),e&&t.classList.remove("dark:bg-slate-800/80")},!1)}),r.addEventListener("change",n=>{const d=n.target.files[0];if(d){const p=a.querySelector("#auto-save-toggle");this.isAutoSave=p?p.checked:!1,this.handleFile(d)}}),t.addEventListener("drop",n=>{const p=n.dataTransfer.files[0];if(p){const i=a.querySelector("#auto-save-toggle");this.isAutoSave=i?i.checked:!1,this.handleFile(p)}},!1)}})},handleFile(e){if(!e.name.toLowerCase().endsWith(".csv")&&!e.name.toLowerCase().endsWith(".txt")){b.fire("Invalid File","Please upload a valid .csv or .txt file.","error");return}const o=new FileReader;o.onload=s=>{const a=s.target.result;this.parseCSV(a)},o.readAsText(e)},async parseCSV(e){let o=[],s="",a=!1;for(let r=0;r<e.length;r++){let t=e[r];t==='"'&&(a=!a),!a&&(t===`
`||t==="\r")?(t==="\r"&&e[r+1]===`
`&&r++,s.trim()!==""&&o.push(s),s=""):s+=t}s.trim()!==""&&o.push(s),this.queue=[];for(let r=0;r<o.length;r++){let t=o[r].trim();if(!t)continue;let c=[],n="",d=!1;for(let p=0;p<t.length;p++){let i=t[p];i==='"'?d=!d:i===","&&!d?(c.push(n.replace(/(^"|"$)/g,"").trim()),n=""):n+=i}if(c.push(n.replace(/(^"|"$)/g,"").trim()),c.length>=2){const p=c[3];if(!p||isNaN(parseInt(p)))continue;const i=c[1];if(!i||i.toLowerCase()==="name"||i.toLowerCase()==="full name")continue;const l=c[2];let u=c[4]?c[4].toUpperCase().trim():"",f="";(u==="F"||u.includes("FEMALE"))&&(f="Female"),(u==="M"||u.includes("MALE"))&&(f="Male");const m=c[5],w=c[6],x=c[7],C=this.formatDate(c[8]),A=this.formatDate(c[9]);this.queue.push({name:i,address:l,age:p,gender:f,education:m,startDate:C,endDate:A,office:w,designation:x})}}if(this.queue.length>0){try{b.fire({title:"Checking duplicates...",html:'<p class="text-sm">Please wait while we cross-reference your data.</p>',allowOutsideClick:!1,didOpen:()=>{b.showLoading()}});const r=this.queue.map(n=>n.name),c=await(await fetch(`${T()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({names:r})})).json();if(c.success&&c.duplicates&&c.duplicates.length>0){const n=new Set(c.duplicates.map(p=>p.toLowerCase().trim())),d=this.queue.length;this.queue=this.queue.filter(p=>{const i=n.has(p.name.toLowerCase().trim());return i&&console.warn(`%c[Bulk Add] SKIPPED: ${p.name} already exists in database.`,"color: #ff9800; font-weight: bold;"),!i}),console.log(`[Bulk Add] Removed ${d-this.queue.length} duplicates ahead of time.`)}}catch(r){console.error("Bulk duplicate check failed:",r)}if(this.queue.length===0){b.fire("All Skipped","All beneficiaries in the CSV already exist in the database.","info");return}this.isActive=!0,this.currentIndex=0,b.close(),this.processNext()}else b.fire("Error","No valid beneficiary data found. Please ensure your CSV formatting matches the requirements.","error")},showProgressModal(){z();const e=Math.round(this.currentIndex/this.queue.length*100),o=`
        ...
        `;if(b.isVisible()&&b.getPopup().querySelector("#bulk-progress-bar")){const s=document.getElementById("bulk-progress-bar"),a=b.getPopup().querySelector("span.text-\\[10px\\]"),r=document.getElementById("bulk-current-name");s&&(s.style.width=`${e}%`),a&&(a.textContent=`${this.currentIndex} / ${this.queue.length}`),r&&(r.textContent=this.queue[this.currentIndex]?.name||"...")}else b.fire({html:o,showConfirmButton:!1,allowOutsideClick:!1,allowEscapeKey:!1,width:"450px",customClass:{container:"font-montserrat",popup:"rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-800"},didOpen:s=>{s.querySelector("#stop-bulk-btn").onclick=()=>this.onCancel()}})},formatDate(e){if(!e||e.trim()==="")return"";const o=new Date(e);if(isNaN(o.getTime())){const t=e.split("/");return t.length===3?`${t[2]}-${t[1].padStart(2,"0")}-${t[0].padStart(2,"0")}`:""}const s=o.getFullYear(),a=String(o.getMonth()+1).padStart(2,"0"),r=String(o.getDate()).padStart(2,"0");return`${s}-${a}-${r}`},async processNext(){if(this.currentIndex<this.queue.length){const e=this.queue[this.currentIndex];e._isBulk=!0,e._bulkCurrent=this.currentIndex+1,e._bulkTotal=this.queue.length,this.isAutoSave?(this.showProgressModal(),window.addBeneficiaryData?(async()=>{try{const s=e.startDate?new Date(e.startDate).getFullYear():new Date().getFullYear(),[a,r]=await Promise.all([F(`api/beneficiaries.php?next_id&year=${s}`),F(`api/beneficiaries.php?next_series_no&year=${s}`)]);a.success&&a.data?.success&&a.data?.nextId&&(e.gip_id=a.data.nextId,e.id=null),r.success&&r.data?.success&&r.data?.nextSeries&&(e.seriesNo=r.data.nextSeries)}catch(s){console.warn("[Bulk Add] Identifier fetch failed, continuing:",s?.message||s)}const o=await window.addBeneficiaryData(e);this.isActive&&(o?this.onSaveSuccess():ce(e))})():ce(e)):ce(e)}else this.isActive=!1,this.lastInteractionTime=Date.now(),b.fire({icon:"success",title:"Bulk Add Complete!",text:`Successfully processed ${this.queue.length} beneficiaries.`,confirmButtonColor:"#2e7d32"})},onSaveSuccess(){if(this.isActive){this.currentIndex++;const e=this.isAutoSave?100:1500;setTimeout(()=>{this.processNext()},e)}},onCancel(){this.isActive&&(this.isActive=!1,this.lastInteractionTime=Date.now(),this.queue=[],this.currentIndex=0,b.fire({icon:"info",title:"Bulk Add Cancelled",text:"Stopped processing the remaining beneficiaries and cleared the queue.",confirmButtonColor:"#3085d6"}).then(()=>{this.showUploadModal()}))}};window.BulkApp=de;function he(e){if(!e)return"";const o=new Date(e),s=new Date;let a=s.getFullYear()-o.getFullYear();const r=s.getMonth()-o.getMonth();return(r<0||r===0&&s.getDate()<o.getDate())&&a--,a>=0?a:0}function xe(e){return e?e.includes("DOLE")?"bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/40 dark:text-blue-300 dark:border-blue-800":e.includes("DepEd")?"bg-orange-100 text-orange-700 border-orange-200 dark:bg-orange-900/40 dark:text-orange-300 dark:border-orange-800":e.includes("LGU")?"bg-purple-100 text-purple-700 border-purple-200 dark:bg-purple-900/40 dark:text-purple-300 dark:border-purple-800":e.includes("DICT")?"bg-cyan-100 text-cyan-700 border-cyan-200 dark:bg-cyan-900/40 dark:text-cyan-300 dark:border-cyan-800":"bg-gray-100 text-gray-700 border-gray-200 dark:text-gray-300":"bg-gray-100 text-gray-700 border-gray-200 dark:text-gray-300"}function Ge(e){if(!e)return"bg-gray-100 text-gray-600 border-gray-200 dark:text-gray-300";const o=e.toUpperCase();return o==="ONGOING"?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800":o==="EXPIRED"?"bg-red-400 text-white border-red-400 dark:bg-red-900/60 dark:border-red-800":o==="RESIGNED"?"bg-[#ce1126] text-white border-[#ce1126] dark:bg-red-900/80 dark:border-red-900":o==="ABSORBED"?"bg-[#2e7d32] text-white border-[#2e7d32] dark:bg-green-900/80 dark:border-green-900":"bg-gray-100 text-gray-600 border-gray-200 dark:text-gray-300"}function ie(e,o=0){e={...e,id:e?.id||e?.gip_id||"N/A",name:e?.name||"N/A",office:e?.office||"N/A",remarks:e?.remarks||"N/A",designation:e?.designation||"N/A"};const s=window.innerWidth<640?"top":"top-start";let a=o;const r=e.arLogs||[],t=e.dtrLogs||[],c=e.docs||[],n=["GIP FORM","BIRTH CERTIFICATE","DIPLOMA","TOR","VALID ID"],d=n.map(l=>{const u=c.find(f=>f.name.toUpperCase()===l.toUpperCase());return u||{name:l,status:"PENDING",id:null}});c.forEach(l=>{n.some(f=>f.toUpperCase()===l.name.toUpperCase())||d.push(l)});const p=`
<div class="border-b border-default pb-4 mb-5 flex flex-col relative w-full pt-4 font-montserrat user-select-none">
    ...
    <div class="grid grid-cols-2 gap-4 mt-6 w-full">
        ...
        <div class="flex flex-col gap-1 text-left overflow-hidden">
            ...
            <div class="flex items-center flex-wrap gap-1.5 min-h-[30px]">
                <span class="${Ge(e.remarks)} text-[10px] font-black px-2 py-1 rounded border uppercase tracking-widest shadow-sm border-l-4 ${e.remarks==="ONGOING"?"border-l-green-600":"border-l-red-600"}">${e.remarks}</span>
                <!-- Desktop Only Office Badge (Pill Style) -->
                <span class="${xe(e.office)} hidden sm:inline-block text-[9px] font-black px-2.5 py-1 rounded-full border shadow-sm truncate max-w-[120px] lg:max-w-none" title="${e.office}">${e.office}</span>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col gap-1 text-left mt-4 sm:hidden">
        ...
        <span class="${xe(e.office)} text-[10px] font-black px-2.5 py-2 rounded border border-gray-200 dark:border-slate-700 uppercase tracking-widest shadow-sm border-l-4 border-l-indigo-500 w-full truncate" title="${e.office}">${e.office}</span>
    </div>
</div>
...
<div class="flex flex-col gap-4 text-sm mt-3 px-1">
    ...
    <div class="flex justify-between items-center group">
        ...
        <span class="${e.age||he(e.birthday)?"font-black text-heading":"font-bold text-gray-300 italic"} text-right">${e.age||he(e.birthday)||"N/A"}</span>
    </div>
    <div class="flex justify-between items-center group">
...
<div class="mt-8 pt-6 border-t border-default relative">
    ...
    <div id="drawer-page-1" class="hidden flex-1 flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-3">
            ${d.map(l=>{const u=l.status.toUpperCase(),m={VERIFIED:"COMPLETED",DECLINED:"REJECTED",PENDING:"PENDING"}[u]||u,w=m==="COMPLETED",x=m==="REJECTED",C=w?"text-green-500":x?"text-red-500":"text-gray-400 dark:text-gray-500",A=w?"bg-green-50/50 dark:bg-green-900/10":x?"bg-red-50/50 dark:bg-red-900/10":"bg-gray-50/50 dark:bg-slate-800/50",M=w?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800 hover:bg-green-200 cursor-pointer":x?"bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800 hover:bg-red-200 cursor-pointer":"bg-white text-gray-500 border-gray-200 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer";let N='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';return w?N='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>':x&&(N='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>'),`
                <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-0.5 ${A}">
                    <div class="flex items-center gap-3 w-full">
                        <div class="flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center bg-white dark:bg-slate-800 shadow-sm border border-gray-100 dark:border-slate-700 ${C}">
                            ${N}
                        </div>
                        <span class="text-xs sm:text-sm font-black ${w?"text-heading":"text-gray-500 dark:text-gray-400"} uppercase tracking-tight flex-1">${l.name}</span>
                    </div>
                    <button type="button" class="ml-3 ${M} text-[10px] font-black px-3 py-1.5 rounded-full border uppercase tracking-widest transition-colors flex-shrink-0 drawer-doc-btn" data-id="${l.id}" data-name="${l.name}" data-status="${m} cursosr-pointer">
                        ${m}
                    </button>
                </div>
                `}).join("")}
    ...
    <div id="drawer-page-2" class="hidden flex-1 flex flex-col gap-6">
         ...
         <div class="flex flex-col gap-5">
            <div>
                ...
                <div class="space-y-2">
                    ${t.length?t.map(l=>{const u=l.status||"PENDING";let f=u==="VERIFIED"||u==="COMPLETED"?"text-green-500":u==="REJECTED"||u==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500";return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-blue-100 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors edit-log-btn" data-type="dtr" data-id="${l.id}" data-val="${l.date||l.createdAt}" data-status="${u}">
                            ...
                            <span class="text-[11px] font-bold ${f} uppercase tracking-widest truncate max-w-[50%] text-right pr-6 group-hover:pr-12 pointer-events-none transition-all">${u}</span>
            ...
            <div>
                ...
                <div class="space-y-2">
                    ${r.length?r.map(l=>{const u=l.status||"PENDING";let f=u==="VERIFIED"||u==="COMPLETED"?"text-green-500":u==="REJECTED"||u==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500";return`
...
<style>
    ...
    `;let i=document.getElementById("beneficiary-drawer-container");i&&i.remove(),i=document.createElement("div"),i.id="beneficiary-drawer-container",i.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-neutral-primary-soft dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl",i.setAttribute("tabindex","-1"),i.setAttribute("data-drawer-backdrop","true"),i.innerHTML=p,document.body.appendChild(i),ye(async()=>{const{Drawer:l}=await import("./vendor-flowbite-bBKPL--D.js");return{Drawer:l}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:l})=>{const u={placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{i&&i.parentNode&&i.remove()},300)}},f=new l(i,u);f.show(),i.querySelector("#close-drawer-btn").addEventListener("click",()=>f.hide());const w=i.querySelector("#drawer-prev-btn"),x=i.querySelector("#drawer-next-btn"),C=2,A=()=>{i.querySelectorAll("[id^=drawer-page-]").forEach((y,B)=>{y.classList.toggle("hidden",B!==a)}),w.disabled=a===0,x.disabled=a===C,w.classList.toggle("opacity-50",a===0),x.classList.toggle("opacity-50",a===C)};w.addEventListener("click",()=>{a>0&&a--,A()}),x.addEventListener("click",()=>{a<C&&a++,A()}),A(),i.querySelectorAll(".drawer-doc-btn").forEach(y=>{y.addEventListener("click",async()=>{const B=y.dataset.name,E=y.dataset.status,L=await b.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Update Document</span>',html:`
                    ...
                    `,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest">Update</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest">Cancel</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs px-6 py-2.5 rounded-xl border border-gray-200 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1,preConfirm:()=>document.getElementById("swal-doc-status").value});if(L.isConfirmed){const $=L.value;if($===E)return;try{const U={COMPLETED:"VERIFIED",REJECTED:"DECLINED",PENDING:"PENDING"}[$]||$,H=await ue("api/logs.php?type=docs",{gip_id:e.id,doc_name:B,status:U}),V=H.success?H.data:{success:!1,error:H.error};V.success?(b.fire({toast:!0,position:s,icon:"success",title:"Status updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):b.fire("Error",V.error||"Failed to update","error")}catch(O){b.fire("Error",O.message,"error")}}})});const M=()=>{if(!t.length)return new Date().toISOString().split("T")[0];const y=new Date(t[0].date);return y.setDate(y.getDate()+1),y.toISOString().split("T")[0]},N=()=>{if(!r.length){const L=new Date,$=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"][L.getMonth()];return L.getDate()<=15?`${$} 1-15`:`${$} 16-${new Date(L.getFullYear(),L.getMonth()+1,0).getDate()}`}const y=r[0].period||"",B=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"],E=y.toUpperCase().match(/([A-Z]{3})\s+(\d+)-(\d+)/);if(E){let L=E[1],$=parseInt(E[2]),O=B.indexOf(L);if(O!==-1)return $===1?`${L} 16-${new Date(new Date().getFullYear(),O+1,0).getDate()}`:`${B[(O+1)%12]} 1-15`}return"NEW PERIOD"},j=async(y,B)=>{b.fire({title:"Adding...",allowOutsideClick:!1,showConfirmButton:!1}),b.showLoading();try{const E={gip_id:e.id};y==="dtr"&&(E.record_date=B),y==="ar"&&(E.period=B);const L=await ue(`api/logs.php?type=${y}`,E);(L.success?L.data:{success:!1,error:L.error}).success?(b.fire({toast:!0,position:s,icon:"success",title:"Auto-Added!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):b.fire("Error","Failed to add log.","error")}catch(E){b.fire("Error",E.message,"error")}},G=async(y,B,E,L,$)=>{const O=B==="dtr"?"Record Date":"Period";z();const U="flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[10px] sm:text-xs gap-2 ",H='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>',V='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>',{value:q}=await b.fire({title:`<span class="text-xl font-black text-heading uppercase tracking-tight">Edit ${y} Log</span>`,html:`
                    <div class="flex flex-col gap-5 text-left font-montserrat mt-2">
                        <div>
                            <label class="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-1">${O}</label>
                            <input id="swal-log-val" value="${L}" class="bg-gray-50 dark:bg-slate-800/80 border border-gray-200 dark:border-slate-600 text-heading text-sm rounded-xl focus:ring-brand focus:border-brand block w-full p-2.5 font-bold uppercase transition-all" placeholder="Enter value">
                        </div>
                        <div>
                            <label class="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-2">Validation Status</label>
                            <div class="grid grid-cols-2 gap-3">
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-log-status" value="VERIFIED" class="peer sr-only" ${$==="VERIFIED"?"checked":""}>
                                    <div class="${U} border-gray-100 bg-gray-50 text-gray-400 dark:border-slate-700 dark:bg-slate-800/50 dark:text-gray-500 peer-checked:border-green-500 peer-checked:bg-green-50 peer-checked:text-green-600 dark:peer-checked:border-green-500 dark:peer-checked:bg-green-900/20 dark:peer-checked:text-green-400 hover:bg-gray-100 dark:hover:bg-slate-700 group">
                                        ${H}
                                        <span>Verify</span>
                                    </div>
                                </label>
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-log-status" value="REJECTED" class="peer sr-only" ${$==="REJECTED"||$==="DECLINED"?"checked":""}>
                ...
                `,focusConfirm:!1,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">SAVE REVISIONS</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">CANCEL</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2"},buttonsStyling:!1,preConfirm:()=>{const D=document.querySelector('input[name="swal-log-status"]:checked');return{val:document.getElementById("swal-log-val").value.trim().toUpperCase(),status:D?D.value:"PENDING"}}});if(q&&(q.val!==L||q.status!==$))try{const D={type:B,id:E,status:q.status};B==="dtr"&&(D.record_date=q.val),B==="ar"&&(D.period=q.val);const W=await je("api/logs.php",D),g=W.success?W.data:{success:!1,error:W.error};g.success?(b.fire({toast:!0,position:s,icon:"success",title:"Log Updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):b.fire("Error",g.error||"Failed to update log.","error")}catch(D){b.fire("Error",D.message,"error")}},I=i.querySelector("#add-dtr-log-btn");I&&I.addEventListener("click",()=>j("dtr",M()));const _=i.querySelector("#add-ar-log-btn");_&&_.addEventListener("click",()=>j("ar",N())),i.querySelectorAll(".edit-log-btn").forEach(y=>{y.addEventListener("click",B=>{if(B.target.closest(".delete-log-btn"))return;const E=y.dataset.type,L=y.dataset.id,$=y.dataset.val,O=y.dataset.status;G(E.toUpperCase(),E,L,$,O)})}),i.querySelectorAll(".delete-log-btn").forEach(y=>{y.addEventListener("click",async()=>{const B=y.dataset.id,E=y.dataset.type;if((await b.fire({title:'<span class="text-xl font-black text-philippine-red uppercase tracking-tight">Delete item?</span>',text:"This action cannot be undone.",icon:"warning",showCancelButton:!0,confirmButtonText:'<span class="font-black tracking-widest uppercase">Delete</span>',cancelButtonText:'<span class="font-black tracking-widest uppercase">Wait</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100",confirmButton:"bg-philippine-red text-white hover:bg-red-700 text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs px-6 py-2.5 rounded-xl border border-gray-200 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1})).isConfirmed)try{const $=await ue(`api/logs.php?type=${E}`,{log_id:B,action:"delete"});($.success?$.data:{success:!1,error:$.error}).success?(b.fire({toast:!0,position:s,icon:"success",title:"Deleted",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):b.fire("Error","Failed to delete data.","error")}catch($){b.fire("Error",$.message,"error")}})})}).catch(console.error)}function Ue(e){const o=z(),s=`w-full bg-transparent border-b-2 ${o?"border-slate-700 text-white focus:border-brand placeholder-slate-600":"border-gray-200 text-gray-900 focus:border-brand placeholder-gray-300"} px-1 py-1 text-sm font-black outline-none transition-all focus:ring-0`,a=`w-full bg-transparent border-none ${o?"text-white":"text-royal-blue"} px-0 py-0 text-xl sm:text-2xl font-black leading-tight tracking-tight focus:ring-0 outline-none placeholder-gray-300 resize-none overflow-hidden`,r=["DOLE Field Office","LGU","DEPED","DICT","PCA"];function t(d){if(!d)return"";const p=new Date(d),i=new Date;let l=i.getFullYear()-p.getFullYear();const u=i.getMonth()-p.getMonth();return(u<0||u===0&&i.getDate()<p.getDate())&&l--,l>=0?l:0}const c=`
    ...
    <div class="flex flex-col gap-4 text-sm mt-3 px-1 pb-24">
        ...
        <div class="flex flex-col gap-2 pt-3 mt-1 border-t border-gray-50 dark:border-slate-800/60">
            ...
            <div id="edit-education-suggestions-box" class="hidden mt-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                ${$e.map(d=>`<button type="button" class="edit-education-option w-full text-left px-3 py-2 text-[11px] font-bold text-slate-700 dark:text-slate-200 hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer"><span class="option-text">${d.name}</span></button>`).join("")}
            </div>
        </div>
    </div>
...
.editable-indicator {
    ...
    `;let n=document.getElementById("edit-drawer-container");n&&n.remove(),n=document.createElement("div"),n.id="edit-drawer-container",n.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-white dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",n.setAttribute("tabindex","-1"),n.innerHTML=c,document.body.appendChild(n),setTimeout(()=>{const d=n.querySelector('textarea[name="name"]');d&&(d.style.height="auto",d.style.height=d.scrollHeight+"px")},10),(async()=>{try{const d=await oe("api/beneficiaries.php?get_offices=1");if(d.success&&d.data.offices){const p=n.querySelector("#edit-office-suggestions-box");if(p){const i=["DOLE Field Office","LGU","DEPED","DICT","PCA"],l=[...new Set([...i,...d.data.offices])];p.innerHTML=l.map(u=>`
                    ...
                    `).join("")}}}catch(d){console.error("Error fetching office suggestions:",d)}})(),ye(async()=>{const{Drawer:d}=await import("./vendor-flowbite-bBKPL--D.js");return{Drawer:d}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:d})=>{const p=new d(n,{placement:"right",backdrop:!0,bodyScrolling:!1,backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{n.parentNode&&n.remove()},300)}});p.show();const i=()=>{document.activeElement&&typeof document.activeElement.blur=="function"&&document.activeElement.blur(),p.hide()};n.querySelector("#close-edit-drawer-btn").addEventListener("click",i),n.querySelector("#edit-drawer-cancel-btn").addEventListener("click",i);const l=n.querySelector("#edit-beneficiary-drawer-form"),u=n.querySelector("#edit-bday-input"),f=n.querySelector("#edit-age-display");let m=!1;u&&f&&(f.addEventListener("input",()=>{m=!0}),u.addEventListener("change",x=>{x.target.value&&(!m||!f.value)&&(f.value=t(x.target.value))}));const w=(x,C,A)=>{const M=n.querySelector(x),N=n.querySelector(C);if(!M||!N)return;const j=()=>N.classList.add("hidden"),G=()=>N.classList.remove("hidden");M.addEventListener("focus",G),M.addEventListener("input",()=>{const I=M.value.toLowerCase().trim();let _=0;N.querySelectorAll(A).forEach(y=>{const E=(y.querySelector(".option-text")?.textContent||y.textContent||"").toLowerCase().includes(I);y.style.display=E?"block":"none",E&&_++}),_>0?G():j()}),N.addEventListener("click",I=>{const _=I.target.closest(A);_&&(M.value=(_.querySelector(".option-text")?.textContent||_.textContent||"").trim(),j(),M.dispatchEvent(new Event("change")))}),document.addEventListener("click",I=>{!M.contains(I.target)&&!N.contains(I.target)&&j()})};w("#edit-education-input","#edit-education-suggestions-box",".edit-education-option"),w("#edit-designation-input","#edit-designation-suggestions-box",".edit-designation-option"),w("#edit-office-input","#edit-office-suggestions-box",".edit-office-option"),l.querySelectorAll("input, select, textarea").forEach(x=>{const C=(x.getAttribute("type")||"").toLowerCase(),A=!x.disabled&&!x.readOnly&&C!=="hidden";x.classList.remove("editable-indicator"),A&&x.classList.add("editable-indicator")}),l.addEventListener("submit",x=>{x.preventDefault();const C=new FormData(l),A={};C.forEach((M,N)=>A[N]=M),A.id=e.id,A.gip_id=e.id,window.addBeneficiaryData&&(async()=>await window.addBeneficiaryData(A,!0,!1)&&(i(),window.viewBeneficiary&&setTimeout(()=>window.viewBeneficiary({id:e.id},0),100),b.fire({toast:!0,position:"top-end",icon:"success",title:"Record Updated",showConfirmButton:!1,timer:3e3})))()})})}function at(){window.viewBeneficiary=async function(e,o=0){const s=e?.id||e?.gip_id||null;if(!s)return;const a=!!(e?.name&&e?.office&&e?.remarks);let r={...e,id:s};if(!a){const n=await F(`api/beneficiaries.php?id=${encodeURIComponent(s)}`);n.success&&n.data?.success&&n.data?.beneficiary&&(r={...n.data.beneficiary,...r,id:s})}const t=`logs_cache_${s}`;let c=!1;try{if(window.__doleDB&&window.__doleDB.getSecureCache){const x=await window.__doleDB.getSecureCache(t);x&&(r.arLogs=x.arLogs||[],r.dtrLogs=x.dtrLogs||[],r.docs=x.docs||[],ie(r,o),c=!0)}let n=[],d=[],p=[];const[i,l,u,f]=await Promise.all([F(`api/logs.php?type=ar&gip_id=${encodeURIComponent(s)}`),F(`api/logs.php?type=dtr&gip_id=${encodeURIComponent(s)}`),F(`api/logs.php?type=docs&gip_id=${encodeURIComponent(s)}`),F(`api/logs.php?type=absorption&gip_id=${encodeURIComponent(s)}`)]);n=i.success&&i.data?.success?i.data.logs:[],d=l.success&&l.data?.success?l.data.logs:[],p=u.success&&u.data?.success?u.data.logs:[];const m=f.success&&f.data?.success?f.data.logs:[];if(m.length>0){const x=m[0];r.absorbDate=x.absorption_datetime,r.absorb_where=x.where||x.absorb_where,r.absorb_position=x.position||x.absorb_position,r.absorb_agency=x.agency||x.absorb_agency}const w={arLogs:n,dtrLogs:d,docs:p};window.__doleDB&&window.__doleDB.setSecureCache&&await window.__doleDB.setSecureCache(t,w),r.arLogs=n,r.dtrLogs=d,r.docs=p,c?document.getElementById("beneficiary-drawer-container")&&ie(r,o):ie(r,o)}catch(n){console.error("Error fetching logs/docs:",n),c||(r.arLogs=[],r.dtrLogs=[],r.docs=[],ie(r,o))}},window.showAddDataModal=function(e){ce(e)},window.editBeneficiary=function(e){Ue(e)},window.showExportConfigModal=function(e){Ve(e)},window.showProfileModal=function(){He()}}async function He(){try{if(we()&&pe){let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=a.id)}catch{}if(!e)throw new Error("User not logged in");const{data:o,error:s}=await pe.from("users").select("*").eq("id",e).single();if(s)throw s;me(o)}else{let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=`?user_id=${a.id}`)}catch{}const s=await(await fetch(`${T()}api/profile.php${e}`)).json();if(s.success){const a=s.profile;me(a)}else b.fire({icon:"error",title:"Error",text:s.error||"Failed to load profile"})}}catch(e){console.error("Error fetching profile:",e)}}function me(e){const o=e.profile_picture_path?`${T()}${e.profile_picture_path}`:null,s=e.full_name?e.full_name.split(" ").map(r=>r[0]).join("").substring(0,2).toUpperCase():"US",a=`
    ...
    `;b.fire({html:a,width:"450px",showConfirmButton:!1,showCloseButton:!0,padding:"1.5rem",customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl overflow-visible ldn-modal-popup",closeButton:"focus:outline-none bg-gray-50 border-none swal2-custom-close cursor-pointer"},didOpen:r=>{const t=r.querySelector("#profile-edit-form"),c=r.querySelector("#profile-pic-input"),n=r.querySelector("#profile-avatar-preview");c.addEventListener("change",d=>{const p=d.target.files[0];if(p){const i=new FileReader;i.onload=l=>{n.innerHTML=`<img src="${l.target.result}" class="w-full h-full object-cover" />`},i.readAsDataURL(p)}}),t.addEventListener("submit",async d=>{d.preventDefault();const p=new FormData(t);try{const i=JSON.parse(localStorage.getItem("user"));i&&i.id&&p.append("user_id",i.id)}catch{}try{const l=await(await fetch(`${T()}api/profile.php`,{method:"POST",body:p})).json();l.success?(l.profile&&(localStorage.setItem("user",JSON.stringify(l.profile)),ze(l.profile)),b.close(),b.fire({toast:!0,position:"top-end",icon:"success",title:"Profile Updated",showConfirmButton:!1,timer:3e3,timerProgressBar:!0})):b.fire({icon:"error",title:"Update Failed",text:l.error||"Something went wrong"})}catch(i){console.error("Error saving profile:",i)}})}})}function ze(e){const o=e.profile_picture_path?`${T()}${e.profile_picture_path}`:null,s=e.full_name?e.full_name.split(" ").map(t=>t[0]).join("").substring(0,2).toUpperCase():"US",a=document.querySelectorAll(".sidebar-user-name"),r=document.querySelectorAll(".sidebar-user-avatar");a.forEach(t=>t.textContent=e.full_name),r.forEach(t=>{o?t.innerHTML=`<img src="${o}" class="w-full h-full object-cover" />`:t.textContent=s}),localStorage.setItem("user_full_name",e.full_name),o&&localStorage.setItem("user_avatar",o)}function Ve(e){const o=window.getExportFilters?window.getExportFilters():{office:"ALL",status:"ALL",search:"",sort:"name",section:"ALL",columns:["id","name","office","position","startdate","enddate","status"],preparedBy:localStorage.getItem("ldn_export_prepared")||"",approvedBy:localStorage.getItem("ldn_export_approved")||""},s=`
        <div class="text-left font-montserrat p-1 overflow-visible">
            <div class="flex items-center gap-3 mb-6">
                <div class="w-12 h-12 bg-royal-blue/10 rounded-2xl flex items-center justify-center">
    ...
    `;b.fire({html:c,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:n=>{const d=n.querySelector("#cancel-modal-btn");d&&d.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),b.close(),e?._isBulk&&de.onCancel()});const p=n.querySelector("#bulk-add-btn");p&&p.addEventListener("click",()=>{b.close(),de.init()});const i=n.querySelector("#birthday-input"),l=n.querySelector("#age-display");i&&l&&(i.addEventListener("change",g=>{g.target.value&&(l.value=ve(g.target.value))}),i.value&&!e?.age&&(l.value=ve(i.value)));const u=n.querySelector("#name-input-field"),f=n.querySelector("#duplicate-warning");if(u&&f){let g;u.addEventListener("input",h=>{const v=h.target.value.trim();if(clearTimeout(g),v.length<3){f.classList.add("hidden");return}g=setTimeout(async()=>{try{const S=await(await fetch(`${T()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:v})})).json();S.success&&S.exists?(f.classList.remove("hidden"),z()?f.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":f.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):f.classList.add("hidden")}catch(k){console.error("Duplicate check error:",k)}},500)}),e?.name&&(f.classList.add("hidden"),(async()=>{const v=await(await fetch(`${T()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();v.success&&v.exists&&(f.classList.remove("hidden"),z()?f.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":f.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const m=n.querySelector("#full-id-input"),w=n.querySelector("#series-no-input"),x=n.querySelector('input[name="startDate"]'),C=n.querySelector('input[name="endDate"]'),A=n.querySelectorAll('input[name="remarks"]'),M=n.querySelector("#extension-log-container"),N=async g=>{if(!o){if(m){m.classList.add("animate-pulse"),m.placeholder="Loading...";try{const h=await F(`api/beneficiaries.php?next_id&year=${encodeURIComponent(g)}`);h.success&&h.data?.success&&h.data?.nextId&&(m.value=h.data.nextId)}finally{m.classList.remove("animate-pulse")}}if(w){w.classList.add("animate-pulse"),w.placeholder="Loading...";try{const h=await F(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(g)}`);h.success&&h.data?.success&&h.data?.nextSeries&&(w.value=h.data.nextSeries)}finally{w.classList.remove("animate-pulse")}}}},j=n.querySelector("#replacement-search-input"),G=n.querySelector("#replacement-hidden"),I=n.querySelector("#replacement-suggestions"),_=g=>{const h=(g.name||"").toUpperCase().trim(),v=g.startDateFormatted||g.startDate||"N/A",k=g.endDateFormatted||g.endDate||"N/A";return`${h} (${v.toUpperCase()} - ${k.toUpperCase()})`},y=g=>{if(I){if(!g.length){I.innerHTML=`<div class="px-3 py-2 text-[10px] font-bold ${t.textCourseOpt}">No matching beneficiary found.</div>`,I.classList.remove("hidden");return}I.innerHTML=g.map(h=>{const v=_(h);return`
                    ...
                    `}).join(""),I.classList.remove("hidden"),I.querySelectorAll(".replacement-option").forEach(h=>{h.addEventListener("click",()=>{const v=h.getAttribute("data-replacement")||"";j&&(j.value=v),G&&(G.value=v),I.classList.add("hidden")})})}};let B=null;const E=async(g="")=>{const h=(g||"").trim(),v=`api/beneficiaries.php?replacement_candidates=1&limit=20${h?`&q=${encodeURIComponent(h)}`:""}`,k=await F(v);k.success&&k.data?.success&&Array.isArray(k.data.candidates)&&y(k.data.candidates)};j&&G&&I&&(j.addEventListener("focus",()=>{E(j.value||"")}),j.addEventListener("input",()=>{G.value=j.value.trim(),clearTimeout(B),B=setTimeout(()=>{E(j.value||"")},250)}),document.addEventListener("click",g=>{!j.contains(g.target)&&!I.contains(g.target)&&I.classList.add("hidden")}));const L=()=>{const g=n.querySelector('input[name="remarks"]:checked');return g?g.value:"ONGOING"},$=g=>{const h=n.querySelector(`input[name="remarks"][value="${g}"]`);h&&(h.checked=!0,U())},O=()=>{if(C&&C.value){const g=C.value.split("-"),h=new Date(g[0],g[1]-1,g[2]),v=new Date;v.setHours(0,0,0,0);let k="ONGOING";h<v&&(k="EXPIRED"),$(k)}},U=()=>{if(!M)return;if(L()==="ABSORBED"){const h=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});M.innerHTML=`
                    ...
                    `}else M.innerHTML=""};if(x&&(x.addEventListener("change",g=>{const h=g.target.value;if(h){const v=new Date(h).getFullYear();if(N(v),C&&!C.value){const k=new Date(h);k.setMonth(k.getMonth()+1),C.value=k.toISOString().split("T")[0]}O()}}),!o)){const g=x.value?new Date(x.value).getFullYear():new Date().getFullYear();N(g)}C&&C.addEventListener("change",O),A.forEach(g=>g.addEventListener("change",U));const H=n.querySelector("#resign-btn"),V=n.querySelector("#absorb-btn");H&&H.addEventListener("click",()=>$("RESIGNED")),V&&V.addEventListener("click",()=>$("ABSORBED")),n.querySelectorAll('input[type="text"], textarea').forEach(g=>{["id-number-input","full-id-input"].includes(g.id)||g.addEventListener("input",()=>{const h=g.selectionStart,v=g.selectionEnd;g.value=g.value.toUpperCase(),g.setSelectionRange(h,v)})}),O(),U(),q("education-input","course-suggestions","course-option"),q("designation-input","work-suggestions","work-option"),q("office-input","office-suggestions","office-option");function q(g,h,v){const k=n.querySelector(`#${g}`),S=n.querySelector(`#${h}`);if(!k||!S)return;let J=!1;k.addEventListener("focus",()=>S.classList.remove("hidden")),document.addEventListener("click",P=>{!k.contains(P.target)&&!S.contains(P.target)&&S.classList.add("hidden")}),k.addEventListener("input",()=>{if(J){J=!1;return}const P=k.value.toLowerCase(),Y=S.querySelectorAll(`.${v}`);let ae=!1;Y.forEach(K=>{const R=K.querySelector(".option-text");(R?R.innerText:K.innerText).toLowerCase().includes(P)?(K.style.display="block",ae=!0):K.style.display="none"}),ae?S.classList.remove("hidden"):S.classList.add("hidden")}),S.querySelectorAll(`.${v}`).forEach(P=>{P.addEventListener("click",()=>{const Y=P.querySelector(".option-text");k.value=Y?Y.innerText.trim():P.innerText.trim(),J=!0,S.classList.add("hidden"),k.dispatchEvent(new Event("change"))})})}const D=n.querySelector("#add-beneficiary-form"),W="add_beneficiary_draft";if(!o){const g=localStorage.getItem(W);if(g)try{const h=JSON.parse(g);Object.entries(h).forEach(([v,k])=>{const S=D.elements[v];S&&S.type!=="file"&&S.type!=="hidden"&&(S.value=k)})}catch(h){console.error("Error loading draft",h)}}D.addEventListener("input",g=>{if(!o){const h=new FormData(D),v={};h.forEach((k,S)=>v[S]=k),localStorage.setItem(W,JSON.stringify(v))}}),D&&D.addEventListener("submit",g=>{g.preventDefault(),D.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(Z=>{Z.classList.remove("ring-2","ring-red-500","!border-red-500")});const v=new FormData(D);let k=!1;const S=Z=>{const Q=D.querySelector(`[name="${Z}"]`);Q&&Q.classList.add("ring-2","ring-red-500","!border-red-500"),k=!0},J=v.get("name"),P=v.get("contact"),Y=v.get("startDate"),ae=v.get("endDate"),K=(v.get("designation")||"").trim();if((!J||J.trim()===""||/[0-9]/.test(J))&&S("name"),P&&P.trim()!==""&&/[^0-9]/.test(P.replace(/[\s\-\+\(\)]/g,""))&&S("contact"),Y||S("startDate"),ae||S("endDate"),k)return;const R={};v.forEach((Z,Q)=>{R[Q]=Z}),K||(R.designation="N/A"),R.replacement||(R.replacement="");const te=n.querySelector("#full-id-input")?.value;o?(R.id=e?.id,te&&(R.gip_id=te)):(R.id=null,te&&(R.gip_id=te)),window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(R)){if(!o){const Q="add_beneficiary_draft",Ce=D.querySelector('[name="office"]')?.value||"",Be=D.querySelector('[name="designation"]')?.value||"",Le=D.querySelector('[name="education"]')?.value||"";localStorage.setItem(Q,JSON.stringify({office:Ce,designation:Be,education:Le}))}b.close(),setTimeout(()=>{b.fire({toast:!0,position:"top-end",icon:"success",title:`Record ${o?"Updated":"Added"} Successfully`,showConfirmButton:!1,timer:3e3,timerProgressBar:!0}),!o&&e?._isBulk&&de.onSaveSuccess()},100)}else b.fire({icon:"error",title:"Save Failed",text:"There was an error saving the record to the database."})})()})}})}window.handleContactSubmit=async function(e){e.preventDefault();const o=e.target,s=o.querySelector('button[type="submit"]'),a=s.innerHTML;s.disabled=!0,s.innerHTML=`
        <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        Sending...
    `;try{const r=new FormData(o);if((await fetch(o.action,{method:"POST",body:r,headers:{Accept:"application/json"}})).ok)b.fire({toast:!0,position:"top-end",icon:"success",title:"Message Sent Successfully!",text:"Thank you for reaching out. I will get back to you soon!",showConfirmButton:!1,timer:4e3,timerProgressBar:!0,didOpen:c=>{c.addEventListener("mouseenter",b.stopTimer),c.addEventListener("mouseleave",b.resumeTimer)},customClass:{popup:"rounded-2xl border-l-4 border-teal-500 shadow-2xl"}}),o.reset();else throw new Error("Failed to send")}catch{b.fire({toast:!0,position:"top-end",icon:"error",title:"Oops! Something went wrong.",text:"Could not send your message. Please try again later.",showConfirmButton:!1,timer:4e3,customClass:{popup:"rounded-2xl border-l-4 border-red-500 shadow-2xl"}})}finally{s.disabled=!1,s.innerHTML=a}return!1};export{ye as _,F as a,oe as b,Qe as c,rt as d,tt as e,We as f,T as g,ot as h,we as i,Ye as j,Ke as k,X as l,Ze as m,at as n,Pe as p,et as r,pe as s,ze as u};
