        ...
        <div class="flex flex-col gap-3">
            ${n.map(i=>{const u=i.status.toUpperCase(),x={VERIFIED:"COMPLETED",DECLINED:"REJECTED",PENDING:"PENDING"}[u]||u,f=x==="COMPLETED",m=x==="REJECTED",I=f?"text-green-500":m?"text-red-500":"text-gray-400 dark:text-gray-500",S=f?"bg-green-50/50 dark:bg-green-900/10":m?"bg-red-50/50 dark:bg-red-900/10":"bg-gray-50/50 dark:bg-slate-800/50",A=f?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800 hover:bg-green-200 cursor-pointer":m?"bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800 hover:bg-red-200 cursor-pointer":"bg-white text-gray-500 border-gray-200 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer";let B='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';return f?B='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>':m&&(B='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>'),`
                <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-0.5 ${S}">
                    ...
                    <button type="button" class="ml-3 ${A} text-[10px] font-black px-3 py-1.5 rounded-full border uppercase tracking-widest transition-colors flex-shrink-0 drawer-doc-btn" data-id="${i.id}" data-name="${i.name}" data-status="${x} cursosr-pointer">
                        ${x}
                    </button>
                </div>
                `}).join("")}
...
<style>
    ...
    `;let l=document.getElementById("beneficiary-drawer-container");l&&l.remove(),l=document.createElement("div"),l.id="beneficiary-drawer-container",l.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-neutral-primary-soft dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl",l.setAttribute("tabindex","-1"),l.setAttribute("data-drawer-backdrop","true"),l.innerHTML=c,document.body.appendChild(l),ue(async()=>{const{Drawer:i}=await import("./vendor-flowbite-B2TFTxm5.js");return{Drawer:i}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:i})=>{const u={placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{l&&l.parentNode&&l.remove()},300)}},g=new i(l,u);g.show(),l.querySelector("#close-drawer-btn").addEventListener("click",()=>g.hide());const f=l.querySelector("#drawer-prev-btn"),m=l.querySelector("#drawer-next-btn"),I=2,S=()=>{l.querySelectorAll("[id^=drawer-page-]").forEach((w,C)=>{w.classList.toggle("hidden",C!==a)}),f.disabled=a===0,m.disabled=a===I,f.classList.toggle("opacity-50",a===0),m.classList.toggle("opacity-50",a===I)};f.addEventListener("click",()=>{a>0&&a--,S()}),m.addEventListener("click",()=>{a<I&&a++,S()}),S(),l.querySelectorAll(".drawer-doc-btn").forEach(w=>{w.addEventListener("click",async()=>{const C=w.dataset.name,E=w.dataset.status,D=await p.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Update Document</span>',html:`
                    ...
                    `,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest">Update</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest">Cancel</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs px-6 py-2.5 rounded-xl border border-gray-200 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1,preConfirm:()=>document.getElementById("swal-doc-status").value});if(D.isConfirmed){const y=D.value;if(y===E)return;try{const M=new FormData;M.append("gip_id",e.id),M.append("doc_name",C);const h={COMPLETED:"VERIFIED",REJECTED:"DECLINED",PENDING:"PENDING"}[y]||y;M.append("status",h);const k=await(await fetch(`${L()}api/logs.php?type=docs`,{method:"POST",body:M})).text();let v={success:!1};try{v=JSON.parse(k)}catch{}v.success?(p.fire({toast:!0,position:"top-end",icon:"success",title:"Status updated!",showConfirmButton:!1,timer:1500}),g.hide(),setTimeout(()=>window.viewBeneficiary(e,a),400)):p.fire("Error",v.error||"Failed to update","error")}catch(M){p.fire("Error",M.message,"error")}}})});const A=()=>{if(!r.length)return new Date().toISOString().split("T")[0];const w=new Date(r[0].date);return w.setDate(w.getDate()+1),w.toISOString().split("T")[0]},B=()=>{if(!s.length){const D=new Date,y=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"][D.getMonth()];return D.getDate()<=15?`${y} 1-15`:`${y} 16-${new Date(D.getFullYear(),D.getMonth()+1,0).getDate()}`}const w=s[0].period||"",C=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"],E=w.toUpperCase().match(/([A-Z]{3})\s+(\d+)-(\d+)/);if(E){let D=E[1],y=parseInt(E[2]),M=C.indexOf(D);if(M!==-1)return y===1?`${D} 16-${new Date(new Date().getFullYear(),M+1,0).getDate()}`:`${C[(M+1)%12]} 1-15`}return"NEW PERIOD"},T=async(w,C)=>{p.fire({title:"Adding...",allowOutsideClick:!1,showConfirmButton:!1}),p.showLoading();try{const E=new FormData;E.append("gip_id",e.id),w==="dtr"&&E.append("record_date",C),w==="ar"&&E.append("period",C),(await(await fetch(`${L()}api/logs.php?type=${w}`,{method:"POST",body:E})).json()).success?(p.fire({toast:!0,position:"top-end",icon:"success",title:"Auto-Added!",showConfirmButton:!1,timer:1500}),g.hide(),setTimeout(()=>window.viewBeneficiary(e,a),400)):p.fire("Error","Failed to add log.","error")}catch(E){p.fire("Error",E.message,"error")}},F=async(w,C,E,D,y)=>{const M=C==="dtr"?"Record Date":"Period";_();const b="flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[10px] sm:text-xs gap-2 ",h='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>',$='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>',{value:k}=await p.fire({title:`<span class="text-xl font-black text-heading uppercase tracking-tight">Edit ${w} Log</span>`,html:`
...
.editable-indicator {
    ...
    `;let n=document.getElementById("edit-drawer-container");n&&n.remove(),n=document.createElement("div"),n.id="edit-drawer-container",n.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-white dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",n.setAttribute("tabindex","-1"),n.innerHTML=d,document.body.appendChild(n),setTimeout(()=>{const c=n.querySelector('textarea[name="name"]');c&&(c.style.height="auto",c.style.height=c.scrollHeight+"px")},10),(async()=>{try{const c=await re("api/beneficiaries.php?get_offices=1");if(c.success&&c.data.offices){const l=n.querySelector("#edit-office-suggestions-box");if(l){const i=["DOLE Field Office","LGU","DEPED","DICT","PCA"],u=[...new Set([...i,...c.data.offices])];l.innerHTML=u.map(g=>`
                    ...
                    `).join("")}}}catch(c){console.error("Error fetching office suggestions:",c)}})(),ue(async()=>{const{Drawer:c}=await import("./vendor-flowbite-B2TFTxm5.js");return{Drawer:c}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:c})=>{const l=new c(n,{placement:"right",backdrop:!0,bodyScrolling:!1,backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{n.parentNode&&n.remove()},300)}});l.show(),n.querySelector("#close-edit-drawer-btn").addEventListener("click",()=>l.hide()),n.querySelector("#edit-drawer-cancel-btn").addEventListener("click",()=>l.hide());const i=n.querySelector("#edit-beneficiary-drawer-form"),u=n.querySelector("#edit-bday-input"),g=n.querySelector("#edit-age-display");let x=!1;u&&g&&(g.addEventListener("input",()=>{x=!0}),u.addEventListener("change",m=>{m.target.value&&(!x||!g.value)&&(g.value=t(m.target.value))}));const f=(m,I,S)=>{const A=n.querySelector(m),B=n.querySelector(I);if(!A||!B)return;const T=()=>B.classList.add("hidden"),F=()=>B.classList.remove("hidden");A.addEventListener("focus",F),A.addEventListener("input",()=>{const N=A.value.toLowerCase().trim();let O=0;B.querySelectorAll(S).forEach(w=>{const E=(w.querySelector(".option-text")?.textContent||w.textContent||"").toLowerCase().includes(N);w.style.display=E?"block":"none",E&&O++}),O>0?F():T()}),B.addEventListener("click",N=>{const O=N.target.closest(S);O&&(A.value=(O.querySelector(".option-text")?.textContent||O.textContent||"").trim(),T(),A.dispatchEvent(new Event("change")))}),document.addEventListener("click",N=>{!A.contains(N.target)&&!B.contains(N.target)&&T()})};f("#edit-education-input","#edit-education-suggestions-box",".edit-education-option"),f("#edit-designation-input","#edit-designation-suggestions-box",".edit-designation-option"),f("#edit-office-input","#edit-office-suggestions-box",".edit-office-option"),i.querySelectorAll("input, select, textarea").forEach(m=>{const I=(m.getAttribute("type")||"").toLowerCase(),S=!m.disabled&&!m.readOnly&&I!=="hidden";m.classList.remove("editable-indicator"),S&&m.classList.add("editable-indicator")}),i.addEventListener("submit",m=>{m.preventDefault();const I=new FormData(i),S={};I.forEach((A,B)=>S[B]=A),S.id=e.id,S.gip_id=e.id,window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(S,!0,!1)){l.hide();const B=document.querySelector("#close-drawer-btn");B&&B.click(),p.fire({toast:!0,position:"top-end",icon:"success",title:"Record Updated",showConfirmButton:!1,timer:3e3})}})()})})}function ie(e="Incorrect Username or Password"){p.fire({html:`
            <div class="p-6">
        ...
        `,timer:e?800:3e3,timerProgressBar:!0,showConfirmButton:!1,showCloseButton:!1,width:"350px",padding:"0",customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl overflow-hidden border border-gray-100",timerProgressBar:"bg-green-500 h-1.5"},backdrop:"rgba(0, 0, 0, 0.4)"})}function _e(){window.viewBeneficiary=async function(e,o=0){const a=`logs_cache_${e.id}`;let s=!1;try{if(window.__doleDB&&window.__doleDB.getSecureCache){const f=await window.__doleDB.getSecureCache(a);f&&(e.arLogs=f.arLogs||[],e.dtrLogs=f.dtrLogs||[],e.docs=f.docs||[],Y(e,o),s=!0)}const[r,t,d]=await Promise.all([fetch(`${L()}api/logs.php?type=ar&gip_id=${encodeURIComponent(e.id)}`),fetch(`${L()}api/logs.php?type=dtr&gip_id=${encodeURIComponent(e.id)}`),fetch(`${L()}api/logs.php?type=docs&gip_id=${encodeURIComponent(e.id)}`)]),n=await r.json(),c=await t.json(),l=await d.json(),i=n.success?n.logs:[],u=c.success?c.logs:[],g=l.success?l.logs:[],x={arLogs:i,dtrLogs:u,docs:g};window.__doleDB&&window.__doleDB.setSecureCache&&await window.__doleDB.setSecureCache(a,x),e.arLogs=i,e.dtrLogs=u,e.docs=g,s?document.getElementById("beneficiary-drawer-container")&&Y(e,o):Y(e,o)}catch(r){console.error("Error fetching logs/docs:",r),s||(e.arLogs=[],e.dtrLogs=[],e.docs=[],Y(e,o))}},window.showAddDataModal=function(e){ee(e)},window.editBeneficiary=function(e){Ce(e)},window.showExportConfigModal=function(e){Ae(e)},window.showProfileModal=function(){De()}}async function De(){try{let e="";try{const s=JSON.parse(localStorage.getItem("user"));s&&s.id&&(e=`?user_id=${s.id}`)}catch{}const a=await(await fetch(`${L()}api/profile.php${e}`)).json();if(a.success){const s=a.profile;Le(s)}else p.fire({icon:"error",title:"Error",text:a.error||"Failed to load profile"})}catch(e){console.error("Error fetching profile:",e)}}function Le(e){const o=e.profile_picture_path?`${L()}${e.profile_picture_path}`:null,a=e.full_name?e.full_name.split(" ").map(r=>r[0]).join("").substring(0,2).toUpperCase():"US",s=`
    ...
    `;p.fire({html:d,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:n=>{const c=n.querySelector("#cancel-modal-btn");c&&c.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),p.close(),e?._isBulk&&Z.onCancel()});const l=n.querySelector("#bulk-add-btn");l&&l.addEventListener("click",()=>{p.close(),Z.init()});const i=n.querySelector("#birthday-input"),u=n.querySelector("#age-display");i&&u&&(i.addEventListener("change",b=>{b.target.value&&(u.value=le(b.target.value))}),i.value&&!e?.age&&(u.value=le(i.value)));const g=n.querySelector("#name-input-field"),x=n.querySelector("#duplicate-warning");if(g&&x){let b;g.addEventListener("input",h=>{const $=h.target.value.trim();if(clearTimeout(b),$.length<3){x.classList.add("hidden");return}b=setTimeout(async()=>{try{const v=await(await fetch(`${L()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:$})})).json();v.success&&v.exists?(x.classList.remove("hidden"),_()?x.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":x.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):x.classList.add("hidden")}catch(k){console.error("Duplicate check error:",k)}},500)}),e?.name&&(x.classList.add("hidden"),(async()=>{const $=await(await fetch(`${L()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();$.success&&$.exists&&(x.classList.remove("hidden"),_()?x.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":x.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const f=n.querySelector("#full-id-input"),m=n.querySelector("#series-no-input"),I=n.querySelector('input[name="startDate"]'),S=n.querySelector('input[name="endDate"]'),A=n.querySelectorAll('input[name="remarks"]'),B=n.querySelector("#extension-log-container"),T=b=>{o||(f&&(f.classList.add("animate-pulse"),f.placeholder="Loading...",fetch(`${L()}api/beneficiaries.php?next_id&year=${b}`).then(h=>h.json()).then(h=>{f.classList.remove("animate-pulse"),h.success&&h.nextId&&(f.value=h.nextId)})),m&&(m.classList.add("animate-pulse"),m.placeholder="Loading...",fetch(`${L()}api/beneficiaries.php?next_series_no&year=${b}`).then(h=>h.json()).then(h=>{m.classList.remove("animate-pulse"),h.success&&h.nextSeries&&(m.value=h.nextSeries)})))},F=()=>{const b=n.querySelector('input[name="remarks"]:checked');return b?b.value:"ONGOING"},N=b=>{const h=n.querySelector(`input[name="remarks"][value="${b}"]`);h&&(h.checked=!0,w())},O=()=>{if(S&&S.value){const b=S.value.split("-"),h=new Date(b[0],b[1]-1,b[2]),$=new Date;$.setHours(0,0,0,0);let k="ONGOING";h<$&&(k="EXPIRED"),N(k)}},w=()=>{if(!B)return;if(F()==="ABSORBED"){const h=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});B.innerHTML=`
                    ...
                    `}else B.innerHTML=""};if(I&&(I.addEventListener("change",b=>{const h=b.target.value;if(h){const $=new Date(h).getFullYear();if(T($),S&&!S.value){const k=new Date(h);k.setMonth(k.getMonth()+1),S.value=k.toISOString().split("T")[0]}O()}}),!o)){const b=I.value?new Date(I.value).getFullYear():new Date().getFullYear();T(b)}S&&S.addEventListener("change",O),A.forEach(b=>b.addEventListener("change",w));const C=n.querySelector("#resign-btn"),E=n.querySelector("#absorb-btn");C&&C.addEventListener("click",()=>N("RESIGNED")),E&&E.addEventListener("click",()=>N("ABSORBED")),n.querySelectorAll('input[type="text"], textarea').forEach(b=>{["id-number-input","full-id-input"].includes(b.id)||b.addEventListener("input",()=>{const h=b.selectionStart,$=b.selectionEnd;b.value=b.value.toUpperCase(),b.setSelectionRange(h,$)})}),O(),w(),D("education-input","course-suggestions","course-option"),D("designation-input","work-suggestions","work-option"),D("office-input","office-suggestions","office-option");function D(b,h,$){const k=n.querySelector(`#${b}`),v=n.querySelector(`#${h}`);if(!k||!v)return;let P=!1;k.addEventListener("focus",()=>v.classList.remove("hidden")),document.addEventListener("click",j=>{!k.contains(j.target)&&!v.contains(j.target)&&v.classList.add("hidden")}),k.addEventListener("input",()=>{if(P){P=!1;return}const j=k.value.toLowerCase(),G=v.querySelectorAll(`.${$}`);let X=!1;G.forEach(q=>{const R=q.querySelector(".option-text");(R?R.innerText:q.innerText).toLowerCase().includes(j)?(q.style.display="block",X=!0):q.style.display="none"}),X?v.classList.remove("hidden"):v.classList.add("hidden")}),v.querySelectorAll(`.${$}`).forEach(j=>{j.addEventListener("click",()=>{const G=j.querySelector(".option-text");k.value=G?G.innerText.trim():j.innerText.trim(),P=!0,v.classList.add("hidden"),k.dispatchEvent(new Event("change"))})})}const y=n.querySelector("#add-beneficiary-form"),M="add_beneficiary_draft";if(!o){const b=localStorage.getItem(M);if(b)try{const h=JSON.parse(b);Object.entries(h).forEach(([$,k])=>{const v=y.elements[$];v&&v.type!=="file"&&v.type!=="hidden"&&(v.value=k)})}catch(h){console.error("Error loading draft",h)}}y.addEventListener("input",b=>{if(!o){const h=new FormData(y),$={};h.forEach((k,v)=>$[v]=k),localStorage.setItem(M,JSON.stringify($))}}),y&&y.addEventListener("submit",b=>{b.preventDefault(),y.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(U=>{U.classList.remove("ring-2","ring-red-500","!border-red-500")});const $=new FormData(y);let k=!1;const v=U=>{const H=y.querySelector(`[name="${U}"]`);H&&H.classList.add("ring-2","ring-red-500","!border-red-500"),k=!0},P=$.get("name"),j=$.get("contact"),G=$.get("startDate"),X=$.get("endDate"),q=$.get("designation");if((!P||P.trim()===""||/[0-9]/.test(P))&&v("name"),j&&j.trim()!==""&&/[^0-9]/.test(j.replace(/[\s\-\+\(\)]/g,""))&&v("contact"),G||v("startDate"),X||v("endDate"),(!q||q.trim()==="")&&v("designation"),k)return;const R={};$.forEach((U,H)=>{R[H]=U});const V=n.querySelector("#full-id-input")?.value;o?(R.id=e?.id,V&&(R.gip_id=V)):(R.id=null,V&&(R.gip_id=V)),window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(R)){if(!o){const H="add_beneficiary_draft",be=y.querySelector('[name="office"]')?.value||"",fe=y.querySelector('[name="designation"]')?.value||"",he=y.querySelector('[name="education"]')?.value||"";localStorage.setItem(H,JSON.stringify({office:be,designation:fe,education:he}))}p.close(),setTimeout(()=>{p.fire({toast:!0,position:"top-end",icon:"success",title:`Record ${o?"Updated":"Added"} Successfully`,showConfirmButton:!1,timer:3e3,timerProgressBar:!0}),!o&&e?._isBulk&&Z.onSaveSuccess()},100)}else p.fire({icon:"error",title:"Save Failed",text:"There was an error saving the record to the database."})})()})}})}window.handleContactSubmit=async function(e){e.preventDefault();const o=e.target,a=o.querySelector('button[type="submit"]'),s=a.innerHTML;a.disabled=!0,a.innerHTML=`
        <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        Sending...
    `;try{const r=new FormData(o);if((await fetch(o.action,{method:"POST",body:r,headers:{Accept:"application/json"}})).ok)p.fire({toast:!0,position:"top-end",icon:"success",title:"Message Sent Successfully!",text:"Thank you for reaching out. I will get back to you soon!",showConfirmButton:!1,timer:4e3,timerProgressBar:!0,didOpen:d=>{d.addEventListener("mouseenter",p.stopTimer),d.addEventListener("mouseleave",p.resumeTimer)},customClass:{popup:"rounded-2xl border-l-4 border-teal-500 shadow-2xl"}}),o.reset();else throw new Error("Failed to send")}catch{p.fire({toast:!0,position:"top-end",icon:"error",title:"Oops! Something went wrong.",text:"Could not send your message. Please try again later.",showConfirmButton:!1,timer:4e3,customClass:{popup:"rounded-2xl border-l-4 border-red-500 shadow-2xl"}})}finally{a.disabled=!1,a.innerHTML=s}return!1};"true".toLowerCase();function L(){const e=window.location.pathname,o="/dole-system/",a=e.toLowerCase().indexOf(o.toLowerCase());return a!==-1?e.substring(0,a+o.length):"/"}function qe(){const e=localStorage.getItem("hasVisitedBefore"),o=document.getElementById("page-loader");e&&(document.documentElement.classList.add("loaded"),o&&(o.style.display="none")),window.addEventListener("load",()=>{const a=document.querySelector("body > *:not(.page-loader)");a&&a.classList.add("content-fade-in"),setTimeout(()=>{document.documentElement.classList.add("loaded"),o&&o.classList.add("hidden"),document.body.style.overflow="auto",localStorage.setItem("hasVisitedBefore","true")},300)})}const K={_key:"DOLE-GIP-SECURE-KEY-2026",encrypt:function(e){try{const o=r=>r.split("").map(t=>t.charCodeAt(0)),a=r=>("0"+Number(r).toString(16)).substr(-2),s=r=>o(this._key).reduce((t,d)=>t^d,r);return e.split("").map(o).map(s).map(a).join("")}catch(o){return console.error("Encryption Failed",o),null}},decrypt:function(e){try{const o=s=>s.split("").map(r=>r.charCodeAt(0)),a=s=>o(this._key).reduce((r,t)=>r^t,s);return e.match(/.{1,2}/g).map(s=>parseInt(s,16)).map(a).map(s=>String.fromCharCode(s)).join("")}catch(o){return console.error("Decryption Failed",o),null}}};function Fe(){document.querySelectorAll(".login-form-shared").forEach(o=>{const a=o.querySelector('input[name="username"]'),s=o.querySelector('input[name="password"]'),r=o.querySelector('input[name="rememberMe"]');if(a&&s&&r){const t=localStorage.getItem("secure_user"),d=localStorage.getItem("secure_pass");if(t&&d){const n=K.decrypt(t),c=K.decrypt(d);n&&c&&(a.value=n,s.value=c,r.checked=!0)}}o.addEventListener("submit",async t=>{t.preventDefault();try{const n=await(await fetch(`${L()}api/auth.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:a.value,password:s.value})})).json();if(n.success){r.checked?(localStorage.setItem("secure_user",K.encrypt(a.value)),localStorage.setItem("secure_pass",K.encrypt(s.value))):(localStorage.removeItem("secure_user"),localStorage.removeItem("secure_pass"));const c=localStorage.getItem("hasLoggedInBefore")==="true";localStorage.setItem("isLoggedIn","true"),localStorage.setItem("hasLoggedInBefore","true"),localStorage.setItem("user",JSON.stringify(n.user));const l=document.getElementById("drawer-login");if(l){l.classList.add("translate-y-full");const i=l.querySelector("[data-drawer-hide]");i&&i.click()}await Be(c),Me(c)}else{const c=document.getElementById("drawer-login");c?(c.classList.add("translate-y-full"),setTimeout(()=>{ie(),setTimeout(()=>{c.classList.remove("translate-y-full"),s.value="",s.focus()},600)},400)):(ie(),s.value="",s.focus())}}catch(d){console.error("Login Error:",d),p.fire({icon:"error",title:"Login Error",text:"Unable to reach the server. Please check your connection."})}})})}function Me(e=!1){const o=document.getElementById("left-panel"),a=document.getElementById("right-panel"),s=document.getElementById("left-panel-content"),r=document.getElementById("right-panel-content");s&&(s.style.opacity="0"),r&&(r.style.opacity="0");const t=document.createElement("div");t.className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none";const d=e?"":"animate__delay-1s",n=e?"animation-duration: 0.8s;":"animation-duration: 2s;";t.innerHTML=`
        <img src="${L()}frontend/images/logo/doleiligan.png" class="w-64 h-64 md:w-96 md:h-96 object-contain bg-white/20 backdrop-blur-sm shadow-2xl rounded-full p-4 animate__animated animate__rotateOut ${d}" style="${n}" alt="DOLE Logo">
    `,document.body.appendChild(t);const c=e?0:1e3,l=e?600:1500;setTimeout(()=>{o&&o.classList.add("animate-slide-left"),a&&a.classList.add("animate-slide-right"),setTimeout(()=>{window.location.href=`${L()}frontend/dashboard/`},l)},c)}function Ge(){const e=document.getElementById("logoutBtn");e&&e.addEventListener("click",()=>{localStorage.removeItem("isLoggedIn"),window.location.href=`${L()}`})}function Ue(){const e=document.getElementById("mobile-splash"),o=document.getElementById("show-login-btn"),a=document.getElementById("back-to-splash"),s=document.getElementById("mobile-bg-content"),r=document.getElementById("mobile-welcome-text"),t=document.getElementById("reopen-login-drawer"),d=document.getElementById("request-notifications-btn"),n=async()=>{if("Notification"in window){const f=await Notification.requestPermission();console.log("Notification permission:",f),f==="granted"&&d&&d.classList.add("hidden")}};Notification.permission==="default"&&d&&(d.classList.remove("hidden"),d.addEventListener("click",n));const c=()=>{e&&(e.style.transform="translateY(-100%)",setTimeout(()=>{e.style.visibility="hidden",e.style.pointerEvents="none",e.style.zIndex="-1",Notification.permission==="default"&&n();const f=document.getElementById("drawer-login");f&&f.classList.remove("translate-y-full")},800))},l=()=>{e&&(e.style.zIndex="9999",e.style.pointerEvents="auto",e.style.visibility="visible",e.style.transform="translateY(0)")};o&&o.addEventListener("click",c),document.querySelectorAll(".forgot-password-link").forEach(f=>{f.addEventListener("click",m=>{m.target.closest("#mobile-splash")&&c()})}),a&&a.addEventListener("click",()=>{const f=document.getElementById("drawer-login");if(f){f.classList.add("translate-y-full");const m=f.querySelector("[data-drawer-hide]");m&&m.click()}l()});const u=document.getElementById("drawer-login"),g=document.getElementById("curved-welcome"),x=document.getElementById("peoples-bg");u&&new MutationObserver(m=>{m.forEach(I=>{I.attributeName==="class"&&(u.classList.contains("translate-y-full")?(s&&(s.style.transform="translateY(0)"),r&&(r.style.opacity="1",r.style.transform="translateY(0) scale(1)"),g&&(g.style.opacity="0",g.style.transform="scale(0.5)"),t&&e&&e.style.visibility==="hidden"&&(t.style.opacity="1",t.style.transform="scale(1)"),x&&(x.classList.add("opacity-0","scale-0"),x.classList.remove("opacity-40","scale-[1.6]"))):(s&&(s.style.transform="translateY(-35%)"),r&&(r.style.opacity="0",r.style.transform="translateY(20px) scale(0.9)"),g&&(g.style.opacity="1",g.style.transform="scale(1)"),t&&(t.style.opacity="0",t.style.transform="scale(0)"),x&&(x.classList.remove("opacity-0","scale-0"),x.classList.add("opacity-40","scale-[1.6]"))))})}).observe(u,{attributes:!0})}export{ue as _,re as a,Re as b,oe as c,Oe as d,Ne as e,Fe as f,L as g,Ge as h,qe as i,Ue as j,_e as k,W as l,ke as p,Te as r,Pe as s,Ie as u};
