...
import b from"./vendor-swal-BSk0fVSb.js";import{a as Ie}from"./vendor-DHtNC8Ux.js";const De="true".toLowerCase()==="true";function we(){return De}function _(){const e=window.location.pathname,o="/dole-system/",s=e.toLowerCase().indexOf(o.toLowerCase());if(s!==-1)return e.substring(0,s+o.length);const a=e.indexOf("/frontend/");if(a!==-1)return e.substring(0,a+1);const r=e.indexOf("/backend/");return r!==-1?e.substring(0,r+1):"/"}function be(e="Incorrect Username or Password"){b.fire({html:`
            <div class="p-6">
    ...
    <div id="drawer-page-2" class="hidden flex-1 flex flex-col gap-6">
         ...
         <div class="flex flex-col gap-5">
            <div>
                ...
                <div class="space-y-2">
                    ${r.length?r.map(i=>{const u=i.status||"PENDING";let f=u==="VERIFIED"||u==="COMPLETED"?"text-green-500":u==="REJECTED"||u==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500";return`
            ...
            <div>
                ...
                <div class="space-y-2">
                    ${a.length?a.map(i=>{const u=i.status||"PENDING";let f=u==="VERIFIED"||u==="COMPLETED"?"text-green-500":u==="REJECTED"||u==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500";return`
...
<style>
    ...
    `;let d=document.getElementById("beneficiary-drawer-container");d&&d.remove(),d=document.createElement("div"),d.id="beneficiary-drawer-container",d.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-neutral-primary-soft dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl",d.setAttribute("tabindex","-1"),d.setAttribute("data-drawer-backdrop","true"),d.innerHTML=c,document.body.appendChild(d),ye(async()=>{const{Drawer:i}=await import("./vendor-flowbite-bBKPL--D.js");return{Drawer:i}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:i})=>{const u={placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{d&&d.parentNode&&d.remove()},300)}},f=new i(d,u);f.show(),d.querySelector("#close-drawer-btn").addEventListener("click",()=>f.hide());const m=d.querySelector("#drawer-prev-btn"),x=d.querySelector("#drawer-next-btn"),L=2,k=()=>{d.querySelectorAll("[id^=drawer-page-]").forEach((w,B)=>{w.classList.toggle("hidden",B!==s)}),m.disabled=s===0,x.disabled=s===L,m.classList.toggle("opacity-50",s===0),x.classList.toggle("opacity-50",s===L)};m.addEventListener("click",()=>{s>0&&s--,k()}),x.addEventListener("click",()=>{s<L&&s++,k()}),k(),d.querySelectorAll(".drawer-doc-btn").forEach(w=>{w.addEventListener("click",async()=>{const B=w.dataset.name,E=w.dataset.status,C=await b.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Update Document</span>',html:`
                    ...
                    `,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest">Update</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest">Cancel</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs px-6 py-2.5 rounded-xl border border-gray-200 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1,preConfirm:()=>document.getElementById("swal-doc-status").value});if(C.isConfirmed){const $=C.value;if($===E)return;try{const F={COMPLETED:"VERIFIED",REJECTED:"DECLINED",PENDING:"PENDING"}[$]||$,G=await ue("api/logs.php?type=docs",{gip_id:e.id,doc_name:B,status:F}),V=G.success?G.data:{success:!1,error:G.error};V.success?(b.fire({toast:!0,position:"top-end",icon:"success",title:"Status updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,s)):b.fire("Error",V.error||"Failed to update","error")}catch(M){b.fire("Error",M.message,"error")}}})});const D=()=>{if(!r.length)return new Date().toISOString().split("T")[0];const w=new Date(r[0].date);return w.setDate(w.getDate()+1),w.toISOString().split("T")[0]},A=()=>{if(!a.length){const C=new Date,$=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"][C.getMonth()];return C.getDate()<=15?`${$} 1-15`:`${$} 16-${new Date(C.getFullYear(),C.getMonth()+1,0).getDate()}`}const w=a[0].period||"",B=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"],E=w.toUpperCase().match(/([A-Z]{3})\s+(\d+)-(\d+)/);if(E){let C=E[1],$=parseInt(E[2]),M=B.indexOf(C);if(M!==-1)return $===1?`${C} 16-${new Date(new Date().getFullYear(),M+1,0).getDate()}`:`${B[(M+1)%12]} 1-15`}return"NEW PERIOD"},q=async(w,B)=>{b.fire({title:"Adding...",allowOutsideClick:!1,showConfirmButton:!1}),b.showLoading();try{const E={gip_id:e.id};w==="dtr"&&(E.record_date=B),w==="ar"&&(E.period=B);const C=await ue(`api/logs.php?type=${w}`,E);(C.success?C.data:{success:!1,error:C.error}).success?(b.fire({toast:!0,position:"top-end",icon:"success",title:"Auto-Added!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,s)):b.fire("Error","Failed to add log.","error")}catch(E){b.fire("Error",E.message,"error")}},j=async(w,B,E,C,$)=>{const M=B==="dtr"?"Record Date":"Period";z();const F="flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[10px] sm:text-xs gap-2 ",G='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>',V='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>',{value:U}=await b.fire({title:`<span class="text-xl font-black text-heading uppercase tracking-tight">Edit ${w} Log</span>`,html:`
                ...
                `,focusConfirm:!1,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">SAVE REVISIONS</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">CANCEL</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2"},buttonsStyling:!1,preConfirm:()=>{const P=document.querySelector('input[name="swal-log-status"]:checked');return{val:document.getElementById("swal-log-val").value.trim().toUpperCase(),status:P?P.value:"PENDING"}}});if(U&&(U.val!==C||U.status!==$))try{const P={type:B,id:E,status:U.status};B==="dtr"&&(P.record_date=U.val),B==="ar"&&(P.period=U.val);const N=await je("api/logs.php",P),ee=N.success?N.data:{success:!1,error:N.error};ee.success?(b.fire({toast:!0,position:"top-end",icon:"success",title:"Log Updated!",showConfirmButton:!1,timer:1500}),f.hide(),setTimeout(()=>window.viewBeneficiary(e,s),400)):b.fire("Error",ee.error||"Failed to update log.","error")}catch(P){b.fire("Error",P.message,"error")}},O=d.querySelector("#add-dtr-log-btn");O&&O.addEventListener("click",()=>q("dtr",D()));const I=d.querySelector("#add-ar-log-btn");I&&I.addEventListener("click",()=>q("ar",A())),d.querySelectorAll(".edit-log-btn").forEach(w=>{w.addEventListener("click",B=>{if(B.target.closest(".delete-log-btn"))return;const E=w.dataset.type,C=w.dataset.id,$=w.dataset.val,M=w.dataset.status;j(E.toUpperCase(),E,C,$,M)})}),d.querySelectorAll(".delete-log-btn").forEach(w=>{w.addEventListener("click",async()=>{const B=w.dataset.id,E=w.dataset.type;if((await b.fire({title:'<span class="text-xl font-black text-philippine-red uppercase tracking-tight">Delete item?</span>',text:"This action cannot be undone.",icon:"warning",showCancelButton:!0,confirmButtonText:'<span class="font-black tracking-widest uppercase">Delete</span>',cancelButtonText:'<span class="font-black tracking-widest uppercase">Wait</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100",confirmButton:"bg-philippine-red text-white hover:bg-red-700 text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs px-6 py-2.5 rounded-xl border border-gray-200 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1})).isConfirmed)try{const $=await ue(`api/logs.php?type=${E}`,{log_id:B,action:"delete"});($.success?$.data:{success:!1,error:$.error}).success?(b.fire({toast:!0,position:"top-end",icon:"success",title:"Deleted",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,s)):b.fire("Error","Failed to delete data.","error")}catch($){b.fire("Error",$.message,"error")}})})}).catch(console.error)}function Ue(e){const o=z(),s=`w-full bg-transparent border-b-2 ${o?"border-slate-700 text-white focus:border-brand placeholder-slate-600":"border-gray-200 text-gray-900 focus:border-brand placeholder-gray-300"} px-1 py-1 text-sm font-black outline-none transition-all focus:ring-0`,a=`w-full bg-transparent border-none ${o?"text-white":"text-royal-blue"} px-0 py-0 text-xl sm:text-2xl font-black leading-tight tracking-tight focus:ring-0 outline-none placeholder-gray-300 resize-none overflow-hidden`,r=["DOLE Field Office","LGU","DEPED","DICT","PCA"];function t(c){if(!c)return"";const d=new Date(c),i=new Date;let u=i.getFullYear()-d.getFullYear();const f=i.getMonth()-d.getMonth();return(f<0||f===0&&i.getDate()<d.getDate())&&u--,u>=0?u:0}const l=`
    ...
    <div class="flex flex-col gap-4 text-sm mt-3 px-1 pb-24">
        ...
        <div class="flex flex-col gap-2 pt-3 mt-1 border-t border-gray-50 dark:border-slate-800/60">
            ...
            <div id="edit-education-suggestions-box" class="hidden mt-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                ${$e.map(c=>`<button type="button" class="edit-education-option w-full text-left px-3 py-2 text-[11px] font-bold text-slate-700 dark:text-slate-200 hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer"><span class="option-text">${c.name}</span></button>`).join("")}
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col gap-4 text-sm mt-4 px-1">
        ...
        <div class="flex flex-col gap-2">
            ...
            <div id="edit-designation-suggestions-box" class="hidden mt-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                ${Se.map(c=>`<button type="button" class="edit-designation-option w-full text-left px-3 py-2 text-[11px] font-bold text-slate-700 dark:text-slate-200 hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer"><span class="option-text">${c}</span></button>`).join("")}
            </div>
        </div>
...
.editable-indicator {
    ...
    `;let n=document.getElementById("edit-drawer-container");n&&n.remove(),n=document.createElement("div"),n.id="edit-drawer-container",n.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-white dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",n.setAttribute("tabindex","-1"),n.innerHTML=l,document.body.appendChild(n),setTimeout(()=>{const c=n.querySelector('textarea[name="name"]');c&&(c.style.height="auto",c.style.height=c.scrollHeight+"px")},10),(async()=>{try{const c=await oe("api/beneficiaries.php?get_offices=1");if(c.success&&c.data.offices){const d=n.querySelector("#edit-office-suggestions-box");if(d){const i=["DOLE Field Office","LGU","DEPED","DICT","PCA"],u=[...new Set([...i,...c.data.offices])];d.innerHTML=u.map(f=>`
                    ...
                    `).join("")}}}catch(c){console.error("Error fetching office suggestions:",c)}})(),ye(async()=>{const{Drawer:c}=await import("./vendor-flowbite-bBKPL--D.js");return{Drawer:c}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:c})=>{const d=new c(n,{placement:"right",backdrop:!0,bodyScrolling:!1,backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{n.parentNode&&n.remove()},300)}});d.show(),n.querySelector("#close-edit-drawer-btn").addEventListener("click",()=>d.hide()),n.querySelector("#edit-drawer-cancel-btn").addEventListener("click",()=>d.hide());const i=n.querySelector("#edit-beneficiary-drawer-form"),u=n.querySelector("#edit-bday-input"),f=n.querySelector("#edit-age-display");let p=!1;u&&f&&(f.addEventListener("input",()=>{p=!0}),u.addEventListener("change",x=>{x.target.value&&(!p||!f.value)&&(f.value=t(x.target.value))}));const m=(x,L,k)=>{const D=n.querySelector(x),A=n.querySelector(L);if(!D||!A)return;const q=()=>A.classList.add("hidden"),j=()=>A.classList.remove("hidden");D.addEventListener("focus",j),D.addEventListener("input",()=>{const O=D.value.toLowerCase().trim();let I=0;A.querySelectorAll(k).forEach(w=>{const E=(w.querySelector(".option-text")?.textContent||w.textContent||"").toLowerCase().includes(O);w.style.display=E?"block":"none",E&&I++}),I>0?j():q()}),A.addEventListener("click",O=>{const I=O.target.closest(k);I&&(D.value=(I.querySelector(".option-text")?.textContent||I.textContent||"").trim(),q(),D.dispatchEvent(new Event("change")))}),document.addEventListener("click",O=>{!D.contains(O.target)&&!A.contains(O.target)&&q()})};m("#edit-education-input","#edit-education-suggestions-box",".edit-education-option"),m("#edit-designation-input","#edit-designation-suggestions-box",".edit-designation-option"),m("#edit-office-input","#edit-office-suggestions-box",".edit-office-option"),i.querySelectorAll("input, select, textarea").forEach(x=>{const L=(x.getAttribute("type")||"").toLowerCase(),k=!x.disabled&&!x.readOnly&&L!=="hidden";x.classList.remove("editable-indicator"),k&&x.classList.add("editable-indicator")}),i.addEventListener("submit",x=>{x.preventDefault();const L=new FormData(i),k={};L.forEach((D,A)=>k[A]=D),k.id=e.id,k.gip_id=e.id,window.addBeneficiaryData&&(async()=>await window.addBeneficiaryData(k,!0,!1)&&(d.hide(),window.viewBeneficiary&&setTimeout(()=>window.viewBeneficiary({id:e.id},0),100),b.fire({toast:!0,position:"top-end",icon:"success",title:"Record Updated",showConfirmButton:!1,timer:3e3})))()})})}function at(){window.viewBeneficiary=async function(e,o=0){const s=`logs_cache_${e.id}`;let a=!1;try{if(window.__doleDB&&window.__doleDB.getSecureCache){const p=await window.__doleDB.getSecureCache(s);p&&(e.arLogs=p.arLogs||[],e.dtrLogs=p.dtrLogs||[],e.docs=p.docs||[],ie(e,o),a=!0)}let r=[],t=[],l=[];const[n,c,d,i]=await Promise.all([H(`api/logs.php?type=ar&gip_id=${encodeURIComponent(e.id)}`),H(`api/logs.php?type=dtr&gip_id=${encodeURIComponent(e.id)}`),H(`api/logs.php?type=docs&gip_id=${encodeURIComponent(e.id)}`),H(`api/logs.php?type=absorption&gip_id=${encodeURIComponent(e.id)}`)]);r=n.success&&n.data?.success?n.data.logs:[],t=c.success&&c.data?.success?c.data.logs:[],l=d.success&&d.data?.success?d.data.logs:[];const u=i.success&&i.data?.success?i.data.logs:[];if(u.length>0){const p=u[0];e.absorbDate=p.absorption_datetime,e.absorb_where=p.where||p.absorb_where,e.absorb_position=p.position||p.absorb_position,e.absorb_agency=p.agency||p.absorb_agency}const f={arLogs:r,dtrLogs:t,docs:l};window.__doleDB&&window.__doleDB.setSecureCache&&await window.__doleDB.setSecureCache(s,f),e.arLogs=r,e.dtrLogs=t,e.docs=l,a?document.getElementById("beneficiary-drawer-container")&&ie(e,o):ie(e,o)}catch(r){console.error("Error fetching logs/docs:",r),a||(e.arLogs=[],e.dtrLogs=[],e.docs=[],ie(e,o))}},window.showAddDataModal=function(e){ce(e)},window.editBeneficiary=function(e){Ue(e)},window.showExportConfigModal=function(e){Ve(e)},window.showProfileModal=function(){He()}}async function He(){try{if(we()&&pe){let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=a.id)}catch{}if(!e)throw new Error("User not logged in");const{data:o,error:s}=await pe.from("users").select("*").eq("id",e).single();if(s)throw s;me(o)}else{let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=`?user_id=${a.id}`)}catch{}const s=await(await fetch(`${_()}api/profile.php${e}`)).json();if(s.success){const a=s.profile;me(a)}else b.fire({icon:"error",title:"Error",text:s.error||"Failed to load profile"})}}catch(e){console.error("Error fetching profile:",e)}}function me(e){const o=e.profile_picture_path?`${_()}${e.profile_picture_path}`:null,s=e.full_name?e.full_name.split(" ").map(r=>r[0]).join("").substring(0,2).toUpperCase():"US",a=`
    ...
    `;b.fire({html:l,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:n=>{const c=n.querySelector("#cancel-modal-btn");c&&c.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),b.close(),e?._isBulk&&de.onCancel()});const d=n.querySelector("#bulk-add-btn");d&&d.addEventListener("click",()=>{b.close(),de.init()});const i=n.querySelector("#birthday-input"),u=n.querySelector("#age-display");i&&u&&(i.addEventListener("change",g=>{g.target.value&&(u.value=ve(g.target.value))}),i.value&&!e?.age&&(u.value=ve(i.value)));const f=n.querySelector("#name-input-field"),p=n.querySelector("#duplicate-warning");if(f&&p){let g;f.addEventListener("input",h=>{const v=h.target.value.trim();if(clearTimeout(g),v.length<3){p.classList.add("hidden");return}g=setTimeout(async()=>{try{const S=await(await fetch(`${_()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:v})})).json();S.success&&S.exists?(p.classList.remove("hidden"),z()?p.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":p.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):p.classList.add("hidden")}catch(y){console.error("Duplicate check error:",y)}},500)}),e?.name&&(p.classList.add("hidden"),(async()=>{const v=await(await fetch(`${_()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();v.success&&v.exists&&(p.classList.remove("hidden"),z()?p.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":p.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const m=n.querySelector("#full-id-input"),x=n.querySelector("#series-no-input"),L=n.querySelector('input[name="startDate"]'),k=n.querySelector('input[name="endDate"]'),D=n.querySelectorAll('input[name="remarks"]'),A=n.querySelector("#extension-log-container"),q=async g=>{if(!o){if(m){m.classList.add("animate-pulse"),m.placeholder="Loading...";try{const h=await H(`api/beneficiaries.php?next_id&year=${encodeURIComponent(g)}`);h.success&&h.data?.success&&h.data?.nextId&&(m.value=h.data.nextId)}finally{m.classList.remove("animate-pulse")}}if(x){x.classList.add("animate-pulse"),x.placeholder="Loading...";try{const h=await H(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(g)}`);h.success&&h.data?.success&&h.data?.nextSeries&&(x.value=h.data.nextSeries)}finally{x.classList.remove("animate-pulse")}}}},j=n.querySelector("#replacement-search-input"),O=n.querySelector("#replacement-hidden"),I=n.querySelector("#replacement-suggestions"),w=g=>{const h=(g.name||"").toUpperCase().trim(),v=g.startDateFormatted||g.startDate||"N/A",y=g.endDateFormatted||g.endDate||"N/A";return`${h} (${v.toUpperCase()} - ${y.toUpperCase()})`},B=g=>{if(I){if(!g.length){I.innerHTML=`<div class="px-3 py-2 text-[10px] font-bold ${t.textCourseOpt}">No matching beneficiary found.</div>`,I.classList.remove("hidden");return}I.innerHTML=g.map(h=>{const v=w(h);return`
                    ...
                    `}).join(""),I.classList.remove("hidden"),I.querySelectorAll(".replacement-option").forEach(h=>{h.addEventListener("click",()=>{const v=h.getAttribute("data-replacement")||"";j&&(j.value=v),O&&(O.value=v),I.classList.add("hidden")})})}};let E=null;const C=async(g="")=>{const h=(g||"").trim(),v=`api/beneficiaries.php?replacement_candidates=1&limit=20${h?`&q=${encodeURIComponent(h)}`:""}`,y=await H(v);y.success&&y.data?.success&&Array.isArray(y.data.candidates)&&B(y.data.candidates)};j&&O&&I&&(j.addEventListener("focus",()=>{C(j.value||"")}),j.addEventListener("input",()=>{O.value=j.value.trim(),clearTimeout(E),E=setTimeout(()=>{C(j.value||"")},250)}),document.addEventListener("click",g=>{!j.contains(g.target)&&!I.contains(g.target)&&I.classList.add("hidden")}));const $=()=>{const g=n.querySelector('input[name="remarks"]:checked');return g?g.value:"ONGOING"},M=g=>{const h=n.querySelector(`input[name="remarks"][value="${g}"]`);h&&(h.checked=!0,G())},F=()=>{if(k&&k.value){const g=k.value.split("-"),h=new Date(g[0],g[1]-1,g[2]),v=new Date;v.setHours(0,0,0,0);let y="ONGOING";h<v&&(y="EXPIRED"),M(y)}},G=()=>{if(!A)return;if($()==="ABSORBED"){const h=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});A.innerHTML=`
                    ...
                    `}else A.innerHTML=""};if(L&&(L.addEventListener("change",g=>{const h=g.target.value;if(h){const v=new Date(h).getFullYear();if(q(v),k&&!k.value){const y=new Date(h);y.setMonth(y.getMonth()+1),k.value=y.toISOString().split("T")[0]}F()}}),!o)){const g=L.value?new Date(L.value).getFullYear():new Date().getFullYear();q(g)}k&&k.addEventListener("change",F),D.forEach(g=>g.addEventListener("change",G));const V=n.querySelector("#resign-btn"),U=n.querySelector("#absorb-btn");V&&V.addEventListener("click",()=>M("RESIGNED")),U&&U.addEventListener("click",()=>M("ABSORBED")),n.querySelectorAll('input[type="text"], textarea').forEach(g=>{["id-number-input","full-id-input"].includes(g.id)||g.addEventListener("input",()=>{const h=g.selectionStart,v=g.selectionEnd;g.value=g.value.toUpperCase(),g.setSelectionRange(h,v)})}),F(),G(),P("education-input","course-suggestions","course-option"),P("designation-input","work-suggestions","work-option"),P("office-input","office-suggestions","office-option");function P(g,h,v){const y=n.querySelector(`#${g}`),S=n.querySelector(`#${h}`);if(!y||!S)return;let J=!1;y.addEventListener("focus",()=>S.classList.remove("hidden")),document.addEventListener("click",R=>{!y.contains(R.target)&&!S.contains(R.target)&&S.classList.add("hidden")}),y.addEventListener("input",()=>{if(J){J=!1;return}const R=y.value.toLowerCase(),W=S.querySelectorAll(`.${v}`);let ae=!1;W.forEach(Y=>{const T=Y.querySelector(".option-text");(T?T.innerText:Y.innerText).toLowerCase().includes(R)?(Y.style.display="block",ae=!0):Y.style.display="none"}),ae?S.classList.remove("hidden"):S.classList.add("hidden")}),S.querySelectorAll(`.${v}`).forEach(R=>{R.addEventListener("click",()=>{const W=R.querySelector(".option-text");y.value=W?W.innerText.trim():R.innerText.trim(),J=!0,S.classList.add("hidden"),y.dispatchEvent(new Event("change"))})})}const N=n.querySelector("#add-beneficiary-form"),ee="add_beneficiary_draft";if(!o){const g=localStorage.getItem(ee);if(g)try{const h=JSON.parse(g);Object.entries(h).forEach(([v,y])=>{const S=N.elements[v];S&&S.type!=="file"&&S.type!=="hidden"&&(S.value=y)})}catch(h){console.error("Error loading draft",h)}}N.addEventListener("input",g=>{if(!o){const h=new FormData(N),v={};h.forEach((y,S)=>v[S]=y),localStorage.setItem(ee,JSON.stringify(v))}}),N&&N.addEventListener("submit",g=>{g.preventDefault(),N.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(K=>{K.classList.remove("ring-2","ring-red-500","!border-red-500")});const v=new FormData(N);let y=!1;const S=K=>{const Z=N.querySelector(`[name="${K}"]`);Z&&Z.classList.add("ring-2","ring-red-500","!border-red-500"),y=!0},J=v.get("name"),R=v.get("contact"),W=v.get("startDate"),ae=v.get("endDate"),Y=(v.get("designation")||"").trim();if((!J||J.trim()===""||/[0-9]/.test(J))&&S("name"),R&&R.trim()!==""&&/[^0-9]/.test(R.replace(/[\s\-\+\(\)]/g,""))&&S("contact"),W||S("startDate"),ae||S("endDate"),y)return;const T={};v.forEach((K,Z)=>{T[Z]=K}),Y||(T.designation="N/A"),T.replacement||(T.replacement="");const te=n.querySelector("#full-id-input")?.value;o?(T.id=e?.id,te&&(T.gip_id=te)):(T.id=null,te&&(T.gip_id=te)),window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(T)){if(!o){const Z="add_beneficiary_draft",Ce=N.querySelector('[name="office"]')?.value||"",Be=N.querySelector('[name="designation"]')?.value||"",Le=N.querySelector('[name="education"]')?.value||"";localStorage.setItem(Z,JSON.stringify({office:Ce,designation:Be,education:Le}))}b.close(),setTimeout(()=>{b.fire({toast:!0,position:"top-end",icon:"success",title:`Record ${o?"Updated":"Added"} Successfully`,showConfirmButton:!1,timer:3e3,timerProgressBar:!0}),!o&&e?._isBulk&&de.onSaveSuccess()},100)}else b.fire({icon:"error",title:"Save Failed",text:"There was an error saving the record to the database."})})()})}})}window.handleContactSubmit=async function(e){e.preventDefault();const o=e.target,s=o.querySelector('button[type="submit"]'),a=s.innerHTML;s.disabled=!0,s.innerHTML=`
        <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        Sending...
    `;try{const r=new FormData(o);if((await fetch(o.action,{method:"POST",body:r,headers:{Accept:"application/json"}})).ok)b.fire({toast:!0,position:"top-end",icon:"success",title:"Message Sent Successfully!",text:"Thank you for reaching out. I will get back to you soon!",showConfirmButton:!1,timer:4e3,timerProgressBar:!0,didOpen:l=>{l.addEventListener("mouseenter",b.stopTimer),l.addEventListener("mouseleave",b.resumeTimer)},customClass:{popup:"rounded-2xl border-l-4 border-teal-500 shadow-2xl"}}),o.reset();else throw new Error("Failed to send")}catch{b.fire({toast:!0,position:"top-end",icon:"error",title:"Oops! Something went wrong.",text:"Could not send your message. Please try again later.",showConfirmButton:!1,timer:4e3,customClass:{popup:"rounded-2xl border-l-4 border-red-500 shadow-2xl"}})}finally{s.disabled=!1,s.innerHTML=a}return!1};export{ye as _,H as a,oe as b,Qe as c,rt as d,je as e,tt as f,_ as g,We as h,we as i,ot as j,Ye as k,X as l,Ke as m,Ze as n,at as o,Pe as p,et as r,pe as s,ze as u};
