...
import C from"./vendor-swal-BSk0fVSb.js";import{a as qe}from"./vendor-DHtNC8Ux.js";const Fe="true".toLowerCase()==="true";function me(){return Fe}function se(){const e=window.location.pathname,r="/dole-system/",n=e.toLowerCase().indexOf(r.toLowerCase());if(n!==-1)return e.substring(0,n+r.length);const a=e.indexOf("/frontend/");if(a!==-1)return e.substring(0,a+1);const o=e.indexOf("/backend/");return o!==-1?e.substring(0,o+1):"/"}function De(e="Incorrect Username or Password"){C.fire({html:`
        ...
        `,timer:e?800:3e3,timerProgressBar:!0,showConfirmButton:!1,showCloseButton:!1,width:"350px",padding:"0",customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl overflow-hidden border border-gray-100",timerProgressBar:"bg-green-500 h-1.5"},backdrop:"rgba(0, 0, 0, 0.4)"})}function ut(){const e=localStorage.getItem("hasVisitedBefore"),r=document.getElementById("page-loader");e&&(document.documentElement.classList.add("loaded"),r&&(r.style.display="none")),window.addEventListener("load",()=>{const n=document.querySelector("body > *:not(.page-loader)");n&&n.classList.add("content-fade-in"),setTimeout(()=>{document.documentElement.classList.add("loaded"),r&&r.classList.add("hidden"),document.body.style.overflow="auto",localStorage.setItem("hasVisitedBefore","true")},300)})}const we={_key:"DOLE-GIP-SECURE-KEY-2026",encrypt:function(e){try{const r=o=>o.split("").map(t=>t.charCodeAt(0)),n=o=>("0"+Number(o).toString(16)).substr(-2),a=o=>r(this._key).reduce((t,f)=>t^f,o);return e.split("").map(r).map(a).map(n).join("")}catch(r){return console.error("Encryption Failed",r),null}},decrypt:function(e){try{const r=a=>a.split("").map(o=>o.charCodeAt(0)),n=a=>r(this._key).reduce((o,t)=>o^t,a);return e.match(/.{1,2}/g).map(a=>parseInt(a,16)).map(n).map(a=>String.fromCharCode(a)).join("")}catch(r){return console.error("Decryption Failed",r),null}}};function gt(){document.querySelectorAll(".login-form-shared").forEach(r=>{const n=r.querySelector('input[name="username"]'),a=r.querySelector('input[name="password"]'),o=r.querySelector('input[name="rememberMe"]');if(n&&a&&o){const t=localStorage.getItem("secure_user"),f=localStorage.getItem("secure_pass");if(t&&f){const s=we.decrypt(t),m=we.decrypt(f);s&&m&&(n.value=s,a.value=m,o.checked=!0)}}r.addEventListener("submit",async t=>{t.preventDefault();try{const s=await(await fetch(`${se()}api/auth.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:n.value,password:a.value})})).json();if(s.success){o.checked?(localStorage.setItem("secure_user",we.encrypt(n.value)),localStorage.setItem("secure_pass",we.encrypt(a.value))):(localStorage.removeItem("secure_user"),localStorage.removeItem("secure_pass"));const m=localStorage.getItem("hasLoggedInBefore")==="true";localStorage.setItem("isLoggedIn","true"),localStorage.setItem("hasLoggedInBefore","true"),localStorage.setItem("user",JSON.stringify(s.user));const w=document.getElementById("drawer-login");if(w){w.classList.add("translate-y-full");const l=w.querySelector("[data-drawer-hide]");l&&l.click()}await Ve(m),Ue(m)}else{const m=document.getElementById("drawer-login");m?(m.classList.add("translate-y-full"),setTimeout(()=>{De(),setTimeout(()=>{m.classList.remove("translate-y-full"),a.value="",a.focus()},600)},400)):(De(),a.value="",a.focus())}}catch(f){console.error("Login Error:",f),C.fire({icon:"error",title:"Login Error",text:"Unable to reach the server. Please check your connection."})}})})}function Ue(e=!1){const r=document.getElementById("left-panel"),n=document.getElementById("right-panel"),a=document.getElementById("left-panel-content"),o=document.getElementById("right-panel-content");a&&(a.style.opacity="0"),o&&(o.style.opacity="0");const t=document.createElement("div");t.className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none";const f=e?"":"animate__delay-1s",s=e?"animation-duration: 0.8s;":"animation-duration: 2s;";t.innerHTML=`
        <img src="${se()}frontend/images/logo/doleiligan.png" class="w-64 h-64 md:w-96 md:h-96 object-contain bg-white/20 backdrop-blur-sm shadow-2xl rounded-full p-4 animate__animated animate__rotateOut ${f}" style="${s}" alt="DOLE Logo">
    `,document.body.appendChild(t);const m=e?0:1e3,w=e?600:1500;setTimeout(()=>{r&&r.classList.add("animate-slide-left"),n&&n.classList.add("animate-slide-right"),setTimeout(()=>{window.location.href=`${se()}frontend/dashboard/`},w)},m)}function bt(){const e=document.getElementById("logoutBtn");e&&e.addEventListener("click",()=>{localStorage.removeItem("isLoggedIn"),window.location.href=`${se()}`})}function ft(){const e=document.getElementById("mobile-splash"),r=document.getElementById("show-login-btn"),n=document.getElementById("back-to-splash"),a=document.getElementById("mobile-bg-content"),o=document.getElementById("mobile-welcome-text"),t=document.getElementById("reopen-login-drawer"),f=document.getElementById("request-notifications-btn"),s=async()=>{if("Notification"in window){const c=await Notification.requestPermission();console.log("Notification permission:",c),c==="granted"&&f&&f.classList.add("hidden")}};Notification.permission==="default"&&f&&(f.classList.remove("hidden"),f.addEventListener("click",s));const m=()=>{e&&(e.style.transform="translateY(-100%)",setTimeout(()=>{e.style.visibility="hidden",e.style.pointerEvents="none",e.style.zIndex="-1",Notification.permission==="default"&&s();const c=document.getElementById("drawer-login");c&&c.classList.remove("translate-y-full")},800))},w=()=>{e&&(e.style.zIndex="9999",e.style.pointerEvents="auto",e.style.visibility="visible",e.style.transform="translateY(0)")};r&&r.addEventListener("click",m),document.querySelectorAll(".forgot-password-link").forEach(c=>{c.addEventListener("click",A=>{A.target.closest("#mobile-splash")&&m()})}),n&&n.addEventListener("click",()=>{const c=document.getElementById("drawer-login");if(c){c.classList.add("translate-y-full");const A=c.querySelector("[data-drawer-hide]");A&&A.click()}w()});const d=document.getElementById("drawer-login"),L=document.getElementById("curved-welcome"),h=document.getElementById("peoples-bg");d&&new MutationObserver(A=>{A.forEach(E=>{E.attributeName==="class"&&(d.classList.contains("translate-y-full")?(a&&(a.style.transform="translateY(0)"),o&&(o.style.opacity="1",o.style.transform="translateY(0) scale(1)"),L&&(L.style.opacity="0",L.style.transform="scale(0.5)"),t&&e&&e.style.visibility==="hidden"&&(t.style.opacity="1",t.style.transform="scale(1)"),h&&(h.classList.add("opacity-0","scale-0"),h.classList.remove("opacity-40","scale-[1.6]"))):(a&&(a.style.transform="translateY(-35%)"),o&&(o.style.opacity="0",o.style.transform="translateY(20px) scale(0.9)"),L&&(L.style.opacity="1",L.style.transform="scale(1)"),t&&(t.style.opacity="0",t.style.transform="scale(0)"),h&&(h.classList.remove("opacity-0","scale-0"),h.classList.add("opacity-40","scale-[1.6]"))))})}).observe(d,{attributes:!0})}const he=()=>"false".toLowerCase()==="true";function Ge(e){try{return JSON.stringify(e)}catch{return"[unserializable]"}}const ge={debug(...e){he()&&console.debug(...e)},info(...e){he()&&console.info(...e)},warn(...e){he()&&console.warn(...e)},error(...e){console.error(...e)},table(e){he()&&console.table(e)},json(e,r){he()&&console.debug(e,Ge(r))}},ye=new Map;async function ve(e,r={}){const a=`${se()}${e}`;let o=null;try{const l=JSON.parse(localStorage.getItem("user"));l&&(o=l.user_id||l.id||null)}catch{}const t={headers:{"Content-Type":"application/json",...o?{"X-User-Id":o}:{},...r.headers},...r},s=(t.method||"GET").toUpperCase()==="GET"?2:1;let m=null;for(let l=1;l<=s;l++)try{if(ge.debug("[API] Request",{url:a,method:t.method||"GET",hasUserId:!!o}),t.body)try{ge.json("[API] Payload",JSON.parse(t.body))}catch{ge.debug("[API] Payload (raw)",t.body)}const d=await fetch(a,t);if(!d.ok)throw new Error(`HTTP ${d.status}: ${d.statusText}`);const L=await d.json();return ye.has(a)&&(ye.delete(a),ge.info?.("[API] Recovered",{url:a})),ge.debug("[API] Response",{url:a,ok:!0}),{success:!0,data:L}}catch(d){if(m=d,d instanceof TypeError&&/fetch/i.test(d.message||"")&&l<s){await new Promise(c=>setTimeout(c,1200));continue}}return m instanceof TypeError&&/fetch/i.test(m.message||"")?ye.get(a)||(ye.set(a,!0),ge.error("API Request Network Error (suppressed for repeats):",{url:a,message:m.message})):ge.error("API Request Error:",m),{success:!1,error:m?.message||"Unknown request error"}}async function K(e){return ve(e,{method:"GET"})}async function Se(e,r){return ve(e,{method:"POST",body:JSON.stringify(r)})}async function ze(e,r){return ve(e,{method:"PUT",body:JSON.stringify(r)})}async function ht(e,r){const n=new URLSearchParams(r).toString();return ve(`${e}?${n}`,{method:"PATCH"})}class Ye{constructor(){this.intervals=new Map,this.isPageVisible=!0,document.addEventListener("visibilitychange",()=>{this.isPageVisible=!document.hidden,this.isPageVisible?console.log("[Polling] Page visible - resuming normal activity"):console.log("[Polling] Page hidden - reducing activity")})}start(r,n,a=1e4){this.stop(r),n();const o=setInterval(async()=>{this.isPageVisible&&await n()},a);this.intervals.set(r,o),console.log(`[Polling] Started: ${r} (every ${a}ms)`)}stop(r){this.intervals.has(r)&&(clearInterval(this.intervals.get(r)),this.intervals.delete(r),console.log(`[Polling] Stopped: ${r}`))}stopAll(){this.intervals.forEach((r,n)=>this.stop(n)),console.log("[Polling] All intervals stopped")}getActivePolls(){return Array.from(this.intervals.keys())}}const Je=new Ye;function xt(){typeof window.initFlowbite=="function"?(window.initFlowbite(),console.log("[Flowbite] Components re-initialized")):console.warn("[Flowbite] initFlowbite() not available globally")}function mt(e){return JSON.stringify(e)}window.addEventListener("beforeunload",()=>{Je.stopAll()});const Ze="modulepreload",We=function(e,r){return new URL(e,r).href},Ie={},Te=function(r,n,a){let o=Promise.resolve();if(n&&n.length>0){let w=function(l){return Promise.all(l.map(d=>Promise.resolve(d).then(L=>({status:"fulfilled",value:L}),L=>({status:"rejected",reason:L}))))};const f=document.getElementsByTagName("link"),s=document.querySelector("meta[property=csp-nonce]"),m=s?.nonce||s?.getAttribute("nonce");o=w(n.map(l=>{if(l=We(l,a),l in Ie)return;Ie[l]=!0;const d=l.endsWith(".css"),L=d?'[rel="stylesheet"]':"";if(a)for(let c=f.length-1;c>=0;c--){const A=f[c];if(A.href===l&&(!d||A.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${l}"]${L}`))return;const h=document.createElement("link");if(h.rel=d?"stylesheet":Ze,d||(h.as="script"),h.crossOrigin="",h.href=l,m&&h.setAttribute("nonce",m),document.head.appendChild(h),d)return new Promise((c,A)=>{h.addEventListener("load",c),h.addEventListener("error",()=>A(new Error(`Unable to preload CSS for ${l}`)))})}))}function t(f){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=f,window.dispatchEvent(s),!s.defaultPrevented)throw f}return o.then(f=>{for(const s of f||[])s.status==="rejected"&&t(s.reason);return r().catch(t)})};let ce=null;if(me()){const e="https://llnddycvbcetztzwbdpx.supabase.co",r="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxsbmRkeWN2YmNldHp0endiZHB4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ1MDMwMTEsImV4cCI6MjA5MDA3OTAxMX0.apIpDlZYja7SNuA-joXgFSvu_wtbBcVVP3OQSPrh7fA";try{ce=qe(e,r),console.log("[SUPABASE SDK] Client Initialized Successfully")}catch(n){console.error("[SUPABASE SDK] Failed to initialize client:",n)}}else console.log("[SUPABASE SDK] Supabase mode is disabled (Localhost PHP mode active).");function Xe(e=new Date().getFullYear()){const r=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"],n=[];for(let a=0;a<12;a++){const o=new Date(e,a+1,0).getDate();n.push(`${r[a]} 1-15, ${e}`),n.push(`${r[a]} 16-${o}, ${e}`)}return n}function Ke(e,r,n){if(n==="ar")return(e.period||"").toUpperCase().trim()===r.toUpperCase().trim();{const a=r.match(/([A-Z]+)\s+(\d+)-(\d+),\s+(\d+)/i);if(!a)return!1;const t=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"].indexOf(a[1].toUpperCase());if(t===-1)return!1;const f=parseInt(a[4]),s=parseInt(a[2]),m=parseInt(a[3]),w=(e.date||"").substring(0,10),l=new Date(w+"T00:00:00");return isNaN(l)?!1:l.getFullYear()===f&&l.getMonth()===t&&l.getDate()>=s&&l.getDate()<=m}}function Qe(e){if(!e)return"-";const r=e.toUpperCase();return r==="VERIFIED"||r==="COMPLETED"?"✓":r==="REJECTED"||r==="DECLINED"?"X":r==="PENDING"?"?":r}function Be(e,r,n,a){const o=e.map(t=>{const f=r[t.id]||[],s=a.map(m=>{const w=f.find(l=>Ke(l,m,n));return w?Qe(w.status):"-"});return{name:t.name||t.id,cells:s}});return{periods:a,rows:o}}function Ae(e,r,n){const{periods:a,rows:o}=r,t=a.length+1;let f='<table border="1" style="border-collapse:collapse;font-family:Arial,sans-serif;font-size:10px;">';return f+=`<tr><td colspan="${t}" style="background:${n};color:#fff;font-weight:bold;font-size:13px;padding:8px 12px;text-transform:uppercase;letter-spacing:1px;">${e}</td></tr>`,f+=`<tr><th style="background:${n};color:#fff;padding:6px 10px;min-width:200px;text-align:left;">BENEFICIARY NAME</th>`,a.forEach(s=>{f+=`<th style="background:${n};color:#fff;padding:6px 8px;text-align:center;white-space:nowrap;">${s}</th>`}),f+="</tr>",o.forEach((s,m)=>{const w=m%2===0?"#ffffff":"#f5f5f5";f+="<tr>",f+=`<td style="background:${w};padding:5px 10px;font-weight:bold;text-transform:uppercase;">${s.name}</td>`,s.cells.forEach(l=>{f+=`<td style="background:${w};padding:5px 8px;text-align:center;font-weight:bold;color:${l==="✓"?"#15803d":l==="X"?"#dc2626":"#9ca3af"};">${l}</td>`}),f+="</tr>"}),f+="</table>",f}async function vt(e){const r="flex flex-col items-center justify-center p-3 rounded-2xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[10px] gap-1.5",n=e.length,a=await C.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Export DTR/AR',html:`
        ...
        `,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Generate Excel</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Cancel</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-emerald-600 text-white text-xs px-6 py-2.5 rounded-xl shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1,didOpen:()=>{document.querySelectorAll('input[name="exp-count"]').forEach(m=>{m.addEventListener("change",()=>{const w=document.getElementById("custom-count-wrap");w.classList.toggle("hidden",m.value!=="custom"||!m.checked);const l=document.querySelector('input[name="exp-count"]:checked');w.classList.toggle("hidden",l?.value!=="custom")})})},preConfirm:()=>{const m=document.querySelector('input[name="exp-type"]:checked')?.value||"dtr",w=document.querySelector('input[name="exp-count"]:checked')?.value||"10";let l=parseInt(w==="custom"?document.getElementById("exp-custom-count")?.value||n:w,10);(isNaN(l)||l<1)&&(l=10),l=Math.min(l,n);const d=parseInt(document.getElementById("exp-year")?.value||new Date().getFullYear(),10);return{type:m,count:l,year:d}}});if(!a.isConfirmed||!a.value)return;const{type:o,count:t,year:f}=a.value,s=e.slice(0,t);await _e(s,o,f)}async function _e(e,r,n){C.fire({title:'<span class="text-lg font-black text-emerald-600 uppercase tracking-tight">Building Excel File</span>',html:`<div class="flex flex-col items-center p-4">
        ...
        </div>`,showConfirmButton:!1,allowOutsideClick:!1,customClass:{popup:"rounded-[1.5rem] shadow-2xl font-montserrat dark:bg-slate-900 border border-gray-100 dark:border-slate-800"}});try{const a=Xe(n),o=se();async function t(A){const I=await(await fetch(`${o}api/logs.php?type=${A}`,{headers:{"X-User-Id":(()=>{try{return JSON.parse(localStorage.getItem("user"))?.id||""}catch{return""}})()}})).json();return I.success?I.logs||[]:[]}let f={},s={};(r==="dtr"||r==="both")&&(await t("dtr")).forEach(E=>{const I=String(E.gip_id||E.beneficiary_id||E.id||"");f[I]||(f[I]=[]),f[I].push(E)}),(r==="ar"||r==="both")&&(await t("ar")).forEach(E=>{const I=String(E.gip_id||E.beneficiary_id||E.id||"");s[I]||(s[I]=[]),s[I].push(E)});const m=e.map(A=>({...A,mapKey:String(A.id||A.gip_id||A.beneficiary_id)}));let w="";const l=new Intl.DateTimeFormat("en-PH",{timeZone:"Asia/Manila",dateStyle:"long",timeStyle:"short"}).format(new Date);if(r==="dtr"||r==="both"){const A=m.map(I=>({...I,id:I.mapKey})),E=Be(A,f,"dtr",a);w+="<br>"+Ae(`DTR – Daily Time Records (${n})`,E,"#1d4ed8")}if(r==="ar"||r==="both"){const A=m.map(I=>({...I,id:I.mapKey})),E=Be(A,s,"ar",a);w+="<br><br>"+Ae(`AR – Accomplishment Reports (${n})`,E,"#d97706")}const d=`
    ...
    <div class="grid grid-cols-2 gap-4 mt-6 w-full">
        ...
        <div class="flex flex-col gap-1 text-left overflow-hidden">
            ...
            <div class="flex items-center flex-wrap gap-2.5 min-h-[30px] font-montserrat">
                <span class="${at(e.remarks)} text-[10px] sm:text-[11px] font-black px-4 sm:px-6 py-2 rounded-lg border uppercase tracking-widest shadow-sm border-l-4 ${e.remarks==="ONGOING"?"border-l-green-600":"border-l-red-600"} min-w-[110px] sm:min-w-[140px] text-center inline-block">${e.remarks}</span>
                <span class="${ot(e.office)} inline-block text-[9px] sm:text-[10px] font-black px-4 sm:px-6 py-2 rounded-full border shadow-sm truncate max-w-[180px] sm:max-w-[280px] lg:max-w-none min-w-[110px] sm:min-w-[140px] text-center" title="${e.office}">${e.office}</span>
            </div>
        </div>
    </div>
</div>
    ...
    <div id="drawer-page-0" class="flex-1 flex flex-col gap-4">
        ...
        <div class="bg-[#e8f5e9]/50 dark:bg-green-900/10 p-4 rounded-xl border border-[#c8e6c9] dark:border-green-900/30 mt-2">
            ...
            <div class="flex flex-col gap-2">
                <div class="flex justify-between items-start group">
                    ...
                    <span class="font-black text-[#1b5e20] dark:text-green-400 text-xs text-right whitespace-nowrap">${(()=>{if(!e.absorbDate||String(e.absorbDate).includes("0000-00-00"))return"N/A";const h=new Date(e.absorbDate);return isNaN(h.getTime())||h.getFullYear()<1900?"N/A":(h.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric"})+" "+h.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})).toUpperCase()})()}</span>
                </div>
                <div class="flex justify-between items-start group">
        ...
        <div class="bg-[#ffebee]/50 dark:bg-red-900/10 p-4 rounded-xl border border-[#ffcdd2] dark:border-red-900/30 mt-2">
            ...
            <div class="flex flex-col gap-2">
                <div class="flex justify-between items-start group">
                    <span class="text-gray-500 font-medium text-[10px] uppercase font-bold tracking-widest mr-4">Date</span>
                    <span class="font-black text-[#b71c1c] dark:text-red-400 text-xs text-right whitespace-nowrap">${(()=>{if(!e.resignedDate||String(e.resignedDate).includes("0000-00-00"))return"N/A";const h=new Date(e.resignedDate);return isNaN(h.getTime())||h.getFullYear()<1900?"N/A":(h.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric"})+" "+h.toLocaleTimeString("en-US",{hour:"2-digit",minute:"2-digit"})).toUpperCase()})()}</span>
                </div>
                <div class="flex justify-between items-start group">
    ...
    <div id="drawer-page-1" class="hidden flex-1 flex flex-col gap-6">
         ...
         <div class="flex flex-col gap-5">
            <div>
                ...
                <div class="space-y-2">
                    ${t.length?t.map(h=>{const c=h.status||"PENDING";let A=c==="VERIFIED"||c==="COMPLETED"?"text-green-500":c==="REJECTED"||c==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500",E=h.date||h.createdAt,I=E;if(E){const j=/^\d{4}-\d{2}-\d{2}$/.test(E)?new Date(E+"T00:00:00Z"):new Date(E);isNaN(j)||(I=j.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric",timeZone:"Asia/Manila"}).toUpperCase())}return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-blue-100 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors edit-log-btn" data-type="dtr" data-id="${h.id}" data-val="${h.day||E}" data-status="${c}">
                            <span class="text-xs font-black text-royal-blue dark:text-blue-400 capitalize whitespace-nowrap pointer-events-none">${h.day||I}</span>
                            <span class="text-[11px] font-bold ${A} uppercase tracking-widest truncate max-w-[50%] text-right pr-6 group-hover:pr-12 pointer-events-none transition-all">${c}</span>
                            <button class="absolute top-0 right-0 h-full w-10 bg-red-500 hover:bg-red-600 text-white flex items-center justify-center translate-x-full group-hover:translate-x-0 transition-transform cursor-pointer delete-log-btn" data-type="dtr" data-id="${h.id}">
                ...
                <div class="space-y-2">
                    ${o.length?o.map(h=>{const c=h.status||"PENDING";let A=c==="VERIFIED"||c==="COMPLETED"?"text-green-500":c==="REJECTED"||c==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500",E=h.period||h.createdAt,I=E;if(E){const j=/^\d{4}-\d{2}-\d{2}$/.test(E)?new Date(E+"T00:00:00Z"):new Date(E);isNaN(j)||(I=j.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric",timeZone:"Asia/Manila"}).toUpperCase())}return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-orange-100 dark:border-orange-900/50 bg-orange-50/50 dark:bg-orange-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-orange-100 dark:hover:bg-orange-900/40 transition-colors edit-log-btn" data-type="ar" data-id="${h.id}" data-val="${E}" data-status="${c}">
                            ...
                            <span class="text-[11px] font-bold ${A} uppercase tracking-widest truncate max-w-[50%] text-right pr-6 group-hover:pr-12 pointer-events-none transition-all">${c}</span>
                            <button class="absolute top-0 right-0 h-full w-10 bg-red-500 hover:bg-red-600 text-white flex items-center justify-center translate-x-full group-hover:translate-x-0 transition-transform cursor-pointer delete-log-btn" data-type="ar" data-id="${h.id}">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>
                            </button>
                        </div>
            ...
            ${m.map(h=>{const c=h.status.toUpperCase(),E={VERIFIED:"COMPLETED",DECLINED:"REJECTED",PENDING:"PENDING"}[c]||c,I=E==="COMPLETED",j=E==="REJECTED",re=I?"text-green-500":j?"text-red-500":"text-gray-400 dark:text-gray-500",X=I?"bg-green-50/50 dark:bg-green-900/10":j?"bg-red-50/50 dark:bg-red-900/10":"bg-gray-50/50 dark:bg-slate-800/50",U=I?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800 hover:bg-green-200 cursor-pointer":j?"bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800 hover:bg-red-200 cursor-pointer":"bg-white text-gray-500 border-gray-200 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer";let ne='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';return I?ne='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>':j&&(ne='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>'),`
                <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-0.5 ${X}">
                    <div class="flex items-center gap-3 w-full">
                        ...
                        <span class="text-xs sm:text-sm font-black ${I?"text-heading":"text-gray-500 dark:text-gray-400"} uppercase tracking-tight flex-1">${h.name}</span>
                    </div>
                    <button type="button" class="ml-3 ${U} text-[10px] font-black px-3 py-1.5 rounded-full border uppercase tracking-widest transition-colors flex-shrink-0 drawer-doc-btn" data-id="${h.id}" data-name="${h.name}" data-status="${E} cursosr-pointer">
                        ${E}
                    </button>
                </div>
                `}).join("")}
...
<style>
    ...
    `,l=!!e._noAnimation;let d=document.getElementById("beneficiary-drawer-container");const L=l&&!!d&&d.dataset.beneficiaryId===String(e.id||"");if(L){const h=d.scrollTop;d.innerHTML=w,d.scrollTop=h}else d&&(d.remove(),document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden")),d=document.createElement("div"),d.id="beneficiary-drawer-container",d.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-neutral-primary-soft dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl",d.setAttribute("tabindex","-1"),d.setAttribute("data-drawer-backdrop","true"),d.innerHTML=w,document.body.appendChild(d),document.documentElement.classList.add("overflow-hidden"),document.body.classList.add("overflow-hidden");d.dataset.beneficiaryId=String(e.id||""),Te(async()=>{const{Drawer:h}=await import("./vendor-flowbite-B7rSfpuT.js").then(c=>c.b);return{Drawer:h}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:h})=>{let c=L?d.__drawerInstance:null;if(!c){const O={placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden"),setTimeout(()=>{d&&d.parentNode&&d.remove()},300)}};c=new h(d,O),d.__drawerInstance=c,c.show()}d.querySelector("#close-drawer-btn").addEventListener("click",()=>c.hide());const E=d.querySelector("#drawer-prev-btn"),I=d.querySelector("#drawer-next-btn"),j=2,re=()=>{d.querySelectorAll("[id^=drawer-page-]").forEach((T,G)=>{T.classList.toggle("hidden",G!==a)});const O=d.querySelector("#drawer-section-title");O&&O.classList.toggle("invisible",a!==0);const _=d.querySelector("#personal-profile-section");_&&_.classList.toggle("hidden",a!==0),E.disabled=a===0,I.disabled=a===j,E.classList.toggle("opacity-50",a===0),I.classList.toggle("opacity-50",a===j)};E.addEventListener("click",()=>{a>0&&a--,re()}),I.addEventListener("click",()=>{a<j&&a++,re()}),re(),d.querySelectorAll(".drawer-doc-btn").forEach(O=>{O.addEventListener("click",async()=>{const _=O.dataset.name,T=O.dataset.status,G="flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[9px] gap-2 transition-all duration-300 ",p=await C.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Update Document</span>',html:`
                        <div class="font-montserrat text-left">
                            <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-4 ps-1">Set status for <span class="text-brand font-black">${_}</span></label>
                    ...
                    <div class="flex flex-col gap-5 text-left font-montserrat mt-2">
                        ...
                        <div>
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-log-status" value="VERIFIED" class="peer sr-only" ${p==="VERIFIED"?"checked":""}>
                                ...
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-log-status" value="REJECTED" class="peer sr-only" ${p==="REJECTED"||p==="DECLINED"?"checked":""}>
                ...
                `,focusConfirm:!1,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">SAVE REVISIONS</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">CANCEL</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2"},buttonsStyling:!1,preConfirm:()=>{const q=document.querySelector('input[name="swal-log-status"]:checked');return{val:document.getElementById("swal-log-val").value.trim().toUpperCase(),status:q?q.value:"PENDING"}}});if(v&&(v.val!==G||v.status!==p))try{const q={type:_,id:T,status:v.status};if(_==="dtr"){const b=v.val.toUpperCase().match(/([A-Z]{3})\s+(\d+)-\d+,\s*(\d{4})/);if(b){const y=U.indexOf(b[1]),x=parseInt(b[2]),D=parseInt(b[3]);let M=new Date(D,y,x);for(;!ne(M.toISOString().split("T")[0]);)M.setDate(M.getDate()+1);q.record_date=M.toISOString().split("T")[0]}else q.record_date=new Date().toISOString().split("T")[0];q.weekday=v.val}_==="ar"&&(q.period=v.val);const i=await ze("api/logs.php",q),g=i.success?i.data:{success:!1,error:i.error};g.success?(C.fire({toast:!0,position:n,icon:"success",title:"Log Updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):C.fire("Error",g.error||"Failed to update log.","error")}catch(q){C.fire("Error",q.message,"error")}},pe=d.querySelector("#add-dtr-log-btn");pe&&pe.addEventListener("click",()=>de("dtr",ie()));const ue=d.querySelector("#add-ar-log-btn");ue&&ue.addEventListener("click",()=>de("ar",Q()));const be=d.querySelector("#export-log-btn");be&&be.addEventListener("click",async()=>{const O="flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[11px] gap-2 ",_=await C.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Export DTR/AR',html:`
                        <div class="font-montserrat text-left">
...
.pulse-highlight {
    ...
    `;let s=document.getElementById("edit-drawer-container");s&&(s.remove(),document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden")),s=document.createElement("div"),s.id="edit-drawer-container",s.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-white dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",s.setAttribute("tabindex","-1"),s.innerHTML=f,document.body.appendChild(s),document.documentElement.classList.add("overflow-hidden"),document.body.classList.add("overflow-hidden"),setTimeout(()=>{const l=s.querySelector('textarea[name="name"]');l&&(l.style.height="auto",l.style.height=l.scrollHeight+"px")},10);const m=s.querySelector("#edit-education-suggestions-box");m&&(m.innerHTML=Le.map(l=>`
        ...
        `).join("")),Te(async()=>{const{Drawer:l}=await import("./vendor-flowbite-B7rSfpuT.js").then(d=>d.b);return{Drawer:l}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:l})=>{const d=new l(s,{placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden"),setTimeout(()=>{s&&s.parentNode&&s.remove()},400)}});d.show(),window.initFlowbite&&window.initFlowbite();const L=()=>{document.activeElement&&typeof document.activeElement.blur=="function"&&document.activeElement.blur(),d.hide()};s.querySelector("#close-edit-drawer-btn").addEventListener("click",L),s.querySelector("#edit-drawer-cancel-btn").addEventListener("click",L);const h=s.querySelector("#edit-beneficiary-drawer-form"),c=s.querySelector("#edit-bday-input"),A=s.querySelector("#edit-age-display"),E=s.querySelector("#edit-startDate-input"),I=s.querySelector("#edit-endDate-input"),j=s.querySelector('input[name="seriesNo"]'),re=s.querySelector('input[name="gip_id"]'),X=s.querySelector("#edit-drawer-remarks"),U=s.querySelector("#edit-extension-log-container"),ne=()=>{if(!U)return;const p=X.value,u=le();if(p==="ABSORBED"){const k=e.absorbDate&&!String(e.absorbDate).includes("0000-00-00")?new Date(e.absorbDate):new Date,B=k.getTimezoneOffset()*6e4,$=new Date(k.getTime()-B).toISOString().slice(0,16);U.innerHTML=`
                    <div class="mt-4 pt-4 border-t ${u?"border-slate-800":"border-gray-100"}">
                        <p class="text-[9px] uppercase font-black ${u?"text-green-500":"text-[#2e7d32]"} border-b ${u?"border-slate-800":"border-green-100"} pb-1 flex items-center gap-2 mb-3"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg> Absorption Details</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                            <div class="group">
                                <label class="text-[9px] ${u?"text-green-500":"text-[#2e7d32]"} font-black uppercase block mb-1 cursor-pointer" for="absorbDateInput">Absorption Date</label>
                                <input type="datetime-local" id="absorbDateInput" name="absorbDate" value="${$}" class="w-full ${u?"bg-slate-800 text-white border-slate-700":"bg-green-50 text-slate-900 border-green-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm font-mono cursor-pointer">
                            </div>
                            <div class="group">
                                <label class="text-[9px] ${u?"text-slate-500":"text-gray-400"} font-black uppercase block mb-1">Where?</label>
                                <input type="text" name="absorb_where" value="${e.absorb_where||""}" class="w-full ${u?"bg-slate-800 text-white border-slate-700":"bg-gray-50 text-slate-900 border-gray-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm" placeholder="Where to absorb?">
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                            <div class="group">
                                <label class="text-[9px] ${u?"text-slate-500":"text-gray-400"} font-black uppercase block mb-1">Position</label>
                                <input type="text" name="absorb_position" value="${e.absorb_position||""}" class="w-full ${u?"bg-slate-800 text-white border-slate-700":"bg-gray-50 text-slate-900 border-gray-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm" placeholder="What kind of position?">
                            </div>
                            <div class="group">
                                <label class="text-[9px] ${u?"text-slate-500":"text-gray-400"} font-black uppercase block mb-1">Agency</label>
                                <input type="text" name="absorb_agency" value="${e.absorb_agency||""}" class="w-full ${u?"bg-slate-800 text-white border-slate-700":"bg-gray-50 text-slate-900 border-gray-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm" placeholder="On what agency?">
                            </div>
                        </div>
                    </div>
                `}else if(p==="RESIGNED"){const k=e.resignedDate&&!String(e.resignedDate).includes("0000-00-00")?new Date(e.resignedDate):new Date,B=k.getTimezoneOffset()*6e4,$=new Date(k.getTime()-B).toISOString().slice(0,16);U.innerHTML=`
                    <div class="mt-4 pt-4 border-t ${u?"border-slate-800":"border-gray-100"}">
                        <p class="text-[9px] uppercase font-black ${u?"text-red-500":"text-[#ce1126]"} border-b ${u?"border-slate-800":"border-red-100"} pb-1 flex items-center gap-2 mb-3"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg> Resignation Details</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                            <div class="group">
                                <label class="text-[9px] ${u?"text-red-500":"text-[#ce1126]"} font-black uppercase block mb-1 cursor-pointer" for="resignedDateInput">Resignation Date</label>
                                <input type="datetime-local" id="resignedDateInput" name="resignedDate" value="${$}" class="w-full ${u?"bg-slate-800 text-white border-slate-700":"bg-red-50 text-slate-900 border-red-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm font-mono cursor-pointer">
                            </div>
                            <div class="group">
                                <label class="text-[9px] ${u?"text-slate-500":"text-gray-400"} font-black uppercase block mb-1">Reason (Optional)</label>
                                <input type="text" name="resigned_reason" value="${e.resigned_reason||""}" class="w-full ${u?"bg-slate-800 text-white border-slate-700":"bg-gray-50 text-slate-900 border-gray-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm" placeholder="Why resigned?">
                            </div>
                        </div>
                    </div>
                `}};X&&X.addEventListener("change",p=>{const u="text-[10px] sm:text-[11px] font-black px-2.5 py-2.5 rounded-lg border uppercase tracking-widest shadow-sm outline-none focus:ring-2 focus:ring-brand w-full cursor-pointer transition-colors duration-300 h-[42px]";X.className=`${t(p.target.value)} ${u} editable-indicator`,ne(),(X.value==="ABSORBED"||X.value==="RESIGNED")&&setTimeout(()=>{U.scrollIntoView({behavior:"smooth",block:"center"}),U.classList.add("pulse-highlight"),setTimeout(()=>U.classList.remove("pulse-highlight"),1500)},50)}),ne();let ae=!1,ie=!1;const Q=(p,u)=>{p.addEventListener("paste",k=>{k.preventDefault();let B=(k.clipboardData||window.clipboardData).getData("text");if(B){B=B.replace(/[-.\s]/g,"/");const $=B.split("/");if($.length===3){const v=$[0].padStart(2,"0"),q=$[1].padStart(2,"0");let i=$[2];if(i.length===2){const x=new Date().getFullYear(),D=Math.floor(x/100)*100;i=String(D+parseInt(i))}else i=i.padStart(4,"0");const g=`${v}/${q}/${i}`;p.value=g;const b=new Event("input",{bubbles:!0});p.dispatchEvent(b);const y=window.__parseFormattedDate(g);if(y&&u&&(ie||u(y),document.activeElement===p&&p.blur()),p._datepicker)p._datepicker.hide();else{const x=p.parentNode&&p.parentNode._datepicker;x&&typeof x.hide=="function"&&x.hide()}}}}),p.addEventListener("input",k=>{const B=k.target.value,$=window.__maskDate(B);if(B!==$&&(k.target.value=$),$.length===10){const v=window.__parseFormattedDate($);if(v&&u)if(ie||u(v),document.activeElement===p&&p.blur(),p._datepicker)p._datepicker.hide();else{const q=p.parentNode&&p.parentNode._datepicker;q&&typeof q.hide=="function"&&q.hide()}}}),p.addEventListener("changeDate",k=>{k.detail&&k.detail.date&&u&&(ie||u(k.detail.date),p._datepicker&&p._datepicker.hide())})};c&&Q(c,p=>{A&&(!ae||!A.value)&&(A.value=window.calculateAge(p))}),E&&Q(E,p=>{if(I){const k=new Date(p);k.setDate(k.getDate()+243);const B=String(k.getMonth()+1).padStart(2,"0"),$=String(k.getDate()).padStart(2,"0"),v=k.getFullYear();I.value=`${B}/${$}/${v}`}const u=p.getFullYear();u>1900&&re&&j&&Promise.all([K(`api/beneficiaries.php?next_id&year=${encodeURIComponent(u)}`),K(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(u)}`)]).then(([k,B])=>{const $=k.success&&k.data?.success?k.data.nextId:null,v=B.success&&B.data?.success?B.data.nextSeries:null,q=String(re.value||"").match(/^ROX-RD-ESIG-(\d{4})-\d{4}$/),i=String(j.value||"").match(/^(\d{4})-\d{2}-\d{3}$/),g=q?Number(q[1]):null,b=i?Number(i[1]):null;$&&(g===null||g!==u)&&(re.value=$),v&&(b===null||b!==u)&&(j.value=v)}).catch(k=>{console.error("Edit drawer identifier sync error:",k)})}),I&&Q(I);const de=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null),ee=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null);de&&c&&(c._datepicker=new de(c,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"}));const pe=s.querySelector("#edit-date-range-picker");if(ee&&pe){const p=new ee(pe,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0,orientation:"auto"});E&&(E._datepicker=p.datepickers[0]),I&&(I._datepicker=p.datepickers[1])}e.id&&(ie=!0,K(`api/beneficiaries.php?id=${encodeURIComponent(e.id)}`).then(p=>{if(p.success&&p.data&&p.data.beneficiary){const u=p.data.beneficiary;if(E&&u.startDate){const k=new Date(u.startDate);isNaN(k)||(E.value=u.startDateFormatted||"",E._datepicker&&E._datepicker.setDate(k))}if(I&&u.endDate){const k=new Date(u.endDate);isNaN(k)||(I.value=u.endDateFormatted||"",I._datepicker&&I._datepicker.setDate(k))}}setTimeout(()=>{ie=!1},100)}).catch(p=>{console.error("Error fetching accurate beneficiary dates:",p),ie=!1})),A&&A.addEventListener("input",()=>ae=!0);const ue=(p,u,k)=>{const B=s.querySelector(p),$=s.querySelector(u);if(!B||!$)return;const v=()=>$.classList.add("hidden"),q=()=>$.classList.remove("hidden");B.addEventListener("focus",q),B.addEventListener("input",()=>{const i=B.value.toLowerCase().trim();let g=0;$.querySelectorAll(k).forEach(b=>{const x=(b.querySelector(".option-text")?.textContent||b.textContent||"").toLowerCase().includes(i);b.style.display=x?"block":"none",x&&g++}),g>0?q():v()}),$.addEventListener("click",i=>{const g=i.target.closest(k);g&&(B.value=(g.querySelector(".option-text")?.textContent||g.textContent||"").trim(),v(),B.dispatchEvent(new Event("change")))}),document.addEventListener("click",i=>{!B.contains(i.target)&&!$.contains(i.target)&&v()})};ue("#edit-education-input","#edit-education-suggestions-box",".edit-education-option"),ue("#edit-designation-input","#edit-designation-suggestions-box",".edit-designation-option"),(()=>{const p=s.querySelector("#edit-office-input"),u=s.querySelector("#edit-office-suggestions-box");if(!p||!u)return;u.classList.add("mt-[52px]");let k="OFFICES",B=null,$=[];const v={textLabel:r?"text-slate-400":"text-slate-500",borderDivide:r?"border-slate-800":"border-slate-100",courseHover:r?"hover:bg-slate-800/80":"hover:bg-blue-50",textCourseOpt:r?"text-slate-300":"text-slate-700"},q=async()=>{const g="dole_offices_cache",b=async()=>{let x=[];try{const D=await K("api/beneficiaries.php?get_offices_advanced=1");D.success&&D.data?.success&&Array.isArray(D.data.offices)&&(x=D.data.offices)}catch(D){console.error("Office fetch failed:",D)}return x.length>0&&($=x,localStorage.setItem(g,JSON.stringify({data:x,timestamp:Date.now()}))),x},y=localStorage.getItem(g);if(y)try{const{data:x,timestamp:D}=JSON.parse(y);return $=x,Date.now()-D>1800*1e3&&b().then(()=>{k==="OFFICES"&&i("OFFICES",B,p.value)}),x}catch{localStorage.removeItem(g)}return $.length===0?await b():$},i=async(g="OFFICES",b=null,y="")=>{if(k=g,B=b,g==="OFFICES"){const D=(await q()).filter(S=>S.office.toLowerCase().includes(y.toLowerCase()));u.innerHTML=`
                        ...
                        <div class="max-h-64 overflow-y-auto scrollbar-hide">
                            ${D.length>0?D.map(S=>{const R=parseInt(S.location_count||0)>0;return`
                                    <div class="office-code-option group/opt px-3 py-2 text-[9px] font-bold ${v.textCourseOpt} ${v.courseHover} rounded-lg ${R?"cursor-pointer":"cursor-default opacity-60"} transition-all flex items-center justify-between group active:scale-[0.98] mx-1 mb-0.5"
                    ...
                    `;const M=y.trim(),H=u.querySelector("#add-office-location-row-edit"),F=u.querySelector("#new-office-loc-input-edit"),J=u.querySelector("#confirm-office-with-loc-edit"),V=u.querySelector("#add-office-with-loc-btn-edit"),N=u.querySelector("#skip-office-loc-btn-edit");if(V&&H&&V.addEventListener("click",S=>{S.stopPropagation(),H.classList.remove("hidden"),H.classList.add("flex"),setTimeout(()=>F?.focus(),50)}),J&&F){const S=R=>{R.stopPropagation();const z=F.value.trim();p.value=z?`${M} - ${z}`:M,u.classList.add("hidden"),p.dispatchEvent(new Event("change"))};J.addEventListener("click",S),F.addEventListener("keydown",R=>{R.key==="Enter"&&S(R)}),F.addEventListener("click",R=>R.stopPropagation())}N&&N.addEventListener("click",S=>{S.stopPropagation(),p.value=M,u.classList.add("hidden"),p.dispatchEvent(new Event("change"))}),u.querySelectorAll(".office-code-option").forEach(S=>{S.addEventListener("click",R=>{R.stopPropagation(),S.dataset.hasLocations==="true"?i("LOCATIONS",{id:S.dataset.id,name:S.dataset.name}):(p.value=S.dataset.name,u.classList.add("hidden"),p.dispatchEvent(new Event("change")))})})}else{u.innerHTML=`
                        ...
                        <div class="p-2 border-b ${v.borderDivide}">
                            <div class="relative group">
                                ...
                                <input type="text" id="location-search-edit" placeholder="Search in ${b.name}..." 
                    ...
                    `;const x=u.querySelector("#loc-list-edit"),D=u.querySelector("#location-search-edit"),M=`dole_locs_cache_${b.id}`;let H=[];const F=localStorage.getItem(M);if(F)try{const{data:N,timestamp:S}=JSON.parse(F);H=N,Date.now()-S<3600*1e3}catch{localStorage.removeItem(M)}const J=async()=>{let N=[];if(ce&&me()){const{data:S,error:R}=await ce.from("office_locations").select("location").eq("office_id",b.id).order("location");!R&&S&&(N=S)}if(N.length===0)try{const S=await K(`api/beneficiaries.php?get_office_locations=1&office_id=${b.id}`);S.success&&S.data?.success&&Array.isArray(S.data.locations)&&(N=S.data.locations)}catch(S){console.error("Office locations fetch failed:",S)}N.length>0&&(H=N,localStorage.setItem(M,JSON.stringify({data:N,timestamp:Date.now()})),V(D.value))},V=(N="")=>{const S=H.filter(z=>z.location.toLowerCase().includes(N.toLowerCase())),R=N.trim();S.length>0?x.innerHTML=S.map(z=>`
                            ...
                            `).join(""):H.length===0?x.innerHTML=`<div class="px-3 py-4 text-center text-[9px] font-bold ${v.textLabel} animate-pulse">Fetching...</div>`:(x.innerHTML=`
                            ...
                            `,R&&x.querySelector("#add-new-location-edit")?.addEventListener("click",()=>{p.value=`${b.name} - ${R}`,u.classList.add("hidden"),p.dispatchEvent(new Event("change"))})),x.querySelectorAll(".location-option-edit").forEach(z=>{z.addEventListener("click",()=>{p.value=`${b.name} - ${z.dataset.location}`,u.classList.add("hidden"),p.dispatchEvent(new Event("change"))})})};V(D.value),J(),setTimeout(()=>D.focus(),50),D.addEventListener("input",()=>V(D.value)),D.addEventListener("click",N=>N.stopPropagation()),u.querySelector("#back-to-offices-edit").addEventListener("click",N=>{N.stopPropagation(),i("OFFICES")})}};p.addEventListener("focus",()=>{u.classList.remove("hidden"),i(k,B,p.value)}),p.addEventListener("input",()=>{k==="OFFICES"&&i("OFFICES",null,p.value)}),document.addEventListener("click",g=>{!p.contains(g.target)&&!u.contains(g.target)&&u.classList.add("hidden")})})();const O=s.querySelector("#edit-replacement-input"),_=s.querySelector("#edit-replacement-suggestions-box"),T=s.querySelector("#edit-replacement-loading");let G=null;O&&_&&(O.addEventListener("input",p=>{const u=p.target.value.trim();clearTimeout(G),_.classList.add("hidden"),!(u.length<2)&&(T&&T.classList.remove("hidden"),G=setTimeout(async()=>{try{const k=await ve(`api/beneficiaries.php?replacement_candidates=1&q=${encodeURIComponent(u)}&limit=10`);k.success&&k.data&&k.data.candidates&&k.data.candidates.length>0?(_.innerHTML=k.data.candidates.map(B=>`
            ...
            <form id="export-config-form" class="space-y-6">
                ...
                <div class="bg-gray-50/50 rounded-2xl p-4 border border-gray-100 mt-4">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-2">
                        ${["ID","Name","Age","Office","Position","Start Date","End Date","Status"].map(a=>{const o=a.toLowerCase().replace(" ",""),t=r.columns.includes(o),f=`col-switch-${o}`;return`
                                <label for="${f}" class="flex items-center gap-2 px-3 py-2 bg-white border border-gray-100 rounded-lg cursor-pointer hover:border-emerald-500/30 transition-all group select-none shadow-sm">
                                    <div class="relative flex items-center shrink-0 scale-90">
                                        <input type="checkbox" id="${f}" name="export-column" value="${o}" ${t?"checked":""} class="sr-only peer">
                        ...
                        <div class="group">
                            ...
                            <div class="flex flex-wrap gap-2 items-center">
                                <div class="flex flex-wrap gap-2 p-1.5 ${t.bgStatusWrap} border ${t.borderStatus} rounded-xl shadow-inner flex-1">
                                    ${(()=>{const s={ONGOING:"peer-checked:bg-green-400 peer-checked:text-white peer-checked:border-green-400",EXPIRED:"peer-checked:bg-red-400 peer-checked:text-white peer-checked:border-red-400",RESIGNED:"peer-checked:bg-[#ce1126] peer-checked:text-white peer-checked:border-[#ce1126]",ABSORBED:"peer-checked:bg-[#2e7d32] peer-checked:text-white peer-checked:border-[#2e7d32]"};return["ONGOING","EXPIRED","RESIGNED","ABSORBED"].map(m=>`
                                            <label class="cursor-pointer relative min-w-[80px]">
                                                <input type="radio" name="remarks" value="${m}" ${e?.remarks===m?"checked":""} class="hidden peer remarks-radio">
                                                <span class="px-3 py-1.5 rounded-lg border border-transparent text-[10px] font-black text-gray-400 dark:text-white! uppercase tracking-widest ${s[m]||""} transition-all block text-center cursor-pointer shadow-sm">
                                                    ${m}
                                                </span>
                                            </label>
                                        `).join("")})()}
    ...
    `;C.fire({html:f,width:window.innerWidth<640?"96vw":window.innerWidth<1024?"90vw":"1120px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<640?"0.75rem":window.innerWidth<1024?"1.25rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},willOpen:()=>{document.documentElement.classList.add("overflow-hidden"),document.body.classList.add("overflow-hidden")},didClose:()=>{document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden")},didOpen:s=>{window.initFlowbite&&window.initFlowbite();const m=s.querySelector("#cancel-modal-btn");m&&m.addEventListener("click",()=>{!r&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),C.close(),e?._isBulk&&$e.onCancel()});const w=s.querySelector("#bulk-add-btn");w&&w.addEventListener("click",()=>{C.close(),$e.init()});const l=(i,g)=>{i.addEventListener("paste",b=>{b.preventDefault();let y=(b.clipboardData||window.clipboardData).getData("text");if(y){y=y.replace(/[-.\s]/g,"/");const x=y.split("/");if(x.length===3){const D=x[0].padStart(2,"0"),M=x[1].padStart(2,"0");let H=x[2];if(H.length===2){const S=new Date().getFullYear(),R=Math.floor(S/100)*100;H=String(R+parseInt(H))}else H=H.padStart(4,"0");const F=`${D}/${M}/${H}`;i.value=F;const J=new Event("input",{bubbles:!0});i.dispatchEvent(J);const V=window.__parseFormattedDate(F);V&&g&&(g(V),document.activeElement===i&&i.blur());const N=i._datepicker||i.parentNode&&i.parentNode._datepicker;N&&typeof N.hide=="function"&&N.hide()}}}),i.addEventListener("input",b=>{const y=window.__maskDate(b.target.value);if(b.target.value!==y&&(b.target.value=y),y.length===10){const x=window.__parseFormattedDate(y);if(x&&g){g(x),document.activeElement===i&&i.blur();const D=i._datepicker||i.parentNode&&i.parentNode._datepicker;D&&typeof D.hide=="function"&&D.hide()}}}),i.addEventListener("changeDate",b=>{if(b.detail&&b.detail.date&&g){g(b.detail.date);const y=i._datepicker||i.parentNode&&i.parentNode._datepicker;y&&typeof y.hide=="function"&&y.hide()}})},d=s.querySelector("#birthday-input"),L=s.querySelector("#age-display"),h=s.querySelector("#age-warning"),c=s.querySelector("#submit-beneficiary-btn"),A=i=>{if(!i)return h&&h.classList.add("hidden"),c&&(c.disabled=!1,c.classList.remove("opacity-50","cursor-not-allowed","grayscale"),c.classList.add("cursor-pointer")),!0;const g=parseInt(i),b=!isNaN(g)&&g>=18&&g<=29;return h&&(h.className=`mt-1 text-[10px] font-bold ${b?"hidden":"flex"} items-center gap-1.5 animate-pulse ${le()?"text-red-400":"text-red-600"}`),c&&(b?(c.disabled=!1,c.classList.remove("opacity-50","cursor-not-allowed","grayscale"),c.classList.add("cursor-pointer","active:scale-[0.98]")):(c.disabled=!0,c.classList.add("opacity-50","cursor-not-allowed","grayscale"),c.classList.remove("cursor-pointer","active:scale-[0.98]"))),b};if(L&&(L.addEventListener("input",i=>{A(i.target.value)}),L.value&&A(L.value)),d){l(d,g=>{L&&(L.value=window.calculateAge(g),A(L.value),L.classList.add("animate-pulse"),setTimeout(()=>L.classList.remove("animate-pulse"),400))});const i=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);i&&(d._datepicker=new i(d,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"}))}const E=s.querySelector("#name-input-field"),I=s.querySelector("#duplicate-warning");if(E&&I){let i;const g=()=>{try{return JSON.parse(localStorage.getItem("user")||"{}")?.id||null}catch{return null}},b=(x,D="")=>{I.className=`mt-1 text-[10px] font-bold ${x?"flex":"hidden"} items-center gap-1.5 animate-pulse ${le()?"text-red-400":"text-red-600"}`;const M=I.querySelector("span");M&&(M.textContent=D?`Beneficiary already exists: ${D}`:"Beneficiary already exists")},y=async x=>{const D=g(),M=await fetch(`${se()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json",...D?{"X-User-Id":String(D)}:{}},body:JSON.stringify({name:x,user_id:D,exclude_id:r?e?.id:null})});if(!M.ok)throw new Error(`Duplicate check failed (${M.status})`);return M.json()};E.addEventListener("input",x=>{const D=x.target.value.trim();if(clearTimeout(i),D.length<3){b(!1);return}i=setTimeout(async()=>{try{const M=await y(D);M.success&&M.exists?b(!0,M.match||M.name):b(!1)}catch(M){console.error("Duplicate check error:",M)}},500)}),e?.name&&(b(!1),(async()=>{const x=await y(e.name);x.success&&x.exists&&b(!0,x.match||x.name)})())}const j=s.querySelector("#full-id-input"),re=s.querySelector("#series-no-input"),X=s.querySelector('input[name="startDate"]'),U=s.querySelector('input[name="endDate"]'),ne=s.querySelectorAll('input[name="remarks"]'),ae=s.querySelector("#extension-log-container"),ie=async i=>{if(!i)return;const g=[j,re].filter(Boolean);g.forEach(b=>{b.classList.add("animate-pulse"),b.placeholder="Syncing..."});try{const[b,y]=await Promise.all([K(`api/beneficiaries.php?next_id&year=${encodeURIComponent(i)}`),K(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(i)}`)]),x=b.success&&b.data?.success?b.data.nextId:null,D=y.success&&y.data?.success?y.data.nextSeries:null;x&&j&&(j.value=x),D&&re&&(re.value=D)}catch(b){console.error("ID Sync error:",b)}finally{g.forEach(b=>b.classList.remove("animate-pulse"))}},Q=s.querySelector("#replacement-search-input"),de=s.querySelector("#replacement-hidden"),ee=s.querySelector("#replacement-suggestions"),pe=i=>{const g=(i.name||"").toUpperCase().trim(),b=i.startDateFormatted||i.startDate||"N/A",y=i.endDateFormatted||i.endDate||"N/A";return`${g} (${b.toUpperCase()} - ${y.toUpperCase()})`},ue=i=>{if(ee){if(!i.length){ee.innerHTML=`<div class="px-3 py-2 text-[10px] font-bold ${t.textCourseOpt}">No matching beneficiary found.</div>`,ee.classList.remove("hidden");return}ee.innerHTML=i.map(g=>{const b=pe(g);return`
                        <button type="button" class="replacement-option w-full text-left px-3 py-2 text-[10px] font-bold ${t.textCourseOpt} ${t.courseHover} rounded-md cursor-pointer transition-colors"
                            data-replacement="${b.replace(/"/g,"&quot;")}">
                            ${b}
                        </button>
                    `}).join(""),ee.classList.remove("hidden"),ee.querySelectorAll(".replacement-option").forEach(g=>{g.addEventListener("click",()=>{const b=g.getAttribute("data-replacement")||"";Q&&(Q.value=b),de&&(de.value=b),ee.classList.add("hidden")})})}};let be=null;const O=async(i="")=>{const g=(i||"").trim(),b=`api/beneficiaries.php?replacement_candidates=1&limit=20${g?`&q=${encodeURIComponent(g)}`:""}`,y=await K(b);y.success&&y.data?.success&&Array.isArray(y.data.candidates)&&ue(y.data.candidates)};Q&&de&&ee&&(Q.addEventListener("focus",()=>{O(Q.value||"")}),Q.addEventListener("input",()=>{de.value=Q.value.trim(),clearTimeout(be),be=setTimeout(()=>{O(Q.value||"")},250)}),document.addEventListener("click",i=>{Q&&ee&&!Q.contains(i.target)&&!ee.contains(i.target)&&ee.classList.add("hidden")}));const _=()=>{const i=s.querySelector('input[name="remarks"]:checked');return i?i.value:"ONGOING"},T=i=>{const g=s.querySelector(`input[name="remarks"][value="${i}"]`);g&&(g.checked=!0,p())},G=()=>{if(U&&U.value){const i=window.__parseFormattedDate(U.value);if(!i)return;const g=new Date;g.setHours(0,0,0,0);let b="ONGOING";i<g&&(b="EXPIRED"),T(b)}},p=()=>{if(!ae)return;const i=_();if(i==="ABSORBED"){const g=e?.absorbDate&&!String(e.absorbDate).includes("0000-00-00")?new Date(e.absorbDate):new Date,b=g.getTimezoneOffset()*6e4,y=new Date(g.getTime()-b).toISOString().slice(0,16);ae.innerHTML=`
                    ...
                    `}else if(i==="RESIGNED"){const g=e?.resignedDate&&!String(e.resignedDate).includes("0000-00-00")?new Date(e.resignedDate):new Date,b=g.getTimezoneOffset()*6e4,y=new Date(g.getTime()-b).toISOString().slice(0,16);ae.innerHTML=`
                        <div class="mt-4 pt-4 border-t ${o?"border-slate-800":"border-gray-100"}">
                    ...
                    `}else ae.innerHTML=""};if(X){let i=null;l(X,x=>{const D=x.getFullYear();if(U){const M=new Date(x);M.setDate(M.getDate()+243);const H=String(M.getMonth()+1).padStart(2,"0"),F=String(M.getDate()).padStart(2,"0"),J=M.getFullYear();U.value=`${H}/${F}/${J}`}G(),D>1900&&D!==i&&(i=D,ie(D))}),U&&l(U,()=>G());const g=s.querySelector("#date-range-picker"),b=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null),y=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);if(b&&g&&X&&U){const x=new b(g,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0,orientation:"auto"});X._datepicker=x.datepickers?.[0]||null,U._datepicker=x.datepickers?.[1]||null}else y&&(X&&(X._datepicker=new y(X,{format:"mm/dd/yyyy",autohide:!0,orientation:"auto"})),U&&(U._datepicker=new y(U,{format:"mm/dd/yyyy",autohide:!0,orientation:"auto"})));if(!r){const x=new Date().getFullYear();ie(x)}}ne.forEach(i=>i.addEventListener("change",p));const u=s.querySelector("#resign-btn"),k=s.querySelector("#absorb-btn");u&&u.addEventListener("click",()=>T("RESIGNED")),k&&k.addEventListener("click",()=>T("ABSORBED")),s.querySelectorAll('input[type="text"], textarea').forEach(i=>{["id-number-input","full-id-input"].includes(i.id)||i.addEventListener("input",()=>{const g=i.selectionStart,b=i.selectionEnd;i.value=i.value.toUpperCase(),i.setSelectionRange(g,b)})}),G(),p(),$("education-input","course-suggestions","course-option"),$("designation-input","work-suggestions","work-option"),(()=>{const i=s.querySelector("#office-input"),g=s.querySelector("#office-suggestions");if(!i||!g)return;let b="OFFICES",y=null,x=[];const D=async()=>{const H="dole_offices_cache",F=async()=>{let V=[];try{if(ce&&me()){const[{data:N,error:S},{data:R}]=await Promise.all([ce.from("offices").select("*").order("office"),ce.from("office_locations").select("office_id")]);if(!S&&N?.length){const z={};R&&R.forEach(Z=>{z[Z.office_id]=(z[Z.office_id]||0)+1}),V=N.map(Z=>({id:Z.id??Z.office_id,office:Z.office||Z.office_name||"",location_count:z[Z.id??Z.office_id]||0})).filter(Z=>Z.office)}}}catch{}if(!V.length)try{const N=await K("api/beneficiaries.php?get_offices_advanced=1");N.success&&N.data?.success&&Array.isArray(N.data.offices)&&(V=N.data.offices)}catch(N){console.error("Office fetch failed:",N)}return V.length>0&&(x=V,localStorage.setItem(H,JSON.stringify({data:V,timestamp:Date.now()}))),V},J=localStorage.getItem(H);if(J)try{const{data:V,timestamp:N}=JSON.parse(J);return x=V,Date.now()-N>300*1e3&&F().then(()=>{b==="OFFICES"&&M("OFFICES",y,i.value)}),V}catch{localStorage.removeItem(H)}return x.length===0?await F():x},M=async(H="OFFICES",F=null,J="")=>{if(b=H,y=F,H==="OFFICES"){const N=(await D()).filter(P=>P.office.toLowerCase().includes(J.toLowerCase()));g.innerHTML=`
                            ...
                            <div class="max-h-64 overflow-y-auto scrollbar-hide">
                                ${N.length>0?N.map(P=>{const W=parseInt(P.location_count||0)>0;return`
                                        <div class="office-code-option group/opt px-3 py-2 text-[9px] font-bold ${t.textCourseOpt} ${t.courseHover} rounded-lg ${W?"cursor-pointer":"cursor-default opacity-60"} transition-all flex items-center justify-between group active:scale-[0.98] mx-1 mb-0.5" 
                                    ...
                                    ${J.trim()?`
                                    <div class="px-2 pb-2 flex flex-col gap-1.5">
                                        <div class="text-[7px] font-black uppercase tracking-widest ${t.textLabel} opacity-50 px-1">New office: "${J.trim()}"</div>
                        ...
                        `;const S=J.trim(),R=g.querySelector("#add-office-location-row-modal"),z=g.querySelector("#new-office-loc-input-modal"),Z=g.querySelector("#confirm-office-with-loc-modal"),te=g.querySelector("#add-office-with-loc-btn-modal"),Y=g.querySelector("#skip-office-loc-btn-modal");if(te&&R&&te.addEventListener("click",P=>{P.stopPropagation(),R.classList.remove("hidden"),R.classList.add("flex"),setTimeout(()=>z?.focus(),50)}),Z&&z){const P=W=>{W.stopPropagation();const oe=z.value.trim();i.value=oe?`${S} - ${oe}`:S,i.dataset.locationName=oe||"",g.classList.add("hidden"),i.dispatchEvent(new Event("change"))};Z.addEventListener("click",P),z.addEventListener("keydown",W=>{W.key==="Enter"&&P(W)}),z.addEventListener("click",W=>W.stopPropagation())}Y&&Y.addEventListener("click",P=>{P.stopPropagation(),i.value=S,i.dataset.locationName="",g.classList.add("hidden"),i.dispatchEvent(new Event("change"))}),g.querySelectorAll(".office-code-option").forEach(P=>{P.addEventListener("click",W=>{W.stopPropagation(),P.dataset.hasLocations==="true"?M("LOCATIONS",{id:P.dataset.id,name:P.dataset.name}):(i.value=P.dataset.name,i.dataset.officeId=P.dataset.id,delete i.dataset.locationName,g.classList.add("hidden"),i.dispatchEvent(new Event("change")))})})}else{g.innerHTML=`
                        ...
                        `;const V=g.querySelector("#locations-list-container"),N=g.querySelector("#location-search-internal"),S=`dole_locs_cache_${F.id}`;let R=[];const z=localStorage.getItem(S);if(z)try{const{data:Y,timestamp:P}=JSON.parse(z);R=Y}catch{localStorage.removeItem(S)}const Z=async()=>{let Y=[];if(ce&&me()){const{data:P,error:W}=await ce.from("office_locations").select("location").eq("office_id",F.id).order("location");!W&&P&&(Y=P)}if(Y.length===0)try{const P=await K(`api/beneficiaries.php?get_office_locations=1&office_id=${F.id}`);P.success&&P.data?.success&&Array.isArray(P.data.locations)&&(Y=P.data.locations)}catch(P){console.error("Office locations fetch failed:",P)}Y.length>0&&(R=Y,localStorage.setItem(S,JSON.stringify({data:Y,timestamp:Date.now()})),te(N.value))},te=(Y="")=>{const P=R.filter(oe=>oe.location.toLowerCase().includes(Y.toLowerCase())),W=Y.trim();P.length>0?V.innerHTML=P.map(oe=>`
                                ...
                                `,W&&V.querySelector("#add-new-location-modal")?.addEventListener("click",()=>{i.value=`${F.name} - ${W}`,i.dataset.officeId=F.id,i.dataset.locationName=W,g.classList.add("hidden"),i.dispatchEvent(new Event("change"))})),V.querySelectorAll(".location-option").forEach(oe=>{oe.addEventListener("click",dt=>{const Ce=oe.dataset.location;i.value=`${F.name} - ${Ce}`,i.dataset.officeId=F.id,i.dataset.locationName=Ce,g.classList.add("hidden"),i.dispatchEvent(new Event("change"))})})};te(N.value),Z(),setTimeout(()=>N.focus(),50),N.addEventListener("input",()=>te(N.value)),N.addEventListener("click",Y=>Y.stopPropagation()),g.querySelector("#back-to-offices").addEventListener("click",Y=>{Y.stopPropagation(),M("OFFICES")})}};i.addEventListener("focus",()=>{g.classList.remove("hidden"),M(b,y,i.value)}),i.addEventListener("input",()=>{delete i.dataset.officeId,delete i.dataset.locationName,b="OFFICES",y=null,g.classList.remove("hidden"),M("OFFICES",null,i.value)}),document.addEventListener("click",H=>{!i.contains(H.target)&&!g.contains(H.target)&&(g.classList.add("hidden"),i.value||(b="OFFICES",y=null))})})();function $(i,g,b){const y=s.querySelector(`#${i}`),x=s.querySelector(`#${g}`);if(!y||!x)return;let D=!1;y.addEventListener("focus",()=>x.classList.remove("hidden")),document.addEventListener("click",M=>{!y.contains(M.target)&&!x.contains(M.target)&&x.classList.add("hidden")}),y.addEventListener("input",()=>{if(D){D=!1;return}const M=y.value.toLowerCase(),H=x.querySelectorAll(`.${b}`);let F=!1;H.forEach(J=>{const V=J.querySelector(".option-text");(V?V.innerText:J.innerText).toLowerCase().includes(M)?(J.style.display="block",F=!0):J.style.display="none"}),F?x.classList.remove("hidden"):x.classList.add("hidden")}),x.addEventListener("click",M=>{const H=M.target.closest(`.${b}`);if(!H)return;const F=H.querySelector(".option-text");y.value=F?F.innerText.trim():H.innerText.trim(),D=!0,x.classList.add("hidden"),y.dispatchEvent(new Event("change"))})}const v=s.querySelector("#add-beneficiary-form"),q="add_beneficiary_draft";if(!r){const i=localStorage.getItem(q);if(i)try{const g=JSON.parse(i);Object.entries(g).forEach(([b,y])=>{const x=v.elements[b];x&&x.type!=="file"&&x.type!=="hidden"&&(x.value=y)})}catch(g){console.error("Error loading draft",g)}}v.addEventListener("input",i=>{if(!r){const g=new FormData(v),b={};g.forEach((y,x)=>b[x]=y),localStorage.setItem(q,JSON.stringify(b))}}),v&&v.addEventListener("submit",i=>{i.preventDefault(),v.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(Z=>{Z.classList.remove("ring-2","ring-red-500","!border-red-500")});const b=new FormData(v);let y=!1;const x=Z=>{const te=v.querySelector(`[name="${Z}"]`);te&&te.classList.add("ring-2","ring-red-500","!border-red-500"),y=!0},D=b.get("name"),M=b.get("contact"),H=b.get("startDate"),F=b.get("endDate"),J=(b.get("designation")||"").trim();(!D||D.trim()===""||/[0-9]/.test(D))&&x("name"),M&&M.trim()!==""&&/[^0-9]/.test(M.replace(/[\s\-\+\(\)]/g,""))&&x("contact"),H||x("startDate"),F||x("endDate");const V=b.get("age"),N=parseInt(V);if((!V||isNaN(N)||N<18||N>29)&&(y=!0,h&&(h.className=`mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse ${typeof le=="function"&&le()?"text-red-400":"text-red-600"}`),c&&(c.disabled=!0,c.classList.add("opacity-50","cursor-not-allowed","grayscale"),c.classList.remove("cursor-pointer","active:scale-[0.98]"))),y)return;const S={};b.forEach((Z,te)=>{if(["birthday","startDate","endDate"].includes(te)){const Y=window.__parseFormattedDate(Z);if(Y){const P=Y.getFullYear(),W=String(Y.getMonth()+1).padStart(2,"0"),oe=String(Y.getDate()).padStart(2,"0");S[te]=`${P}-${W}-${oe}`}else S[te]=Z}else S[te]=Z}),J||(S.designation="N/A"),S.replacement||(S.replacement="");const R=s.querySelector("#office-input");R?.dataset.officeId&&(S.officeId=R.dataset.officeId),R?.dataset.locationName&&(S.locationName=R.dataset.locationName);const z=s.querySelector("#full-id-input")?.value;r?(S.id=e?.id,z&&(S.gip_id=z)):(S.id=null,z&&(S.gip_id=z)),window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(S)){if(!r){const te="add_beneficiary_draft",Y=v.querySelector('[name="office"]')?.value||"",P=v.querySelector('[name="designation"]')?.value||"",W=v.querySelector('[name="education"]')?.value||"";localStorage.setItem(te,JSON.stringify({office:Y,designation:P,education:W}))}C.close(),setTimeout(()=>{C.fire({toast:!0,position:"top-end",icon:"success",title:`Record ${r?"Updated":"Added"} Successfully`,showConfirmButton:!1,timer:3e3,timerProgressBar:!0}),!r&&e?._isBulk&&$e.onSaveSuccess()},100)}else C.fire({icon:"error",title:"Save Failed",text:"There was an error saving the record to the database."})})()})}})}window.handleContactSubmit=async function(e){e.preventDefault();const r=e.target,n=r.querySelector('button[type="submit"]'),a=n.innerHTML;n.disabled=!0,n.innerHTML=`
        <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        Sending...
    `;try{const o=new FormData(r);if((await fetch(r.action,{method:"POST",body:o,headers:{Accept:"application/json"}})).ok)C.fire({toast:!0,position:"top-end",icon:"success",title:"Message Sent Successfully!",text:"Thank you for reaching out. I will get back to you soon!",showConfirmButton:!1,timer:4e3,timerProgressBar:!0,didOpen:f=>{f.addEventListener("mouseenter",C.stopTimer),f.addEventListener("mouseleave",C.resumeTimer)},customClass:{popup:"rounded-2xl border-l-4 border-teal-500 shadow-2xl"}}),r.reset();else throw new Error("Failed to send")}catch{C.fire({toast:!0,position:"top-end",icon:"error",title:"Oops! Something went wrong.",text:"Could not send your message. Please try again later.",showConfirmButton:!1,timer:4e3,customClass:{popup:"rounded-2xl border-l-4 border-red-500 shadow-2xl"}})}finally{n.disabled=!1,n.innerHTML=a}return!1};export{Te as _,K as a,ve as b,mt as c,ht as d,vt as e,ut as f,se as g,wt as h,me as i,gt as j,bt as k,ge as l,ft as m,yt as n,xt as r,ce as s,it as u};
