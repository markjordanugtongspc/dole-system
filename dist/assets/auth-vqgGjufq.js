...
import p from"./vendor-swal-BSk0fVSb.js";const G="color-theme",fe=3600*24*365;function be(e,o,a){document.cookie=`${e}=${o}; max-age=${a}; path=/; SameSite=Lax`}function se(e){const o=document.cookie.match(new RegExp("(?:^|; )"+e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+"=([^;]*)"));return o?decodeURIComponent(o[1]):null}function ne(){const e=localStorage.getItem(G)||se(G);return e==="dark"||e==="light"?e:"light"}function J(e){const o=document.documentElement;e==="dark"?o.classList.add("dark"):o.classList.remove("dark"),localStorage.setItem(G,e),be(G,e,fe),xe(e),document.dispatchEvent(new CustomEvent("themeChanged",{detail:{theme:e}}))}function Q(){const e=ne();J(e==="dark"?"light":"dark")}function xe(e){const o=e==="dark",a=document.getElementById("pref-dark-mode");a&&(a.checked=o);const n=document.getElementById("theme-toggle-dark-icon"),r=document.getElementById("theme-toggle-light-icon");n&&r&&(n.classList.toggle("hidden",o),r.classList.toggle("hidden",!o));const t=document.getElementById("sidebar-theme-label");t&&(t.textContent=o?"Light Mode":"Dark Mode")}function Ie(){const e=ne();J(e);const o=document.getElementById("pref-dark-mode");o&&o.addEventListener("change",()=>{J(o.checked?"dark":"light")});const a=document.getElementById("theme-toggle-btn");a&&a.addEventListener("click",Q),document.querySelectorAll("[data-theme-toggle]").forEach(n=>{n.addEventListener("click",Q)}),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",n=>{localStorage.getItem(G)||se(G)||J(n.matches?"dark":"light")})}function F(){return document.documentElement.classList.contains("dark")}window.toggleTheme=Q;window.isDarkMode=F;const X={queue:[],currentIndex:0,isActive:!1,isAutoSave:!1,lastInteractionTime:0,init(){this.showUploadModal()},showUploadModal(){const e=F(),o={bgCard:e?"bg-slate-900/40":"bg-gray-50/40",borderCard:e?"border-slate-800":"border-gray-100",textHeading:e?"text-green-500":"text-[#2e7d32]",textSubtitle:e?"text-slate-500":"text-gray-400 dark:text-gray-300",bgUpload:e?"bg-slate-800":"bg-white",borderUpload:e?"border-slate-700":"border-gray-200",textUpload:e?"text-slate-400":"text-gray-500",hoverUpload:e?"hover:bg-slate-800/80 hover:border-blue-500":"hover:bg-blue-50/50 hover:border-royal-blue",iconText:e?"text-green-400":"text-[#2e7d32]",iconBg:e?"bg-green-900/20":"bg-[#e8f5e9]",iconBorder:e?"border-green-800/30":"border-[#c8e6c9]"},a=`
...
<div class="border-b border-default pb-4 mb-5 flex flex-col relative w-full pt-4 font-montserrat user-select-none">
    ...
    <div class="grid grid-cols-2 gap-4 mt-6 w-full">
        ...
        <div class="flex flex-col gap-1 text-left overflow-hidden">
            ...
            <div class="flex items-center flex-wrap gap-1.5 min-h-[30px]">
                ...
                <span class="${te(e.office)} hidden sm:inline-block text-[9px] font-black px-2.5 py-1 rounded-full border shadow-sm truncate max-w-[120px] lg:max-w-none" title="${e.office}">${e.office}</span>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col gap-1 text-left mt-4 sm:hidden">
        ...
        <span class="${te(e.office)} text-[10px] font-black px-2.5 py-2 rounded border border-gray-200 dark:border-slate-700 uppercase tracking-widest shadow-sm border-l-4 border-l-indigo-500 w-full truncate" title="${e.office}">${e.office}</span>
    </div>
</div>
    ...
    <div class="flex flex-col gap-4 text-sm mt-3 px-1">
        ...
        <div class="flex flex-col gap-2 pt-3 mt-1 border-t border-gray-50 dark:border-slate-800/60">
            ...
            <datalist id="course-suggestions">
                ${de.map(s=>`<option value="${s.name}"></option>`).join("")}
            </datalist>
        </div>
    </div>
    ...
    <div class="flex flex-col gap-4 text-sm mt-4 px-1">
        ...
        <div class="flex flex-col gap-2">
            ...
            <datalist id="work-suggestions">
                ${ce.map(s=>`<option value="${s}"></option>`).join("")}
            </datalist>
        </div>
...
.text-right-select { text-align-last: right; }
    ...
    `;p.fire({html:i,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:s=>{const c=s.querySelector("#cancel-modal-btn");c&&c.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),p.close(),e?._isBulk&&X.onCancel()});const d=s.querySelector("#bulk-add-btn");d&&d.addEventListener("click",()=>{p.close(),X.init()});const l=s.querySelector("#birthday-input"),g=s.querySelector("#age-display");l&&g&&(l.addEventListener("change",u=>{u.target.value&&(g.value=ae(u.target.value))}),l.value&&!e?.age&&(g.value=ae(l.value)));const b=s.querySelector("#name-input-field"),h=s.querySelector("#duplicate-warning");if(b&&h){let u;b.addEventListener("input",f=>{const m=f.target.value.trim();if(clearTimeout(u),m.length<3){h.classList.add("hidden");return}u=setTimeout(async()=>{try{const y=await(await fetch(`${B()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:m})})).json();y.success&&y.exists?(h.classList.remove("hidden"),F()?h.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":h.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):h.classList.add("hidden")}catch(k){console.error("Duplicate check error:",k)}},500)}),e?.name&&(h.classList.add("hidden"),(async()=>{const m=await(await fetch(`${B()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();m.success&&m.exists&&(h.classList.remove("hidden"),F()?h.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":h.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const x=s.querySelector("#full-id-input"),$=s.querySelector("#series-no-input"),D=s.querySelector('input[name="startDate"]'),I=s.querySelector('input[name="endDate"]'),N=s.querySelectorAll('input[name="remarks"]'),A=s.querySelector("#extension-log-container"),H=u=>{o||(x&&(x.classList.add("animate-pulse"),x.placeholder="Loading...",fetch(`${B()}api/beneficiaries.php?next_id&year=${u}`).then(f=>f.json()).then(f=>{x.classList.remove("animate-pulse"),f.success&&f.nextId&&(x.value=f.nextId)})),$&&($.classList.add("animate-pulse"),$.placeholder="Loading...",fetch(`${B()}api/beneficiaries.php?next_series_no&year=${u}`).then(f=>f.json()).then(f=>{$.classList.remove("animate-pulse"),f.success&&f.nextSeries&&($.value=f.nextSeries)})))},K=()=>{const u=s.querySelector('input[name="remarks"]:checked');return u?u.value:"ONGOING"},T=u=>{const f=s.querySelector(`input[name="remarks"][value="${u}"]`);f&&(f.checked=!0,w())},P=()=>{if(I&&I.value){const u=I.value.split("-"),f=new Date(u[0],u[1]-1,u[2]),m=new Date;m.setHours(0,0,0,0);let k="ONGOING";f<m&&(k="EXPIRED"),T(k)}},w=()=>{if(!A)return;if(K()==="ABSORBED"){const f=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});A.innerHTML=`
