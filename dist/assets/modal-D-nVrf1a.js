...
import g from"./vendor-swal-BSk0fVSb.js";import{a as ke}from"./vendor-DHtNC8Ux.js";const Ee="true".toLowerCase()==="true";function ge(){return Ee}function N(){const e=window.location.pathname,o="/dole-system/",s=e.toLowerCase().indexOf(o.toLowerCase());if(s!==-1)return e.substring(0,s+o.length);const a=e.indexOf("/frontend/");if(a!==-1)return e.substring(0,a+1);const r=e.indexOf("/backend/");return r!==-1?e.substring(0,r+1):"/"}function ie(e="Incorrect Username or Password"){g.fire({html:`
            <div class="p-6">
        ...
        `,timer:e?800:3e3,timerProgressBar:!0,showConfirmButton:!1,showCloseButton:!1,width:"350px",padding:"0",customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl overflow-hidden border border-gray-100",timerProgressBar:"bg-green-500 h-1.5"},backdrop:"rgba(0, 0, 0, 0.4)"})}function Ge(){const e=localStorage.getItem("hasVisitedBefore"),o=document.getElementById("page-loader");e&&(document.documentElement.classList.add("loaded"),o&&(o.style.display="none")),window.addEventListener("load",()=>{const s=document.querySelector("body > *:not(.page-loader)");s&&s.classList.add("content-fade-in"),setTimeout(()=>{document.documentElement.classList.add("loaded"),o&&o.classList.add("hidden"),document.body.style.overflow="auto",localStorage.setItem("hasVisitedBefore","true")},300)})}const Z={_key:"DOLE-GIP-SECURE-KEY-2026",encrypt:function(e){try{const o=r=>r.split("").map(t=>t.charCodeAt(0)),s=r=>("0"+Number(r).toString(16)).substr(-2),a=r=>o(this._key).reduce((t,l)=>t^l,r);return e.split("").map(o).map(a).map(s).join("")}catch(o){return console.error("Encryption Failed",o),null}},decrypt:function(e){try{const o=a=>a.split("").map(r=>r.charCodeAt(0)),s=a=>o(this._key).reduce((r,t)=>r^t,a);return e.match(/.{1,2}/g).map(a=>parseInt(a,16)).map(s).map(a=>String.fromCharCode(a)).join("")}catch(o){return console.error("Decryption Failed",o),null}}};function Ue(){document.querySelectorAll(".login-form-shared").forEach(o=>{const s=o.querySelector('input[name="username"]'),a=o.querySelector('input[name="password"]'),r=o.querySelector('input[name="rememberMe"]');if(s&&a&&r){const t=localStorage.getItem("secure_user"),l=localStorage.getItem("secure_pass");if(t&&l){const n=Z.decrypt(t),c=Z.decrypt(l);n&&c&&(s.value=n,a.value=c,r.checked=!0)}}o.addEventListener("submit",async t=>{t.preventDefault();try{const n=await(await fetch(`${N()}api/auth.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:s.value,password:a.value})})).json();if(n.success){r.checked?(localStorage.setItem("secure_user",Z.encrypt(s.value)),localStorage.setItem("secure_pass",Z.encrypt(a.value))):(localStorage.removeItem("secure_user"),localStorage.removeItem("secure_pass"));const c=localStorage.getItem("hasLoggedInBefore")==="true";localStorage.setItem("isLoggedIn","true"),localStorage.setItem("hasLoggedInBefore","true"),localStorage.setItem("user",JSON.stringify(n.user));const d=document.getElementById("drawer-login");if(d){d.classList.add("translate-y-full");const i=d.querySelector("[data-drawer-hide]");i&&i.click()}await $e(c),Se(c)}else{const c=document.getElementById("drawer-login");c?(c.classList.add("translate-y-full"),setTimeout(()=>{ie(),setTimeout(()=>{c.classList.remove("translate-y-full"),a.value="",a.focus()},600)},400)):(ie(),a.value="",a.focus())}}catch(l){console.error("Login Error:",l),g.fire({icon:"error",title:"Login Error",text:"Unable to reach the server. Please check your connection."})}})})}function Se(e=!1){const o=document.getElementById("left-panel"),s=document.getElementById("right-panel"),a=document.getElementById("left-panel-content"),r=document.getElementById("right-panel-content");a&&(a.style.opacity="0"),r&&(r.style.opacity="0");const t=document.createElement("div");t.className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none";const l=e?"":"animate__delay-1s",n=e?"animation-duration: 0.8s;":"animation-duration: 2s;";t.innerHTML=`
        <img src="${N()}frontend/images/logo/doleiligan.png" class="w-64 h-64 md:w-96 md:h-96 object-contain bg-white/20 backdrop-blur-sm shadow-2xl rounded-full p-4 animate__animated animate__rotateOut ${l}" style="${n}" alt="DOLE Logo">
    `,document.body.appendChild(t);const c=e?0:1e3,d=e?600:1500;setTimeout(()=>{o&&o.classList.add("animate-slide-left"),s&&s.classList.add("animate-slide-right"),setTimeout(()=>{window.location.href=`${N()}frontend/dashboard/`},d)},c)}function He(){const e=document.getElementById("logoutBtn");e&&e.addEventListener("click",()=>{localStorage.removeItem("isLoggedIn"),window.location.href=`${N()}`})}function ze(){const e=document.getElementById("mobile-splash"),o=document.getElementById("show-login-btn"),s=document.getElementById("back-to-splash"),a=document.getElementById("mobile-bg-content"),r=document.getElementById("mobile-welcome-text"),t=document.getElementById("reopen-login-drawer"),l=document.getElementById("request-notifications-btn"),n=async()=>{if("Notification"in window){const m=await Notification.requestPermission();console.log("Notification permission:",m),m==="granted"&&l&&l.classList.add("hidden")}};Notification.permission==="default"&&l&&(l.classList.remove("hidden"),l.addEventListener("click",n));const c=()=>{e&&(e.style.transform="translateY(-100%)",setTimeout(()=>{e.style.visibility="hidden",e.style.pointerEvents="none",e.style.zIndex="-1",Notification.permission==="default"&&n();const m=document.getElementById("drawer-login");m&&m.classList.remove("translate-y-full")},800))},d=()=>{e&&(e.style.zIndex="9999",e.style.pointerEvents="auto",e.style.visibility="visible",e.style.transform="translateY(0)")};o&&o.addEventListener("click",c),document.querySelectorAll(".forgot-password-link").forEach(m=>{m.addEventListener("click",x=>{x.target.closest("#mobile-splash")&&c()})}),s&&s.addEventListener("click",()=>{const m=document.getElementById("drawer-login");if(m){m.classList.add("translate-y-full");const x=m.querySelector("[data-drawer-hide]");x&&x.click()}d()});const u=document.getElementById("drawer-login"),b=document.getElementById("curved-welcome"),p=document.getElementById("peoples-bg");u&&new MutationObserver(x=>{x.forEach(L=>{L.attributeName==="class"&&(u.classList.contains("translate-y-full")?(a&&(a.style.transform="translateY(0)"),r&&(r.style.opacity="1",r.style.transform="translateY(0) scale(1)"),b&&(b.style.opacity="0",b.style.transform="scale(0.5)"),t&&e&&e.style.visibility==="hidden"&&(t.style.opacity="1",t.style.transform="scale(1)"),p&&(p.classList.add("opacity-0","scale-0"),p.classList.remove("opacity-40","scale-[1.6]"))):(a&&(a.style.transform="translateY(-35%)"),r&&(r.style.opacity="0",r.style.transform="translateY(20px) scale(0.9)"),b&&(b.style.opacity="1",b.style.transform="scale(1)"),t&&(t.style.opacity="0",t.style.transform="scale(0)"),p&&(p.classList.remove("opacity-0","scale-0"),p.classList.add("opacity-40","scale-[1.6]"))))})}).observe(u,{attributes:!0})}const W=()=>"false".toLowerCase()==="true";function Ce(e){try{return JSON.stringify(e)}catch{return"[unserializable]"}}const F={debug(...e){W()&&console.debug(...e)},info(...e){W()&&console.info(...e)},warn(...e){W()&&console.warn(...e)},error(...e){console.error(...e)},table(e){W()&&console.table(e)},json(e,o){W()&&console.debug(e,Ce(o))}},Q=new Map;async function Y(e,o={}){const a=`${N()}${e}`;let r=null;try{const l=JSON.parse(localStorage.getItem("user"));l&&(r=l.user_id||l.id||null)}catch{}const t={headers:{"Content-Type":"application/json",...r?{"X-User-Id":r}:{},...o.headers},...o};try{if(F.debug("[API] Request",{url:a,method:t.method||"GET",hasUserId:!!r}),t.body)try{F.json("[API] Payload",JSON.parse(t.body))}catch{F.debug("[API] Payload (raw)",t.body)}const l=await fetch(a,t);if(!l.ok)throw new Error(`HTTP ${l.status}: ${l.statusText}`);const n=await l.json();return Q.has(a)&&(Q.delete(a),F.info?.("[API] Recovered",{url:a})),F.debug("[API] Response",{url:a,ok:!0}),{success:!0,data:n}}catch(l){return l instanceof TypeError&&/fetch/i.test(l.message||"")?Q.get(a)||(Q.set(a,!0),F.error("API Request Network Error (suppressed for repeats):",{url:a,message:l.message})):F.error("API Request Error:",l),{success:!1,error:l.message}}}async function V(e){return Y(e,{method:"GET"})}async function ae(e,o){return Y(e,{method:"POST",body:JSON.stringify(o)})}async function Be(e,o){return Y(e,{method:"PUT",body:JSON.stringify(o)})}async function Ve(e,o){const s=new URLSearchParams(o).toString();return Y(`${e}?${s}`,{method:"PATCH"})}class Le{constructor(){this.intervals=new Map,this.isPageVisible=!0,document.addEventListener("visibilitychange",()=>{this.isPageVisible=!document.hidden,this.isPageVisible?console.log("[Polling] Page visible - resuming normal activity"):console.log("[Polling] Page hidden - reducing activity")})}start(o,s,a=1e4){this.stop(o),s();const r=setInterval(async()=>{this.isPageVisible&&await s()},a);this.intervals.set(o,r),console.log(`[Polling] Started: ${o} (every ${a}ms)`)}stop(o){this.intervals.has(o)&&(clearInterval(this.intervals.get(o)),this.intervals.delete(o),console.log(`[Polling] Stopped: ${o}`))}stopAll(){this.intervals.forEach((o,s)=>this.stop(s)),console.log("[Polling] All intervals stopped")}getActivePolls(){return Array.from(this.intervals.keys())}}const Ie=new Le;function Je(){typeof window.initFlowbite=="function"?(window.initFlowbite(),console.log("[Flowbite] Components re-initialized")):console.warn("[Flowbite] initFlowbite() not available globally")}function Xe(e,o="",s="info"){g.fire({toast:!0,position:"top-end",icon:s,title:e,text:o,showConfirmButton:!1,timer:3e3,timerProgressBar:!0})}function We(e){return JSON.stringify(e)}window.addEventListener("beforeunload",()=>{Ie.stopAll()});const De="modulepreload",Ae=function(e,o){return new URL(e,o).href},le={},be=function(o,s,a){let r=Promise.resolve();if(s&&s.length>0){let d=function(i){return Promise.all(i.map(u=>Promise.resolve(u).then(b=>({status:"fulfilled",value:b}),b=>({status:"rejected",reason:b}))))};const l=document.getElementsByTagName("link"),n=document.querySelector("meta[property=csp-nonce]"),c=n?.nonce||n?.getAttribute("nonce");r=d(s.map(i=>{if(i=Ae(i,a),i in le)return;le[i]=!0;const u=i.endsWith(".css"),b=u?'[rel="stylesheet"]':"";if(a)for(let m=l.length-1;m>=0;m--){const x=l[m];if(x.href===i&&(!u||x.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${i}"]${b}`))return;const p=document.createElement("link");if(p.rel=u?"stylesheet":De,u||(p.as="script"),p.crossOrigin="",p.href=i,c&&p.setAttribute("nonce",c),document.head.appendChild(p),u)return new Promise((m,x)=>{p.addEventListener("load",m),p.addEventListener("error",()=>x(new Error(`Unable to preload CSS for ${i}`)))})}))}function t(l){const n=new Event("vite:preloadError",{cancelable:!0});if(n.payload=l,window.dispatchEvent(n),!n.defaultPrevented)throw l}return r.then(l=>{for(const n of l||[])n.status==="rejected"&&t(n.reason);return o().catch(t)})};let se=null;if(ge()){const e="https://llnddycvbcetztzwbdpx.supabase.co",o="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxsbmRkeWN2YmNldHp0endiZHB4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ1MDMwMTEsImV4cCI6MjA5MDA3OTAxMX0.apIpDlZYja7SNuA-joXgFSvu_wtbBcVVP3OQSPrh7fA";try{se=ke(e,o),console.log("[SUPABASE SDK] Client Initialized Successfully")}catch(s){console.error("[SUPABASE SDK] Failed to initialize client:",s)}}else console.log("[SUPABASE SDK] Supabase mode is disabled (Localhost PHP mode active).");const J="color-theme",Me=3600*24*365;function Ne(e,o,s){document.cookie=`${e}=${o}; max-age=${s}; path=/; SameSite=Lax`}function fe(e){const o=document.cookie.match(new RegExp("(?:^|; )"+e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+"=([^;]*)"));return o?decodeURIComponent(o[1]):null}function he(){const e=localStorage.getItem(J)||fe(J);return e==="dark"||e==="light"?e:"light"}function te(e){const o=document.documentElement;e==="dark"?o.classList.add("dark"):o.classList.remove("dark"),localStorage.setItem(J,e),Ne(J,e,Me),je(e),document.dispatchEvent(new CustomEvent("themeChanged",{detail:{theme:e}}))}function ne(){const e=he();te(e==="dark"?"light":"dark")}function je(e){const o=e==="dark",s=document.getElementById("pref-dark-mode");s&&(s.checked=o);const a=document.getElementById("theme-toggle-dark-icon"),r=document.getElementById("theme-toggle-light-icon");a&&r&&(a.classList.toggle("hidden",o),r.classList.toggle("hidden",!o));const t=document.getElementById("sidebar-theme-label");t&&(t.textContent=o?"Light Mode":"Dark Mode")}function Ye(){const e=he();te(e);const o=document.getElementById("pref-dark-mode");o&&o.addEventListener("change",()=>{te(o.checked?"dark":"light")});const s=document.getElementById("theme-toggle-btn");s&&s.addEventListener("click",ne),document.querySelectorAll("[data-theme-toggle]").forEach(a=>{a.addEventListener("click",ne)}),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",a=>{localStorage.getItem(J)||fe(J)||te(a.matches?"dark":"light")})}function _(){return document.documentElement.classList.contains("dark")}window.toggleTheme=ne;window.isDarkMode=_;const re={queue:[],currentIndex:0,isActive:!1,isAutoSave:!1,lastInteractionTime:0,init(){this.showUploadModal()},showUploadModal(){const e=_(),o={bgCard:e?"bg-slate-900/40":"bg-gray-50/40",borderCard:e?"border-slate-800":"border-gray-100",textHeading:e?"text-green-500":"text-[#2e7d32]",textSubtitle:e?"text-slate-500":"text-gray-400 dark:text-gray-300",bgUpload:e?"bg-slate-800":"bg-white",borderUpload:e?"border-slate-700":"border-gray-200",textUpload:e?"text-slate-400":"text-gray-500",hoverUpload:e?"hover:bg-slate-800/80 hover:border-blue-500":"hover:bg-blue-50/50 hover:border-royal-blue",iconText:e?"text-green-400":"text-[#2e7d32]",iconBg:e?"bg-green-900/20":"bg-[#e8f5e9]",iconBorder:e?"border-green-800/30":"border-[#c8e6c9]"},s=`
...
<div class="border-b border-default pb-4 mb-5 flex flex-col relative w-full pt-4 font-montserrat user-select-none">
    ...
    <div class="grid grid-cols-2 gap-4 mt-6 w-full">
        ...
        <div class="flex flex-col gap-1 text-left overflow-hidden">
            ...
            <div class="flex items-center flex-wrap gap-1.5 min-h-[30px]">
                <span class="${Oe(e.remarks)} text-[10px] font-black px-2 py-1 rounded border uppercase tracking-widest shadow-sm border-l-4 ${e.remarks==="ONGOING"?"border-l-green-600":"border-l-red-600"}">${e.remarks}</span>
                <!-- Desktop Only Office Badge (Pill Style) -->
                <span class="${ce(e.office)} hidden sm:inline-block text-[9px] font-black px-2.5 py-1 rounded-full border shadow-sm truncate max-w-[120px] lg:max-w-none" title="${e.office}">${e.office}</span>
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col gap-1 text-left mt-4 sm:hidden">
        ...
        <span class="${ce(e.office)} text-[10px] font-black px-2.5 py-2 rounded border border-gray-200 dark:border-slate-700 uppercase tracking-widest shadow-sm border-l-4 border-l-indigo-500 w-full truncate" title="${e.office}">${e.office}</span>
    </div>
</div>
...
<div class="flex flex-col gap-4 text-sm mt-3 px-1">
    ...
    <div class="flex justify-between items-center group">
        ...
        <span class="${e.age||de(e.birthday)?"font-black text-heading":"font-bold text-gray-300 italic"} text-right">${e.age||de(e.birthday)||"N/A"}</span>
    </div>
    <div class="flex justify-between items-center group">
...
<div class="mt-8 pt-6 border-t border-default relative">
    ...
    <div id="drawer-page-1" class="hidden flex-1 flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-3">
            ${n.map(i=>{const u=i.status.toUpperCase(),p={VERIFIED:"COMPLETED",DECLINED:"REJECTED",PENDING:"PENDING"}[u]||u,m=p==="COMPLETED",x=p==="REJECTED",L=m?"text-green-500":x?"text-red-500":"text-gray-400 dark:text-gray-500",$=m?"bg-green-50/50 dark:bg-green-900/10":x?"bg-red-50/50 dark:bg-red-900/10":"bg-gray-50/50 dark:bg-slate-800/50",I=m?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800 hover:bg-green-200 cursor-pointer":x?"bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800 hover:bg-red-200 cursor-pointer":"bg-white text-gray-500 border-gray-200 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer";let D='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';return m?D='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>':x&&(D='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>'),`
                <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-0.5 ${$}">
                    ...
                    <button type="button" class="ml-3 ${I} text-[10px] font-black px-3 py-1.5 rounded-full border uppercase tracking-widest transition-colors flex-shrink-0 drawer-doc-btn" data-id="${i.id}" data-name="${i.name}" data-status="${p} cursosr-pointer">
                        ${p}
                    </button>
                </div>
                `}).join("")}
...
<style>
    ...
    `;let d=document.getElementById("beneficiary-drawer-container");d&&d.remove(),d=document.createElement("div"),d.id="beneficiary-drawer-container",d.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-neutral-primary-soft dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl",d.setAttribute("tabindex","-1"),d.setAttribute("data-drawer-backdrop","true"),d.innerHTML=c,document.body.appendChild(d),be(async()=>{const{Drawer:i}=await import("./vendor-flowbite-bBKPL--D.js");return{Drawer:i}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:i})=>{const u={placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{d&&d.parentNode&&d.remove()},300)}},b=new i(d,u);b.show(),d.querySelector("#close-drawer-btn").addEventListener("click",()=>b.hide());const m=d.querySelector("#drawer-prev-btn"),x=d.querySelector("#drawer-next-btn"),L=2,$=()=>{d.querySelectorAll("[id^=drawer-page-]").forEach((v,B)=>{v.classList.toggle("hidden",B!==s)}),m.disabled=s===0,x.disabled=s===L,m.classList.toggle("opacity-50",s===0),x.classList.toggle("opacity-50",s===L)};m.addEventListener("click",()=>{s>0&&s--,$()}),x.addEventListener("click",()=>{s<L&&s++,$()}),$(),d.querySelectorAll(".drawer-doc-btn").forEach(v=>{v.addEventListener("click",async()=>{const B=v.dataset.name,S=v.dataset.status,C=await g.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Update Document</span>',html:`
    ...
    <div class="flex flex-col gap-4 text-sm mt-3 px-1 pb-24">
        ...
        <div class="flex flex-col gap-2 pt-3 mt-1 border-t border-gray-50 dark:border-slate-800/60">
            ...
            <div id="edit-education-suggestions-box" class="hidden mt-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                ${xe.map(c=>`<button type="button" class="edit-education-option w-full text-left px-3 py-2 text-[11px] font-bold text-slate-700 dark:text-slate-200 hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer"><span class="option-text">${c.name}</span></button>`).join("")}
            </div>
        </div>
    </div>
    ...
    <div class="flex flex-col gap-4 text-sm mt-4 px-1">
        ...
        <div class="flex flex-col gap-2">
            ...
            <div id="edit-designation-suggestions-box" class="hidden mt-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                ${me.map(c=>`<button type="button" class="edit-designation-option w-full text-left px-3 py-2 text-[11px] font-bold text-slate-700 dark:text-slate-200 hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer"><span class="option-text">${c}</span></button>`).join("")}
            </div>
        </div>
...
.editable-indicator {
    ...
    `;let n=document.getElementById("edit-drawer-container");n&&n.remove(),n=document.createElement("div"),n.id="edit-drawer-container",n.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-white dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",n.setAttribute("tabindex","-1"),n.innerHTML=l,document.body.appendChild(n),setTimeout(()=>{const c=n.querySelector('textarea[name="name"]');c&&(c.style.height="auto",c.style.height=c.scrollHeight+"px")},10),(async()=>{try{const c=await Y("api/beneficiaries.php?get_offices=1");if(c.success&&c.data.offices){const d=n.querySelector("#edit-office-suggestions-box");if(d){const i=["DOLE Field Office","LGU","DEPED","DICT","PCA"],u=[...new Set([...i,...c.data.offices])];d.innerHTML=u.map(b=>`
                    ...
                    `).join("")}}}catch(c){console.error("Error fetching office suggestions:",c)}})(),be(async()=>{const{Drawer:c}=await import("./vendor-flowbite-bBKPL--D.js");return{Drawer:c}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:c})=>{const d=new c(n,{placement:"right",backdrop:!0,bodyScrolling:!1,backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{n.parentNode&&n.remove()},300)}});d.show(),n.querySelector("#close-edit-drawer-btn").addEventListener("click",()=>d.hide()),n.querySelector("#edit-drawer-cancel-btn").addEventListener("click",()=>d.hide());const i=n.querySelector("#edit-beneficiary-drawer-form"),u=n.querySelector("#edit-bday-input"),b=n.querySelector("#edit-age-display");let p=!1;u&&b&&(b.addEventListener("input",()=>{p=!0}),u.addEventListener("change",x=>{x.target.value&&(!p||!b.value)&&(b.value=t(x.target.value))}));const m=(x,L,$)=>{const I=n.querySelector(x),D=n.querySelector(L);if(!I||!D)return;const T=()=>D.classList.add("hidden"),G=()=>D.classList.remove("hidden");I.addEventListener("focus",G),I.addEventListener("input",()=>{const j=I.value.toLowerCase().trim();let O=0;D.querySelectorAll($).forEach(v=>{const S=(v.querySelector(".option-text")?.textContent||v.textContent||"").toLowerCase().includes(j);v.style.display=S?"block":"none",S&&O++}),O>0?G():T()}),D.addEventListener("click",j=>{const O=j.target.closest($);O&&(I.value=(O.querySelector(".option-text")?.textContent||O.textContent||"").trim(),T(),I.dispatchEvent(new Event("change")))}),document.addEventListener("click",j=>{!I.contains(j.target)&&!D.contains(j.target)&&T()})};m("#edit-education-input","#edit-education-suggestions-box",".edit-education-option"),m("#edit-designation-input","#edit-designation-suggestions-box",".edit-designation-option"),m("#edit-office-input","#edit-office-suggestions-box",".edit-office-option"),i.querySelectorAll("input, select, textarea").forEach(x=>{const L=(x.getAttribute("type")||"").toLowerCase(),$=!x.disabled&&!x.readOnly&&L!=="hidden";x.classList.remove("editable-indicator"),$&&x.classList.add("editable-indicator")}),i.addEventListener("submit",x=>{x.preventDefault();const L=new FormData(i),$={};L.forEach((I,D)=>$[D]=I),$.id=e.id,$.gip_id=e.id,window.addBeneficiaryData&&(async()=>await window.addBeneficiaryData($,!0,!1)&&(d.hide(),window.viewBeneficiary&&setTimeout(()=>window.viewBeneficiary({id:e.id},0),100),g.fire({toast:!0,position:"top-end",icon:"success",title:"Record Updated",showConfirmButton:!1,timer:3e3})))()})})}function Ke(){window.viewBeneficiary=async function(e,o=0){const s=`logs_cache_${e.id}`;let a=!1;try{if(window.__doleDB&&window.__doleDB.getSecureCache){const p=await window.__doleDB.getSecureCache(s);p&&(e.arLogs=p.arLogs||[],e.dtrLogs=p.dtrLogs||[],e.docs=p.docs||[],ee(e,o),a=!0)}let r=[],t=[],l=[];const[n,c,d,i]=await Promise.all([V(`api/logs.php?type=ar&gip_id=${encodeURIComponent(e.id)}`),V(`api/logs.php?type=dtr&gip_id=${encodeURIComponent(e.id)}`),V(`api/logs.php?type=docs&gip_id=${encodeURIComponent(e.id)}`),V(`api/logs.php?type=absorption&gip_id=${encodeURIComponent(e.id)}`)]);r=n.success&&n.data?.success?n.data.logs:[],t=c.success&&c.data?.success?c.data.logs:[],l=d.success&&d.data?.success?d.data.logs:[];const u=i.success&&i.data?.success?i.data.logs:[];if(u.length>0){const p=u[0];e.absorbDate=p.absorption_datetime,e.absorb_where=p.where||p.absorb_where,e.absorb_position=p.position||p.absorb_position,e.absorb_agency=p.agency||p.absorb_agency}const b={arLogs:r,dtrLogs:t,docs:l};window.__doleDB&&window.__doleDB.setSecureCache&&await window.__doleDB.setSecureCache(s,b),e.arLogs=r,e.dtrLogs=t,e.docs=l,a?document.getElementById("beneficiary-drawer-container")&&ee(e,o):ee(e,o)}catch(r){console.error("Error fetching logs/docs:",r),a||(e.arLogs=[],e.dtrLogs=[],e.docs=[],ee(e,o))}},window.showAddDataModal=function(e){oe(e)},window.editBeneficiary=function(e){Pe(e)},window.showExportConfigModal=function(e){_e(e)},window.showProfileModal=function(){Te()}}async function Te(){try{if(ge()&&se){let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=a.id)}catch{}if(!e)throw new Error("User not logged in");const{data:o,error:s}=await se.from("users").select("*").eq("id",e).single();if(s)throw s;ue(o)}else{let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=`?user_id=${a.id}`)}catch{}const s=await(await fetch(`${N()}api/profile.php${e}`)).json();if(s.success){const a=s.profile;ue(a)}else g.fire({icon:"error",title:"Error",text:s.error||"Failed to load profile"})}}catch(e){console.error("Error fetching profile:",e)}}function ue(e){const o=e.profile_picture_path?`${N()}${e.profile_picture_path}`:null,s=e.full_name?e.full_name.split(" ").map(r=>r[0]).join("").substring(0,2).toUpperCase():"US",a=`
    ...
    `;g.fire({html:a,width:"450px",showConfirmButton:!1,showCloseButton:!0,padding:"1.5rem",customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl overflow-visible ldn-modal-popup",closeButton:"focus:outline-none bg-gray-50 border-none swal2-custom-close cursor-pointer"},didOpen:r=>{const t=r.querySelector("#profile-edit-form"),l=r.querySelector("#profile-pic-input"),n=r.querySelector("#profile-avatar-preview");l.addEventListener("change",c=>{const d=c.target.files[0];if(d){const i=new FileReader;i.onload=u=>{n.innerHTML=`<img src="${u.target.result}" class="w-full h-full object-cover" />`},i.readAsDataURL(d)}}),t.addEventListener("submit",async c=>{c.preventDefault();const d=new FormData(t);try{const i=JSON.parse(localStorage.getItem("user"));i&&i.id&&d.append("user_id",i.id)}catch{}try{const u=await(await fetch(`${N()}api/profile.php`,{method:"POST",body:d})).json();u.success?(u.profile&&(localStorage.setItem("user",JSON.stringify(u.profile)),Re(u.profile)),g.close(),g.fire({toast:!0,position:"top-end",icon:"success",title:"Profile Updated",showConfirmButton:!1,timer:3e3,timerProgressBar:!0})):g.fire({icon:"error",title:"Update Failed",text:u.error||"Something went wrong"})}catch(i){console.error("Error saving profile:",i)}})}})}function Re(e){const o=e.profile_picture_path?`${N()}${e.profile_picture_path}`:null,s=e.full_name?e.full_name.split(" ").map(t=>t[0]).join("").substring(0,2).toUpperCase():"US",a=document.querySelectorAll(".sidebar-user-name"),r=document.querySelectorAll(".sidebar-user-avatar");a.forEach(t=>t.textContent=e.full_name),r.forEach(t=>{o?t.innerHTML=`<img src="${o}" class="w-full h-full object-cover" />`:t.textContent=s}),localStorage.setItem("user_full_name",e.full_name),o&&localStorage.setItem("user_avatar",o)}function _e(e){const o=window.getExportFilters?window.getExportFilters():{office:"ALL",status:"ALL",search:"",sort:"name",section:"ALL",columns:["id","name","office","position","startdate","enddate","status"],preparedBy:localStorage.getItem("ldn_export_prepared")||"",approvedBy:localStorage.getItem("ldn_export_approved")||""},s=`
        <div class="text-left font-montserrat p-1 overflow-visible">
            <div class="flex items-center gap-3 mb-6">
                <div class="w-12 h-12 bg-royal-blue/10 rounded-2xl flex items-center justify-center">
    ...
    `;g.fire({html:l,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:n=>{const c=n.querySelector("#cancel-modal-btn");c&&c.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),g.close(),e?._isBulk&&re.onCancel()});const d=n.querySelector("#bulk-add-btn");d&&d.addEventListener("click",()=>{g.close(),re.init()});const i=n.querySelector("#birthday-input"),u=n.querySelector("#age-display");i&&u&&(i.addEventListener("change",f=>{f.target.value&&(u.value=pe(f.target.value))}),i.value&&!e?.age&&(u.value=pe(i.value)));const b=n.querySelector("#name-input-field"),p=n.querySelector("#duplicate-warning");if(b&&p){let f;b.addEventListener("input",h=>{const k=h.target.value.trim();if(clearTimeout(f),k.length<3){p.classList.add("hidden");return}f=setTimeout(async()=>{try{const y=await(await fetch(`${N()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:k})})).json();y.success&&y.exists?(p.classList.remove("hidden"),_()?p.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":p.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):p.classList.add("hidden")}catch(E){console.error("Duplicate check error:",E)}},500)}),e?.name&&(p.classList.add("hidden"),(async()=>{const k=await(await fetch(`${N()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();k.success&&k.exists&&(p.classList.remove("hidden"),_()?p.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":p.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const m=n.querySelector("#full-id-input"),x=n.querySelector("#series-no-input"),L=n.querySelector('input[name="startDate"]'),$=n.querySelector('input[name="endDate"]'),I=n.querySelectorAll('input[name="remarks"]'),D=n.querySelector("#extension-log-container"),T=f=>{o||(m&&(m.classList.add("animate-pulse"),m.placeholder="Loading...",fetch(`${N()}api/beneficiaries.php?next_id&year=${f}`).then(h=>h.json()).then(h=>{m.classList.remove("animate-pulse"),h.success&&h.nextId&&(m.value=h.nextId)})),x&&(x.classList.add("animate-pulse"),x.placeholder="Loading...",fetch(`${N()}api/beneficiaries.php?next_series_no&year=${f}`).then(h=>h.json()).then(h=>{x.classList.remove("animate-pulse"),h.success&&h.nextSeries&&(x.value=h.nextSeries)})))},G=()=>{const f=n.querySelector('input[name="remarks"]:checked');return f?f.value:"ONGOING"},j=f=>{const h=n.querySelector(`input[name="remarks"][value="${f}"]`);h&&(h.checked=!0,v())},O=()=>{if($&&$.value){const f=$.value.split("-"),h=new Date(f[0],f[1]-1,f[2]),k=new Date;k.setHours(0,0,0,0);let E="ONGOING";h<k&&(E="EXPIRED"),j(E)}},v=()=>{if(!D)return;if(G()==="ABSORBED"){const h=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});D.innerHTML=`
                    ...
                    `}else D.innerHTML=""};if(L&&(L.addEventListener("change",f=>{const h=f.target.value;if(h){const k=new Date(h).getFullYear();if(T(k),$&&!$.value){const E=new Date(h);E.setMonth(E.getMonth()+1),$.value=E.toISOString().split("T")[0]}O()}}),!o)){const f=L.value?new Date(L.value).getFullYear():new Date().getFullYear();T(f)}$&&$.addEventListener("change",O),I.forEach(f=>f.addEventListener("change",v));const B=n.querySelector("#resign-btn"),S=n.querySelector("#absorb-btn");B&&B.addEventListener("click",()=>j("RESIGNED")),S&&S.addEventListener("click",()=>j("ABSORBED")),n.querySelectorAll('input[type="text"], textarea').forEach(f=>{["id-number-input","full-id-input"].includes(f.id)||f.addEventListener("input",()=>{const h=f.selectionStart,k=f.selectionEnd;f.value=f.value.toUpperCase(),f.setSelectionRange(h,k)})}),O(),v(),C("education-input","course-suggestions","course-option"),C("designation-input","work-suggestions","work-option"),C("office-input","office-suggestions","office-option");function C(f,h,k){const E=n.querySelector(`#${f}`),y=n.querySelector(`#${h}`);if(!E||!y)return;let P=!1;E.addEventListener("focus",()=>y.classList.remove("hidden")),document.addEventListener("click",A=>{!E.contains(A.target)&&!y.contains(A.target)&&y.classList.add("hidden")}),E.addEventListener("input",()=>{if(P){P=!1;return}const A=E.value.toLowerCase(),U=y.querySelectorAll(`.${k}`);let K=!1;U.forEach(q=>{const R=q.querySelector(".option-text");(R?R.innerText:q.innerText).toLowerCase().includes(A)?(q.style.display="block",K=!0):q.style.display="none"}),K?y.classList.remove("hidden"):y.classList.add("hidden")}),y.querySelectorAll(`.${k}`).forEach(A=>{A.addEventListener("click",()=>{const U=A.querySelector(".option-text");E.value=U?U.innerText.trim():A.innerText.trim(),P=!0,y.classList.add("hidden"),E.dispatchEvent(new Event("change"))})})}const w=n.querySelector("#add-beneficiary-form"),M="add_beneficiary_draft";if(!o){const f=localStorage.getItem(M);if(f)try{const h=JSON.parse(f);Object.entries(h).forEach(([k,E])=>{const y=w.elements[k];y&&y.type!=="file"&&y.type!=="hidden"&&(y.value=E)})}catch(h){console.error("Error loading draft",h)}}w.addEventListener("input",f=>{if(!o){const h=new FormData(w),k={};h.forEach((E,y)=>k[y]=E),localStorage.setItem(M,JSON.stringify(k))}}),w&&w.addEventListener("submit",f=>{f.preventDefault(),w.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(H=>{H.classList.remove("ring-2","ring-red-500","!border-red-500")});const k=new FormData(w);let E=!1;const y=H=>{const z=w.querySelector(`[name="${H}"]`);z&&z.classList.add("ring-2","ring-red-500","!border-red-500"),E=!0},P=k.get("name"),A=k.get("contact"),U=k.get("startDate"),K=k.get("endDate"),q=k.get("designation");if((!P||P.trim()===""||/[0-9]/.test(P))&&y("name"),A&&A.trim()!==""&&/[^0-9]/.test(A.replace(/[\s\-\+\(\)]/g,""))&&y("contact"),U||y("startDate"),K||y("endDate"),(!q||q.trim()==="")&&y("designation"),E)return;const R={};k.forEach((H,z)=>{R[z]=H});const X=n.querySelector("#full-id-input")?.value;o?(R.id=e?.id,X&&(R.gip_id=X)):(R.id=null,X&&(R.gip_id=X)),window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(R)){if(!o){const z="add_beneficiary_draft",ve=w.querySelector('[name="office"]')?.value||"",we=w.querySelector('[name="designation"]')?.value||"",ye=w.querySelector('[name="education"]')?.value||"";localStorage.setItem(z,JSON.stringify({office:ve,designation:we,education:ye}))}g.close(),setTimeout(()=>{g.fire({toast:!0,position:"top-end",icon:"success",title:`Record ${o?"Updated":"Added"} Successfully`,showConfirmButton:!1,timer:3e3,timerProgressBar:!0}),!o&&e?._isBulk&&re.onSaveSuccess()},100)}else g.fire({icon:"error",title:"Save Failed",text:"There was an error saving the record to the database."})})()})}})}window.handleContactSubmit=async function(e){e.preventDefault();const o=e.target,s=o.querySelector('button[type="submit"]'),a=s.innerHTML;s.disabled=!0,s.innerHTML=`
        <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        Sending...
    `;try{const r=new FormData(o);if((await fetch(o.action,{method:"POST",body:r,headers:{Accept:"application/json"}})).ok)g.fire({toast:!0,position:"top-end",icon:"success",title:"Message Sent Successfully!",text:"Thank you for reaching out. I will get back to you soon!",showConfirmButton:!1,timer:4e3,timerProgressBar:!0,didOpen:l=>{l.addEventListener("mouseenter",g.stopTimer),l.addEventListener("mouseleave",g.resumeTimer)},customClass:{popup:"rounded-2xl border-l-4 border-teal-500 shadow-2xl"}}),o.reset();else throw new Error("Failed to send")}catch{g.fire({toast:!0,position:"top-end",icon:"error",title:"Oops! Something went wrong.",text:"Could not send your message. Please try again later.",showConfirmButton:!1,timer:4e3,customClass:{popup:"rounded-2xl border-l-4 border-red-500 shadow-2xl"}})}finally{s.disabled=!1,s.innerHTML=a}return!1};export{be as _,V as a,Y as b,Ve as c,We as d,Be as e,Xe as f,N as g,Ge as h,ge as i,Ye as j,Ue as k,F as l,He as m,ze as n,Ke as o,Ie as p,Je as r,se as s,Re as u};
