            ...
            ${l.map(d=>{const g=d.status.toUpperCase(),v={VERIFIED:"COMPLETED",DECLINED:"REJECTED",PENDING:"PENDING"}[g]||g,k=v==="COMPLETED",y=v==="REJECTED",T=k?"text-green-500":y?"text-red-500":"text-gray-400 dark:text-gray-500",R=k?"bg-green-50/50 dark:bg-green-900/10":y?"bg-red-50/50 dark:bg-red-900/10":"bg-gray-50/50 dark:bg-slate-800/50",te=k?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800 hover:bg-green-200 cursor-pointer":y?"bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800 hover:bg-red-200 cursor-pointer":"bg-white text-gray-500 border-gray-200 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer";let Y='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';return k?Y='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>':y&&(Y='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>'),`
                <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-0.5 ${R}">
                    <div class="flex items-center gap-3 w-full">
                        ...
                        <span class="text-xs sm:text-sm font-black ${k?"text-heading":"text-gray-500 dark:text-gray-400"} uppercase tracking-tight flex-1">${d.name}</span>
                    </div>
                    <button type="button" class="ml-3 ${te} text-[10px] font-black px-3 py-1.5 rounded-full border uppercase tracking-widest transition-colors flex-shrink-0 drawer-doc-btn" data-id="${d.id}" data-name="${d.name}" data-status="${v} cursosr-pointer">
                        ${v}
                    </button>
                </div>
                `}).join("")}
    ...
    <div id="drawer-page-2" class="hidden flex-1 flex flex-col gap-6">
         ...
         <div class="flex flex-col gap-5">
            <div>
                ...
                <div class="space-y-2">
                    ${t.length?t.map(d=>{const g=d.status||"PENDING";let b=g==="VERIFIED"||g==="COMPLETED"?"text-green-500":g==="REJECTED"||g==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500";return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-blue-100 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors edit-log-btn" data-type="dtr" data-id="${d.id}" data-val="${d.date||d.createdAt}" data-status="${g}">
                            ...
                            <span class="text-[11px] font-bold ${b} uppercase tracking-widest truncate max-w-[50%] text-right pr-6 group-hover:pr-12 pointer-events-none transition-all">${g}</span>
            ...
            <div>
                ...
                <div class="space-y-2">
                    ${r.length?r.map(d=>{const g=d.status||"PENDING";let b=g==="VERIFIED"||g==="COMPLETED"?"text-green-500":g==="REJECTED"||g==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500";return`
...
<style>
    ...
    `;let i=document.getElementById("beneficiary-drawer-container");i&&i.remove(),i=document.createElement("div"),i.id="beneficiary-drawer-container",i.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-neutral-primary-soft dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl",i.setAttribute("tabindex","-1"),i.setAttribute("data-drawer-backdrop","true"),i.innerHTML=p,document.body.appendChild(i),Ce(async()=>{const{Drawer:d}=await import("./vendor-flowbite-B7rSfpuT.js").then(g=>g.b);return{Drawer:d}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:d})=>{const g={placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{i&&i.parentNode&&i.remove()},300)}},b=new d(i,g);b.show(),i.querySelector("#close-drawer-btn").addEventListener("click",()=>b.hide());const k=i.querySelector("#drawer-prev-btn"),y=i.querySelector("#drawer-next-btn"),T=2,R=()=>{i.querySelectorAll("[id^=drawer-page-]").forEach((L,A)=>{L.classList.toggle("hidden",A!==a)}),k.disabled=a===0,y.disabled=a===T,k.classList.toggle("opacity-50",a===0),y.classList.toggle("opacity-50",a===T)};k.addEventListener("click",()=>{a>0&&a--,R()}),y.addEventListener("click",()=>{a<T&&a++,R()}),R(),i.querySelectorAll(".drawer-doc-btn").forEach(L=>{L.addEventListener("click",async()=>{const A=L.dataset.name,B=L.dataset.status,D="flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[9px] gap-2 transition-all duration-300 ",M=await x.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Update Document</span>',html:`
                    ...
                    `,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Update Status</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Cancel</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2"},buttonsStyling:!1,preConfirm:()=>{const j=document.querySelector('input[name="swal-doc-status"]:checked');return j?j.value:null}});if(M.isConfirmed){const j=M.value;if(j===B)return;try{const S={COMPLETED:"VERIFIED",REJECTED:"DECLINED",PENDING:"PENDING"}[j]||j,h=await xe("api/logs.php?type=docs",{gip_id:e.id,doc_name:A,status:S}),C=h.success?h.data:{success:!1,error:h.error};C.success?(x.fire({toast:!0,position:s,icon:"success",title:"Status updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):x.fire("Error",C.error||"Failed to update","error")}catch(m){x.fire("Error",m.message,"error")}}})});const te=()=>{if(!t.length)return new Date().toISOString().split("T")[0];const L=new Date(t[0].date);return L.setDate(L.getDate()+1),L.toISOString().split("T")[0]},Y=()=>{if(!r.length){const D=new Date,M=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"][D.getMonth()];return D.getDate()<=15?`${M} 1-15`:`${M} 16-${new Date(D.getFullYear(),D.getMonth()+1,0).getDate()}`}const L=r[0].period||"",A=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"],B=L.toUpperCase().match(/([A-Z]{3})\s+(\d+)-(\d+)/);if(B){let D=B[1],M=parseInt(B[2]),j=A.indexOf(D);if(j!==-1)return M===1?`${D} 16-${new Date(new Date().getFullYear(),j+1,0).getDate()}`:`${A[(j+1)%12]} 1-15`}return"NEW PERIOD"},K=async(L,A)=>{x.fire({title:"Adding...",allowOutsideClick:!1,showConfirmButton:!1}),x.showLoading();try{const B={gip_id:e.id};L==="dtr"&&(B.record_date=A),L==="ar"&&(B.period=A);const D=await xe(`api/logs.php?type=${L}`,B);(D.success?D.data:{success:!1,error:D.error}).success?(x.fire({toast:!0,position:s,icon:"success",title:"Auto-Added!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):x.fire("Error","Failed to add log.","error")}catch(B){x.fire("Error",B.message,"error")}},G=async(L,A,B,D,M)=>{const j=A==="dtr"?"Record Date":"Period";oe();const m="flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[10px] sm:text-xs gap-2 ",S='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>',h='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>',{value:C}=await x.fire({title:`<span class="text-xl font-black text-heading uppercase tracking-tight">Edit ${L} Log</span>`,html:`
...
.editable-indicator {
    ...
    `;let l=document.getElementById("edit-drawer-container");l&&l.remove(),l=document.createElement("div"),l.id="edit-drawer-container",l.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-white dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",l.setAttribute("tabindex","-1"),l.innerHTML=n,document.body.appendChild(l),setTimeout(()=>{const p=l.querySelector('textarea[name="name"]');p&&(p.style.height="auto",p.style.height=p.scrollHeight+"px")},10),(async()=>{try{const p=await ne("api/beneficiaries.php?get_offices=1");if(p.success&&p.data.offices){const i=l.querySelector("#edit-office-suggestions-box");if(i){const d=["DOLE Field Office","LGU","DEPED","DICT","PCA"],g=[...new Set([...d,...p.data.offices])];i.innerHTML=g.map(b=>`
                    ...
                    `).join("")}}}catch(p){console.error("Error fetching office suggestions:",p)}})(),Ce(async()=>{const{Drawer:p}=await import("./vendor-flowbite-B7rSfpuT.js").then(i=>i.b);return{Drawer:p}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:p})=>{const i=new p(l,{placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{setTimeout(()=>{l&&l.parentNode&&l.remove()},400)}});i.show(),window.initFlowbite&&window.initFlowbite();const d=()=>{document.activeElement&&typeof document.activeElement.blur=="function"&&document.activeElement.blur(),i.hide()};l.querySelector("#close-edit-drawer-btn").addEventListener("click",d),l.querySelector("#edit-drawer-cancel-btn").addEventListener("click",d);const g=l.querySelector("#edit-beneficiary-drawer-form"),b=l.querySelector("#edit-bday-input"),v=l.querySelector("#edit-age-display"),k=l.querySelector("#edit-startDate-input"),y=l.querySelector("#edit-endDate-input"),T=l.querySelector('input[name="seriesNo"]'),R=l.querySelector('input[name="gip_id"]'),te=l.querySelector("#edit-drawer-remarks");te&&te.addEventListener("change",m=>{const S="text-[10px] sm:text-[11px] font-black px-2.5 py-2.5 rounded-lg border uppercase tracking-widest shadow-sm outline-none focus:ring-2 focus:ring-brand w-full cursor-pointer transition-colors duration-300";te.className=`${u(m.target.value)} ${S} editable-indicator`});let Y=!1,K=!1;const G=(m,S)=>{m.addEventListener("input",h=>{const C=h.target.value,I=window.__maskDate(C);if(C!==I&&(h.target.value=I),I.length===10){const q=window.__parseFormattedDate(I);q&&S&&(K||S(q),document.activeElement===m&&m.blur())}}),m.addEventListener("changeDate",h=>{h.detail&&h.detail.date&&S&&(K||S(h.detail.date),m._datepicker&&m._datepicker.hide())})};b&&G(b,m=>{v&&(!Y||!v.value)&&(v.value=window.calculateAge(m))}),k&&G(k,m=>{if(y){const h=new Date(m);h.setMonth(h.getMonth()+3),h.setDate(h.getDate()+1);const C=String(h.getMonth()+1).padStart(2,"0"),I=String(h.getDate()).padStart(2,"0"),q=h.getFullYear();y.value=`${C}/${I}/${q}`}const S=m.getFullYear();S>1900&&R&&T&&Promise.all([z(`api/beneficiaries.php?next_id&year=${encodeURIComponent(S)}`),z(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(S)}`)]).then(([h,C])=>{const I=h.success&&h.data?.success?h.data.nextId:null,q=C.success&&C.data?.success?C.data.nextSeries:null,Z=String(R.value||"").match(/^ROX-RD-ESIG-(\d{4})-\d{4}$/),_=String(T.value||"").match(/^(\d{4})-\d{2}-\d{3}$/),J=Z?Number(Z[1]):null,c=_?Number(_[1]):null;I&&(J===null||J!==S)&&(R.value=I),q&&(c===null||c!==S)&&(T.value=q)}).catch(h=>{console.error("Edit drawer identifier sync error:",h)})}),y&&G(y);const ee=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null),F=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null);ee&&b&&(b._datepicker=new ee(b,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"}));const L=l.querySelector("#edit-date-range-picker");if(F&&L){const m=new F(L,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0,orientation:"bottom right"});k&&(k._datepicker=m.datepickers[0]),y&&(y._datepicker=m.datepickers[1])}e.id&&(K=!0,z(`api/beneficiaries.php?id=${encodeURIComponent(e.id)}`).then(m=>{if(m.success&&m.data&&m.data.beneficiary){const S=m.data.beneficiary;if(k&&S.startDate){const h=new Date(S.startDate);isNaN(h)||(k.value=S.startDateFormatted||"",k._datepicker&&k._datepicker.setDate(h))}if(y&&S.endDate){const h=new Date(S.endDate);isNaN(h)||(y.value=S.endDateFormatted||"",y._datepicker&&y._datepicker.setDate(h))}}setTimeout(()=>{K=!1},100)}).catch(m=>{console.error("Error fetching accurate beneficiary dates:",m),K=!1})),v&&v.addEventListener("input",()=>Y=!0);const A=(m,S,h)=>{const C=l.querySelector(m),I=l.querySelector(S);if(!C||!I)return;const q=()=>I.classList.add("hidden"),Z=()=>I.classList.remove("hidden");C.addEventListener("focus",Z),C.addEventListener("input",()=>{const _=C.value.toLowerCase().trim();let J=0;I.querySelectorAll(h).forEach(c=>{const f=(c.querySelector(".option-text")?.textContent||c.textContent||"").toLowerCase().includes(_);c.style.display=f?"block":"none",f&&J++}),J>0?Z():q()}),I.addEventListener("click",_=>{const J=_.target.closest(h);J&&(C.value=(J.querySelector(".option-text")?.textContent||J.textContent||"").trim(),q(),C.dispatchEvent(new Event("change")))}),document.addEventListener("click",_=>{!C.contains(_.target)&&!I.contains(_.target)&&q()})};A("#edit-education-input","#edit-education-suggestions-box",".edit-education-option"),A("#edit-designation-input","#edit-designation-suggestions-box",".edit-designation-option"),A("#edit-office-input","#edit-office-suggestions-box",".edit-office-option");const B=l.querySelector("#edit-replacement-input"),D=l.querySelector("#edit-replacement-suggestions-box"),M=l.querySelector("#edit-replacement-loading");let j=null;B&&D&&(B.addEventListener("input",m=>{const S=m.target.value.trim();clearTimeout(j),D.classList.add("hidden"),!(S.length<2)&&(M&&M.classList.remove("hidden"),j=setTimeout(async()=>{try{const h=await ne(`api/beneficiaries.php?replacement_candidates=1&q=${encodeURIComponent(S)}&limit=10`);h.success&&h.data&&h.data.candidates&&h.data.candidates.length>0?(D.innerHTML=h.data.candidates.map(C=>`
    ...
    `;x.fire({html:u,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:n=>{window.initFlowbite&&window.initFlowbite();const l=n.querySelector("#cancel-modal-btn");l&&l.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),x.close(),e?._isBulk&&ge.onCancel()});const p=n.querySelector("#bulk-add-btn");p&&p.addEventListener("click",()=>{x.close(),ge.init()});const i=(c,w)=>{c.addEventListener("input",f=>{const $=window.__maskDate(f.target.value);if(f.target.value!==$&&(f.target.value=$),$.length===10){const E=window.__parseFormattedDate($);E&&w&&(w(E),document.activeElement===c&&c.blur())}}),c.addEventListener("changeDate",f=>{if(f.detail&&f.detail.date&&w){w(f.detail.date);const $=c._datepicker||c.parentNode&&c.parentNode._datepicker;$&&typeof $.hide=="function"&&$.hide()}})},d=n.querySelector("#birthday-input"),g=n.querySelector("#age-display");if(d){i(d,w=>{g&&(g.value=window.calculateAge(w),g.classList.add("animate-pulse"),setTimeout(()=>g.classList.remove("animate-pulse"),400))});const c=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);c&&(d._datepicker=new c(d,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"}))}const b=n.querySelector("#name-input-field"),v=n.querySelector("#duplicate-warning");if(b&&v){let c;b.addEventListener("input",w=>{const f=w.target.value.trim();if(clearTimeout(c),f.length<3){v.classList.add("hidden");return}c=setTimeout(async()=>{try{const E=await(await fetch(`${W()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:f})})).json();E.success&&E.exists?(v.classList.remove("hidden"),oe()?v.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":v.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600"):v.classList.add("hidden")}catch($){console.error("Duplicate check error:",$)}},500)}),e?.name&&(v.classList.add("hidden"),(async()=>{const f=await(await fetch(`${W()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:e.name})})).json();f.success&&f.exists&&(v.classList.remove("hidden"),oe()?v.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-400":v.className="mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse text-red-600")})())}const k=n.querySelector("#full-id-input"),y=n.querySelector("#series-no-input"),T=n.querySelector('input[name="startDate"]'),R=n.querySelector('input[name="endDate"]'),te=n.querySelectorAll('input[name="remarks"]'),Y=n.querySelector("#extension-log-container"),K=async c=>{if(!c)return;const w=`cache_next_id_${c}`,f=`cache_next_series_${c}`,$=`cache_expiry_${c}`,E=Date.now(),H=localStorage.getItem(w),P=localStorage.getItem(f),U=localStorage.getItem($);if(H&&P&&U&&E<parseInt(U,10)){k&&(k.value=H,k.classList.remove("placeholder:text-gray-300")),y&&(y.value=P);return}const V=[k,y].filter(Boolean);V.forEach(N=>{N.classList.add("animate-pulse"),N.placeholder="Syncing..."});try{const[N,O]=await Promise.all([z(`api/beneficiaries.php?next_id&year=${encodeURIComponent(c)}`),z(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(c)}`)]),Q=N.success&&N.data?.success?N.data.nextId:null,X=O.success&&O.data?.success?O.data.nextSeries:null;Q&&k&&(k.value=Q,localStorage.setItem(w,Q)),X&&y&&(y.value=X,localStorage.setItem(f,X)),localStorage.setItem($,(E+3e5).toString())}catch(N){console.error("ID Sync error:",N)}finally{V.forEach(N=>N.classList.remove("animate-pulse"))}},G=n.querySelector("#replacement-search-input"),ee=n.querySelector("#replacement-hidden"),F=n.querySelector("#replacement-suggestions"),L=c=>{const w=(c.name||"").toUpperCase().trim(),f=c.startDateFormatted||c.startDate||"N/A",$=c.endDateFormatted||c.endDate||"N/A";return`${w} (${f.toUpperCase()} - ${$.toUpperCase()})`},A=c=>{if(F){if(!c.length){F.innerHTML=`<div class="px-3 py-2 text-[10px] font-bold ${t.textCourseOpt}">No matching beneficiary found.</div>`,F.classList.remove("hidden");return}F.innerHTML=c.map(w=>{const f=L(w);return`
                    ...
                    `}).join(""),F.classList.remove("hidden"),F.querySelectorAll(".replacement-option").forEach(w=>{w.addEventListener("click",()=>{const f=w.getAttribute("data-replacement")||"";G&&(G.value=f),ee&&(ee.value=f),F.classList.add("hidden")})})}};let B=null;const D=async(c="")=>{const w=(c||"").trim(),f=`api/beneficiaries.php?replacement_candidates=1&limit=20${w?`&q=${encodeURIComponent(w)}`:""}`,$=await z(f);$.success&&$.data?.success&&Array.isArray($.data.candidates)&&A($.data.candidates)};G&&ee&&F&&(G.addEventListener("focus",()=>{D(G.value||"")}),G.addEventListener("input",()=>{ee.value=G.value.trim(),clearTimeout(B),B=setTimeout(()=>{D(G.value||"")},250)}),document.addEventListener("click",c=>{!G.contains(c.target)&&!F.contains(c.target)&&F.classList.add("hidden")}));const M=()=>{const c=n.querySelector('input[name="remarks"]:checked');return c?c.value:"ONGOING"},j=c=>{const w=n.querySelector(`input[name="remarks"][value="${c}"]`);w&&(w.checked=!0,S())},m=()=>{if(R&&R.value){const c=window.__parseFormattedDate(R.value);if(!c)return;const w=new Date;w.setHours(0,0,0,0);let f="ONGOING";c<w&&(f="EXPIRED"),j(f)}},S=()=>{if(!Y)return;if(M()==="ABSORBED"){const w=e?.absorbDate||new Date().toLocaleString("en-US",{timeZone:"Asia/Manila",month:"short",day:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit",hour12:!0});Y.innerHTML=`
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                                <div class="${r?"bg-green-900/10 border-green-900/30":"bg-[#e8f5e9]/50 border-[#c8e6c9]"} rounded-lg p-2.5 flex items-center justify-between border">
                                    <div>
                                        ...
                                        <p class="text-[10px] font-black ${r?"text-green-400":"text-[#1b5e20]"} uppercase">${w}</p>
                                        <input type="hidden" name="absorbDate" value="${w}">
                                    </div>
                    ...
                    `}else Y.innerHTML=""};if(T){let c=null;i(T,E=>{const H=E.getFullYear();if(R){const P=new Date(E);P.setMonth(P.getMonth()+3),P.setDate(P.getDate()+1);const U=String(P.getMonth()+1).padStart(2,"0"),V=String(P.getDate()).padStart(2,"0"),N=P.getFullYear();R.value=`${U}/${V}/${N}`}m(),H>1900&&H!==c&&(c=H,K(H))}),R&&i(R,()=>m());const w=n.querySelector("#date-range-picker"),f=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null),$=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);if(f&&w&&T&&R){const E=new f(w,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0,orientation:"bottom right"});T._datepicker=E.datepickers?.[0]||null,R._datepicker=E.datepickers?.[1]||null}else $&&(T&&(T._datepicker=new $(T,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"})),R&&(R._datepicker=new $(R,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"})));if(!o){const E=new Date().getFullYear();K(E)}}te.forEach(c=>c.addEventListener("change",S));const h=n.querySelector("#resign-btn"),C=n.querySelector("#absorb-btn");h&&h.addEventListener("click",()=>j("RESIGNED")),C&&C.addEventListener("click",()=>j("ABSORBED")),n.querySelectorAll('input[type="text"], textarea').forEach(c=>{["id-number-input","full-id-input"].includes(c.id)||c.addEventListener("input",()=>{const w=c.selectionStart,f=c.selectionEnd;c.value=c.value.toUpperCase(),c.setSelectionRange(w,f)})}),m(),S(),Z("education-input","course-suggestions","course-option"),Z("designation-input","work-suggestions","work-option"),Z("office-input","office-suggestions","office-option");const I={publicSector:["DOLE","LGU","DEPED"],privateSector:["DICT","PCA"]},q=(c=[])=>{const w=n.querySelector("#office-suggestions");if(!w)return;const f=N=>String(N||"").trim().toUpperCase(),$=new Set(I.publicSector.map(f)),E=new Set(I.privateSector.map(f)),H=new Set([...$,...E]),U=[...new Set((c||[]).map(f).filter(Boolean))].filter(N=>!H.has(N)),V=(N,O)=>O.length?`
                        ...
                        `).join("")}
                    `:"";w.innerHTML=[V("Public Sector",[...$]),V("Private Sector",[...E]),V("Other Offices",U)].join("")};q(),(async()=>{try{const c=await z("api/beneficiaries.php?get_offices=1");c.success&&c.data?.success&&Array.isArray(c.data.offices)&&q(c.data.offices)}catch(c){console.error("Office suggestion sync failed:",c)}})();function Z(c,w,f){const $=n.querySelector(`#${c}`),E=n.querySelector(`#${w}`);if(!$||!E)return;let H=!1;$.addEventListener("focus",()=>E.classList.remove("hidden")),document.addEventListener("click",P=>{!$.contains(P.target)&&!E.contains(P.target)&&E.classList.add("hidden")}),$.addEventListener("input",()=>{if(H){H=!1;return}const P=$.value.toLowerCase(),U=E.querySelectorAll(`.${f}`);let V=!1;U.forEach(N=>{const O=N.querySelector(".option-text");(O?O.innerText:N.innerText).toLowerCase().includes(P)?(N.style.display="block",V=!0):N.style.display="none"}),V?E.classList.remove("hidden"):E.classList.add("hidden")}),E.addEventListener("click",P=>{const U=P.target.closest(`.${f}`);if(!U)return;const V=U.querySelector(".option-text");$.value=V?V.innerText.trim():U.innerText.trim(),H=!0,E.classList.add("hidden"),$.dispatchEvent(new Event("change"))})}const _=n.querySelector("#add-beneficiary-form"),J="add_beneficiary_draft";if(!o){const c=localStorage.getItem(J);if(c)try{const w=JSON.parse(c);Object.entries(w).forEach(([f,$])=>{const E=_.elements[f];E&&E.type!=="file"&&E.type!=="hidden"&&(E.value=$)})}catch(w){console.error("Error loading draft",w)}}_.addEventListener("input",c=>{if(!o){const w=new FormData(_),f={};w.forEach(($,E)=>f[E]=$),localStorage.setItem(J,JSON.stringify(f))}}),_&&_.addEventListener("submit",c=>{c.preventDefault(),_.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(X=>{X.classList.remove("ring-2","ring-red-500","!border-red-500")});const f=new FormData(_);let $=!1;const E=X=>{const re=_.querySelector(`[name="${X}"]`);re&&re.classList.add("ring-2","ring-red-500","!border-red-500"),$=!0},H=f.get("name"),P=f.get("contact"),U=f.get("startDate"),V=f.get("endDate"),N=(f.get("designation")||"").trim();if((!H||H.trim()===""||/[0-9]/.test(H))&&E("name"),P&&P.trim()!==""&&/[^0-9]/.test(P.replace(/[\s\-\+\(\)]/g,""))&&E("contact"),U||E("startDate"),V||E("endDate"),$)return;const O={};f.forEach((X,re)=>{if(["birthday","startDate","endDate"].includes(re)){const se=window.__parseFormattedDate(X);if(se){const be=se.getFullYear(),he=String(se.getMonth()+1).padStart(2,"0"),Me=String(se.getDate()).padStart(2,"0");O[re]=`${be}-${he}-${Me}`}else O[re]=X}else O[re]=X}),N||(O.designation="N/A"),O.replacement||(O.replacement="");const Q=n.querySelector("#full-id-input")?.value;o?(O.id=e?.id,Q&&(O.gip_id=Q)):(O.id=null,Q&&(O.gip_id=Q)),window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(O)){if(!o){const re="add_beneficiary_draft",se=_.querySelector('[name="office"]')?.value||"",be=_.querySelector('[name="designation"]')?.value||"",he=_.querySelector('[name="education"]')?.value||"";localStorage.setItem(re,JSON.stringify({office:se,designation:be,education:he}))}x.close(),setTimeout(()=>{x.fire({toast:!0,position:"top-end",icon:"success",title:`Record ${o?"Updated":"Added"} Successfully`,showConfirmButton:!1,timer:3e3,timerProgressBar:!0}),!o&&e?._isBulk&&ge.onSaveSuccess()},100)}else x.fire({icon:"error",title:"Save Failed",text:"There was an error saving the record to the database."})})()})}})}window.handleContactSubmit=async function(e){e.preventDefault();const o=e.target,s=o.querySelector('button[type="submit"]'),a=s.innerHTML;s.disabled=!0,s.innerHTML=`
        <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        Sending...
    `;try{const r=new FormData(o);if((await fetch(o.action,{method:"POST",body:r,headers:{Accept:"application/json"}})).ok)x.fire({toast:!0,position:"top-end",icon:"success",title:"Message Sent Successfully!",text:"Thank you for reaching out. I will get back to you soon!",showConfirmButton:!1,timer:4e3,timerProgressBar:!0,didOpen:u=>{u.addEventListener("mouseenter",x.stopTimer),u.addEventListener("mouseleave",x.resumeTimer)},customClass:{popup:"rounded-2xl border-l-4 border-teal-500 shadow-2xl"}}),o.reset();else throw new Error("Failed to send")}catch{x.fire({toast:!0,position:"top-end",icon:"error",title:"Oops! Something went wrong.",text:"Could not send your message. Please try again later.",showConfirmButton:!1,timer:4e3,customClass:{popup:"rounded-2xl border-l-4 border-red-500 shadow-2xl"}})}finally{s.disabled=!1,s.innerHTML=a}return!1};export{Ce as _,z as a,ne as b,rt as c,st as d,at as e,Ke as f,W as g,nt as h,Se as i,Qe as j,et as k,ae as l,tt as m,it as n,Te as p,ot as r,me as s,Ze as u};
