...
import C from"./vendor-swal-BSk0fVSb.js";import{a as Fe}from"./vendor-DHtNC8Ux.js";const He="true".toLowerCase()==="true";function $e(){return He}function Q(){const e=window.location.pathname,o="/dole-system/",n=e.toLowerCase().indexOf(o.toLowerCase());if(n!==-1)return e.substring(0,n+o.length);const a=e.indexOf("/frontend/");if(a!==-1)return e.substring(0,a+1);const r=e.indexOf("/backend/");return r!==-1?e.substring(0,r+1):"/"}function De(e="Incorrect Username or Password"){C.fire({html:`
            <div class="p-6">
        ...
        `,timer:e?800:3e3,timerProgressBar:!0,showConfirmButton:!1,showCloseButton:!1,width:"350px",padding:"0",customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl overflow-hidden border border-gray-100",timerProgressBar:"bg-green-500 h-1.5"},backdrop:"rgba(0, 0, 0, 0.4)"})}function pt(){const e=localStorage.getItem("hasVisitedBefore"),o=document.getElementById("page-loader");e&&(document.documentElement.classList.add("loaded"),o&&(o.style.display="none")),window.addEventListener("load",()=>{const n=document.querySelector("body > *:not(.page-loader)");n&&n.classList.add("content-fade-in"),setTimeout(()=>{document.documentElement.classList.add("loaded"),o&&o.classList.add("hidden"),document.body.style.overflow="auto",localStorage.setItem("hasVisitedBefore","true")},300)})}const me={_key:"DOLE-GIP-SECURE-KEY-2026",encrypt:function(e){try{const o=r=>r.split("").map(t=>t.charCodeAt(0)),n=r=>("0"+Number(r).toString(16)).substr(-2),a=r=>o(this._key).reduce((t,u)=>t^u,r);return e.split("").map(o).map(a).map(n).join("")}catch(o){return console.error("Encryption Failed",o),null}},decrypt:function(e){try{const o=a=>a.split("").map(r=>r.charCodeAt(0)),n=a=>o(this._key).reduce((r,t)=>r^t,a);return e.match(/.{1,2}/g).map(a=>parseInt(a,16)).map(n).map(a=>String.fromCharCode(a)).join("")}catch(o){return console.error("Decryption Failed",o),null}}};function ut(){document.querySelectorAll(".login-form-shared").forEach(o=>{const n=o.querySelector('input[name="username"]'),a=o.querySelector('input[name="password"]'),r=o.querySelector('input[name="rememberMe"]');if(n&&a&&r){const t=localStorage.getItem("secure_user"),u=localStorage.getItem("secure_pass");if(t&&u){const s=me.decrypt(t),f=me.decrypt(u);s&&f&&(n.value=s,a.value=f,r.checked=!0)}}o.addEventListener("submit",async t=>{t.preventDefault();try{const s=await(await fetch(`${Q()}api/auth.php`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:n.value,password:a.value})})).json();if(s.success){r.checked?(localStorage.setItem("secure_user",me.encrypt(n.value)),localStorage.setItem("secure_pass",me.encrypt(a.value))):(localStorage.removeItem("secure_user"),localStorage.removeItem("secure_pass"));const f=localStorage.getItem("hasLoggedInBefore")==="true";localStorage.setItem("isLoggedIn","true"),localStorage.setItem("hasLoggedInBefore","true"),localStorage.setItem("user",JSON.stringify(s.user));const x=document.getElementById("drawer-login");if(x){x.classList.add("translate-y-full");const i=x.querySelector("[data-drawer-hide]");i&&i.click()}await qe(f),Ve(f)}else{const f=document.getElementById("drawer-login");f?(f.classList.add("translate-y-full"),setTimeout(()=>{De(),setTimeout(()=>{f.classList.remove("translate-y-full"),a.value="",a.focus()},600)},400)):(De(),a.value="",a.focus())}}catch(u){console.error("Login Error:",u),C.fire({icon:"error",title:"Login Error",text:"Unable to reach the server. Please check your connection."})}})})}function Ve(e=!1){const o=document.getElementById("left-panel"),n=document.getElementById("right-panel"),a=document.getElementById("left-panel-content"),r=document.getElementById("right-panel-content");a&&(a.style.opacity="0"),r&&(r.style.opacity="0");const t=document.createElement("div");t.className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none";const u=e?"":"animate__delay-1s",s=e?"animation-duration: 0.8s;":"animation-duration: 2s;";t.innerHTML=`
        <img src="${Q()}frontend/images/logo/doleiligan.png" class="w-64 h-64 md:w-96 md:h-96 object-contain bg-white/20 backdrop-blur-sm shadow-2xl rounded-full p-4 animate__animated animate__rotateOut ${u}" style="${s}" alt="DOLE Logo">
    `,document.body.appendChild(t);const f=e?0:1e3,x=e?600:1500;setTimeout(()=>{o&&o.classList.add("animate-slide-left"),n&&n.classList.add("animate-slide-right"),setTimeout(()=>{window.location.href=`${Q()}frontend/dashboard/`},x)},f)}function gt(){const e=document.getElementById("logoutBtn");e&&e.addEventListener("click",()=>{localStorage.removeItem("isLoggedIn"),window.location.href=`${Q()}`})}function bt(){const e=document.getElementById("mobile-splash"),o=document.getElementById("show-login-btn"),n=document.getElementById("back-to-splash"),a=document.getElementById("mobile-bg-content"),r=document.getElementById("mobile-welcome-text"),t=document.getElementById("reopen-login-drawer"),u=document.getElementById("request-notifications-btn"),s=async()=>{if("Notification"in window){const b=await Notification.requestPermission();console.log("Notification permission:",b),b==="granted"&&u&&u.classList.add("hidden")}};Notification.permission==="default"&&u&&(u.classList.remove("hidden"),u.addEventListener("click",s));const f=()=>{e&&(e.style.transform="translateY(-100%)",setTimeout(()=>{e.style.visibility="hidden",e.style.pointerEvents="none",e.style.zIndex="-1",Notification.permission==="default"&&s();const b=document.getElementById("drawer-login");b&&b.classList.remove("translate-y-full")},800))},x=()=>{e&&(e.style.zIndex="9999",e.style.pointerEvents="auto",e.style.visibility="visible",e.style.transform="translateY(0)")};o&&o.addEventListener("click",f),document.querySelectorAll(".forgot-password-link").forEach(b=>{b.addEventListener("click",D=>{D.target.closest("#mobile-splash")&&f()})}),n&&n.addEventListener("click",()=>{const b=document.getElementById("drawer-login");if(b){b.classList.add("translate-y-full");const D=b.querySelector("[data-drawer-hide]");D&&D.click()}x()});const l=document.getElementById("drawer-login"),v=document.getElementById("curved-welcome"),E=document.getElementById("peoples-bg");l&&new MutationObserver(D=>{D.forEach($=>{$.attributeName==="class"&&(l.classList.contains("translate-y-full")?(a&&(a.style.transform="translateY(0)"),r&&(r.style.opacity="1",r.style.transform="translateY(0) scale(1)"),v&&(v.style.opacity="0",v.style.transform="scale(0.5)"),t&&e&&e.style.visibility==="hidden"&&(t.style.opacity="1",t.style.transform="scale(1)"),E&&(E.classList.add("opacity-0","scale-0"),E.classList.remove("opacity-40","scale-[1.6]"))):(a&&(a.style.transform="translateY(-35%)"),r&&(r.style.opacity="0",r.style.transform="translateY(20px) scale(0.9)"),v&&(v.style.opacity="1",v.style.transform="scale(1)"),t&&(t.style.opacity="0",t.style.transform="scale(0)"),E&&(E.classList.remove("opacity-0","scale-0"),E.classList.add("opacity-40","scale-[1.6]"))))})}).observe(l,{attributes:!0})}const fe=()=>"false".toLowerCase()==="true";function Ue(e){try{return JSON.stringify(e)}catch{return"[unserializable]"}}const ue={debug(...e){fe()&&console.debug(...e)},info(...e){fe()&&console.info(...e)},warn(...e){fe()&&console.warn(...e)},error(...e){console.error(...e)},table(e){fe()&&console.table(e)},json(e,o){fe()&&console.debug(e,Ue(o))}},ve=new Map;async function xe(e,o={}){const a=`${Q()}${e}`;let r=null;try{const i=JSON.parse(localStorage.getItem("user"));i&&(r=i.user_id||i.id||null)}catch{}const t={headers:{"Content-Type":"application/json",...r?{"X-User-Id":r}:{},...o.headers},...o},s=(t.method||"GET").toUpperCase()==="GET"?2:1;let f=null;for(let i=1;i<=s;i++)try{if(ue.debug("[API] Request",{url:a,method:t.method||"GET",hasUserId:!!r}),t.body)try{ue.json("[API] Payload",JSON.parse(t.body))}catch{ue.debug("[API] Payload (raw)",t.body)}const l=await fetch(a,t);if(!l.ok)throw new Error(`HTTP ${l.status}: ${l.statusText}`);const v=await l.json();return ve.has(a)&&(ve.delete(a),ue.info?.("[API] Recovered",{url:a})),ue.debug("[API] Response",{url:a,ok:!0}),{success:!0,data:v}}catch(l){if(f=l,l instanceof TypeError&&/fetch/i.test(l.message||"")&&i<s){await new Promise(b=>setTimeout(b,1200));continue}}return f instanceof TypeError&&/fetch/i.test(f.message||"")?ve.get(a)||(ve.set(a,!0),ue.error("API Request Network Error (suppressed for repeats):",{url:a,message:f.message})):ue.error("API Request Error:",f),{success:!1,error:f?.message||"Unknown request error"}}async function W(e){return xe(e,{method:"GET"})}async function Se(e,o){return xe(e,{method:"POST",body:JSON.stringify(o)})}async function Ge(e,o){return xe(e,{method:"PUT",body:JSON.stringify(o)})}async function ft(e,o){const n=new URLSearchParams(o).toString();return xe(`${e}?${n}`,{method:"PATCH"})}class ze{constructor(){this.intervals=new Map,this.isPageVisible=!0,document.addEventListener("visibilitychange",()=>{this.isPageVisible=!document.hidden,this.isPageVisible?console.log("[Polling] Page visible - resuming normal activity"):console.log("[Polling] Page hidden - reducing activity")})}start(o,n,a=1e4){this.stop(o),n();const r=setInterval(async()=>{this.isPageVisible&&await n()},a);this.intervals.set(o,r),console.log(`[Polling] Started: ${o} (every ${a}ms)`)}stop(o){this.intervals.has(o)&&(clearInterval(this.intervals.get(o)),this.intervals.delete(o),console.log(`[Polling] Stopped: ${o}`))}stopAll(){this.intervals.forEach((o,n)=>this.stop(n)),console.log("[Polling] All intervals stopped")}getActivePolls(){return Array.from(this.intervals.keys())}}const Ye=new ze;function ht(){typeof window.initFlowbite=="function"?(window.initFlowbite(),console.log("[Flowbite] Components re-initialized")):console.warn("[Flowbite] initFlowbite() not available globally")}function xt(e){return JSON.stringify(e)}window.addEventListener("beforeunload",()=>{Ye.stopAll()});const Ze="modulepreload",Je=function(e,o){return new URL(e,o).href},Le={},Oe=function(o,n,a){let r=Promise.resolve();if(n&&n.length>0){let x=function(i){return Promise.all(i.map(l=>Promise.resolve(l).then(v=>({status:"fulfilled",value:v}),v=>({status:"rejected",reason:v}))))};const u=document.getElementsByTagName("link"),s=document.querySelector("meta[property=csp-nonce]"),f=s?.nonce||s?.getAttribute("nonce");r=x(n.map(i=>{if(i=Je(i,a),i in Le)return;Le[i]=!0;const l=i.endsWith(".css"),v=l?'[rel="stylesheet"]':"";if(a)for(let b=u.length-1;b>=0;b--){const D=u[b];if(D.href===i&&(!l||D.rel==="stylesheet"))return}else if(document.querySelector(`link[href="${i}"]${v}`))return;const E=document.createElement("link");if(E.rel=l?"stylesheet":Ze,l||(E.as="script"),E.crossOrigin="",E.href=i,f&&E.setAttribute("nonce",f),document.head.appendChild(E),l)return new Promise((b,D)=>{E.addEventListener("load",b),E.addEventListener("error",()=>D(new Error(`Unable to preload CSS for ${i}`)))})}))}function t(u){const s=new Event("vite:preloadError",{cancelable:!0});if(s.payload=u,window.dispatchEvent(s),!s.defaultPrevented)throw u}return r.then(u=>{for(const s of u||[])s.status==="rejected"&&t(s.reason);return o().catch(t)})};let ge=null;if($e()){const e="https://llnddycvbcetztzwbdpx.supabase.co",o="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxsbmRkeWN2YmNldHp0endiZHB4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzQ1MDMwMTEsImV4cCI6MjA5MDA3OTAxMX0.apIpDlZYja7SNuA-joXgFSvu_wtbBcVVP3OQSPrh7fA";try{ge=Fe(e,o),console.log("[SUPABASE SDK] Client Initialized Successfully")}catch(n){console.error("[SUPABASE SDK] Failed to initialize client:",n)}}else console.log("[SUPABASE SDK] Supabase mode is disabled (Localhost PHP mode active).");function Xe(e=new Date().getFullYear()){const o=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"],n=[];for(let a=0;a<12;a++){const r=new Date(e,a+1,0).getDate();n.push(`${o[a]} 1-15, ${e}`),n.push(`${o[a]} 16-${r}, ${e}`)}return n}function We(e,o,n){if(n==="ar")return(e.period||"").toUpperCase().trim()===o.toUpperCase().trim();{const a=o.match(/([A-Z]+)\s+(\d+)-(\d+),\s+(\d+)/i);if(!a)return!1;const t=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"].indexOf(a[1].toUpperCase());if(t===-1)return!1;const u=parseInt(a[4]),s=parseInt(a[2]),f=parseInt(a[3]),x=(e.date||"").substring(0,10),i=new Date(x+"T00:00:00");return isNaN(i)?!1:i.getFullYear()===u&&i.getMonth()===t&&i.getDate()>=s&&i.getDate()<=f}}function Ke(e){if(!e)return"-";const o=e.toUpperCase();return o==="VERIFIED"||o==="COMPLETED"?"✓":o==="REJECTED"||o==="DECLINED"?"X":o==="PENDING"?"?":o}function Ie(e,o,n,a){const r=e.map(t=>{const u=o[t.id]||[],s=a.map(f=>{const x=u.find(i=>We(i,f,n));return x?Ke(x.status):"-"});return{name:t.name||t.id,cells:s}});return{periods:a,rows:r}}function Be(e,o,n){const{periods:a,rows:r}=o,t=a.length+1;let u='<table border="1" style="border-collapse:collapse;font-family:Arial,sans-serif;font-size:10px;">';return u+=`<tr><td colspan="${t}" style="background:${n};color:#fff;font-weight:bold;font-size:13px;padding:8px 12px;text-transform:uppercase;letter-spacing:1px;">${e}</td></tr>`,u+=`<tr><th style="background:${n};color:#fff;padding:6px 10px;min-width:200px;text-align:left;">BENEFICIARY NAME</th>`,a.forEach(s=>{u+=`<th style="background:${n};color:#fff;padding:6px 8px;text-align:center;white-space:nowrap;">${s}</th>`}),u+="</tr>",r.forEach((s,f)=>{const x=f%2===0?"#ffffff":"#f5f5f5";u+="<tr>",u+=`<td style="background:${x};padding:5px 10px;font-weight:bold;text-transform:uppercase;">${s.name}</td>`,s.cells.forEach(i=>{u+=`<td style="background:${x};padding:5px 8px;text-align:center;font-weight:bold;color:${i==="✓"?"#15803d":i==="X"?"#dc2626":"#9ca3af"};">${i}</td>`}),u+="</tr>"}),u+="</table>",u}async function mt(e){const o="flex flex-col items-center justify-center p-3 rounded-2xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[10px] gap-1.5",n=e.length,a=await C.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Export DTR/AR',html:`
            <div class="font-montserrat text-left">
                ...
                <div class="grid grid-cols-3 gap-2 mb-5">
                    <label class="relative block cursor-pointer">
                        ...
                        <div class="${o} border-gray-100 bg-gray-50 text-gray-400 peer-checked:border-blue-500 peer-checked:bg-blue-50 peer-checked:text-blue-600 hover:bg-blue-50/50">
                    ...
                    <label class="relative block cursor-pointer">
                        ...
                        <div class="${o} border-gray-100 bg-gray-50 text-gray-400 peer-checked:border-orange-500 peer-checked:bg-orange-50 peer-checked:text-orange-600 hover:bg-orange-50/50">
                    ...
                    <label class="relative block cursor-pointer">
                        ...
                        <div class="${o} border-gray-100 bg-gray-50 text-gray-400 peer-checked:border-emerald-500 peer-checked:bg-emerald-50 peer-checked:text-emerald-600 hover:bg-emerald-50/50">
                ...
                <div class="grid grid-cols-3 gap-2 mb-3" id="count-options">
                    <label class="relative block cursor-pointer">
                        ...
                        <div class="${o} border-gray-100 bg-gray-50 text-gray-400 peer-checked:border-royal-blue peer-checked:bg-blue-50 peer-checked:text-royal-blue hover:bg-blue-50/50">
        ...
        `,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Generate Excel</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Cancel</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-emerald-600 text-white text-xs px-6 py-2.5 rounded-xl shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1,didOpen:()=>{document.querySelectorAll('input[name="exp-count"]').forEach(f=>{f.addEventListener("change",()=>{const x=document.getElementById("custom-count-wrap");x.classList.toggle("hidden",f.value!=="custom"||!f.checked);const i=document.querySelector('input[name="exp-count"]:checked');x.classList.toggle("hidden",i?.value!=="custom")})})},preConfirm:()=>{const f=document.querySelector('input[name="exp-type"]:checked')?.value||"dtr",x=document.querySelector('input[name="exp-count"]:checked')?.value||"10";let i=parseInt(x==="custom"?document.getElementById("exp-custom-count")?.value||n:x,10);(isNaN(i)||i<1)&&(i=10),i=Math.min(i,n);const l=parseInt(document.getElementById("exp-year")?.value||new Date().getFullYear(),10);return{type:f,count:i,year:l}}});if(!a.isConfirmed||!a.value)return;const{type:r,count:t,year:u}=a.value,s=e.slice(0,t);await Ne(s,r,u)}async function Ne(e,o,n){C.fire({title:'<span class="text-lg font-black text-emerald-600 uppercase tracking-tight">Building Excel File</span>',html:`<div class="flex flex-col items-center p-4">
        ...
        </div>`,showConfirmButton:!1,allowOutsideClick:!1,customClass:{popup:"rounded-[1.5rem] shadow-2xl font-montserrat dark:bg-slate-900 border border-gray-100 dark:border-slate-800"}});try{const a=Xe(n),r=Q();async function t(D){const T=await(await fetch(`${r}api/logs.php?type=${D}`,{headers:{"X-User-Id":(()=>{try{return JSON.parse(localStorage.getItem("user"))?.id||""}catch{return""}})()}})).json();return T.success?T.logs||[]:[]}let u={},s={};(o==="dtr"||o==="both")&&(await t("dtr")).forEach($=>{const T=String($.gip_id||$.beneficiary_id||$.id||"");u[T]||(u[T]=[]),u[T].push($)}),(o==="ar"||o==="both")&&(await t("ar")).forEach($=>{const T=String($.gip_id||$.beneficiary_id||$.id||"");s[T]||(s[T]=[]),s[T].push($)});const f=e.map(D=>({...D,mapKey:String(D.id||D.gip_id||D.beneficiary_id)}));let x="";const i=new Intl.DateTimeFormat("en-PH",{timeZone:"Asia/Manila",dateStyle:"long",timeStyle:"short"}).format(new Date);if(o==="dtr"||o==="both"){const D=f.map(T=>({...T,id:T.mapKey})),$=Ie(D,u,"dtr",a);x+="<br>"+Be(`DTR – Daily Time Records (${n})`,$,"#1d4ed8")}if(o==="ar"||o==="both"){const D=f.map(T=>({...T,id:T.mapKey})),$=Ie(D,s,"ar",a);x+="<br><br>"+Be(`AR – Accomplishment Reports (${n})`,$,"#d97706")}const l=`
        ...
        `,v=new Blob([l],{type:"application/vnd.ms-excel"}),E=URL.createObjectURL(v),b=document.createElement("a");b.href=E,b.download=`LDN_LOGS_${o.toUpperCase()}_${n}.xls`,document.body.appendChild(b),b.click(),URL.revokeObjectURL(E),document.body.removeChild(b),C.fire({toast:!0,position:"top-end",icon:"success",title:"Excel file downloaded!",showConfirmButton:!1,timer:2500})}catch(a){console.error("[LogsExport] Error:",a),C.fire("Error",a.message||"Failed to generate export.","error")}}const be="color-theme",Qe=3600*24*365;function et(e,o,n){document.cookie=`${e}=${o}; max-age=${n}; path=/; SameSite=Lax`}function Te(e){const o=document.cookie.match(new RegExp("(?:^|; )"+e.replace(/[.*+?^${}()|[\]\\]/g,"\\$&")+"=([^;]*)"));return o?decodeURIComponent(o[1]):null}function Re(){const e=localStorage.getItem(be)||Te(be);return e==="dark"||e==="light"?e:"light"}function ye(e){const o=document.documentElement;e==="dark"?o.classList.add("dark"):o.classList.remove("dark"),localStorage.setItem(be,e),et(be,e,Qe),tt(e),document.dispatchEvent(new CustomEvent("themeChanged",{detail:{theme:e}}))}function Ee(){const e=Re();ye(e==="dark"?"light":"dark")}function tt(e){const o=e==="dark",n=document.getElementById("pref-dark-mode");n&&(n.checked=o);const a=document.getElementById("theme-toggle-dark-icon"),r=document.getElementById("theme-toggle-light-icon");a&&r&&(a.classList.toggle("hidden",o),r.classList.toggle("hidden",!o));const t=document.getElementById("sidebar-theme-label");t&&(t.textContent=o?"Light Mode":"Dark Mode")}function vt(){const e=Re();ye(e);const o=document.getElementById("pref-dark-mode");o&&o.addEventListener("change",()=>{ye(o.checked?"dark":"light")});const n=document.getElementById("theme-toggle-btn");n&&n.addEventListener("click",Ee),document.querySelectorAll("[data-theme-toggle]").forEach(a=>{a.addEventListener("click",Ee)}),window.matchMedia("(prefers-color-scheme: dark)").addEventListener("change",a=>{localStorage.getItem(be)||Te(be)||ye(a.matches?"dark":"light")})}function ae(){return document.documentElement.classList.contains("dark")}window.toggleTheme=Ee;window.isDarkMode=ae;const ke={queue:[],currentIndex:0,isActive:!1,isAutoSave:!1,lastInteractionTime:0,init(){this.showUploadModal()},showUploadModal(){const e=ae(),o={bgCard:e?"bg-slate-900/40":"bg-gray-50/40",borderCard:e?"border-slate-800":"border-gray-100",textHeading:e?"text-green-500":"text-[#2e7d32]",textSubtitle:e?"text-slate-500":"text-gray-400 dark:text-gray-300",bgUpload:e?"bg-slate-800":"bg-white",borderUpload:e?"border-slate-700":"border-gray-200",textUpload:e?"text-slate-400":"text-gray-500",hoverUpload:e?"hover:bg-slate-800/80 hover:border-blue-500":"hover:bg-blue-50/50 hover:border-royal-blue",iconText:e?"text-green-400":"text-[#2e7d32]",iconBg:e?"bg-green-900/20":"bg-[#e8f5e9]",iconBorder:e?"border-green-800/30":"border-[#c8e6c9]"},n=`
        ...
        `;C.fire({html:n,width:"600px",showConfirmButton:!1,showCloseButton:!0,customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},didOpen:a=>{const r=a.querySelector("#csv-upload"),t=a.querySelector('label[for="csv-upload"]');["dragenter","dragover","dragleave","drop"].forEach(s=>{t.addEventListener(s,u,!1)});function u(s){s.preventDefault(),s.stopPropagation()}["dragenter","dragover"].forEach(s=>{t.addEventListener(s,()=>{t.classList.add("border-blue-500","bg-blue-50/50"),e&&t.classList.add("dark:bg-slate-800/80")},!1)}),["dragleave","drop"].forEach(s=>{t.addEventListener(s,()=>{t.classList.remove("border-blue-500","bg-blue-50/50"),e&&t.classList.remove("dark:bg-slate-800/80")},!1)}),r.addEventListener("change",s=>{const f=s.target.files[0];if(f){const x=a.querySelector("#auto-save-toggle");this.isAutoSave=x?x.checked:!1,this.handleFile(f)}}),t.addEventListener("drop",s=>{const x=s.dataTransfer.files[0];if(x){const i=a.querySelector("#auto-save-toggle");this.isAutoSave=i?i.checked:!1,this.handleFile(x)}},!1)}})},handleFile(e){if(!e.name.toLowerCase().endsWith(".csv")&&!e.name.toLowerCase().endsWith(".txt")){C.fire("Invalid File","Please upload a valid .csv or .txt file.","error");return}const o=new FileReader;o.onload=n=>{const a=n.target.result;this.parseCSV(a)},o.readAsText(e)},async parseCSV(e){let o=[],n="",a=!1;for(let r=0;r<e.length;r++){let t=e[r];t==='"'&&(a=!a),!a&&(t===`
`||t==="\r")?(t==="\r"&&e[r+1]===`
`&&r++,n.trim()!==""&&o.push(n),n=""):n+=t}n.trim()!==""&&o.push(n),this.queue=[];for(let r=0;r<o.length;r++){let t=o[r].trim();if(!t)continue;let u=[],s="",f=!1;for(let x=0;x<t.length;x++){let i=t[x];i==='"'?f=!f:i===","&&!f?(u.push(s.replace(/(^"|"$)/g,"").trim()),s=""):s+=i}if(u.push(s.replace(/(^"|"$)/g,"").trim()),u.length>=2){const x=u[3];if(!x||isNaN(parseInt(x)))continue;const i=u[1];if(!i||i.toLowerCase()==="name"||i.toLowerCase()==="full name")continue;const l=u[2];let v=u[4]?u[4].toUpperCase().trim():"",E="";(v==="F"||v.includes("FEMALE"))&&(E="Female"),(v==="M"||v.includes("MALE"))&&(E="Male");const b=u[5],D=u[6],$=u[7],T=this.formatDate(u[8]),K=this.formatDate(u[9]);this.queue.push({name:i,address:l,age:x,gender:E,education:b,startDate:T,endDate:K,office:D,designation:$})}}if(this.queue.length>0){try{C.fire({title:"Checking duplicates...",html:'<p class="text-sm">Please wait while we cross-reference your data.</p>',allowOutsideClick:!1,didOpen:()=>{C.showLoading()}});const r=this.queue.map(f=>f.name);let t=null;try{t=JSON.parse(localStorage.getItem("user")||"{}")?.id||null}catch{t=null}const s=await(await fetch(`${Q()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json",...t?{"X-User-Id":String(t)}:{}},body:JSON.stringify({names:r,user_id:t})})).json();if(s.success&&s.duplicates&&s.duplicates.length>0){const f=new Set(s.duplicates.map(i=>i.toLowerCase().trim())),x=this.queue.length;this.queue=this.queue.filter(i=>{const l=f.has(i.name.toLowerCase().trim());return l&&console.warn(`%c[Bulk Add] SKIPPED: ${i.name} already exists in database.`,"color: #ff9800; font-weight: bold;"),!l}),console.log(`[Bulk Add] Removed ${x-this.queue.length} duplicates ahead of time.`)}}catch(r){console.error("Bulk duplicate check failed:",r)}if(this.queue.length===0){C.fire("All Skipped","All beneficiaries in the CSV already exist in the database.","info");return}this.isActive=!0,this.currentIndex=0,C.close(),this.processNext()}else C.fire("Error","No valid beneficiary data found. Please ensure your CSV formatting matches the requirements.","error")},showProgressModal(){ae();const e=Math.round(this.currentIndex/this.queue.length*100),o=`
        ...
        `;if(C.isVisible()&&C.getPopup().querySelector("#bulk-progress-bar")){const n=document.getElementById("bulk-progress-bar"),a=C.getPopup().querySelector("span.text-\\[10px\\]"),r=document.getElementById("bulk-current-name");n&&(n.style.width=`${e}%`),a&&(a.textContent=`${this.currentIndex} / ${this.queue.length}`),r&&(r.textContent=this.queue[this.currentIndex]?.name||"...")}else C.fire({html:o,showConfirmButton:!1,allowOutsideClick:!1,allowEscapeKey:!1,width:"450px",customClass:{container:"font-montserrat",popup:"rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-800"},didOpen:n=>{n.querySelector("#stop-bulk-btn").onclick=()=>this.onCancel()}})},formatDate(e){if(!e||e.trim()==="")return"";const o=new Date(e);if(isNaN(o.getTime())){const t=e.split("/");return t.length===3?`${t[2]}-${t[1].padStart(2,"0")}-${t[0].padStart(2,"0")}`:""}const n=o.getFullYear(),a=String(o.getMonth()+1).padStart(2,"0"),r=String(o.getDate()).padStart(2,"0");return`${n}-${a}-${r}`},async processNext(){if(this.currentIndex<this.queue.length){const e=this.queue[this.currentIndex];e._isBulk=!0,e._bulkCurrent=this.currentIndex+1,e._bulkTotal=this.queue.length,this.isAutoSave?(this.showProgressModal(),window.addBeneficiaryData?(async()=>{try{const n=e.startDate?new Date(e.startDate).getFullYear():new Date().getFullYear(),[a,r]=await Promise.all([W(`api/beneficiaries.php?next_id&year=${n}`),W(`api/beneficiaries.php?next_series_no&year=${n}`)]);a.success&&a.data?.success&&a.data?.nextId&&(e.gip_id=a.data.nextId,e.id=null),r.success&&r.data?.success&&r.data?.nextSeries&&(e.seriesNo=r.data.nextSeries)}catch(n){console.warn("[Bulk Add] Identifier fetch failed, continuing:",n?.message||n)}const o=await window.addBeneficiaryData(e);this.isActive&&(o?this.onSaveSuccess():he(e))})():he(e)):he(e)}else this.isActive=!1,this.lastInteractionTime=Date.now(),C.fire({icon:"success",title:"Bulk Add Complete!",text:`Successfully processed ${this.queue.length} beneficiaries.`,confirmButtonColor:"#2e7d32"})},onSaveSuccess(){if(this.isActive){this.currentIndex++;const e=this.isAutoSave?100:1500;setTimeout(()=>{this.processNext()},e)}},onCancel(){this.isActive&&(this.isActive=!1,this.lastInteractionTime=Date.now(),this.queue=[],this.currentIndex=0,C.fire({icon:"info",title:"Bulk Add Cancelled",text:"Stopped processing the remaining beneficiaries and cleared the queue.",confirmButtonColor:"#3085d6"}).then(()=>{this.showUploadModal()}))}};window.BulkApp=ke;function Ae(e){if(!e)return"";const o=new Date(e),n=new Date;let a=n.getFullYear()-o.getFullYear();const r=n.getMonth()-o.getMonth();return(r<0||r===0&&n.getDate()<o.getDate())&&a--,a>=0?a:0}function rt(e){return e?e.includes("DOLE")?"bg-blue-100 text-blue-700 border-blue-200 dark:bg-blue-900/40 dark:text-blue-300 dark:border-blue-800":e.includes("DepEd")?"bg-orange-100 text-orange-700 border-orange-200 dark:bg-orange-900/40 dark:text-orange-300 dark:border-orange-800":e.includes("LGU")?"bg-purple-100 text-purple-700 border-purple-200 dark:bg-purple-900/40 dark:text-purple-300 dark:border-purple-800":e.includes("DICT")?"bg-cyan-100 text-cyan-700 border-cyan-200 dark:bg-cyan-900/40 dark:text-cyan-300 dark:border-cyan-800":"bg-gray-100 text-gray-700 border-gray-200 dark:text-gray-300":"bg-gray-100 text-gray-700 border-gray-200 dark:text-gray-300"}function ot(e){if(!e)return"bg-gray-100 text-gray-600 border-gray-200 dark:text-gray-300";const o=e.toUpperCase();return o==="ONGOING"?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800":o==="EXPIRED"?"bg-red-400 text-white border-red-400 dark:bg-red-900/60 dark:border-red-800":o==="RESIGNED"?"bg-[#ce1126] text-white border-[#ce1126] dark:bg-red-900/80 dark:border-red-900":o==="ABSORBED"?"bg-[#2e7d32] text-white border-[#2e7d32] dark:bg-green-900/80 dark:border-green-900":"bg-gray-100 text-gray-600 border-gray-200 dark:text-gray-300"}function we(e,o=0){e={...e,id:e?.id||e?.gip_id||"N/A",name:e?.name||"N/A",office:e?.office||"N/A",remarks:e?.remarks||"N/A",designation:e?.designation||"N/A",designatedBeneficiary:e?.designatedBeneficiary||"N/A",relationshipToAssured:e?.relationshipToAssured||"N/A"};const n=window.innerWidth<640?"top":"top-start";let a=o;const r=e.arLogs||[],t=e.dtrLogs||[],u=e.docs||[],s=["GIP FORM","BIRTH CERTIFICATE","DIPLOMA","TOR","VALID ID"],f=s.map(l=>{const v=u.find(E=>E.name.toUpperCase()===l.toUpperCase());return v||{name:l,status:"PENDING",id:null}});u.forEach(l=>{s.some(E=>E.toUpperCase()===l.name.toUpperCase())||f.push(l)});const x=`
...
<div id="personal-profile-section" class="transition-all duration-300">
    <div class="flex flex-col gap-4 text-sm mt-3 px-1 mb-8">
        ...
        <div class="flex justify-between items-center group">
            ...
            <span class="${e.age||Ae(e.birthday)?"font-black text-heading":"font-bold text-gray-300 italic"} text-right">${e.age||Ae(e.birthday)||"N/A"}</span>
        </div>
        <div class="flex justify-between items-center group">
...
<div class="relative">
    ...
    <div id="drawer-page-1" class="hidden flex-1 flex flex-col gap-6">
         ...
         <div class="flex flex-col gap-5">
            <div>
                ...
                <div class="space-y-2">
                    ${t.length?t.map(l=>{const v=l.status||"PENDING";let E=v==="VERIFIED"||v==="COMPLETED"?"text-green-500":v==="REJECTED"||v==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500",b=l.date||l.createdAt,D=b;if(b){const $=/^\d{4}-\d{2}-\d{2}$/.test(b)?new Date(b+"T00:00:00Z"):new Date(b);isNaN($)||(D=$.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric",timeZone:"Asia/Manila"}).toUpperCase())}return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-blue-100 dark:border-blue-900/50 bg-blue-50/50 dark:bg-blue-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors edit-log-btn" data-type="dtr" data-id="${l.id}" data-val="${l.day||b}" data-status="${v}">
                            <span class="text-xs font-black text-royal-blue dark:text-blue-400 capitalize whitespace-nowrap pointer-events-none">${l.day||D}</span>
                            <span class="text-[11px] font-bold ${E} uppercase tracking-widest truncate max-w-[50%] text-right pr-6 group-hover:pr-12 pointer-events-none transition-all">${v}</span>
            ...
            <div>
                ...
                <div class="space-y-2">
                    ${r.length?r.map(l=>{const v=l.status||"PENDING";let E=v==="VERIFIED"||v==="COMPLETED"?"text-green-500":v==="REJECTED"||v==="DECLINED"?"text-red-500":"text-gray-400 dark:text-gray-500",b=l.period||l.createdAt,D=b;if(b){const $=/^\d{4}-\d{2}-\d{2}$/.test(b)?new Date(b+"T00:00:00Z"):new Date(b);isNaN($)||(D=$.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric",timeZone:"Asia/Manila"}).toUpperCase())}return`
                        <div class="flex justify-between items-center p-3 rounded-xl border border-orange-100 dark:border-orange-900/50 bg-orange-50/50 dark:bg-orange-900/10 shadow-sm relative group overflow-hidden cursor-pointer hover:bg-orange-100 dark:hover:bg-orange-900/40 transition-colors edit-log-btn" data-type="ar" data-id="${l.id}" data-val="${b}" data-status="${v}">
                            <span class="text-xs font-black text-orange-600 dark:text-orange-400 capitalize whitespace-nowrap pointer-events-none">${b||D}</span>
                            <span class="text-[11px] font-bold ${E} uppercase tracking-widest truncate max-w-[50%] text-right pr-6 group-hover:pr-12 pointer-events-none transition-all">${v}</span>
    ...
    <div id="drawer-page-2" class="hidden flex-1 flex flex-col gap-4">
        ...
        <div class="flex flex-col gap-3">
            ${f.map(l=>{const v=l.status.toUpperCase(),b={VERIFIED:"COMPLETED",DECLINED:"REJECTED",PENDING:"PENDING"}[v]||v,D=b==="COMPLETED",$=b==="REJECTED",T=D?"text-green-500":$?"text-red-500":"text-gray-400 dark:text-gray-500",K=D?"bg-green-50/50 dark:bg-green-900/10":$?"bg-red-50/50 dark:bg-red-900/10":"bg-gray-50/50 dark:bg-slate-800/50",ee=D?"bg-green-100 text-green-700 border-green-200 dark:bg-green-900/40 dark:text-green-300 dark:border-green-800 hover:bg-green-200 cursor-pointer":$?"bg-red-100 text-red-700 border-red-200 dark:bg-red-900/40 dark:text-red-300 dark:border-red-800 hover:bg-red-200 cursor-pointer":"bg-white text-gray-500 border-gray-200 dark:bg-slate-700 dark:text-gray-400 dark:border-slate-600 hover:bg-gray-50 dark:hover:bg-slate-600 cursor-pointer";let P='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>';return D?P='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>':$&&(P='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>'),`
                <div class="flex items-center justify-between p-3.5 rounded-xl border border-gray-100 dark:border-slate-700 shadow-sm transition-all hover:-translate-y-0.5 ${K}">
                    <div class="flex items-center gap-3 w-full">
                        ...
                        <span class="text-xs sm:text-sm font-black ${D?"text-heading":"text-gray-500 dark:text-gray-400"} uppercase tracking-tight flex-1">${l.name}</span>
                    </div>
                    <button type="button" class="ml-3 ${ee} text-[10px] font-black px-3 py-1.5 rounded-full border uppercase tracking-widest transition-colors flex-shrink-0 drawer-doc-btn" data-id="${l.id}" data-name="${l.name}" data-status="${b} cursosr-pointer">
                        ${b}
                    </button>
                </div>
                `}).join("")}
...
<style>
    ...
    `;let i=document.getElementById("beneficiary-drawer-container");i&&(i.remove(),document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden")),i=document.createElement("div"),i.id="beneficiary-drawer-container",i.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-neutral-primary-soft dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl",i.setAttribute("tabindex","-1"),i.setAttribute("data-drawer-backdrop","true"),i.innerHTML=x,document.body.appendChild(i),document.documentElement.classList.add("overflow-hidden"),document.body.classList.add("overflow-hidden"),Oe(async()=>{const{Drawer:l}=await import("./vendor-flowbite-B7rSfpuT.js").then(v=>v.b);return{Drawer:l}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:l})=>{const v={placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden"),setTimeout(()=>{i&&i.parentNode&&i.remove()},300)}},E=new l(i,v);E.show(),i.querySelector("#close-drawer-btn").addEventListener("click",()=>E.hide());const D=i.querySelector("#drawer-prev-btn"),$=i.querySelector("#drawer-next-btn"),T=2,K=()=>{i.querySelectorAll("[id^=drawer-page-]").forEach((I,j)=>{I.classList.toggle("hidden",j!==a)});const M=i.querySelector("#drawer-section-title");if(M){const I=["Personal Profile","Submission Logs","Required Documents"];M.textContent=I[a]||"Details"}const N=i.querySelector("#personal-profile-section");N&&N.classList.toggle("hidden",a!==0),D.disabled=a===0,$.disabled=a===T,D.classList.toggle("opacity-50",a===0),$.classList.toggle("opacity-50",a===T)};D.addEventListener("click",()=>{a>0&&a--,K()}),$.addEventListener("click",()=>{a<T&&a++,K()}),K(),i.querySelectorAll(".drawer-doc-btn").forEach(M=>{M.addEventListener("click",async()=>{const N=M.dataset.name,I=M.dataset.status,j="flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[9px] gap-2 transition-all duration-300 ",_=await C.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Update Document</span>',html:`
                        <div class="font-montserrat text-left">
                            ...
                            <div class="grid grid-cols-3 gap-3">
                                ...
                                <label class="relative block cursor-pointer">
                                    ...
                                    <div class="${j} border-gray-100 bg-gray-50 text-gray-400 dark:border-slate-700 dark:bg-slate-800/50 dark:text-gray-500 peer-checked:border-red-500 peer-checked:bg-red-50 peer-checked:text-red-600 dark:peer-checked:bg-red-900/20 dark:peer-checked:text-red-400 hover:bg-red-50/50 dark:hover:bg-slate-700">
                    ...
                    `,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Update Status</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Cancel</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2"},buttonsStyling:!1,preConfirm:()=>{const g=document.querySelector('input[name="swal-doc-status"]:checked');return g?g.value:null}});if(_.isConfirmed){const g=_.value;if(g===I)return;try{const m={COMPLETED:"VERIFIED",REJECTED:"DECLINED",PENDING:"PENDING"}[g]||g,S=await Se("api/logs.php?type=docs",{gip_id:e.id,doc_name:N,status:m}),y=S.success?S.data:{success:!1,error:S.error};y.success?(C.fire({toast:!0,position:n,icon:"success",title:"Status updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):C.fire("Error",y.error||"Failed to update","error")}catch(h){C.fire("Error",h.message,"error")}}})});const ee=new Set(["2026-01-01","2026-04-02","2026-04-03","2026-04-09","2026-05-01","2026-06-12","2026-08-24","2026-08-31","2026-11-01","2026-11-30","2026-12-25","2026-12-30","2026-12-31"]),P=["JAN","FEB","MAR","APR","MAY","JUN","JUL","AUG","SEP","OCT","NOV","DEC"];function V(M){const I=new Date(M+"T00:00:00").getDay();return I!==0&&I!==6&&!ee.has(M)}function ie(M){const N=M.getDate(),I=P[M.getMonth()],j=M.getFullYear(),_=new Date(j,M.getMonth()+1,0).getDate();return N<=15?`${I} 1-15, ${j}`:`${I} 16-${_}, ${j}`}const le=()=>{const M=new Date;if(!t.length)return ie(M);let N=-1,I="";const j=S=>{const y=(S||"").toUpperCase().match(/([A-Z]{3})\s+(\d+)-(\d+),\s*(\d{4})/);if(!y)return-1;const L=P.indexOf(y[1]),z=parseInt(y[2])===1?0:1;return parseInt(y[4])*100+L*2+z};if(t.forEach(S=>{const y=S.day||S.date||"",L=j(y);L>N&&(N=L,I=y)}),N===-1)return ie(M);const _=I.toUpperCase().match(/([A-Z]{3})\s+(\d+)-(\d+),\s*(\d{4})/),g=P.indexOf(_[1]),h=parseInt(_[2]),m=parseInt(_[4]);if(h===1){const S=new Date(m,g+1,0).getDate();return`${P[g]} 16-${S}, ${m}`}else{const S=(g+1)%12,y=g===11?m+1:m;return`${P[S]} 1-15, ${y}`}},se=()=>{const M=new Date;if(!r.length)return ie(M);let N=-1,I="";const j=S=>{const y=(S||"").toUpperCase().match(/([A-Z]{3})\s+(\d+)-(\d+),\s*(\d{4})/);if(!y)return-1;const L=P.indexOf(y[1]),z=parseInt(y[2])===1?0:1;return parseInt(y[4])*100+L*2+z};if(r.forEach(S=>{const y=j(S.period);y>N&&(N=y,I=S.period)}),N===-1)return ie(M);const _=I.toUpperCase().match(/([A-Z]{3})\s+(\d+)-(\d+),\s*(\d{4})/),g=P.indexOf(_[1]),h=parseInt(_[2]),m=parseInt(_[4]);if(h===1){const S=new Date(m,g+1,0).getDate();return`${P[g]} 16-${S}, ${m}`}else{const S=(g+1)%12,y=g===11?m+1:m;return`${P[S]} 1-15, ${y}`}},Y=async(M,N)=>{C.fire({title:"Adding...",allowOutsideClick:!1,showConfirmButton:!1}),C.showLoading();try{const I={gip_id:e.id};if(M==="dtr"){const g=N.toUpperCase().match(/([A-Z]{3})\s+(\d+)-\d+,\s*(\d{4})/);if(g){const h=P.indexOf(g[1]),m=parseInt(g[2]),S=parseInt(g[3]);let y=new Date(S,h,m);for(;!V(y.toISOString().split("T")[0]);)y.setDate(y.getDate()+1);I.record_date=y.toISOString().split("T")[0]}else I.record_date=new Date().toISOString().split("T")[0];I.weekday=N}M==="ar"&&(I.period=N);const j=await Se(`api/logs.php?type=${M}`,I);(j.success?j.data:{success:!1,error:j.error}).success?(C.fire({toast:!0,position:n,icon:"success",title:"Auto-Added!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):C.fire("Error","Failed to add log.","error")}catch(I){C.fire("Error",I.message,"error")}},de=async(M,N,I,j,_)=>{const g=N==="dtr"?"Record Date":"Period";ae();const h="flex flex-col items-center justify-center p-3 rounded-xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[10px] sm:text-xs gap-2 ",m='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path></svg>',S='<svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M6 18L18 6M6 6l12 12"></path></svg>',{value:y}=await C.fire({title:`<span class="text-xl font-black text-heading uppercase tracking-tight">Edit ${M} Log</span>`,html:`
                    <div class="flex flex-col gap-5 text-left font-montserrat mt-2">
                        <div>
                            <label class="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-1">${g}</label>
                            <input id="swal-log-val" value="${j}" class="bg-gray-50 dark:bg-slate-800/80 border border-gray-200 dark:border-slate-600 text-heading text-sm rounded-xl focus:ring-brand focus:border-brand block w-full p-2.5 font-bold uppercase transition-all" placeholder="Enter value">
                        </div>
                        <div>
                            <label class="text-[10px] font-bold text-gray-500 uppercase tracking-widest block mb-2">Validation Status</label>
                            <div class="grid grid-cols-2 gap-3">
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-log-status" value="VERIFIED" class="peer sr-only" ${_==="VERIFIED"?"checked":""}>
                                ...
                                <label class="relative block cursor-pointer">
                                    <input type="radio" name="swal-log-status" value="REJECTED" class="peer sr-only" ${_==="REJECTED"||_==="DECLINED"?"checked":""}>
                ...
                `,focusConfirm:!1,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">SAVE REVISIONS</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">CANCEL</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-brand text-white hover:bg-brand-strong text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2"},buttonsStyling:!1,preConfirm:()=>{const L=document.querySelector('input[name="swal-log-status"]:checked');return{val:document.getElementById("swal-log-val").value.trim().toUpperCase(),status:L?L.value:"PENDING"}}});if(y&&(y.val!==j||y.status!==_))try{const L={type:N,id:I,status:y.status};if(N==="dtr"){const p=y.val.toUpperCase().match(/([A-Z]{3})\s+(\d+)-\d+,\s*(\d{4})/);if(p){const c=P.indexOf(p[1]),k=parseInt(p[2]),w=parseInt(p[3]);let A=new Date(w,c,k);for(;!V(A.toISOString().split("T")[0]);)A.setDate(A.getDate()+1);L.record_date=A.toISOString().split("T")[0]}else L.record_date=new Date().toISOString().split("T")[0];L.weekday=y.val}N==="ar"&&(L.period=y.val);const z=await Ge("api/logs.php",L),d=z.success?z.data:{success:!1,error:z.error};d.success?(C.fire({toast:!0,position:n,icon:"success",title:"Log Updated!",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):C.fire("Error",d.error||"Failed to update log.","error")}catch(L){C.fire("Error",L.message,"error")}},Z=i.querySelector("#add-dtr-log-btn");Z&&Z.addEventListener("click",()=>Y("dtr",le()));const ce=i.querySelector("#add-ar-log-btn");ce&&ce.addEventListener("click",()=>Y("ar",se()));const pe=i.querySelector("#export-log-btn");pe&&pe.addEventListener("click",async()=>{const M="flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all cursor-pointer font-black uppercase tracking-widest text-[11px] gap-2 ",N=await C.fire({title:'<span class="text-xl font-black text-heading uppercase tracking-tight">Export DTR/AR',html:`
                        <div class="font-montserrat text-left">
                    ...
                    `,showCancelButton:!0,confirmButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Generate Export</span>',cancelButtonText:'<span class="font-black uppercase tracking-widest cursor-pointer">Cancel</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100 dark:border-slate-800 dark:bg-slate-900",confirmButton:"bg-emerald-600 text-white hover:bg-emerald-700 text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600 text-xs px-6 py-2.5 rounded-xl border border-gray-200 dark:border-slate-600 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1,preConfirm:()=>{const I=document.querySelector('input[name="swal-export-type"]:checked');return I?I.value:null}});if(N.isConfirmed&&N.value){const I=N.value,j=new Date().getFullYear();await Ne([e],I,j)}}),i.querySelectorAll(".edit-log-btn").forEach(M=>{M.addEventListener("click",N=>{if(N.target.closest(".delete-log-btn"))return;const I=M.dataset.type,j=M.dataset.id,_=M.dataset.val,g=M.dataset.status;de(I.toUpperCase(),I,j,_,g)})}),i.querySelectorAll(".delete-log-btn").forEach(M=>{M.addEventListener("click",async()=>{const N=M.dataset.id,I=M.dataset.type;if((await C.fire({title:'<span class="text-xl font-black text-philippine-red uppercase tracking-tight">Delete item?</span>',text:"This action cannot be undone.",icon:"warning",showCancelButton:!0,confirmButtonText:'<span class="font-black tracking-widest uppercase">Delete</span>',cancelButtonText:'<span class="font-black tracking-widest uppercase">Wait</span>',customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl border border-gray-100",confirmButton:"bg-philippine-red text-white hover:bg-red-700 text-xs px-6 py-2.5 rounded-xl border border-transparent shadow-sm mx-2 cursor-pointer",cancelButton:"bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs px-6 py-2.5 rounded-xl border border-gray-200 shadow-sm mx-2 cursor-pointer"},buttonsStyling:!1})).isConfirmed)try{const _=await Se(`api/logs.php?type=${I}`,{log_id:N,action:"delete"});(_.success?_.data:{success:!1,error:_.error}).success?(C.fire({toast:!0,position:n,icon:"success",title:"Deleted",showConfirmButton:!1,timer:1500}),window.viewBeneficiary&&window.viewBeneficiary(e,a)):C.fire("Error","Failed to delete data.","error")}catch(_){C.fire("Error",_.message,"error")}})})}).catch(console.error)}function at(e){const o=ae(),n=`w-full bg-transparent border-b-2 ${o?"border-slate-700 text-white focus:border-brand placeholder-slate-600":"border-gray-200 text-gray-900 focus:border-brand placeholder-gray-300"} px-1 py-1 text-sm font-black outline-none transition-all focus:ring-0`,a=`w-full bg-transparent border-none ${o?"text-white":"text-royal-blue"} px-0 py-0 text-xl sm:text-2xl font-black leading-tight tracking-tight focus:ring-0 outline-none placeholder-gray-300 resize-none overflow-hidden`;function r(i){if(!i)return"";const l=new Date(i),v=new Date;let E=v.getFullYear()-l.getFullYear();const b=v.getMonth()-l.getMonth();return(b<0||b===0&&v.getDate()<l.getDate())&&E--,E>=0?E:0}function t(i){if(!i)return"bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-white border-gray-200 dark:border-slate-700";const l=String(i).toUpperCase();return l==="ONGOING"?"bg-green-100 text-green-700 border-green-200":l==="EXPIRED"?"bg-red-400 text-white border-red-400":l==="RESIGNED"?"bg-[#ce1126] text-white border-[#ce1126]":l==="ABSORBED"?"bg-[#2e7d32] text-white border-[#2e7d32]":"bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-white border-gray-200 dark:border-slate-700"}const u=`
<form id="edit-beneficiary-drawer-form" class="h-full flex flex-col pt-4 font-montserrat relative pb-20 overflow-y-auto">
    ...
    <div class="flex flex-col gap-4 text-sm mt-3 px-1 pb-24">
        ...
        <div class="flex justify-between items-center group">
            ...
            <input type="text" name="age" id="edit-age-display" value="${e.age||r(e.birthday)||""}" class="${n} text-right max-w-[80px]" placeholder="Auto">
        </div>
        ...
        <div class="flex flex-col gap-2 pt-3 mt-1 border-t border-gray-50 dark:border-slate-800/60">
            ...
            <div id="edit-education-suggestions-box" class="hidden mt-2 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                ${Ce.map(i=>`<button type="button" class="edit-education-option w-full text-left px-3 py-2 text-[11px] font-bold text-slate-700 dark:text-slate-200 hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer"><span class="option-text">${i.name}</span></button>`).join("")}
            </div>
        </div>
...
.pulse-highlight {
    ...
    `;let s=document.getElementById("edit-drawer-container");s&&(s.remove(),document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden")),s=document.createElement("div"),s.id="edit-drawer-container",s.className="fixed top-0 right-0 z-[100] h-screen p-4 sm:p-6 overflow-y-auto transition-transform duration-500 ease-in-out translate-x-full bg-white dark:bg-slate-900 w-full sm:w-[550px] lg:w-[650px] shadow-2xl pb-0",s.setAttribute("tabindex","-1"),s.innerHTML=u,document.body.appendChild(s),document.documentElement.classList.add("overflow-hidden"),document.body.classList.add("overflow-hidden"),setTimeout(()=>{const i=s.querySelector('textarea[name="name"]');i&&(i.style.height="auto",i.style.height=i.scrollHeight+"px")},10);const f=s.querySelector("#edit-education-suggestions-box");f&&(f.innerHTML=Ce.map(i=>`
        ...
        `).join(""));const x=s.querySelector("#edit-designation-suggestions-box");x&&(x.innerHTML=je.map(i=>`
        ...
        `).join("")),Oe(async()=>{const{Drawer:i}=await import("./vendor-flowbite-B7rSfpuT.js").then(l=>l.b);return{Drawer:i}},__vite__mapDeps([0,1]),import.meta.url).then(({Drawer:i})=>{const l=new i(s,{placement:"right",backdrop:!0,bodyScrolling:!1,edge:!1,edgeOffset:"",backdropClasses:"bg-gray-900/50 dark:bg-gray-900/80 fixed inset-0 z-50",onHide:()=>{document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden"),setTimeout(()=>{s&&s.parentNode&&s.remove()},400)}});l.show(),window.initFlowbite&&window.initFlowbite();const v=()=>{document.activeElement&&typeof document.activeElement.blur=="function"&&document.activeElement.blur(),l.hide()};s.querySelector("#close-edit-drawer-btn").addEventListener("click",v),s.querySelector("#edit-drawer-cancel-btn").addEventListener("click",v);const E=s.querySelector("#edit-beneficiary-drawer-form"),b=s.querySelector("#edit-bday-input"),D=s.querySelector("#edit-age-display"),$=s.querySelector("#edit-startDate-input"),T=s.querySelector("#edit-endDate-input"),K=s.querySelector('input[name="seriesNo"]'),ee=s.querySelector('input[name="gip_id"]'),P=s.querySelector("#edit-drawer-remarks"),V=s.querySelector("#edit-extension-log-container"),ie=()=>{if(!V)return;const g=P.value,h=ae();if(g==="ABSORBED"){const m=e.absorbDate&&!String(e.absorbDate).includes("0000-00-00")?new Date(e.absorbDate):new Date,S=m.getTimezoneOffset()*6e4,y=new Date(m.getTime()-S).toISOString().slice(0,16);V.innerHTML=`
                    <div class="mt-4 pt-4 border-t ${h?"border-slate-800":"border-gray-100"}">
                        <p class="text-[9px] uppercase font-black ${h?"text-green-500":"text-[#2e7d32]"} border-b ${h?"border-slate-800":"border-green-100"} pb-1 flex items-center gap-2 mb-3"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg> Absorption Details</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                            <div class="group">
                                <label class="text-[9px] ${h?"text-green-500":"text-[#2e7d32]"} font-black uppercase block mb-1 cursor-pointer" for="absorbDateInput">Absorption Date</label>
                                <input type="datetime-local" id="absorbDateInput" name="absorbDate" value="${y}" class="w-full ${h?"bg-slate-800 text-white border-slate-700":"bg-green-50 text-slate-900 border-green-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm font-mono cursor-pointer">
                            </div>
                            <div class="group">
                                <label class="text-[9px] ${h?"text-slate-500":"text-gray-400"} font-black uppercase block mb-1">Where?</label>
                                <input type="text" name="absorb_where" value="${e.absorb_where||""}" class="w-full ${h?"bg-slate-800 text-white border-slate-700":"bg-gray-50 text-slate-900 border-gray-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm" placeholder="Where to absorb?">
                            </div>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                            <div class="group">
                                <label class="text-[9px] ${h?"text-slate-500":"text-gray-400"} font-black uppercase block mb-1">Position</label>
                                <input type="text" name="absorb_position" value="${e.absorb_position||""}" class="w-full ${h?"bg-slate-800 text-white border-slate-700":"bg-gray-50 text-slate-900 border-gray-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm" placeholder="What kind of position?">
                            </div>
                            <div class="group">
                                <label class="text-[9px] ${h?"text-slate-500":"text-gray-400"} font-black uppercase block mb-1">Agency</label>
                                <input type="text" name="absorb_agency" value="${e.absorb_agency||""}" class="w-full ${h?"bg-slate-800 text-white border-slate-700":"bg-gray-50 text-slate-900 border-gray-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm" placeholder="On what agency?">
                            </div>
                        </div>
                    </div>
                `}else if(g==="RESIGNED"){const m=e.resignedDate&&!String(e.resignedDate).includes("0000-00-00")?new Date(e.resignedDate):new Date,S=m.getTimezoneOffset()*6e4,y=new Date(m.getTime()-S).toISOString().slice(0,16);V.innerHTML=`
                    <div class="mt-4 pt-4 border-t ${h?"border-slate-800":"border-gray-100"}">
                        <p class="text-[9px] uppercase font-black ${h?"text-red-500":"text-[#ce1126]"} border-b ${h?"border-slate-800":"border-red-100"} pb-1 flex items-center gap-2 mb-3"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" /></svg> Resignation Details</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
                            <div class="group">
                                <label class="text-[9px] ${h?"text-red-500":"text-[#ce1126]"} font-black uppercase block mb-1 cursor-pointer" for="resignedDateInput">Resignation Date</label>
                                <input type="datetime-local" id="resignedDateInput" name="resignedDate" value="${y}" class="w-full ${h?"bg-slate-800 text-white border-slate-700":"bg-red-50 text-slate-900 border-red-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm font-mono cursor-pointer">
                            </div>
                            <div class="group">
                                <label class="text-[9px] ${h?"text-slate-500":"text-gray-400"} font-black uppercase block mb-1">Reason (Optional)</label>
                                <input type="text" name="resigned_reason" value="${e.resigned_reason||""}" class="w-full ${h?"bg-slate-800 text-white border-slate-700":"bg-gray-50 text-slate-900 border-gray-200"} rounded-lg px-3 py-2 text-[11px] font-bold focus:ring-2 focus:ring-brand outline-none transition-all shadow-sm" placeholder="Why resigned?">
                            </div>
                        </div>
                    </div>
                `}};P&&P.addEventListener("change",g=>{const h="text-[10px] sm:text-[11px] font-black px-2.5 py-2.5 rounded-lg border uppercase tracking-widest shadow-sm outline-none focus:ring-2 focus:ring-brand w-full cursor-pointer transition-colors duration-300 h-[42px]";P.className=`${t(g.target.value)} ${h} editable-indicator`,ie(),(P.value==="ABSORBED"||P.value==="RESIGNED")&&setTimeout(()=>{V.scrollIntoView({behavior:"smooth",block:"center"}),V.classList.add("pulse-highlight"),setTimeout(()=>V.classList.remove("pulse-highlight"),1500)},50)}),ie();let le=!1,se=!1;const Y=(g,h)=>{g.addEventListener("input",m=>{const S=m.target.value,y=window.__maskDate(S);if(S!==y&&(m.target.value=y),y.length===10){const L=window.__parseFormattedDate(y);L&&h&&(se||h(L),document.activeElement===g&&g.blur())}}),g.addEventListener("changeDate",m=>{m.detail&&m.detail.date&&h&&(se||h(m.detail.date),g._datepicker&&g._datepicker.hide())})};b&&Y(b,g=>{D&&(!le||!D.value)&&(D.value=window.calculateAge(g))}),$&&Y($,g=>{if(T){const m=new Date(g);m.setDate(m.getDate()+182);const S=String(m.getMonth()+1).padStart(2,"0"),y=String(m.getDate()).padStart(2,"0"),L=m.getFullYear();T.value=`${S}/${y}/${L}`}const h=g.getFullYear();h>1900&&ee&&K&&Promise.all([W(`api/beneficiaries.php?next_id&year=${encodeURIComponent(h)}`),W(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(h)}`)]).then(([m,S])=>{const y=m.success&&m.data?.success?m.data.nextId:null,L=S.success&&S.data?.success?S.data.nextSeries:null,z=String(ee.value||"").match(/^ROX-RD-ESIG-(\d{4})-\d{4}$/),d=String(K.value||"").match(/^(\d{4})-\d{2}-\d{3}$/),p=z?Number(z[1]):null,c=d?Number(d[1]):null;y&&(p===null||p!==h)&&(ee.value=y),L&&(c===null||c!==h)&&(K.value=L)}).catch(m=>{console.error("Edit drawer identifier sync error:",m)})}),T&&Y(T);const de=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null),Z=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null);de&&b&&(b._datepicker=new de(b,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"}));const ce=s.querySelector("#edit-date-range-picker");if(Z&&ce){const g=new Z(ce,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0,orientation:"auto"});$&&($._datepicker=g.datepickers[0]),T&&(T._datepicker=g.datepickers[1])}e.id&&(se=!0,W(`api/beneficiaries.php?id=${encodeURIComponent(e.id)}`).then(g=>{if(g.success&&g.data&&g.data.beneficiary){const h=g.data.beneficiary;if($&&h.startDate){const m=new Date(h.startDate);isNaN(m)||($.value=h.startDateFormatted||"",$._datepicker&&$._datepicker.setDate(m))}if(T&&h.endDate){const m=new Date(h.endDate);isNaN(m)||(T.value=h.endDateFormatted||"",T._datepicker&&T._datepicker.setDate(m))}}setTimeout(()=>{se=!1},100)}).catch(g=>{console.error("Error fetching accurate beneficiary dates:",g),se=!1})),D&&D.addEventListener("input",()=>le=!0);const pe=(g,h,m)=>{const S=s.querySelector(g),y=s.querySelector(h);if(!S||!y)return;const L=()=>y.classList.add("hidden"),z=()=>y.classList.remove("hidden");S.addEventListener("focus",z),S.addEventListener("input",()=>{const d=S.value.toLowerCase().trim();let p=0;y.querySelectorAll(m).forEach(c=>{const w=(c.querySelector(".option-text")?.textContent||c.textContent||"").toLowerCase().includes(d);c.style.display=w?"block":"none",w&&p++}),p>0?z():L()}),y.addEventListener("click",d=>{const p=d.target.closest(m);p&&(S.value=(p.querySelector(".option-text")?.textContent||p.textContent||"").trim(),L(),S.dispatchEvent(new Event("change")))}),document.addEventListener("click",d=>{!S.contains(d.target)&&!y.contains(d.target)&&L()})};pe("#edit-education-input","#edit-education-suggestions-box",".edit-education-option"),pe("#edit-designation-input","#edit-designation-suggestions-box",".edit-designation-option"),(()=>{const g=s.querySelector("#edit-office-input"),h=s.querySelector("#edit-office-suggestions-box");if(!g||!h)return;h.classList.add("mt-[52px]");let m="OFFICES",S=null,y=[];const L={textLabel:o?"text-slate-400":"text-slate-500",borderDivide:o?"border-slate-800":"border-slate-100",courseHover:o?"hover:bg-slate-800/80":"hover:bg-blue-50",textCourseOpt:o?"text-slate-300":"text-slate-700"},z=async()=>{const p="dole_offices_cache",c=async()=>{let w=[];try{const A=await W("api/beneficiaries.php?get_offices_advanced=1");A.success&&A.data?.success&&Array.isArray(A.data.offices)&&(w=A.data.offices)}catch(A){console.error("Office fetch failed:",A)}return w.length>0&&(y=w,localStorage.setItem(p,JSON.stringify({data:w,timestamp:Date.now()}))),w},k=localStorage.getItem(p);if(k)try{const{data:w,timestamp:A}=JSON.parse(k);return y=w,Date.now()-A>1800*1e3&&c().then(()=>{m==="OFFICES"&&d("OFFICES",S,g.value)}),w}catch{localStorage.removeItem(p)}return y.length===0?await c():y},d=async(p="OFFICES",c=null,k="")=>{if(m=p,S=c,p==="OFFICES"){const A=(await z()).filter(B=>B.office.toLowerCase().includes(k.toLowerCase()));h.innerHTML=`
                        <div class="px-2.5 py-1.5 text-[7px] font-black uppercase tracking-widest ${L.textLabel} opacity-70 border-b ${L.borderDivide} mb-1">OFFICE CODE</div>
                        <div class="max-h-64 overflow-y-auto scrollbar-hide">
                            ${A.length>0?A.map(B=>{const F=parseInt(B.location_count||0)>0;return`
                                    <div class="office-code-option group/opt px-3 py-2 text-[9px] font-bold ${L.textCourseOpt} ${L.courseHover} rounded-lg ${F?"cursor-pointer":"cursor-default opacity-60"} transition-all flex items-center justify-between group active:scale-[0.98] mx-1 mb-0.5" 
                                        data-id="${B.id}" data-name="${B.office}" data-has-locations="${F}">
                                        <div class="flex items-center gap-2.5">
                                            ...
                                            <span class="option-text">${B.office}</span>
                                        </div>
                                        ${F?'<svg class="w-3 h-3 text-slate-300 group-hover/opt:text-blue-500 group-hover/opt:translate-x-0.5 transition-all" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M9 5l7 7-7 7"/></svg>':""}
                                    </div>
                                `}).join(""):`<div class="px-3 py-6 text-center text-[9px] font-bold ${L.textLabel} opacity-60">No matching offices.</div>`}
                        </div>
                    `,h.querySelectorAll(".office-code-option").forEach(B=>{B.addEventListener("click",F=>{F.stopPropagation(),B.dataset.hasLocations==="true"?d("LOCATIONS",{id:B.dataset.id,name:B.dataset.name}):(g.value=B.dataset.name,h.classList.add("hidden"),g.dispatchEvent(new Event("change")))})})}else{h.innerHTML=`
                        <div class="flex items-center justify-between px-3 py-2 border-b ${L.borderDivide} bg-slate-50/95 dark:bg-slate-900 sticky top-0 backdrop-blur-sm z-10 rounded-t-xl">
                            <div class="flex items-center gap-2">
                                ...
                                <div class="text-[7px] font-black uppercase tracking-widest ${L.textLabel} opacity-70">OFFICE LOCATION</div>
                            </div>
                        ...
                        <div class="p-2 border-b ${L.borderDivide}">
                            <div class="relative group">
                                ...
                                <input type="text" id="location-search-edit" placeholder="Search in ${c.name}..." 
                        ...
                        <div id="loc-list-edit" class="max-h-64 overflow-y-auto scrollbar-hide p-1">
                            <div class="px-3 py-4 text-center text-[9px] font-bold ${L.textLabel} animate-pulse">Fetching...</div>
                        </div>
                    `;const w=h.querySelector("#loc-list-edit"),A=h.querySelector("#location-search-edit"),B=`dole_locs_cache_${c.id}`;let F=[];const U=localStorage.getItem(B);if(U)try{const{data:R,timestamp:O}=JSON.parse(U);F=R,Date.now()-O<3600*1e3}catch{localStorage.removeItem(B)}const J=async()=>{let R=[];if(ge&&$e()){const{data:O,error:H}=await ge.from("office_locations").select("location").eq("office_id",c.id).order("location");!H&&O&&(R=O)}if(R.length===0)try{const O=await W(`api/beneficiaries.php?get_office_locations=1&office_id=${c.id}`);O.success&&O.data?.success&&Array.isArray(O.data.locations)&&(R=O.data.locations)}catch(O){console.error("Office locations fetch failed:",O)}R.length>0&&(F=R,localStorage.setItem(B,JSON.stringify({data:R,timestamp:Date.now()})),q(A.value))},q=(R="")=>{const O=F.filter(H=>H.location.toLowerCase().includes(R.toLowerCase()));w.innerHTML=O.length>0?O.map(H=>`
                            <div class="location-option-edit group/loc px-3 py-1.5 text-[9px] font-bold ${L.textCourseOpt} ${L.courseHover} rounded-lg cursor-pointer transition-all flex items-center gap-3 active:scale-[0.98] mb-0.5" data-location="${H.location}">
                                <div class="w-1 h-1 rounded-full bg-slate-300 dark:bg-slate-600 group-hover/loc:bg-blue-500 transition-all"></div>
                                <span class="option-text truncate">${H.location}</span>
                            </div>
                        `).join(""):F.length===0?`<div class="px-3 py-4 text-center text-[9px] font-bold ${L.textLabel} animate-pulse">Fetching...</div>`:`<div class="px-3 py-8 text-center text-[9px] font-bold ${L.textLabel} opacity-60">No matching locations.</div>`,w.querySelectorAll(".location-option-edit").forEach(H=>{H.addEventListener("click",()=>{g.value=`${c.name} - ${H.dataset.location}`,h.classList.add("hidden"),g.dispatchEvent(new Event("change"))})})};q(A.value),J(),setTimeout(()=>A.focus(),50),A.addEventListener("input",()=>q(A.value)),A.addEventListener("click",R=>R.stopPropagation()),h.querySelector("#back-to-offices-edit").addEventListener("click",R=>{R.stopPropagation(),d("OFFICES")})}};g.addEventListener("focus",()=>{h.classList.remove("hidden"),d(m,S,g.value)}),g.addEventListener("input",()=>{m==="OFFICES"&&d("OFFICES",null,g.value)}),document.addEventListener("click",p=>{!g.contains(p.target)&&!h.contains(p.target)&&h.classList.add("hidden")})})();const N=s.querySelector("#edit-replacement-input"),I=s.querySelector("#edit-replacement-suggestions-box"),j=s.querySelector("#edit-replacement-loading");let _=null;N&&I&&(N.addEventListener("input",g=>{const h=g.target.value.trim();clearTimeout(_),I.classList.add("hidden"),!(h.length<2)&&(j&&j.classList.remove("hidden"),_=setTimeout(async()=>{try{const m=await xe(`api/beneficiaries.php?replacement_candidates=1&q=${encodeURIComponent(h)}&limit=10`);m.success&&m.data&&m.data.candidates&&m.data.candidates.length>0?(I.innerHTML=m.data.candidates.map(S=>`
                                <button type="button" class="w-full text-left px-4 py-3 border-b border-gray-100 dark:border-slate-700 hover:bg-brand-50 dark:hover:bg-slate-700/50 flex flex-col gap-1 group/cand transition-colors" data-name="${S.name}">
                                    <span class="text-xs font-black text-gray-900 dark:text-white group-hover/cand:text-brand pointer-events-none">${S.name}</span>
                                    <span class="text-[10px] font-bold text-gray-500 uppercase tracking-widest pointer-events-none">${S.id}</span>
                                </button>
                            `).join(""),I.classList.remove("hidden")):(I.innerHTML='<div class="px-4 py-3 text-xs text-gray-500 italic text-center font-medium">No active beneficiaries found.</div>',I.classList.remove("hidden"))}catch(m){console.error("Replacement fetch error:",m)}finally{j&&j.classList.add("hidden")}},400))}),I.addEventListener("click",g=>{const h=g.target.closest("button");h&&(N.value=h.dataset.name,I.classList.add("hidden"))}),document.addEventListener("click",g=>{!N.contains(g.target)&&!I.contains(g.target)&&I.classList.add("hidden")})),E.querySelectorAll("input, select, textarea").forEach(g=>{const h=(g.getAttribute("type")||"").toLowerCase(),m=!g.disabled&&!g.readOnly&&h!=="hidden";g.classList.remove("editable-indicator"),m&&g.classList.add("editable-indicator")}),E.addEventListener("submit",g=>{g.preventDefault();const h=new FormData(E),m={};h.forEach((S,y)=>m[y]=S),m.id=e.id,m.gip_id=m.gip_id||e.id,window.addBeneficiaryData&&(async()=>await window.addBeneficiaryData(m,!0,!1)&&(v(),window.viewBeneficiary&&setTimeout(()=>window.viewBeneficiary({id:e.id},0),100),C.fire({toast:!0,position:"top-end",icon:"success",title:"Record Updated",showConfirmButton:!1,timer:3e3})))()})})}function wt(){window.showAddDataModal=he,window.__maskDate=function(e){let o=e.replace(/\D/g,"").slice(0,8);return o.length>2&&o.length<=4?o=o.slice(0,2)+"/"+o.slice(2):o.length>4&&(o=o.slice(0,2)+"/"+o.slice(2,4)+"/"+o.slice(4)),o},window.__parseFormattedDate=function(e){if(!e)return null;const o=e.split("/");if(o.length===3){const n=parseInt(o[0])-1,a=parseInt(o[1]),r=parseInt(o[2]);if(r>1e3&&n>=0&&n<12&&a>0&&a<=31)return new Date(r,n,a)}return null},window.calculateAge=function(e){if(!e)return"";const o=e instanceof Date?e:new Date(e),n=new Date;let a=n.getFullYear()-o.getFullYear();const r=n.getMonth()-o.getMonth();return(r<0||r===0&&n.getDate()<o.getDate())&&a--,a>=0?a:0},window.viewBeneficiary=async function(e,o=0){const n=e?.id||e?.gip_id||null;if(!n)return;const a=!!(e?.name&&e?.office&&e?.remarks);let r={...e,id:n};if(!a){const s=await W(`api/beneficiaries.php?id=${encodeURIComponent(n)}`);s.success&&s.data?.success&&s.data?.beneficiary&&(r={...s.data.beneficiary,...r,id:n})}const t=`logs_cache_${n}`;let u=!1;try{if(window.__doleDB&&window.__doleDB.getSecureCache){const $=await window.__doleDB.getSecureCache(t);$&&(r.arLogs=$.arLogs||[],r.dtrLogs=$.dtrLogs||[],r.docs=$.docs||[],we(r,o),u=!0)}let s=[],f=[],x=[];const[i,l,v,E]=await Promise.all([W(`api/logs.php?type=ar&gip_id=${encodeURIComponent(n)}`),W(`api/logs.php?type=dtr&gip_id=${encodeURIComponent(n)}`),W(`api/logs.php?type=docs&gip_id=${encodeURIComponent(n)}`),W(`api/logs.php?type=absorption&gip_id=${encodeURIComponent(n)}`)]);s=i.success&&i.data?.success?i.data.logs:[],f=l.success&&l.data?.success?l.data.logs:[],x=v.success&&v.data?.success?v.data.logs:[];const b=E.success&&E.data?.success?E.data.logs:[];if(b.length>0){const $=b[0];r.absorbDate=$.absorption_datetime,r.absorb_where=$.where||$.absorb_where,r.absorb_position=$.position||$.absorb_position,r.absorb_agency=$.agency||$.absorb_agency}const D={arLogs:s,dtrLogs:f,docs:x};window.__doleDB&&window.__doleDB.setSecureCache&&await window.__doleDB.setSecureCache(t,D),r.arLogs=s,r.dtrLogs=f,r.docs=x,u?document.getElementById("beneficiary-drawer-container")&&we(r,o):we(r,o)}catch(s){console.error("Error fetching logs/docs:",s),u||(r.arLogs=[],r.dtrLogs=[],r.docs=[],we(r,o))}},window.showAddDataModal=function(e){he(e)},window.editBeneficiary=function(e){at(e)},window.showExportConfigModal=function(e){it(e)},window.showProfileModal=function(){st()}}async function st(){try{if($e()&&ge){let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=a.id)}catch{}if(!e)throw new Error("User not logged in");const{data:o,error:n}=await ge.from("users").select("*").eq("id",e).single();if(n)throw n;Me(o)}else{let e="";try{const a=JSON.parse(localStorage.getItem("user"));a&&a.id&&(e=`?user_id=${a.id}`)}catch{}const n=await(await fetch(`${Q()}api/profile.php${e}`)).json();if(n.success){const a=n.profile;Me(a)}else C.fire({icon:"error",title:"Error",text:n.error||"Failed to load profile"})}}catch(e){console.error("Error fetching profile:",e)}}function Me(e){const o=e.profile_picture_path?`${Q()}${e.profile_picture_path}`:null,n=e.full_name?e.full_name.split(" ").map(r=>r[0]).join("").substring(0,2).toUpperCase():"US",a=`
                ...
                <div class="flex flex-col items-center mb-6">
                    <div class="relative group">
                        <div id="profile-avatar-preview" class="w-24 h-24 rounded-full border-4 border-royal-blue/10 overflow-hidden bg-gray-100 flex items-center justify-center text-royal-blue text-2xl font-black shadow-lg">
                            ${o?`<img src="${o}" class="w-full h-full object-cover" />`:n}
                        </div>
    ...
    `;C.fire({html:a,width:"450px",showConfirmButton:!1,showCloseButton:!0,padding:"1.5rem",customClass:{container:"font-montserrat",popup:"rounded-[1.5rem] shadow-2xl overflow-visible ldn-modal-popup",closeButton:"focus:outline-none bg-gray-50 border-none swal2-custom-close cursor-pointer"},didOpen:r=>{const t=r.querySelector("#profile-edit-form"),u=r.querySelector("#profile-pic-input"),s=r.querySelector("#profile-avatar-preview");u.addEventListener("change",f=>{const x=f.target.files[0];if(x){const i=new FileReader;i.onload=l=>{s.innerHTML=`<img src="${l.target.result}" class="w-full h-full object-cover" />`},i.readAsDataURL(x)}}),t.addEventListener("submit",async f=>{f.preventDefault();const x=new FormData(t);try{const i=JSON.parse(localStorage.getItem("user"));i&&i.id&&x.append("user_id",i.id)}catch{}try{const l=await(await fetch(`${Q()}api/profile.php`,{method:"POST",body:x})).json();l.success?(l.profile&&(localStorage.setItem("user",JSON.stringify(l.profile)),nt(l.profile)),C.close(),C.fire({toast:!0,position:"top-end",icon:"success",title:"Profile Updated",showConfirmButton:!1,timer:3e3,timerProgressBar:!0})):C.fire({icon:"error",title:"Update Failed",text:l.error||"Something went wrong"})}catch(i){console.error("Error saving profile:",i)}})}})}function nt(e){const o=e.profile_picture_path?`${Q()}${e.profile_picture_path}`:null,n=e.full_name?e.full_name.split(" ").map(t=>t[0]).join("").substring(0,2).toUpperCase():"US",a=document.querySelectorAll(".sidebar-user-name"),r=document.querySelectorAll(".sidebar-user-avatar");a.forEach(t=>t.textContent=e.full_name),r.forEach(t=>{o?t.innerHTML=`<img src="${o}" class="w-full h-full object-cover" />`:t.textContent=n}),localStorage.setItem("user_full_name",e.full_name),o&&localStorage.setItem("user_avatar",o)}function it(e){const o=window.getExportFilters?window.getExportFilters():{office:"ALL",remarks:"ALL",gender:"ALL",ageGroup:"ALL",search:"",sort:"name",section:"ALL",columns:["id","name","age","office","position","startdate","enddate","status"],preparedBy:localStorage.getItem("ldn_export_prepared")||"",approvedBy:localStorage.getItem("ldn_export_approved")||""},n=`
        <div class="text-left font-montserrat p-1 overflow-visible">
            <div class="flex items-center gap-3 mb-6">
                <div class="w-12 h-12 bg-royal-blue/10 rounded-2xl flex items-center justify-center">
    ...
    `;C.fire({html:u,width:window.innerWidth<1024?"96vw":"1000px",showConfirmButton:!1,showCloseButton:!1,padding:window.innerWidth<1024?"0.75rem":"2rem",customClass:{container:"font-montserrat",popup:"rounded-2xl ldn-modal-popup"},willOpen:()=>{document.documentElement.classList.add("overflow-hidden"),document.body.classList.add("overflow-hidden")},didClose:()=>{document.documentElement.classList.remove("overflow-hidden"),document.body.classList.remove("overflow-hidden")},didOpen:s=>{window.initFlowbite&&window.initFlowbite();const f=s.querySelector("#cancel-modal-btn");f&&f.addEventListener("click",()=>{!o&&!e?._isBulk&&localStorage.removeItem("add_beneficiary_draft"),C.close(),e?._isBulk&&ke.onCancel()});const x=s.querySelector("#bulk-add-btn");x&&x.addEventListener("click",()=>{C.close(),ke.init()});const i=(d,p)=>{d.addEventListener("input",c=>{const k=window.__maskDate(c.target.value);if(c.target.value!==k&&(c.target.value=k),k.length===10){const w=window.__parseFormattedDate(k);w&&p&&(p(w),document.activeElement===d&&d.blur())}}),d.addEventListener("changeDate",c=>{if(c.detail&&c.detail.date&&p){p(c.detail.date);const k=d._datepicker||d.parentNode&&d.parentNode._datepicker;k&&typeof k.hide=="function"&&k.hide()}})},l=s.querySelector("#birthday-input"),v=s.querySelector("#age-display"),E=s.querySelector("#age-warning"),b=s.querySelector("#submit-beneficiary-btn"),D=d=>{if(!d)return E&&E.classList.add("hidden"),b&&(b.disabled=!1,b.classList.remove("opacity-50","cursor-not-allowed","grayscale"),b.classList.add("cursor-pointer")),!0;const p=parseInt(d),c=!isNaN(p)&&p>=18&&p<=29;return E&&(E.className=`mt-1 text-[10px] font-bold ${c?"hidden":"flex"} items-center gap-1.5 animate-pulse ${ae()?"text-red-400":"text-red-600"}`),b&&(c?(b.disabled=!1,b.classList.remove("opacity-50","cursor-not-allowed","grayscale"),b.classList.add("cursor-pointer","active:scale-[0.98]")):(b.disabled=!0,b.classList.add("opacity-50","cursor-not-allowed","grayscale"),b.classList.remove("cursor-pointer","active:scale-[0.98]"))),c};if(v&&(v.addEventListener("input",d=>{D(d.target.value)}),v.value&&D(v.value)),l){i(l,p=>{v&&(v.value=window.calculateAge(p),D(v.value),v.classList.add("animate-pulse"),setTimeout(()=>v.classList.remove("animate-pulse"),400))});const d=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);d&&(l._datepicker=new d(l,{format:"mm/dd/yyyy",autohide:!0,orientation:"bottom right"}))}const $=s.querySelector("#name-input-field"),T=s.querySelector("#duplicate-warning");if($&&T){let d;const p=()=>{try{return JSON.parse(localStorage.getItem("user")||"{}")?.id||null}catch{return null}},c=(w,A="")=>{T.className=`mt-1 text-[10px] font-bold ${w?"flex":"hidden"} items-center gap-1.5 animate-pulse ${ae()?"text-red-400":"text-red-600"}`;const B=T.querySelector("span");B&&(B.textContent=A?`Beneficiary already exists: ${A}`:"Beneficiary already exists")},k=async w=>{const A=p(),B=await fetch(`${Q()}api/check_duplicate.php`,{method:"POST",headers:{"Content-Type":"application/json",...A?{"X-User-Id":String(A)}:{}},body:JSON.stringify({name:w,user_id:A,exclude_id:o?e?.id:null})});if(!B.ok)throw new Error(`Duplicate check failed (${B.status})`);return B.json()};$.addEventListener("input",w=>{const A=w.target.value.trim();if(clearTimeout(d),A.length<3){c(!1);return}d=setTimeout(async()=>{try{const B=await k(A);B.success&&B.exists?c(!0,B.match||B.name):c(!1)}catch(B){console.error("Duplicate check error:",B)}},500)}),e?.name&&(c(!1),(async()=>{const w=await k(e.name);w.success&&w.exists&&c(!0,w.match||w.name)})())}const K=s.querySelector("#full-id-input"),ee=s.querySelector("#series-no-input"),P=s.querySelector('input[name="startDate"]'),V=s.querySelector('input[name="endDate"]'),ie=s.querySelectorAll('input[name="remarks"]'),le=s.querySelector("#extension-log-container"),se=async d=>{if(!d)return;const p=[K,ee].filter(Boolean);p.forEach(c=>{c.classList.add("animate-pulse"),c.placeholder="Syncing..."});try{const[c,k]=await Promise.all([W(`api/beneficiaries.php?next_id&year=${encodeURIComponent(d)}`),W(`api/beneficiaries.php?next_series_no&year=${encodeURIComponent(d)}`)]),w=c.success&&c.data?.success?c.data.nextId:null,A=k.success&&k.data?.success?k.data.nextSeries:null;w&&K&&(K.value=w),A&&ee&&(ee.value=A)}catch(c){console.error("ID Sync error:",c)}finally{p.forEach(c=>c.classList.remove("animate-pulse"))}},Y=s.querySelector("#replacement-search-input"),de=s.querySelector("#replacement-hidden"),Z=s.querySelector("#replacement-suggestions"),ce=d=>{const p=(d.name||"").toUpperCase().trim(),c=d.startDateFormatted||d.startDate||"N/A",k=d.endDateFormatted||d.endDate||"N/A";return`${p} (${c.toUpperCase()} - ${k.toUpperCase()})`},pe=d=>{if(Z){if(!d.length){Z.innerHTML=`<div class="px-3 py-2 text-[10px] font-bold ${t.textCourseOpt}">No matching beneficiary found.</div>`,Z.classList.remove("hidden");return}Z.innerHTML=d.map(p=>{const c=ce(p);return`
                        <button type="button" class="replacement-option w-full text-left px-3 py-2 text-[10px] font-bold ${t.textCourseOpt} ${t.courseHover} rounded-md cursor-pointer transition-colors"
                            data-replacement="${c.replace(/"/g,"&quot;")}">
                            ${c}
                        </button>
                    `}).join(""),Z.classList.remove("hidden"),Z.querySelectorAll(".replacement-option").forEach(p=>{p.addEventListener("click",()=>{const c=p.getAttribute("data-replacement")||"";Y&&(Y.value=c),de&&(de.value=c),Z.classList.add("hidden")})})}};let M=null;const N=async(d="")=>{const p=(d||"").trim(),c=`api/beneficiaries.php?replacement_candidates=1&limit=20${p?`&q=${encodeURIComponent(p)}`:""}`,k=await W(c);k.success&&k.data?.success&&Array.isArray(k.data.candidates)&&pe(k.data.candidates)};Y&&de&&Z&&(Y.addEventListener("focus",()=>{N(Y.value||"")}),Y.addEventListener("input",()=>{de.value=Y.value.trim(),clearTimeout(M),M=setTimeout(()=>{N(Y.value||"")},250)}),document.addEventListener("click",d=>{Y&&Z&&!Y.contains(d.target)&&!Z.contains(d.target)&&Z.classList.add("hidden")}));const I=()=>{const d=s.querySelector('input[name="remarks"]:checked');return d?d.value:"ONGOING"},j=d=>{const p=s.querySelector(`input[name="remarks"][value="${d}"]`);p&&(p.checked=!0,g())},_=()=>{if(V&&V.value){const d=window.__parseFormattedDate(V.value);if(!d)return;const p=new Date;p.setHours(0,0,0,0);let c="ONGOING";d<p&&(c="EXPIRED"),j(c)}},g=()=>{if(!le)return;const d=I();if(d==="ABSORBED"){const p=e?.absorbDate&&!String(e.absorbDate).includes("0000-00-00")?new Date(e.absorbDate):new Date,c=p.getTimezoneOffset()*6e4,k=new Date(p.getTime()-c).toISOString().slice(0,16);le.innerHTML=`
                    ...
                    `}else if(d==="RESIGNED"){const p=e?.resignedDate&&!String(e.resignedDate).includes("0000-00-00")?new Date(e.resignedDate):new Date,c=p.getTimezoneOffset()*6e4,k=new Date(p.getTime()-c).toISOString().slice(0,16);le.innerHTML=`
                        <div class="mt-4 pt-4 border-t ${r?"border-slate-800":"border-gray-100"}">
                    ...
                    `}else le.innerHTML=""};if(P){let d=null;i(P,w=>{const A=w.getFullYear();if(V){const B=new Date(w);B.setDate(B.getDate()+182);const F=String(B.getMonth()+1).padStart(2,"0"),U=String(B.getDate()).padStart(2,"0"),J=B.getFullYear();V.value=`${F}/${U}/${J}`}_(),A>1900&&A!==d&&(d=A,se(A))}),V&&i(V,()=>_());const p=s.querySelector("#date-range-picker"),c=window.DateRangePicker||(typeof DateRangePicker<"u"?DateRangePicker:null),k=window.Datepicker||(typeof Datepicker<"u"?Datepicker:null);if(c&&p&&P&&V){const w=new c(p,{format:"mm/dd/yyyy",autohide:!0,allowOneSidedRange:!0,orientation:"auto"});P._datepicker=w.datepickers?.[0]||null,V._datepicker=w.datepickers?.[1]||null}else k&&(P&&(P._datepicker=new k(P,{format:"mm/dd/yyyy",autohide:!0,orientation:"auto"})),V&&(V._datepicker=new k(V,{format:"mm/dd/yyyy",autohide:!0,orientation:"auto"})));if(!o){const w=new Date().getFullYear();se(w)}}ie.forEach(d=>d.addEventListener("change",g));const h=s.querySelector("#resign-btn"),m=s.querySelector("#absorb-btn");h&&h.addEventListener("click",()=>j("RESIGNED")),m&&m.addEventListener("click",()=>j("ABSORBED")),s.querySelectorAll('input[type="text"], textarea').forEach(d=>{["id-number-input","full-id-input"].includes(d.id)||d.addEventListener("input",()=>{const p=d.selectionStart,c=d.selectionEnd;d.value=d.value.toUpperCase(),d.setSelectionRange(p,c)})}),_(),g(),y("education-input","course-suggestions","course-option"),y("designation-input","work-suggestions","work-option"),(()=>{const d=s.querySelector("#office-input"),p=s.querySelector("#office-suggestions");if(!d||!p)return;let c="OFFICES",k=null,w=[];const A=async()=>{const F="dole_offices_cache",U=async()=>{let q=[];try{const R=await W("api/beneficiaries.php?get_offices_advanced=1");R.success&&R.data?.success&&Array.isArray(R.data.offices)&&(q=R.data.offices)}catch(R){console.error("Office fetch failed:",R)}return q.length>0&&(w=q,localStorage.setItem(F,JSON.stringify({data:q,timestamp:Date.now()}))),q},J=localStorage.getItem(F);if(J)try{const{data:q,timestamp:R}=JSON.parse(J);return w=q,Date.now()-R>1800*1e3&&U().then(()=>{c==="OFFICES"&&B("OFFICES",k,d.value)}),q}catch{localStorage.removeItem(F)}return w.length===0?await U():w},B=async(F="OFFICES",U=null,J="")=>{if(c=F,k=U,F==="OFFICES"){const R=(await A()).filter(O=>O.office.toLowerCase().includes(J.toLowerCase()));p.innerHTML=`
                            ...
                            <div class="max-h-64 overflow-y-auto scrollbar-hide">
                                ${R.length>0?R.map(O=>{const H=parseInt(O.location_count||0)>0;return`
                                        <div class="office-code-option group/opt px-3 py-2 text-[9px] font-bold ${t.textCourseOpt} ${t.courseHover} rounded-lg ${H?"cursor-pointer":"cursor-default opacity-60"} transition-all flex items-center justify-between group active:scale-[0.98] mx-1 mb-0.5" 
                                            data-id="${O.id}" data-name="${O.office}" data-has-locations="${H}">
                                            <div class="flex items-center gap-2.5">
                                                <div class="w-2 h-2 rounded-md bg-blue-500/10 group-hover/opt:bg-blue-500/20 flex items-center justify-center transition-colors">
                                                    <div class="w-1 h-1 rounded-full bg-blue-500/40 group-hover/opt:bg-blue-500 transition-colors"></div>
                                                </div>
                        ...
                        `,p.querySelectorAll(".office-code-option").forEach(O=>{O.addEventListener("click",H=>{H.stopPropagation(),O.dataset.hasLocations==="true"?B("LOCATIONS",{id:O.dataset.id,name:O.dataset.name}):(d.value=O.dataset.name,p.classList.add("hidden"),d.dispatchEvent(new Event("change")))})})}else{p.innerHTML=`
                        ...
                        `;const q=p.querySelector("#locations-list-container"),R=p.querySelector("#location-search-internal"),O=`dole_locs_cache_${U.id}`;let H=[];const te=localStorage.getItem(O);if(te)try{const{data:G,timestamp:X}=JSON.parse(te);H=G}catch{localStorage.removeItem(O)}const re=async()=>{let G=[];if(ge&&$e()){const{data:X,error:oe}=await ge.from("office_locations").select("location").eq("office_id",U.id).order("location");!oe&&X&&(G=X)}if(G.length===0)try{const X=await W(`api/beneficiaries.php?get_office_locations=1&office_id=${U.id}`);X.success&&X.data?.success&&Array.isArray(X.data.locations)&&(G=X.data.locations)}catch(X){console.error("Office locations fetch failed:",X)}G.length>0&&(H=G,localStorage.setItem(O,JSON.stringify({data:G,timestamp:Date.now()})),ne(R.value))},ne=(G="")=>{const X=H.filter(oe=>oe.location.toLowerCase().includes(G.toLowerCase()));q.innerHTML=X.length>0?X.map(oe=>`
                            ...
                            `).join(""):H.length===0?`<div class="px-3 py-4 text-center text-[9px] font-bold ${t.textLabel} animate-pulse flex items-center justify-center gap-2"><svg class="w-3.5 h-3.5 animate-spin text-blue-500" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>Fetching...</div>`:`<div class="px-3 py-8 text-center text-[9px] font-bold ${t.textLabel} opacity-60">No matching locations.</div>`,q.querySelectorAll(".location-option").forEach(oe=>{oe.addEventListener("click",lt=>{const Pe=oe.dataset.location;d.value=`${U.name} - ${Pe}`,p.classList.add("hidden"),d.dispatchEvent(new Event("change"))})})};ne(R.value),re(),setTimeout(()=>R.focus(),50),R.addEventListener("input",()=>ne(R.value)),R.addEventListener("click",G=>G.stopPropagation()),p.querySelector("#back-to-offices").addEventListener("click",G=>{G.stopPropagation(),B("OFFICES")})}};d.addEventListener("focus",()=>{p.classList.remove("hidden"),B(c,k,d.value)}),d.addEventListener("input",()=>{p.classList.remove("hidden"),B(c,k,d.value)}),document.addEventListener("click",F=>{!d.contains(F.target)&&!p.contains(F.target)&&(p.classList.add("hidden"),d.value||(c="OFFICES",k=null))})})();function y(d,p,c){const k=s.querySelector(`#${d}`),w=s.querySelector(`#${p}`);if(!k||!w)return;let A=!1;k.addEventListener("focus",()=>w.classList.remove("hidden")),document.addEventListener("click",B=>{!k.contains(B.target)&&!w.contains(B.target)&&w.classList.add("hidden")}),k.addEventListener("input",()=>{if(A){A=!1;return}const B=k.value.toLowerCase(),F=w.querySelectorAll(`.${c}`);let U=!1;F.forEach(J=>{const q=J.querySelector(".option-text");(q?q.innerText:J.innerText).toLowerCase().includes(B)?(J.style.display="block",U=!0):J.style.display="none"}),U?w.classList.remove("hidden"):w.classList.add("hidden")}),w.addEventListener("click",B=>{const F=B.target.closest(`.${c}`);if(!F)return;const U=F.querySelector(".option-text");k.value=U?U.innerText.trim():F.innerText.trim(),A=!0,w.classList.add("hidden"),k.dispatchEvent(new Event("change"))})}const L=s.querySelector("#add-beneficiary-form"),z="add_beneficiary_draft";if(!o){const d=localStorage.getItem(z);if(d)try{const p=JSON.parse(d);Object.entries(p).forEach(([c,k])=>{const w=L.elements[c];w&&w.type!=="file"&&w.type!=="hidden"&&(w.value=k)})}catch(p){console.error("Error loading draft",p)}}L.addEventListener("input",d=>{if(!o){const p=new FormData(L),c={};p.forEach((k,w)=>c[w]=k),localStorage.setItem(z,JSON.stringify(c))}}),L&&L.addEventListener("submit",d=>{d.preventDefault(),L.querySelectorAll('input:not([type="hidden"]), select, textarea').forEach(te=>{te.classList.remove("ring-2","ring-red-500","!border-red-500")});const c=new FormData(L);let k=!1;const w=te=>{const re=L.querySelector(`[name="${te}"]`);re&&re.classList.add("ring-2","ring-red-500","!border-red-500"),k=!0},A=c.get("name"),B=c.get("contact"),F=c.get("startDate"),U=c.get("endDate"),J=(c.get("designation")||"").trim();(!A||A.trim()===""||/[0-9]/.test(A))&&w("name"),B&&B.trim()!==""&&/[^0-9]/.test(B.replace(/[\s\-\+\(\)]/g,""))&&w("contact"),F||w("startDate"),U||w("endDate");const q=c.get("age"),R=parseInt(q);if((!q||isNaN(R)||R<18||R>29)&&(k=!0,E&&(E.className=`mt-1 text-[10px] font-bold flex items-center gap-1.5 animate-pulse ${typeof ae=="function"&&ae()?"text-red-400":"text-red-600"}`),b&&(b.disabled=!0,b.classList.add("opacity-50","cursor-not-allowed","grayscale"),b.classList.remove("cursor-pointer","active:scale-[0.98]"))),k)return;const O={};c.forEach((te,re)=>{if(["birthday","startDate","endDate"].includes(re)){const ne=window.__parseFormattedDate(te);if(ne){const G=ne.getFullYear(),X=String(ne.getMonth()+1).padStart(2,"0"),oe=String(ne.getDate()).padStart(2,"0");O[re]=`${G}-${X}-${oe}`}else O[re]=te}else O[re]=te}),J||(O.designation="N/A"),O.replacement||(O.replacement="");const H=s.querySelector("#full-id-input")?.value;o?(O.id=e?.id,H&&(O.gip_id=H)):(O.id=null,H&&(O.gip_id=H)),window.addBeneficiaryData&&(async()=>{if(await window.addBeneficiaryData(O)){if(!o){const re="add_beneficiary_draft",ne=L.querySelector('[name="office"]')?.value||"",G=L.querySelector('[name="designation"]')?.value||"",X=L.querySelector('[name="education"]')?.value||"";localStorage.setItem(re,JSON.stringify({office:ne,designation:G,education:X}))}C.close(),setTimeout(()=>{C.fire({toast:!0,position:"top-end",icon:"success",title:`Record ${o?"Updated":"Added"} Successfully`,showConfirmButton:!1,timer:3e3,timerProgressBar:!0}),!o&&e?._isBulk&&ke.onSaveSuccess()},100)}else C.fire({icon:"error",title:"Save Failed",text:"There was an error saving the record to the database."})})()})}})}window.handleContactSubmit=async function(e){e.preventDefault();const o=e.target,n=o.querySelector('button[type="submit"]'),a=n.innerHTML;n.disabled=!0,n.innerHTML=`
        <svg class="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
            <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        Sending...
    `;try{const r=new FormData(o);if((await fetch(o.action,{method:"POST",body:r,headers:{Accept:"application/json"}})).ok)C.fire({toast:!0,position:"top-end",icon:"success",title:"Message Sent Successfully!",text:"Thank you for reaching out. I will get back to you soon!",showConfirmButton:!1,timer:4e3,timerProgressBar:!0,didOpen:u=>{u.addEventListener("mouseenter",C.stopTimer),u.addEventListener("mouseleave",C.resumeTimer)},customClass:{popup:"rounded-2xl border-l-4 border-teal-500 shadow-2xl"}}),o.reset();else throw new Error("Failed to send")}catch{C.fire({toast:!0,position:"top-end",icon:"error",title:"Oops! Something went wrong.",text:"Could not send your message. Please try again later.",showConfirmButton:!1,timer:4e3,customClass:{popup:"rounded-2xl border-l-4 border-red-500 shadow-2xl"}})}finally{n.disabled=!1,n.innerHTML=a}return!1};export{Oe as _,W as a,xe as b,ft as c,xt as d,mt as e,pt as f,Q as g,vt as h,$e as i,ut as j,gt as k,ue as l,bt as m,wt as n,ht as r,ge as s,nt as u};
